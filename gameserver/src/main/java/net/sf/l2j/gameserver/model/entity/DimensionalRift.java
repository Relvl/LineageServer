...
public class DimensionalRift {
    ...
    public DimensionalRift(L2Party party, byte type, byte room) {
        ...
        for (L2PcInstance p : party.getPartyMembers()) { p.teleToLocation(coords[0], coords[1], coords[2], 0); }
    ...
    protected void createTeleporterTimer(final boolean reasonTP) {
        ...
        teleporterTimerTask = new TimerTask() {
            @Override
            public void run() {
                if (_choosenRoom > -1) { DimensionalRiftManager.getInstance().getRoom(_type, _choosenRoom).unspawn().setPartyInside(false); }
                ...
                if (reasonTP && _currentJumps < getMaxJumps() && _party.getMemberCount() > deadPlayers.size()) {
                    ...
                    for (L2PcInstance p : _party.getPartyMembers()) { if (!revivedInWaitingRoom.contains(p)) { teleportToNextRoom(p, false); } }

                    createTeleporterTimer(true);
                    createSpawnTimer(_choosenRoom);
                }
                else {
                    for (L2PcInstance p : _party.getPartyMembers()) { if (!revivedInWaitingRoom.contains(p)) { teleportToWaitingRoom(p); } }
        ...
        if (reasonTP) {
            ...
            earthQuakeTask = ThreadPoolManager.getInstance().schedule(new Runnable() {
                @Override
                public void run() {
                    for (L2PcInstance p : _party.getPartyMembers()) { if (!revivedInWaitingRoom.contains(p)) { p.sendPacket(new Earthquake(p.getX(), p.getY(), p.getZ(), 65, 9)); } }
                }
            }, jumpTime - 7000);
        }
        else {
    ...
    public void partyMemberExited(L2PcInstance player) {
        if (deadPlayers.contains(player)) { deadPlayers.remove(player); }

        if (revivedInWaitingRoom.contains(player)) { revivedInWaitingRoom.remove(player); }

        if (_party.getMemberCount() < Config.RIFT_MIN_PARTY_SIZE || _party.getMemberCount() == 1) {
            for (L2PcInstance p : _party.getPartyMembers()) { teleportToWaitingRoom(p); }
    ...
    public void manualTeleport(L2PcInstance player, L2Npc npc) {
        ...
        if (party == null || !party.isInDimensionalRift()) { return; }
        ...
        for (L2PcInstance p : _party.getPartyMembers()) { teleportToNextRoom(p, true); }
    ...
    public void manualExitRift(L2PcInstance player, L2Npc npc) {
        if (!player.isInParty() || !player.getParty().isInDimensionalRift()) { return; }
        ...
        for (L2PcInstance p : player.getParty().getPartyMembers()) { teleportToWaitingRoom(p); }
    ...
    protected void teleportToNextRoom(L2PcInstance player, boolean cantJumpToBossRoom) {
        if (_choosenRoom == -1) {
            ...
            do {
                ...
                if (emptyRooms.isEmpty()) { emptyRooms = DimensionalRiftManager.getInstance().getFreeRooms(_type); }
                ...
                if (cantJumpToBossRoom) {
                    while (_choosenRoom == 9) { _choosenRoom = emptyRooms.get(Rnd.get(1, emptyRooms.size()) - 1); }
                }
            }
    ...
    public void killRift() {
        ...
        if (_party != null) { _party.setDimensionalRift(null); }
    ...
    private long calcTimeToNextJump() {
        ...
        if (isBossRoom) { return (long) (time * Config.RIFT_BOSS_ROOM_TIME_MUTIPLY); }
    ...
    public void usedTeleport(L2PcInstance player) {
        if (!revivedInWaitingRoom.contains(player)) { revivedInWaitingRoom.add(player); }

        if (!deadPlayers.contains(player)) { deadPlayers.add(player); }

        if (_party.getMemberCount() - revivedInWaitingRoom.size() < Config.RIFT_MIN_PARTY_SIZE) {
            for (L2PcInstance p : _party.getPartyMembers()) { if (p != null && !revivedInWaitingRoom.contains(p)) { teleportToWaitingRoom(p); } }
    ...
    public byte getMaxJumps() {
        if (Config.RIFT_MAX_JUMPS <= 8 && Config.RIFT_MAX_JUMPS >= 1) { return (byte) Config.RIFT_MAX_JUMPS; }
