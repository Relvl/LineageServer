...
public class Q345_MethodToRaiseTheDead extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30970-03.htm")) {
            ...
            if (st.hasQuestItems(USELESS_BONE_PIECES)) { htmltext = "30971-02.htm"; }
        }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case DOROTHY:
                        if (cond == 1) { htmltext = (!st.hasQuestItems(VICTIM_ARM_BONE, VICTIM_THIGH_BONE, VICTIM_SKULL, VICTIM_RIB_BONE, VICTIM_SPINE)) ? "30970-04.htm" : "30970-05.htm"; }
                        else if (cond == 2) { htmltext = "30970-07.htm"; }
                        else if (cond > 2 && cond < 6) { htmltext = "30970-08.htm"; }
                        else {
                            ...
                            if (cond == 7) {
                                ...
                                st.rewardItems(ItemConst.ADENA_ID, 3040 + amount);

                                // Reward can be either an Imperial Diamond or bills.
                                if (Rnd.get(100) < 10) { st.giveItems(IMPERIAL_DIAMOND, 1); }
                                else { st.giveItems(BILL_OF_IASON_HEINE, 5); }
                            }
                            // Friends of Dorothy
                            else {
                                htmltext = "30970-09.htm";
                                st.rewardItems(ItemConst.ADENA_ID, 5390 + amount);
                    ...
                    case XENOVIA:
                        if (cond == 2) { htmltext = "30912-01.htm"; }
                        else if (cond > 2) { htmltext = "30912-06.htm"; }
                        break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (Rnd.get(4) == 0) {
            int randomPart = Rnd.get(VICTIM_ARM_BONE, VICTIM_SPINE);
