            ...
            if (_type == 1) {
                ...
                if (_itemId < 6535 || _itemId > 6540) {
                    ...
                    if (_itemId == 6645 || _itemId == 6646 || _itemId == 6647) {
                        ...
                        else { activeChar.sendPacket(SystemMessageId.NO_SERVITOR_CANNOT_AUTOMATE_USE); }
                    }
                    else {
                        // Cannot activate bss automation during Olympiad.
                        ...
                        // start the auto soulshot use
                        if (activeChar.getActiveWeaponItem() != activeChar.getFistsWeaponItem() && item.getItem().getCrystalType() == activeChar.getActiveWeaponItem().getCrystalType()) { activeChar.rechargeShots(true, true); }
                        else {
                            if ((_itemId >= 2509 && _itemId <= 2514) || (_itemId >= 3947 && _itemId <= 3952) || _itemId == 5790) { activeChar.sendPacket(SystemMessageId.SPIRITSHOTS_GRADE_MISMATCH); }
                            else { activeChar.sendPacket(SystemMessageId.SOULSHOTS_GRADE_MISMATCH); }
                        }
