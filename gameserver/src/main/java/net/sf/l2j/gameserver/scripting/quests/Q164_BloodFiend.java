    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getRace() == PlayerRace.DarkElf) { htmltext = "30149-00.htm"; }
                else if (player.getLevel() < 21) { htmltext = "30149-02.htm"; }
                else { htmltext = "30149-03.htm"; }
                break;
            ...
            case QuestState.STATE_STARTED:
                if (st.hasQuestItems(KIRUNAK_SKULL)) {
                    ...
                    st.rewardItems(ItemConst.ADENA_ID, 42130);
                    st.playSound(ESound.ItemSound_quest_finish);
                    st.exitQuest(false);
                }
                else { htmltext = "30149-05.htm"; }
                break;
