...
public class L2DoormenInstance extends L2NpcInstance {
    ...
    public void onBypassFeedback(L2PcInstance player, String command) {
        if (command.startsWith("Chat")) {
            ...
            if (isOwnerClan(player)) {
                if (isUnderSiege()) { cannotManageDoors(player); }
                else { openDoors(player, command); }
            }
            return;
        }
            ...
            if (isOwnerClan(player)) {
                if (isUnderSiege()) { cannotManageDoors(player); }
                else { closeDoors(player, command); }
            }
            return;
        }
        else if (command.startsWith("tele")) {
            if (isOwnerClan(player)) { doTeleport(player, command); }
            return;
        }
    ...
    public void showChatWindow(L2PcInstance player) {
        ...
        if (!isOwnerClan(player)) { html.setFile("data/html/doormen/" + getTemplate().getNpcId() + "-no.htm"); }
        else { html.setFile("data/html/doormen/" + getTemplate().getNpcId() + ".htm"); }
