...
public class L2WarehouseInstance extends L2NpcInstance {
    ...
    private static void showDepositWindowFreight(L2PcInstance player) {
        ...
        if (player.getAccountChars().isEmpty()) { player.sendPacket(SystemMessageId.CHARACTER_DOES_NOT_EXIST); }
    ...
    private void showWithdrawWindowFreight(L2PcInstance player) {
        ...
        if (freight != null) {
            if (freight.getSize() > 0) {
                if (Config.ALT_GAME_FREIGHTS) { freight.setActiveLocation(0); }
                else { freight.setActiveLocation(getWorldRegion().hashCode()); }

                player.setActiveWarehouse(freight);
                player.sendPacket(new WarehouseWithdrawList(player, WarehouseWithdrawList.FREIGHT));
            }
            else { player.sendPacket(SystemMessageId.NO_ITEM_DEPOSITED_IN_WH); }
        }
    }
    ...
    public void onBypassFeedback(L2PcInstance player, String command) {
        ...
        if (command.startsWith("WithdrawP")) { showRetrieveWindow(player); }
        else if (command.equals("DepositP")) { showDepositWindow(player); }
        else if (command.equals("WithdrawC")) { showWithdrawWindowClan(player); }
        else if (command.equals("DepositC")) { showDepositWindowClan(player); }
