...
import net.sf.l2j.gameserver.network.client.game_to_client.Revive;
...
public abstract class L2Playable extends L2Character {
    ...
    public boolean doDie(L2Character killer) {
        ...
        if (isPhoenixBlessed()) {
            ...
            if (getCharmOfLuck()) { stopCharmOfLuck(null); }
            if (isNoblesseBlessed()) { stopNoblesseBlessing(null); }
        }
        ...
        if (region != null) { region.onDeath(this); }

        // Notify Quest of L2Playable's death
        final L2PcInstance actingPlayer = getActingPlayer();
        for (QuestState qs : actingPlayer.getNotifyQuestOfDeath()) { qs.getQuest().notifyDeath((killer == null ? this : killer), actingPlayer); }
    ...
    public void doRevive() {
        ...
        else { getStatus().setCurrentHp(getMaxHp() * Config.RESPAWN_RESTORE_HP); }
    ...
    public boolean checkIfPvP(L2Playable target) {
        ...
        if (player == null || player.getKarma() != 0) { return false; }

        final L2PcInstance targetPlayer = target.getActingPlayer();
        if (targetPlayer == null || targetPlayer == this) { return false; }

        if (targetPlayer.getKarma() != 0 || targetPlayer.getPvpFlag() == 0) { return false; }
    ...
    public final void stopNoblesseBlessing(L2Effect effect) {
        if (effect == null) { stopEffects(L2EffectType.NOBLESSE_BLESSING); }
    ...
    public final void stopPhoenixBlessing(L2Effect effect) {
        if (effect == null) { stopEffects(L2EffectType.PHOENIX_BLESSING); }
        else { removeEffect(effect); }
    ...
    public void stopProtectionBlessing(L2Effect effect) {
        if (effect == null) { stopEffects(L2EffectType.PROTECTION_BLESSING); }
        else { removeEffect(effect); }
    ...
    public final void stopCharmOfLuck(L2Effect effect) {
        if (effect == null) { stopEffects(L2EffectType.CHARM_OF_LUCK); }
        else { removeEffect(effect); }
