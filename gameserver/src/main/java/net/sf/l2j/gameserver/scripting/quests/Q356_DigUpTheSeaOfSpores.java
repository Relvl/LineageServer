...
public class Q356_DigUpTheSeaOfSpores extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30717-06.htm")) {
            ...
            st.rewardItems(ItemConst.ADENA_ID, 20950);
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                if (cond == 1) { htmltext = "30717-07.htm"; }
                else if (cond == 2) {
                    if (st.getQuestItemsCount(HERB_SPORE) >= 50) { htmltext = "30717-08.htm"; }
                    else if (st.getQuestItemsCount(CARN_SPORE) >= 50) { htmltext = "30717-09.htm"; }
                    else { htmltext = "30717-07.htm"; }
                }
                else if (cond == 3) { htmltext = "30717-10.htm"; }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (cond < 3) {
            switch (npc.getNpcId()) {
                case ROTTING_TREE:
                    if (st.dropItems(HERB_SPORE, 1, 50, 630000)) { st.set("cond", (cond == 2) ? "3" : "2"); }
                    break;

                case SPORE_ZOMBIE:
                    if (st.dropItems(CARN_SPORE, 1, 50, 760000)) { st.set("cond", (cond == 2) ? "3" : "2"); }
                    break;
            }
        }
