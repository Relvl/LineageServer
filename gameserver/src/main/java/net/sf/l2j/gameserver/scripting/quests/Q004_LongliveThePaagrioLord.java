    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getRace() != PlayerRace.Orc) { htmltext = "30578-00.htm"; }
                else if (player.getLevel() < 2) { htmltext = "30578-01.htm"; }
                else { htmltext = "30578-02.htm"; }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                if (npcId == 30578) {
                    if (cond == 1) { htmltext = "30578-04.htm"; }
                    else if (cond == 2) {
                        htmltext = "30578-06.htm";
                        st.giveItems(4, 1);
                        for (int item : NPC_GIFTS.values()) { st.takeItems(item, -1); }
                    ...
                    if (st.hasQuestItems(i)) { htmltext = npcId + "-02.htm"; }
                    else {
                        st.giveItems(i, 1);
                        htmltext = npcId + "-01.htm";

                        int count = 0;
                        for (int item : NPC_GIFTS.values()) { count += st.getQuestItemsCount(item); }
