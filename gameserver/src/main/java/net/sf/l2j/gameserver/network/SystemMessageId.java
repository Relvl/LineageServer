...
public final class SystemMessageId {
        ...
        for (final Field field : fields) {
            ...
            if (Modifier.isStatic(mod) && Modifier.isPublic(mod) && Modifier.isFinal(mod) && field.getType().equals(SystemMessageId.class)) {
                try {
                    ...
                    LOGGER.error("SystemMessageId: Failed field access for '" + field.getName() + "'", e);
                }
            }
        }
    ...
    public final void setParamCount(final int params) {
        if (params < 0) { throw new IllegalArgumentException("Invalid negative param count: " + params); }

        if (params > 10) { throw new IllegalArgumentException("Maximum param count exceeded: " + params); }

        if (params != 0) { _staticSystemMessage = null; }
