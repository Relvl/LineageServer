...
public class Castle {
    ...
    public synchronized void engrave(L2Clan clan, L2Object target) {
        if (!_artefacts.contains(target)) { return; }
    ...
    public void addToTreasury(int amount) {
        ...
        if (_name.equalsIgnoreCase("Schuttgart") || _name.equalsIgnoreCase("Goddard")) {
            ...
            if (rune != null) {
                ...
                if (rune._ownerId > 0) { rune.addToTreasury(runeTax); }
                amount -= runeTax;
            }
        }
    ...
    public boolean addToTreasuryNoTax(long amount) {
        ...
        if (amount < 0) {
            ...
            if (_treasury + amount > Integer.MAX_VALUE) { _treasury = Integer.MAX_VALUE; }
    ...
    public void openCloseDoor(L2PcInstance activeChar, int doorId, boolean open) {
        if (activeChar.getClanId() != _ownerId) { return; }
    ...
    public void removeOwner(L2Clan clan) {
        ...
        if (getSiege().isInProgress()) { getSiege().midVictory(); }
        else {
            // Remove circlets only if a siege isn't in progress, and if an owner exists
            if (Config.REMOVE_CASTLE_CIRCLETS) { if (_formerOwner != null) { CastleManager.getInstance().removeCirclet(_formerOwner, _castleId); } }
        }
    }
    ...
    public void upgradeDoor(int doorId, int hp, boolean db) {
        ...
        if (db) {
            try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
                ...
                LOGGER.error("Exception: saveDoorUpgrade(int doorId, int hp): {}", e.getMessage(), e);
            }
        }
    }
    ...
    public void loadDoorUpgrade() {
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            while (rs.next()) { upgradeDoor(rs.getInt("doorId"), rs.getInt("hp"), false); }

            rs.close();
            statement.close();
        }
        catch (Exception e) {
            LOGGER.error("Exception: loadDoorUpgrade(): {}", e.getMessage(), e);
        }
    }
    ...
    public void removeDoorUpgrade() {
        for (L2DoorInstance door : _doors) { door.setUpgradeHpRatio(1); }
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.error("Exception: removeDoorUpgrade(): {}", e.getMessage(), e);
        }
    }
    ...
    public void setSeedProduction(List<SeedProduction> seed, int period) {
        if (period == CastleManorManager.PERIOD_CURRENT) { _production = seed; }
        else { _productionNext = seed; }
    }

    public void setCropProcure(List<CropProcure> crop, int period) {
        if (period == CastleManorManager.PERIOD_CURRENT) { _procure = crop; }
        else { _procureNext = crop; }
    }
    ...
    public long getManorCost(int period) {
        ...
        long total = 0;
        if (production != null) {
            for (SeedProduction seed : production) { total += L2Manor.getInstance().getSeedBuyPrice(seed.getId()) * seed.getStartProduce(); }
        }

        if (procure != null) {
            for (CropProcure crop : procure) { total += crop.getPrice() * crop.getStartAmount(); }
        }
        return total;
    }
    ...
    public void saveSeedData() {
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            if (_production != null) {
                ...
                for (SeedProduction s : _production) { values[count++] = "(" + getCastleId() + "," + s.getId() + "," + s.getCanProduce() + "," + s.getStartProduce() + "," + s.getPrice() + "," + CastleManorManager.PERIOD_CURRENT + ")"; }
            ...
            if (_productionNext != null) {
                ...
                for (SeedProduction s : _productionNext) { values[count++] = "(" + getCastleId() + "," + s.getId() + "," + s.getCanProduce() + "," + s.getStartProduce() + "," + s.getPrice() + "," + CastleManorManager.PERIOD_NEXT + ")"; }
            ...
            LOGGER.error("Error adding seed production data for castle {}: {}", getName(), e.getMessage(), e);
        }
    }
    ...
    public void saveSeedData(int period) {
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            if (prod != null) {
                ...
                for (SeedProduction s : prod) { values[count++] = "(" + getCastleId() + "," + s.getId() + "," + s.getCanProduce() + "," + s.getStartProduce() + "," + s.getPrice() + "," + period + ")"; }
    ...
    public void saveCropData() {
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.error("Error adding crop data for castle {}: {}", getName(), e.getMessage(), e);
        }
    }
    ...
    public void saveCropData(int period) {
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            if (proc != null) {
                ...
                for (CropProcure cp : proc) { values[count++] = "(" + getCastleId() + "," + cp.getId() + "," + cp.getAmount() + "," + cp.getStartAmount() + "," + cp.getPrice() + "," + cp.getReward() + "," + period + ")"; }
    ...
    public int getTrapUpgradeLevel(int towerIndex) {
        TowerSpawn spawn = SiegeManager.getInstance().getFlameTowers(_castleId).get(towerIndex);
    ...
    public void setTrapUpgrade(int towerIndex, int level, boolean save) {
        if (save) {
            try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
                ...
                LOGGER.error("Exception: setTrapUpgradeLevel(int towerIndex, int level, int castleId): {}", e.getMessage(), e);
            }
        }

        TowerSpawn spawn = SiegeManager.getInstance().getFlameTowers(_castleId).get(towerIndex);
        if (spawn != null) { spawn.setUpgradeLevel(level); }
    }
    ...
    public void removeTrapUpgrade() {
        for (TowerSpawn ts : SiegeManager.getInstance().getFlameTowers(_castleId)) { ts.setUpgradeLevel(0); }
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.error("Exception: removeTrapUpgrade(): {}", e.getMessage(), e);
        }
    }
}