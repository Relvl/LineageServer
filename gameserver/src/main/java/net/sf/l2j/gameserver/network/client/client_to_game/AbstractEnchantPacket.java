...
public abstract class AbstractEnchantPacket extends L2GameClientPacket {
    ...
    public static final class EnchantScroll {
        ...
        public final boolean isValid(L2ItemInstance enchantItem) {
            if (enchantItem == null) { return false; }
            ...
            if (_grade != enchantItem.getItem().getCrystalType()) { return false; }
        ...
        public final double getChance(L2ItemInstance enchantItem) {
            if (!isValid(enchantItem)) { return -1; }

            boolean fullBody = enchantItem.getItem().getBodyPart() == EItemBodyPart.SLOT_FULL_ARMOR;
            ...
            if (enchantItem.isArmor()) { chance = Math.pow(Config.ENCHANT_CHANCE_ARMOR, (enchantItem.getEnchantLevel() - 2)); }
            // Weapon formula is 70% for fighter weapon, 40% for mage weapon. Special rates after +14.
            else if (enchantItem.isWeapon()) {
                if (((Weapon) enchantItem.getItem()).isMagical()) { chance = (enchantItem.getEnchantLevel() > 14) ? Config.ENCHANT_CHANCE_WEAPON_MAGIC_15PLUS : Config.ENCHANT_CHANCE_WEAPON_MAGIC; }
                else { chance = (enchantItem.getEnchantLevel() > 14) ? Config.ENCHANT_CHANCE_WEAPON_NONMAGIC_15PLUS : Config.ENCHANT_CHANCE_WEAPON_NONMAGIC; }
            }
