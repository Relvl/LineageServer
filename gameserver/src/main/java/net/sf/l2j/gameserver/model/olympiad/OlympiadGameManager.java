...
public class OlympiadGameManager implements Runnable {
    ...
    protected OlympiadGameManager() {
        ...
        if (zones == null || zones.isEmpty()) { throw new Error("No olympiad stadium zones defined !"); }

        _tasks = new OlympiadGameTask[zones.size()];
        int i = 0;
        for (L2OlympiadStadiumZone zone : zones) { _tasks[i++] = new OlympiadGameTask(zone); }

        LOGGER.info("Olympiad: Loaded {} stadiums.", _tasks.length);
    }
    ...
    public final void run() {
        if (Olympiad.getInstance().isOlympiadEnd()) { return; }
        ...
        if (Olympiad.getInstance().inCompPeriod()) {
            ...
            if (readyClassed != null || readyNonClassed) {
                ...
                for (int i = 0; i < _tasks.length; i++) {
                    ...
                    if (readyClassed == null && !readyNonClassed) { break; }
                }
            }
        }
        else {
            ...
            if (isAllTasksFinished()) {
                ...
                LOGGER.info("Olympiad: All current games finished.");
            }
        }
    }
    ...
    public final OlympiadGameTask getOlympiadTask(int id) {
        if (id < 0 || id >= _tasks.length) { return null; }
    ...
    public final void notifyCompetitorDamage(L2PcInstance player, int damage) {
        ...
        if (id < 0 || id >= _tasks.length) { return; }

        final AbstractOlympiadGame game = _tasks[id].getGame();
        if (game != null) { game.addDamage(player, damage); }
    }
