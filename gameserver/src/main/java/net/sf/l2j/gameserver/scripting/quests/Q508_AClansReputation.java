...
public class Q508_AClansReputation extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (!player.isClanLeader()) { htmltext = "30868-0a.htm"; }
                else if (clan.getLevel() < 5) { htmltext = "30868-0b.htm"; }
                else { htmltext = "30868-0c.htm"; }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                if (!st.hasQuestItems(item)) { htmltext = "30868-" + raid + "a.htm"; }
                else {
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st == null || !st.isStarted()) { return null; }

        // Reward only if quest is setup on good index.
        int raid = st.getInt("raid");
        if (reward_list[raid - 1][0] == npc.getNpcId()) { st.dropItemsAlways(reward_list[raid - 1][1], 1, 1); }
