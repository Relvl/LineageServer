    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getLevel() < 60) { htmltext = "31577-02.htm"; }
                else {
                    QuestState st2 = player.getQuestState("Q053_LinnaeusSpecialBait");
                    if (st2 != null && st2.isCompleted()) { htmltext = "31577-01.htm"; }
                    else { htmltext = "31577-03.htm"; }
                }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case LINNAEUS:
                        if (cond == 1) { htmltext = (!st.hasQuestItems(RED_TREASURE_BOX)) ? "31577-06.htm" : "31577-05.htm"; }
                        else if (cond == 2) { htmltext = "31577-09.htm"; }
                        break;

                    case RUKAL:
                        if (cond == 2) { htmltext = "30629-01.htm"; }
                        break;
                }
                break;
