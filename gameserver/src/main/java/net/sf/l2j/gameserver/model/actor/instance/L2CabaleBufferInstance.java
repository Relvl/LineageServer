...
public class L2CabaleBufferInstance extends L2NpcInstance {
    ...
    private class CabaleAI implements Runnable {
        ...
        public void run() {
            ...
            else if (winningCabal == SevenSigns.CABAL_DUSK) { losingCabal = SevenSigns.CABAL_DAWN; }
            ...
            for (L2PcInstance player : getKnownList().getKnownTypeInRadius(L2PcInstance.class, 900)) {
                if (player.isGM()) { gmsList.add(player); }
                ...
                if (playerCabal != SevenSigns.CABAL_NULL) {
                    ...
                    if (isBuffAWinner && isBuffALoser) { break; }
                }
            }
            ...
            if (_step >= 12) {
                if (!playersList.isEmpty() || !gmsList.isEmpty()) {
                    ...
                    String text;
                    if (_caster.getCollisionHeight() > 30) { text = MESSAGES_LOSER[Rnd.get(MESSAGES_LOSER.length)]; }
                    else { text = MESSAGES_WINNER[Rnd.get(MESSAGES_WINNER.length)]; }
                    ...
                    if (!text.contains("%player_")) {
                        CreatureSay cs = new CreatureSay(getObjectId(), EChatType.ALL, getName(), text);

                        for (L2PcInstance nearbyPlayer : playersList) { nearbyPlayer.sendPacket(cs); }

                        for (L2PcInstance nearbyGM : gmsList) { nearbyGM.sendPacket(cs); }
                    }
                }
                _step = 0;
            }
