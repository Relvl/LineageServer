...
public class L2FlameTowerInstance extends L2Npc {
    ...
    public boolean doDie(L2Character killer) {
        ...
        if (getCastle() != null) {
            ...
            if (_zoneList != null && _upgradeLevel != 0) { getCastle().getSiege().announceToPlayer(SystemMessage.getSystemMessage(SystemMessageId.A_TRAP_DEVICE_HAS_BEEN_STOPPED), false); }
        }
    ...
    public final void enableZones(boolean state) {
        if (_zoneList != null && _upgradeLevel != 0) {
            ...
            for (int i = 0; i < maxIndex; i++) {
                ...
                if (zone != null && zone instanceof ACastleZoneType) { ((ACastleZoneType) zone).setEnabled(state); }
            }
        }
    }
