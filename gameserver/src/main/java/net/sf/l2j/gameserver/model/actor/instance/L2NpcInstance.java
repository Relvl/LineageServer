...
public class L2NpcInstance extends L2Npc {
    ...
    public void addEffect(L2Effect newEffect) {
        if (newEffect instanceof EffectDebuff || newEffect instanceof EffectBuff) { super.addEffect(newEffect); }
    ...
    public static void showSkillList(L2PcInstance player, L2Npc npc, ClassId classId) {
        if (((L2NpcInstance) npc).getClassesToTeach() == null) {
            ...
            html.setHtml("<html><body>I cannot teach you. My class list is empty.<br>Your admin needs to add me teachTo informations.<br>NpcId:" + npc.getTemplate().getNpcId() + ", your classId:" + player.getClassId()
                                                                                                                                                                                                            .getId() + "</body></html>");
        ...
        if (empty) {
            ...
            if (minlevel > 0) { player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.DO_NOT_HAVE_FURTHER_SKILLS_TO_LEARN_S1).addNumber(minlevel)); }
    ...
    public static void showEnchantSkillList(L2PcInstance player, L2Npc npc, ClassId classId) {
        ...
        if (empty) {
            ...
            if (player.getLevel() < 74) { player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.DO_NOT_HAVE_FURTHER_SKILLS_TO_LEARN_S1).addNumber(74)); }
            else { player.sendPacket(SystemMessageId.NO_MORE_SKILLS_TO_LEARN); }
        }
    ...
    public void onBypassFeedback(L2PcInstance player, String command) {
        ...
        else if (command.startsWith("EnchantSkillList")) { showEnchantSkillList(player, this, player.getClassId()); }
