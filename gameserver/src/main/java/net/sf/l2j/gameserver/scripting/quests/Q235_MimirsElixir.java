...
public class Q235_MimirsElixir extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30721-06.htm")) {
            ...
            if (!st.hasQuestItems(MAGISTER_MIXING_STONE)) { htmltext = "31149-havent.htm"; }
        }
        else if (event.equalsIgnoreCase("31149-03.htm")) {
            if (!st.hasQuestItems(MAGISTER_MIXING_STONE, PURE_SILVER)) { htmltext = "31149-havent.htm"; }
        }
        else if (event.equalsIgnoreCase("31149-05.htm")) {
            if (!st.hasQuestItems(MAGISTER_MIXING_STONE, PURE_SILVER, TRUE_GOLD)) { htmltext = "31149-havent.htm"; }
        }
        else if (event.equalsIgnoreCase("31149-07.htm")) {
            if (!st.hasQuestItems(MAGISTER_MIXING_STONE, PURE_SILVER, TRUE_GOLD, BLOOD_FIRE)) { htmltext = "31149-havent.htm"; }
        }
            ...
            else { htmltext = "31149-havent.htm"; }
        }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getLevel() < 75) { htmltext = "30721-01b.htm"; }
                else if (!st.hasQuestItems(STAR_OF_DESTINY)) { htmltext = "30721-01a.htm"; }
                else { htmltext = "30721-01.htm"; }
                break;
                ...
                switch (npc.getNpcId()) {
                    case LADD:
                        if (cond == 1) {
                            ...
                            else { htmltext = "30721-07.htm"; }
                        }
                        else if (cond < 5) { htmltext = "30721-10.htm"; }
                        else if (cond == 5 && st.hasQuestItems(TRUE_GOLD)) { htmltext = "30721-11.htm"; }
                        else if (cond == 6 || cond == 7) { htmltext = "30721-13.htm"; }
                        else if (cond == 8 && st.hasQuestItems(MIMIR_ELIXIR)) { htmltext = "30721-14.htm"; }
                        break;

                    case JOAN:
                        if (cond == 2) { htmltext = "30718-01.htm"; }
                        else if (cond == 3) { htmltext = "30718-04.htm"; }
                        ...
                        else if (cond > 4) { htmltext = "30718-06.htm"; }
                        break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            case 20965:
                if (st.getInt("cond") == 3 && st.dropItems(SAGE_STONE, 1, 1, 200000)) { st.set("cond", "4"); }
                break;

            case 21090:
                if (st.getInt("cond") == 6 && st.dropItems(BLOOD_FIRE, 1, 1, 200000)) { st.set("cond", "7"); }
                break;
        }
