...
public class Q638_SeekersOfTheHolyGrail extends Quest {
    ...
    public Q638_SeekersOfTheHolyGrail() {
        ...
        for (int i = 22138; i < 22175; i++) { addKillId(i); }
    }
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                if (st.getQuestItemsCount(PAGAN_TOTEM) >= 2000) {
                    ...
                    if (chance == 0) { st.rewardItems(959, 1); }
                    else if (chance == 1) { st.rewardItems(960, 1); }
                    else { st.rewardItems(ItemConst.ADENA_ID, 3576000); }
                }
                else { htmltext = "31328-04.htm"; }
                break;
        }
