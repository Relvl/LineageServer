...
public class Q328_SenseForBusiness extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                if (all == 0) { htmltext = "30436-04.htm"; }
                else {
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (npcId < 20069) {
            int rnd = Rnd.get(100);
            if (rnd < (chance + 1)) { st.dropItemsAlways((rnd < chance) ? MONSTER_EYE_CARCASS : MONSTER_EYE_LENS, 1, 0); }
        }
        else { st.dropItems(BASILISK_GIZZARD, 1, 0, chance); }
