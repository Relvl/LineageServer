...
public class Q412_PathToADarkWizard extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30421-05.htm")) {
            if (player.getClassId() != ClassId.darkMage) { htmltext = (player.getClassId() == ClassId.darkWizard) ? "30421-02a.htm" : "30421-03.htm"; }
            else if (player.getLevel() < 19) { htmltext = "30421-02.htm"; }
            else if (st.hasQuestItems(JEWEL_OF_DARKNESS)) { htmltext = "30421-04.htm"; }
            else {
            ...
            if (st.hasQuestItems(SEED_OF_ANGER)) { htmltext = "30421-06.htm"; }
            else if (st.hasQuestItems(LUCKY_KEY)) { htmltext = "30421-08.htm"; }
            else if (st.getQuestItemsCount(FAMILY_REMAINS) == 3) { htmltext = "30421-18.htm"; }
        }
        else if (event.equalsIgnoreCase("30421-10.htm")) {
            if (st.hasQuestItems(SEED_OF_HORROR)) { htmltext = "30421-09.htm"; }
            else if (st.getQuestItemsCount(KNEE_BONE) == 2) { htmltext = "30421-19.htm"; }
        }
        else if (event.equalsIgnoreCase("30421-13.htm")) {
            if (st.hasQuestItems(SEED_OF_LUNACY)) { htmltext = "30421-12.htm"; }
        }
            ...
            case QuestState.STATE_STARTED:
                switch (npc.getNpcId()) {
                    case VARIKA:
                        ...
                        else { htmltext = "30421-17.htm"; }
                        break;

                    case CHARKEREN:
                        if (st.hasQuestItems(SEED_OF_ANGER)) { htmltext = "30415-06.htm"; }
                        else if (!st.hasQuestItems(LUCKY_KEY)) { htmltext = "30415-01.htm"; }
                        ...
                        else { htmltext = "30415-04.htm"; }
                        break;

                    case ANNIKA:
                        if (st.hasQuestItems(SEED_OF_HORROR)) { htmltext = "30418-04.htm"; }
                        else if (!st.hasQuestItems(CANDLE)) { htmltext = "30418-01.htm"; }
                        ...
                        else { htmltext = "30418-03.htm"; }
                        break;

                    case ARKENIA:
                        if (st.hasQuestItems(SEED_OF_LUNACY)) { htmltext = "30419-03.htm"; }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            case 20015:
                if (st.hasQuestItems(LUCKY_KEY)) { st.dropItems(FAMILY_REMAINS, 1, 3, 500000); }
                break;

            case 20022:
            case 20517:
            case 20518:
                if (st.hasQuestItems(CANDLE)) { st.dropItems(KNEE_BONE, 1, 2, 500000); }
                break;

            case 20045:
                if (st.hasQuestItems(HUB_SCENT)) { st.dropItems(HEART_OF_LUNACY, 1, 3, 500000); }
                break;
        }
