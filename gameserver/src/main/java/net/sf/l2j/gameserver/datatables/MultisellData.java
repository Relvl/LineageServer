...
public class MultisellData {
    ...
    private ListContainer generateMultiSell(int listId, boolean inventoryOnly, L2PcInstance player, double taxRate) {
        ListContainer listTemplate = _entries.get(listId);
        if (listTemplate == null) { return EMPTY_CONTAINER; }
        ...
        if (inventoryOnly) {
            ...
            for (L2ItemInstance item : player.getInventory().getUniqueItems(false, false, listTemplate.getMaintainEnchantment())) {
                ...
                if ((item.getItem() instanceof Armor) || (item.getItem() instanceof Weapon)) {
                    int enchantLevel = listTemplate.getMaintainEnchantment() ? item.getEnchantLevel() : 0;
                    ...
                    for (Entry ent : listTemplate.getEntries()) {
                        ...
                        if (doInclude) { list.addEntry(prepareEntry(ent, listTemplate.getApplyTaxes(), listTemplate.getMaintainEnchantment(), enchantLevel, taxRate)); }
                    }
                }
            }
        }
        // this is a list-all type
            ...
            for (Entry ent : listTemplate.getEntries()) { list.addEntry(prepareEntry(ent, listTemplate.getApplyTaxes(), false, 0, taxRate)); }
        }
        return list;
    }
    ...
    private static Entry prepareEntry(Entry templateEntry, boolean applyTaxes, boolean maintainEnchantment, int enchantLevel, double taxRate) {
        ...
        for (Ingredient ing : templateEntry.getIngredients()) {
            ...
            if (ing.getItemId() == ItemConst.ADENA_ID && ing.isTaxIngredient()) {
                if (applyTaxes) { adenaAmount += (int) Math.round(ing.getItemCount() * taxRate); }

                continue; // do not adena yet, as non-taxIngredient adena entries might occur next (order not guaranteed)
            }
            else if (ing.getItemId() == ItemConst.ADENA_ID) {
            ...
            // if it is an armor/weapon, modify the enchantment level appropriately, if necessary
                ...
                if ((tempItem instanceof Armor) || (tempItem instanceof Weapon)) { newIngredient.setEnchantmentLevel(enchantLevel); }
            }
        ...
        if (adenaAmount > 0) { newEntry.addIngredient(new Ingredient(ItemConst.ADENA_ID, adenaAmount, 0, false, false)); }
    ...
    private static ListContainer parseDocument(Document doc) {
        ...
        for (Node n = doc.getFirstChild(); n != null; n = n.getNextSibling()) {
            if ("list".equalsIgnoreCase(n.getNodeName())) {
                ...
                if (attribute == null) { list.setApplyTaxes(false); }
                else { list.setApplyTaxes(Boolean.parseBoolean(attribute.getNodeValue())); }

                attribute = n.getAttributes().getNamedItem("maintainEnchantment");
                if (attribute == null) { list.setMaintainEnchantment(false); }
                else { list.setMaintainEnchantment(Boolean.parseBoolean(attribute.getNodeValue())); }
    ...
    private static Entry parseEntry(Node n) {
        ...
        for (n = first; n != null; n = n.getNextSibling()) {
            if ("ingredient".equalsIgnoreCase(n.getNodeName())) {
                ...
                if (attribute != null) { isTaxIngredient = Boolean.parseBoolean(attribute.getNodeValue()); }

                attribute = n.getAttributes().getNamedItem("mantainIngredient");

                if (attribute != null) { mantainIngredient = Boolean.parseBoolean(attribute.getNodeValue()); }
                ...
                if (attribute != null) { enchant = Integer.parseInt(attribute.getNodeValue()); }
