...
public class SevenSignsFestival implements SpawnListener {
    ...
    private static void addReputationPointsForPartyMemberClan(String partyMemberName) {
        ...
        if (player != null) {
            ...
            try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
                ...
                LOGGER.error("could not get clan name of {}: ", partyMemberName, e);
            }
        }
    }
    ...
    protected final ScheduledFuture<?> getFestivalManagerSchedule() {
        if (_managerScheduledTask == null) { startFestivalManager(); }
    ...
    protected void restoreFestivalData() {
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            for (int i = 0; i < FESTIVAL_COUNT - 1; i++) { StringUtil.append(sb, "accumulated_bonus", i, ", "); }
    ...
    public void saveFestivalData(boolean updateSettings) {
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            if (updateSettings) { SevenSigns.getInstance().saveSevenSignsStatus(); }
        }
        catch (SQLException e) {
    ...
    protected void rewardHighestRanked() {
        ...
        if (overallData != null) {
            ...
            for (String partyMemberName : partyMembers) { addReputationPointsForPartyMemberClan(partyMemberName); }
        }
    ...
    protected void resetFestivalData(boolean updateSettings) {
        ...
        for (int i = 0; i < FESTIVAL_COUNT; i++) { _accumulatedBonuses.set(i, 0); }
        ...
        for (int i = 0; i < FESTIVAL_COUNT * 2; i++) {
            ...
            if (i >= FESTIVAL_COUNT) { festivalId -= FESTIVAL_COUNT; }
            ...
            else { tempStats.set("cabal", SevenSigns.getCabalShortName(SevenSigns.CABAL_DUSK)); }
    ...
    public final int getMinsToNextCycle() {
        if (SevenSigns.getInstance().isSealValidationPeriod()) { return -1; }
    ...
    public final boolean isParticipant(L2PcInstance player) {
        if (SevenSigns.getInstance().isSealValidationPeriod()) { return false; }

        if (_managerInstance == null) { return false; }

        for (List<Integer> participants : _dawnFestivalParticipants.values()) {
            if (participants != null && participants.contains(player.getObjectId())) { return true; }
        }
    ...
    public final List<Integer> getParticipants(int oracle, int festivalId) {
        if (oracle == SevenSigns.CABAL_DAWN) { return _dawnFestivalParticipants.get(festivalId); }
    ...
    public final List<Integer> getPreviousParticipants(int oracle, int festivalId) {
        if (oracle == SevenSigns.CABAL_DAWN) { return _dawnPreviousParticipants.get(festivalId); }
    ...
    public void setParticipants(int oracle, int festivalId, L2Party festivalParty) {
        ...
        if (oracle == SevenSigns.CABAL_DAWN) { _dawnFestivalParticipants.put(festivalId, participants); }
        else { _duskFestivalParticipants.put(festivalId, participants); }
    }
    ...
    public void updateParticipants(L2PcInstance player, L2Party festivalParty) {
        ...
        if (festivalId > -1) {
            if (_festivalInitialized) {
                ...
                else { festivalInst.relocatePlayer(player, true); }
            }
    ...
    public final int getFinalScore(int oracle, int festivalId) {
        if (oracle == SevenSigns.CABAL_DAWN) { return _dawnFestivalScores.get(festivalId); }
    ...
    public final StatsSet getHighestScoreData(int oracle, int festivalId) {
        ...
        if (oracle == SevenSigns.CABAL_DAWN) { offsetId += 5; }
        ...
        try {
            ...
            LOGGER.error("SevenSignsFestival: Data missing for {}, FestivalID = {} (Current Cycle {})", SevenSigns.getCabalName(oracle), festivalId, _signsCycle, e);
        }
    ...
    public final StatsSet getOverallHighestScoreData(int festivalId) {
        ...
        for (Map<Integer, StatsSet> currCycleData : _festivalData.values()) {
            for (StatsSet currFestData : currCycleData.values()) {
                ...
                if (currFestID != festivalId) { continue; }
    ...
    public boolean setFinalScore(L2PcInstance player, int oracle, int festivalId, int offeringScore) {
        ...
        if (offeringScore > thisCabalHighScore) {
            ...
            if (thisCabalHighScore < otherCabalHighScore) { return false; }
            ...
            if (offeringScore > otherCabalHighScore) {
                ...
                LOGGER.info("SevenSignsFestival: This is the highest score overall so far for the {} festival!", getFestivalName(festivalId));
            }
    ...
    public final int getTotalAccumulatedBonus() {
        ...
        for (int accumBonus : _accumulatedBonuses) { totalAccumBonus += accumBonus; }
    ...
    public final int distribAccumulatedBonus(L2PcInstance player) {
        ...
        if (playerCabal != SevenSigns.getInstance().getCabalHighestScore()) { return 0; }
    ...
    public void sendMessageToAll(String senderName, String message) {
        if (_dawnChatGuide == null || _duskChatGuide == null) { return; }

        CreatureSay cs = new CreatureSay(_dawnChatGuide.getObjectId(), EChatType.SHOUT, senderName, message);
        _dawnChatGuide.broadcastPacket(cs);

        cs = new CreatureSay(_duskChatGuide.getObjectId(), EChatType.SHOUT, senderName, message);
    ...
    private static class FestivalSpawn {
        ...
        protected FestivalSpawn(int[] spawnData) {
            ...
            if (spawnData.length > 4) { _npcId = spawnData[4]; }
            else { _npcId = -1; }
        }
    }
    ...
    private class FestivalManager implements Runnable {
        ...
        public synchronized void run() {
            try {
                ...
                if (SevenSigns.getInstance().isSealValidationPeriod()) { return; }
                ...
                for (L2DarknessFestival festivalInst : _festivalInstances.values()) { festivalInst.unspawnMobs(); }
                    ...
                    if ((_duskFestivalParticipants.isEmpty() && _dawnFestivalParticipants.isEmpty())) {
                        try {
                            ...
                            for (L2DarknessFestival festivalInst : _festivalInstances.values()) {
                                if (!festivalInst._npcInsts.isEmpty()) { festivalInst.unspawnMobs(); }
                            }
                        }
                        catch (InterruptedException e) {
                        }
                    }
                    else { _noPartyRegister = false; }
                }
                ...
                for (L2DarknessFestival festivalInst : _festivalInstances.values()) { festivalInst.festivalEnd(); }
        ...
        public final L2DarknessFestival getFestivalInstance(int oracle, int festivalId) {
            if (!isFestivalInitialized()) { return null; }
    ...
    private class L2DarknessFestival {
        ...
        protected L2DarknessFestival(int cabal, int levelRange) {
            ...
            // FOR TESTING!
            if (_participants == null) { _participants = new ArrayList<>(); }
        ...
        protected void festivalInit() {
            ...
            try {
                ...
                LOGGER.error("SevenSignsFestival: Error while spawning Festival Witch ID {}: ", _witchSpawn._npcId, e);
            }
        ...
        protected void spawnFestivalMonsters(int respawnDelay, int spawnType) {
            ...
            if (_npcSpawns != null) {
                for (int[] _npcSpawn : _npcSpawns) {
                    ...
                    if (spawnType == 1 && isFestivalArcher(currSpawn._npcId)) { continue; }
                    ...
                    try {
                        ...
                        if (spawnType == 1) { festivalMob.setOfferingBonus(2); }
                        else if (spawnType == 3) { festivalMob.setOfferingBonus(5); }

                        _npcInsts.add(festivalMob);
                    }
                    catch (Exception e) {
                        LOGGER.error("SevenSignsFestival: Error while spawning NPC ID {}: ", currSpawn._npcId, e);
                    }
                }
            }
        }
        ...
        protected boolean increaseChallenge() {
            if (_challengeIncreased) { return false; }
        ...
        public void sendMessageToParticipants(String message) {
            if (_participants != null && !_participants.isEmpty()) {
                _witchInst.broadcastPacket(new CreatureSay(_witchInst.getObjectId(), EChatType.ALL, "Festival Witch", message));
            }
        }
        ...
        protected void festivalEnd() {
            if (_participants != null && !_participants.isEmpty()) {
                ...
                if (_cabal == SevenSigns.CABAL_DAWN) { _dawnPreviousParticipants.put(_levelRange, _participants); }
                else { _duskPreviousParticipants.put(_levelRange, _participants); }
            }
            _participants = null;
