...
public class Q242_PossessorOfAPreciousSoul extends Quest {
    ...
    private static boolean _unicorn;
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        // Cornerstone
            ...
            if (st.hasQuestItems(ORB_OF_BINDING)) {
                ...
                else { st.set("cornerstone", Integer.toString(cornerstones)); }
            }
            else { htmltext = null; }
        }
        // Spawn Pure Unicorn
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (st.hasQuestItems(VIRGIL_LETTER)) {
                    if (!player.isSubClassActive() || player.getLevel() < 60) { htmltext = "31742-02.htm"; }
                    else {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case VIRGIL:
                        if (cond == 1) { htmltext = "31742-04.htm"; }
                        else if (cond == 2) { htmltext = "31742-05.htm"; }
                        break;

                    case KASSANDRA:
                        if (cond == 1) { htmltext = "31743-01.htm"; }
                        else if (cond == 2) { htmltext = "31743-06.htm"; }
                        else if (cond == 11) {
                    ...
                    case OGMAR:
                        if (cond == 2) { htmltext = "31744-01.htm"; }
                        else if (cond == 3) { htmltext = "31744-03.htm"; }
                        break;

                    case MYSTERIOUS_KNIGHT:
                        if (cond == 3) { htmltext = "31751-01.htm"; }
                        else if (cond == 4) { htmltext = "31751-03.htm"; }
                        else if (cond == 5) {
                        ...
                        else if (cond == 6) { htmltext = "31751-05.htm"; }
                        break;
                    ...
                    case ANGEL_CORPSE:
                        ...
                        else if (cond == 5) { htmltext = "31752-01.htm"; }
                        break;

                    case KALIS:
                        if (cond == 6) { htmltext = "30759-01.htm"; }
                        else if (cond == 7) { htmltext = "30759-03.htm"; }
                        else if (cond == 8) {
                            if (st.hasQuestItems(SORCERY_INGREDIENT)) { htmltext = "30759-04.htm"; }
                            else {
                                htmltext = "30759-03.htm";
                                st.set("cond", "7");
                            }
                        }
                        else if (cond == 9) { htmltext = "30759-06.htm"; }
                        break;

                    case MATILD:
                        if (cond == 7) { htmltext = "30738-01.htm"; }
                        else if (cond == 8) { htmltext = "30738-03.htm"; }
                        break;

                    case CORNERSTONE:
                        if (cond == 9) {
                            if (st.hasQuestItems(ORB_OF_BINDING)) { htmltext = "31748-02.htm"; }
                            else { htmltext = "31748-01.htm"; }
                        }
                        break;

                    case FALLEN_UNICORN:
                        if (cond == 9) { htmltext = "31746-01.htm"; }
                        else if (cond == 10) {
                    ...
                    case PURE_UNICORN:
                        ...
                        else if (cond == 11) { htmltext = "31747-02.htm"; }
                        break;
                }
                break;
