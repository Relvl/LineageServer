...
public class Q384_WarehouseKeepersPastime extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30182-05.htm")) {
            ...
            if (st.getQuestItemsCount(MEDAL) < 10) { htmltext = npcId + "-12.htm"; }
            else {
                st.set("bet", "10");
            ...
            if (st.getQuestItemsCount(MEDAL) < 100) { htmltext = npcId + "-12.htm"; }
            else {
                st.set("bet", "100");
                st.set("board", StringUtil.scrambleString("123456789"));
        ...
        else if (event.startsWith("select_2-")) // pick #2-5
        {
            ...
            if (Util.contains(playerArray.split(""), number)) { htmltext = fillBoard(st, getHtmlText(npcId + "-" + (14 + 2 * playerArray.length()) + ".htm")); }
            else {
                // Stores the final String.
                st.set("playerArray", playerArray.concat(number));

                htmltext = fillBoard(st, getHtmlText(npcId + "-" + (11 + 2 * (playerArray.length() + 1)) + ".htm"));
            }
        }
        else if (event.startsWith("select_3-")) // pick #6
        {
            ...
            if (Util.contains(playerArray.split(""), number)) { htmltext = fillBoard(st, getHtmlText(npcId + "-26.htm")); }
            else {
                ...
                for (int[] map : INDEX_MAP) {
                    ...
                    boolean won = true;
                    for (int index : map) { won &= Util.contains(playerChoice, board[index]); }
                ...
                if (winningLines == 3) {
                    ...
                    for (int[] reward : st.get("bet") == "10" ? _rewards_10_win : _rewards_100_win) {
                        if (chance < reward[0]) {
                            ...
                            if (reward[1] == 2437) { st.giveItems(2463, 1); }
                ...
                else { htmltext = getHtmlText(npcId + "-24.htm"); }
    ...
    private static final String fillBoard(QuestState st, String htmltext) {
        ...
        for (int i = 1; i < 10; i++) { htmltext = htmltext.replace("<?Cell" + i + "?>", (Util.contains(playerArray, board[i])) ? board[i] : "?"); }
