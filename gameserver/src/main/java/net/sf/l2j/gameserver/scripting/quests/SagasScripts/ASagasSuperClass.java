...
public abstract class ASagasSuperClass extends Quest {
    public int[] NPC = { };
    public int[] Items = { };
    public int[] Mob = { };
    public int classid;
    ...
    protected ASagasSuperClass(int id, String descr) {
        super(id, descr);
    }
    ...
    public void registerNPCs() {
        ...
        for (int npc : NPC) { addTalkId(npc); }
        for (int mobid : Mob) { addKillId(mobid); }

        int[] questItemIds = Items.clone();
        questItemIds[0] = 0;
        questItemIds[2] = 0; // remove Ice Crystal and Divine Stone of Wisdom
        setItemsIds(questItemIds);

        for (int Archon_Minion = 21646; Archon_Minion < 21652; Archon_Minion++) { addKillId(Archon_Minion); }
        for (int element : Archon_Hellisha_Norm) { addKillId(element); }
        for (int Guardian_Angel = 27214; Guardian_Angel < 27217; Guardian_Angel++) { addKillId(Guardian_Angel); }
    }
    ...
    public void DeleteSpawn(L2Npc npc) {
    ...
    public QuestState findRightState(L2Npc npc) {
        if (_SpawnList.containsKey(npc)) {
            ...
            if (player != null) { return player.getQuestState(getName()); }
        }
        return null;
    }
    ...
    public void giveHallishaMark(QuestState st2) {
        if (st2.getInt("spawned") == 0) {
            if (st2.getQuestItemsCount(Items[3]) >= 700) {
                ...
                st2.playSound(ESound.ItemSound_quest_itemget);
            }
        }
    }
    ...
    public QuestState findQuest(L2PcInstance player) {
        ...
        if (st != null && player.getClassId().getId() == QuestClass[getQuestId() - 70]) {
            return st;
        }
        return null;
    }
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        String htmltext = "";
        if (st != null) {
            if (event.equalsIgnoreCase("0-011.htm") || event.equalsIgnoreCase("0-012.htm") || event.equalsIgnoreCase("0-013.htm") || event.equalsIgnoreCase("0-014.htm") || event.equalsIgnoreCase("0-015.htm")) { htmltext = event; }
                ...
                if (player.getLevel() < 76) {
                    htmltext = "0-02.htm";
                    if (st.isCreated()) { st.exitQuest(true); }
                }
                else { htmltext = "0-05.htm"; }
            }
                ...
                if (player.getLevel() >= 76) {
                    ...
                    if (!player.isSubClassActive() && player.getBaseClass() == getPrevClass(player)) { player.setBaseClass(Class); }
                ...
                if (Items[11] != 0) { st.takeItems(Items[11], 1); }
                ...
                if (st.getInt("spawned") == 1) { htmltext = "7-03.htm"; }
                ...
                else if (st.getInt("Quest1") == 45) { htmltext = "10-03.htm"; }
                else { htmltext = "10-04.htm"; }
            }
                ...
                if (st.getInt("Quest1") == 45) { st.set("Quest1", "0"); }
                return null;
            }
                ...
                if (st.getInt("Quest0") == 0) {
                    ...
                    if (Rnd.get(2) == 0) { npc.broadcastNpcSay(Text[9].replace("PLAYERNAME", player.getName())); }
                    else { npc.broadcastNpcSay(Text[10].replace("PLAYERNAME", player.getName())); }
                }
                return null;
            }
                ...
                if (st.getInt("Quest0") == 1 || st.getInt("Quest0") == 2 || st.getInt("Quest1") > 3) {
                    st.set("Quest0", "0");
                    if (st.getInt("Quest0") == 1) { npc.broadcastNpcSay(Text[11]); }
                    else { npc.broadcastNpcSay(Text[12]); }
                    DeleteSpawn(npc);
                }
                else { startQuestTimer("Mob_2 has despawned", 1000, npc, player, false); }
                return null;
            }
        }
        else { return null; }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        if (st != null) {
            ...
            if (st.isCompleted() && npcId == NPC[0]) {
                ...
                if (cond == 0) {
                    if (npcId == NPC[0]) { htmltext = "0-01.htm"; }
                }
                else if (cond == 1) {
                    if (npcId == NPC[0]) { htmltext = "0-04.htm"; }
                    else if (npcId == NPC[2]) { htmltext = "2-01.htm"; }
                }
                else if (cond == 2) {
                    if (npcId == NPC[2]) { htmltext = "2-02.htm"; }
                    else if (npcId == NPC[1]) { htmltext = "1-01.htm"; }
                }
                else if (cond == 3) {
                    if (npcId == NPC[1] && st.hasQuestItems(Items[0])) {
                        htmltext = "1-02.htm";
                        if (Items[11] == 0 || st.hasQuestItems(Items[11])) { htmltext = "1-03.htm"; }
                    }
                }
                else if (cond == 4) {
                    if (npcId == NPC[1]) { htmltext = "1-04.htm"; }
                    else if (npcId == NPC[2]) { htmltext = "2-03.htm"; }
                }
                else if (cond == 5) {
                    if (npcId == NPC[2]) { htmltext = "2-04.htm"; }
                    else if (npcId == NPC[5]) { htmltext = "5-01.htm"; }
                }
                else if (cond == 6) {
                    if (npcId == NPC[5]) { htmltext = "5-03.htm"; }
                    else if (npcId == NPC[6]) { htmltext = "6-01.htm"; }
                }
                else if (cond == 7) {
                    if (npcId == NPC[6]) { htmltext = "6-02.htm"; }
                }
                else if (cond == 8) {
                    if (npcId == NPC[6]) { htmltext = "6-04.htm"; }
                    else if (npcId == NPC[7]) { htmltext = "7-01.htm"; }
                }
                else if (cond == 9) {
                    if (npcId == NPC[7]) { htmltext = "7-05.htm"; }
                }
                else if (cond == 10) {
                    if (npcId == NPC[7]) { htmltext = "7-07.htm"; }
                    else if (npcId == NPC[3]) { htmltext = "3-01.htm"; }
                }
                else if (cond == 11 || cond == 12) {
                    if (npcId == NPC[3]) {
                        if (st.hasQuestItems(Items[2])) { htmltext = "3-05.htm"; }
                        else { htmltext = "3-04.htm"; }
                    }
                }
                else if (cond == 13) {
                    if (npcId == NPC[3]) { htmltext = "3-06.htm"; }
                    else if (npcId == NPC[8]) { htmltext = "8-01.htm"; }
                }
                else if (cond == 14) {
                    if (npcId == NPC[8]) { htmltext = "8-03.htm"; }
                    else if (npcId == NPC[11]) { htmltext = "11-01.htm"; }
                }
                else if (cond == 15) {
                    if (npcId == NPC[11]) { htmltext = "11-02.htm"; }
                    else if (npcId == NPC[9]) { htmltext = "9-01.htm"; }
                }
                else if (cond == 16) {
                    if (npcId == NPC[9]) { htmltext = "9-02.htm"; }
                }
                else if (cond == 17) {
                    if (npcId == NPC[9]) { htmltext = "9-04.htm"; }
                    else if (npcId == NPC[10]) { htmltext = "10-01.htm"; }
                }
                else if (cond == 18) {
                    if (npcId == NPC[10]) { htmltext = "10-05.htm"; }
                }
                else if (cond == 19) {
                    if (npcId == NPC[10]) { htmltext = "10-07.htm"; }
                    else if (npcId == NPC[0]) { htmltext = "0-06.htm"; }
                }
                else if (cond == 20) {
                    if (npcId == NPC[0]) {
                        if (player.getLevel() >= 76) {
                            ...
                            player.setClassId(Class);
                            if (!player.isSubClassActive() && player.getBaseClass() == prevClass) { player.setBaseClass(Class); }
                            player.broadcastUserInfo();
                            cast(npc, player, 4339, 1);
                        }
                        else { htmltext = "0-010.htm"; }
                    }
                }
            }
        }
            ...
            if (npcId == NPC[4]) {
                if (cond == 17) {
                    ...
                    if (st2 != null) {
                        ...
                        if (st == st2) {
                            if (st.getInt("Tab") == 1) {
                                if (st.getInt("Quest0") == 0) { htmltext = "4-04.htm"; }
                                else if (st.getInt("Quest0") == 1) { htmltext = "4-06.htm"; }
                            }
                            else {
                                if (st.getInt("Quest0") == 0) { htmltext = "4-01.htm"; }
                                else if (st.getInt("Quest0") == 1) { htmltext = "4-03.htm"; }
                            }
                        }
                        else {
                            if (st.getInt("Tab") == 1) {
                                if (st.getInt("Quest0") == 0) { htmltext = "4-05.htm"; }
                                else if (st.getInt("Quest0") == 1) { htmltext = "4-07.htm"; }
                            }
                            else {
                                if (st.getInt("Quest0") == 0) { htmltext = "4-02.htm"; }
                            }
                        }
                    }
                }
                else if (cond == 18) { htmltext = "4-08.htm"; }
            }
        }
        if (htmltext == "") { npc.showChatWindow(player); }
        return htmltext;
    }
    ...
    public String onAttack(L2Npc npc, L2PcInstance player, int damage, boolean isPet) {
        ...
        if (npcId == Mob[2] && st == st2 && cond == 17) {
            ...
            if (st.getInt("Quest0") == 1) { npc.broadcastNpcSay(Text[16].replace("PLAYERNAME", player.getName())); }
    ...
    public String onSkillSee(L2Npc npc, L2PcInstance player, L2Skill skill, L2Object[] targets, boolean isPet) {
        if (_SpawnList.containsKey(npc) && _SpawnList.get(npc) != player.getObjectId()) {
            ...
            if (quest_player == null) { return null; }
                ...
                if (party != null) {
                    ...
                    for (L2PcInstance player1 : party.getPartyMembers()) {
                        ...
                        if (st1 != null && player1.isInsideRadius(player, Config.ALT_PARTY_RANGE2, false, false)) {
                            if (st1.getInt("cond") == 15) { PartyQuestMembers.add(st1); }
                        }
                    }
                    ...
                    if (st1 != null) {
                        if (st1.getInt("cond") == 15) { giveHallishaMark(st1); }
                    }
                }
            ...
            if (npcId == element) {
                ...
                if (st1 != null) {
                    if (st1.getInt("cond") == 15) {
                        ...
                        st1.set("cond", "16");
                        st1.playSound(ESound.ItemSound_quest_middle);
                    }
                }
                return super.onKill(npc, player, isPet);
            }
            ...
            if (npcId == Guardian_Angel) {
                ...
                if (st1 != null) {
                    if (st1.getInt("cond") == 6) {
                        if (st1.getInt("kills") < 9) { st1.set("kills", String.valueOf(st1.getInt("kills") + 1)); }
                        else {
                            st1.playSound(ESound.ItemSound_quest_middle);
        ...
        if (st != null && npcId != Mob[2]) {
            ...
            if (npcId == Mob[0] && cond == 8) {
                ...
                if (!player.isInParty()) {
                    ...
                    else { npc.broadcastNpcSay(Text[5]); }
                }
                cancelQuestTimer("Archon Hellisha has despawned", npc, st2.getPlayer());
                st2.set("spawned", "0");
                DeleteSpawn(npc);
