...
import net.sf.l2j.gameserver.geoengine.geodata.blocks.*;
...
public final class GeoDriverBlock extends GeoData {
    ...
    public GeoDriverBlock() {
        ...
        LOGGER.info("GeoDriverBlock: Loaded " + loaded + " " + Config.GEODATA_FORMAT.toString() + " region files.");
    ...
    private final boolean loadGeoBlocks(int regionX, int regionY) {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(Config.GEODATA_PATH + filename, "r"); FileChannel fc = raf.getChannel()) {
            ...
            if (buffer.remaining() > 0) { LOGGER.warn("GeoDriverBlock: Region file " + filename + " can be corrupted, remaining " + buffer.remaining() + " bytes to read."); }
            ...
            LOGGER.error("GeoDriverBlock: Error while loading " + filename + " region file.", e);
