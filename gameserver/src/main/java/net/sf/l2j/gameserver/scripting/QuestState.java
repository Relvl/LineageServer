...
public final class QuestState {
    ...
    public void exitQuest(boolean repeatable) {
        ...
        else { setState(Quest.STATE_COMPLETED); }
    ...
    public void addNotifyOfDeath() {
        if (_player != null) { _player.addNotifyQuestOfDeath(this); }
    }
    ...
    private void setCond(int cond, int old) {
        ...
        if (cond < 3 || cond > 31) { unset("__compltdStateFlags"); }
        else { completedStateFlags = getInt("__compltdStateFlags"); }
        ...
        if (_quest.isRealQuest() && cond > 0) { _player.sendPacket(new ExShowQuestMark(_quest.getQuestId())); }
    }
    ...
    public void unset(String var) {
        if (_vars.remove(var) != null) { removeQuestVarInDb(var); }
    }
    ...
    public int getQuestItemsCount(int itemId) {
        ...
        for (L2ItemInstance item : _player.getInventory().getItems()) { if (item != null && item.getItemId() == itemId) { count += item.getCount(); } }
     ...
     */
    public int getItemEquipped(EPaperdollSlot loc) {
    ...
    public void takeItems(int itemId, int itemCount) {
        ...
        if (item.isEquipped()) {
            ...
            for (L2ItemInstance itm : unequiped) { iu.addModifiedItem(itm); }
    ...
    public boolean dropItems(int itemId, int count, int neededCount, int dropChance, byte type) {
        ...
        if (amount > 0) {
            ...
            if (!_player.getInventory().validateCapacityByItemId(itemId)) { return false; }
    ...
    public boolean dropMultipleItems(int[][] rewardsInfos, byte type) {
        ...
        for (int[] info : rewardsInfos) {
            ...
            if (amount > 0) {
                // Limit count to reach required amount.
                if (neededCount > 0) { amount = ((currentCount + amount) >= neededCount) ? neededCount - currentCount : amount; }

                // Inventory slot check.
                if (!_player.getInventory().validateCapacityByItemId(itemId)) { continue; }
                ...
                if (neededCount <= 0 || ((currentCount + amount) < neededCount)) { reached = false; }
            }
        }
