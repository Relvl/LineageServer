...
public class Q365_DevilsLegacy extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30095-02.htm")) {
            ...
            if (!st.hasQuestItems(PIRATE_TREASURE_CHEST)) { htmltext = "30092-02.htm"; }
            else if (st.getQuestItemsCount(ItemConst.ADENA_ID) < 600) { htmltext = "30092-03.htm"; }
            else {
                ...
                if (Rnd.get(100) < 80) {
                    i0 = Rnd.get(100);
                    if (i0 < 1) { st.giveItems(955, 1); }
                    else if (i0 < 4) { st.giveItems(956, 1); }
                    else if (i0 < 36) { st.giveItems(1868, 1); }
                    else if (i0 < 68) { st.giveItems(1884, 1); }
                    else { st.giveItems(1872, 1); }

                    htmltext = "30092-05.htm";
                }
                else {
                    i0 = Rnd.get(1000);
                    if (i0 < 10) { st.giveItems(951, 1); }
                    else if (i0 < 40) { st.giveItems(952, 1); }
                    else if (i0 < 60) { st.giveItems(955, 1); }
                    else if (i0 < 260) { st.giveItems(956, 1); }
                    else if (i0 < 445) { st.giveItems(1879, 1); }
                    else if (i0 < 630) { st.giveItems(1880, 1); }
                    else if (i0 < 815) { st.giveItems(1882, 1); }
                    else { st.giveItems(1881, 1); }

                    htmltext = "30092-06.htm";

                    // Curse effect !
                    L2Skill skill = SkillTable.getInfo(4082, 1);
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                switch (npc.getNpcId()) {
                    case RANDOLF:
                        if (!st.hasQuestItems(PIRATE_TREASURE_CHEST)) { htmltext = "30095-03.htm"; }
                        else {
