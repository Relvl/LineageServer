    ...
    public boolean useAdminCommand(String command, L2PcInstance activeChar) {
        if (command.startsWith("admin_buy")) {
            try {
                ...
                if (list == null) { activeChar.sendMessage("Invalid buylist id."); }
                else { activeChar.sendPacket(new BuyList(list, activeChar.getAdena(), 0)); }
            }
            catch (Exception e) {
                activeChar.sendMessage("Invalid buylist id.");
            }
        }
        else if (command.equals("admin_gmshop")) { AdminHelpPage.showHelpPage(activeChar, "gmshops.htm"); }
