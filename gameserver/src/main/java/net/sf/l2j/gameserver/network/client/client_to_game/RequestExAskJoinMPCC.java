...
import net.sf.l2j.gameserver.network.client.game_to_client.ExAskJoinMPCC;
...
public final class RequestExAskJoinMPCC extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (activeChar.isInParty()) {
            ...
            if (activeParty.getLeader().equals(activeChar)) {
                ...
                if (activeParty.isInCommandChannel() && activeParty.getCommandChannel().getChannelLeader().equals(activeChar)) {
                    ...
                    if (player.isInParty()) {
                        ...
                        if (player.getParty().isInCommandChannel()) { activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_ALREADY_MEMBER_OF_COMMAND_CHANNEL).addPcName(player)); }
                        // ready to open a new CC, send request to targets Party's PartyLeader
                        else { askJoinMPCC(activeChar, player); }
                    }
                    else { activeChar.sendMessage(player.getName() + " doesn't have party and cannot be invited to Command Channel."); }
                }
                // in CC, but not the CCLeader
                else if (activeParty.isInCommandChannel() && !activeParty.getCommandChannel().getChannelLeader().equals(activeChar)) { activeChar.sendPacket(SystemMessageId.CANNOT_INVITE_TO_COMMAND_CHANNEL); }
                else {
                    // target in a party?
                    if (player.isInParty()) {
            ...
            else { activeChar.sendPacket(SystemMessageId.CANNOT_INVITE_TO_COMMAND_CHANNEL); }
        }
    }
    ...
    private static void askJoinMPCC(L2PcInstance requestor, L2PcInstance target) {
        ...
        else { requestor.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_IS_BUSY_TRY_LATER).addPcName(targetLeader)); }
    }
}