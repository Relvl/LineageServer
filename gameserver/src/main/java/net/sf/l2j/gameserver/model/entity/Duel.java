...
public class Duel {
    ...
    private static final PlaySound B04_S01 = new PlaySound(ESound.B04_S01);
    ...
    protected Future<?> _startTask;
    protected Future<?> _checkTask;
    ...
    private static class PlayerCondition {
        ...
        public void restoreCondition(boolean abnormalEnd) {
            ...
            if (_debuffs != null) {
                for (L2Effect skill : _debuffs) { if (skill != null) { skill.exit(); } }
            }
        }
        ...
        public void teleportBack() {
            if (_x != 0 && _y != 0) { _player.teleToLocation(_x, _y, _z, 0); }
        }
    ...
    private class CheckTask implements Runnable {
        ...
        @Override
        public void run() {
            DuelResult status = checkEndDuelCondition();
            ...
            if (status != DuelResult.CONTINUE) {
                ...
                if (status != DuelResult.CANCELED) { playAnimations(); }
    ...
    private void savePlayerConditions() {
        if (_partyDuel) {
            for (L2PcInstance partyPlayer : _playerA.getParty().getPartyMembers()) { _playerConditions.add(new PlayerCondition(partyPlayer, _partyDuel)); }

            for (L2PcInstance partyPlayer : _playerB.getParty().getPartyMembers()) { _playerConditions.add(new PlayerCondition(partyPlayer, _partyDuel)); }
        }
        else {
    ...
    private void restorePlayerConditions(boolean abnormalEnd) {
        ...
        if ((!_partyDuel && !abnormalEnd) || _partyDuel) {
            for (PlayerCondition cond : _playerConditions) { cond.restoreCondition(abnormalEnd); }
        }
    }
    ...
    public void broadcastToTeam1(L2GameServerPacket packet) {
        if (_partyDuel && _playerA.getParty() != null) {
            for (L2PcInstance partyPlayer : _playerA.getParty().getPartyMembers()) { partyPlayer.sendPacket(packet); }
        }
    ...
    public void broadcastToTeam2(L2GameServerPacket packet) {
        if (_partyDuel && _playerB.getParty() != null) {
            for (L2PcInstance partyPlayer : _playerB.getParty().getPartyMembers()) { partyPlayer.sendPacket(packet); }
        }
    ...
    protected void playAnimations() {
        if (_playerA.isOnline()) {
            if (_playerA.getDuelState() == DuelState.WINNER) {
                if (_partyDuel && _playerA.getParty() != null) {
                    for (L2PcInstance partyPlayer : _playerA.getParty().getPartyMembers()) { partyPlayer.broadcastPacket(new SocialAction(partyPlayer, 3)); }
                }
                else { _playerA.broadcastPacket(new SocialAction(_playerA, 3)); }
            }
            else if (_playerA.getDuelState() == DuelState.DEAD) {
                if (_partyDuel && _playerA.getParty() != null) {
                    for (L2PcInstance partyPlayer : _playerA.getParty().getPartyMembers()) { partyPlayer.broadcastPacket(new SocialAction(partyPlayer, 7)); }
                }
                else { _playerA.broadcastPacket(new SocialAction(_playerA, 7)); }
            }
        }
        ...
        if (_playerB.isOnline()) {
            if (_playerB.getDuelState() == DuelState.WINNER) {
                if (_partyDuel && _playerB.getParty() != null) {
                    for (L2PcInstance partyPlayer : _playerB.getParty().getPartyMembers()) { partyPlayer.broadcastPacket(new SocialAction(partyPlayer, 3)); }
                }
                else { _playerB.broadcastPacket(new SocialAction(_playerB, 3)); }
            }
            else if (_playerB.getDuelState() == DuelState.DEAD) {
                if (_partyDuel && _playerB.getParty() != null) {
                    for (L2PcInstance partyPlayer : _playerB.getParty().getPartyMembers()) { partyPlayer.broadcastPacket(new SocialAction(partyPlayer, 7)); }
                }
                else { _playerB.broadcastPacket(new SocialAction(_playerB, 7)); }
            }
        }
    }
    ...
    protected void endDuel(DuelResult result) {
        ...
        switch (result) {
            case TEAM_2_SURRENDER:
                sm = SystemMessage.getSystemMessage((_partyDuel) ? SystemMessageId.SINCE_S1_PARTY_WITHDREW_FROM_THE_DUEL_S2_PARTY_HAS_WON : SystemMessageId.SINCE_S1_WITHDREW_FROM_THE_DUEL_S2_HAS_WON)
                                  .addString(_playerB.getName())
                                  .addString(_playerA.getName());
                broadcastToTeam1(sm);
                broadcastToTeam2(sm);
            case TEAM_1_WIN:
    ...
    protected DuelResult checkEndDuelCondition() {
        ...
        if (!_playerA.isOnline() && !_playerB.isOnline()) { return DuelResult.CANCELED; }
        ...
        if (_surrenderRequest != 0) { return (_surrenderRequest == 1) ? DuelResult.TEAM_1_SURRENDER : DuelResult.TEAM_2_SURRENDER; }

        // Duel timed out.
        if (getRemainingTime() <= 0) { return DuelResult.TIMEOUT; }

        // One of the players is declared winner.
        if (_playerA.getDuelState() == DuelState.WINNER) { return DuelResult.TEAM_1_WIN; }

        if (_playerB.getDuelState() == DuelState.WINNER) { return DuelResult.TEAM_2_WIN; }

        if (!_partyDuel) {
            // Duel was interrupted e.g.: player was attacked by mobs / other players
            if (_playerA.getDuelState() == DuelState.INTERRUPTED || _playerB.getDuelState() == DuelState.INTERRUPTED) { return DuelResult.CANCELED; }

            // Players are too far apart.
            if (!_playerA.isInsideRadius(_playerB, 2000, false, false)) { return DuelResult.CANCELED; }

            // One of the players is engaged in PvP.
            if (_playerA.getPvpFlag() != 0 || _playerB.getPvpFlag() != 0) { return DuelResult.CANCELED; }
            ...
            if (_playerA.getParty() != null) {
                for (L2PcInstance partyMember : _playerA.getParty().getPartyMembers()) {
                    // Duel was interrupted e.g.: player was attacked by mobs / other players
                    if (partyMember.getDuelState() == DuelState.INTERRUPTED) { return DuelResult.CANCELED; }

                    // Players are too far apart.
                    if (!partyMember.isInsideRadius(_playerB, 2000, false, false)) { return DuelResult.CANCELED; }

                    // One of the players is engaged in PvP.
                    if (partyMember.getPvpFlag() != 0) { return DuelResult.CANCELED; }

                    // One of the players is in a Siege, Peace or PvP zone.
                    if (partyMember.isInsideZone(ZoneId.PEACE) || partyMember.isInsideZone(ZoneId.PEACE) || partyMember.isInsideZone(ZoneId.SIEGE)) { return DuelResult.CANCELED; }
                }
            }
            ...
            if (_playerB.getParty() != null) {
                for (L2PcInstance partyMember : _playerB.getParty().getPartyMembers()) {
                    ...
                    // Players are too far apart.
                    if (!partyMember.isInsideRadius(_playerA, 2000, false, false)) { return DuelResult.CANCELED; }
    ...
    public void doSurrender(L2PcInstance player) {
        ...
        if (_partyDuel) {
            if (_playerA.getParty().getPartyMembers().contains(player)) {
                ...
                for (L2PcInstance partyPlayer : _playerA.getParty().getPartyMembers()) { partyPlayer.setDuelState(DuelState.DEAD); }

                for (L2PcInstance partyPlayer : _playerB.getParty().getPartyMembers()) { partyPlayer.setDuelState(DuelState.WINNER); }
            }
            else if (_playerB.getParty().getPartyMembers().contains(player)) {
                _surrenderRequest = 2;

                for (L2PcInstance partyPlayer : _playerB.getParty().getPartyMembers()) { partyPlayer.setDuelState(DuelState.DEAD); }

                for (L2PcInstance partyPlayer : _playerA.getParty().getPartyMembers()) { partyPlayer.setDuelState(DuelState.WINNER); }
            }
        }
        else {
            if (player == _playerA) {
    ...
    public void onPlayerDefeat(L2PcInstance player) {
        ...
        if (_partyDuel) {
            ...
            if (teamDefeated) {
                ...
                for (L2PcInstance partyPlayer : winner.getParty().getPartyMembers()) { partyPlayer.setDuelState(DuelState.WINNER); }
            }
        }
        else {
            if (_playerA == player) { _playerB.setDuelState(DuelState.WINNER); }
            else { _playerA.setDuelState(DuelState.WINNER); }
        }
    }
