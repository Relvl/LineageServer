...
public final class OlympiadGameTask implements Runnable {
    private static final Logger LOGGER = LoggerFactory.getLogger(OlympiadGameTask.class);
        ...
        try {
            ...
            switch (_state) {
                ...
                case TELE_TO_ARENA: {
                    ...
                    if (_countDown <= 0) { _state = GameState.GAME_STARTED; }
                    break;
                }
                ...
                case BATTLE_COUNTDOWN: {
                    ...
                    if (_countDown == 20) { _game.buffAndHealPlayers(); }

                    delay = getDelay(BATTLE_START_TIME);
                    if (_countDown <= 0) { _state = GameState.BATTLE_STARTED; }
                ...
                case BATTLE_STARTED: {
                    ...
                    if (!startBattle()) { _state = GameState.GAME_STOPPED; }
                ...
                case BATTLE_IN_PROGRESS: {
                    _countDown += 1000;
                    if (checkBattle() || _countDown > Config.ALT_OLY_BATTLE) { _state = GameState.GAME_STOPPED; }
                ...
                case TELE_TO_TOWN: {
                    ...
                    delay = getDelay(TELEPORT_TO_TOWN);
                    if (_countDown <= 0) { _state = GameState.CLEANUP; }
    ...
    private final boolean startGame() {
        try {
            ...
            if (_game.checkDefaulted()) { return false; }

            if (!_game.portPlayersToArena(_zone.getSpawns())) { return false; }
