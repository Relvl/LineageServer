...
public class Q421_LittleWingsBigAdventure extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30610-06.htm")) {
            ...
            if (summon != null) { htmltext = (summon.getControlItemId() == st.getInt("summonOid")) ? "30747-04.htm" : "30747-03.htm"; }
        }
        else if (event.equalsIgnoreCase("30747-05.htm")) {
            L2Summon summon = player.getPet();
            if (summon == null || summon.getControlItemId() != st.getInt("summonOid")) { htmltext = "30747-06.htm"; }
            else {
                st.set("cond", "2");
                st.set("iCond", "3");
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                // Wrong level.
                if (player.getLevel() < 45) { htmltext = "30610-01.htm"; }
                // Got more than one flute, or none.
                else if (st.getQuestItemsCount(3500) + st.getQuestItemsCount(3501) + st.getQuestItemsCount(3502) != 1) { htmltext = "30610-02.htm"; }
                else {
                    // Find the level of the hatchling.
                    for (int i = 3500; i < 3503; i++) {
                        L2ItemInstance item = player.getInventory().getItemByItemId(i);
                        if (item != null && item.getEnchantLevel() >= 55) { return "30610-04.htm"; }
                    }
            ...
            case QuestState.STATE_STARTED:
                switch (npc.getNpcId()) {
                    ...
                    case MIMYU:
                        ...
                        {
                            L2Summon summon = player.getPet();
                            if (summon == null) { return "30747-12.htm"; }

                            if (summon.getControlItemId() != st.getInt("summonOid")) { return "30747-14.htm"; }
                        ...
                        {
                            L2Summon summon = player.getPet();
                            if (summon != null && summon.getControlItemId() == st.getInt("summonOid")) { return "30747-15.htm"; }

                            if (st.getQuestItemsCount(3500) + st.getQuestItemsCount(3501) + st.getQuestItemsCount(3502) > 1) { return "30747-17.htm"; }
                            ...
                            L2Skill skill = SkillTable.getInfo(4167, 1);
                            if (skill != null && player.getFirstEffect(skill) == null) { skill.getEffects(npc, player); }
                        }
                        break;
                }
                break;
    ...
    public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isPet) {
        ...
        if (((L2MonsterInstance) npc).hasMinions()) {
            for (L2MonsterInstance ghost : ((L2MonsterInstance) npc).getMinionList().getSpawnedMinions()) {
                if (!ghost.isDead() && Rnd.get(100) < 1) { ghost.broadcastNpcSay("We must protect the fairy tree!"); }
            }
        }
        ...
        if (isPet && attacker.getPet().getControlItemId() == st.getInt("summonOid")) {
            ...
            if (Rnd.get(100) < 1 && st.hasQuestItems(FAIRY_LEAF)) {
                int idMask = (int) Math.pow(2, npc.getNpcId() - 27182 - 1);
    ...
    public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet) {
        ...
        if (Rnd.get(100) < 30) {
            if (originalKiller != null) {
                L2Skill skill = SkillTable.getInfo(4243, 1);
                if (skill != null && originalKiller.getFirstEffect(skill) == null) { skill.getEffects(npc, originalKiller); }
            }
        }

        // Spawn 20 ghosts, attacking the killer.
        for (int i = 0; i < 20; i++) {
            L2Attackable newNpc = (L2Attackable) addSpawn(27189, npc, true, 300000, false);
