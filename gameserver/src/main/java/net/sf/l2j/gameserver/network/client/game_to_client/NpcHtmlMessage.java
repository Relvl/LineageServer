...
public final class NpcHtmlMessage extends L2GameServerPacket {
    ...
    public void runImpl() {
        ...
        for (int i = 0; i < _html.length(); i++) {
            ...
            if (_html.substring(start + 8, start + 10).equals("-h")) { start += 11; }
            else { start += 8; }

            i = finish;
            int finish2 = _html.indexOf("$", start);
            if (finish2 < finish && finish2 > 0) { activeChar.addBypass2(_html.substring(start, finish2).trim()); }
            else { activeChar.addBypass(_html.substring(start, finish).trim()); }
        }
    }
