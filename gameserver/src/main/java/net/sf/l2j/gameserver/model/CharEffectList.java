...
public class CharEffectList {
    ...
    public final L2Effect[] getAllEffects() {
        ...
        if ((_buffs == null || _buffs.isEmpty()) && (_debuffs == null || _debuffs.isEmpty())) { return EMPTY_EFFECTS; }

        synchronized (_buildEffectLock) {
            // If we dont need to rebuild the cache, just return the current one.
            if (!_rebuildCache) { return _effectCache; }
            ...
            if (_buffs != null && !_buffs.isEmpty()) { temp.addAll(_buffs); }
            if (_debuffs != null && !_debuffs.isEmpty()) { temp.addAll(_debuffs); }
    ...
    private boolean doesStack(L2Skill checkSkill) {
        if (_buffs == null || _buffs.isEmpty()) { return false; }

        if (checkSkill._effectTemplates == null || checkSkill._effectTemplates.isEmpty()) { return false; }

        final String stackType = checkSkill._effectTemplates.get(0).stackType;
        if (stackType == null || "none".equals(stackType)) { return false; }

        for (L2Effect e : _buffs) {
            if (e.getStackType() != null && e.getStackType().equals(stackType)) { return true; }
        }
        return false;
    }
    ...
    public int getDanceCount() {
        ...
        int danceCount = 0;
        for (L2Effect e : _buffs) {
            if (e != null && e.getSkill().isDance() && e.getInUse()) { danceCount++; }
        }
        return danceCount;
    }
    ...
    public final void stopAllEffectsExceptThoseThatLastThroughDeath() {
        ...
        // Exit them
        for (L2Effect e : effects) {
            if (e != null && !e.getSkill().isStayAfterDeath()) { e.exit(true); }
        }
    }
    ...
    public final void stopEffects(L2EffectType type) {
        if (_buffs != null && !_buffs.isEmpty()) {
            for (L2Effect e : _buffs) {
                ...
                if (e != null && e.getEffectType() == type) { e.exit(); }
            }
        }
    ...
    public final void stopSkillEffects(int skillId) {
        if (_buffs != null && !_buffs.isEmpty()) {
            for (L2Effect e : _buffs) {
                if (e != null && e.getSkill().getId() == skillId) { e.exit(); }
            }
        }
    ...
    public void stopEffectsOnAction() {
        if (_hasBuffsRemovedOnAnyAction) {
            if (_buffs != null && !_buffs.isEmpty()) {
                for (L2Effect e : _buffs) {
                    if (e != null && e.getSkill().isRemovedOnAnyActionExceptMove()) { e.exit(true); }
                }
            }
        }
    }
    ...
    public void stopEffectsOnDamage(boolean awake) {
        if (_hasBuffsRemovedOnDamage) {
            if (_buffs != null && !_buffs.isEmpty()) {
                for (L2Effect e : _buffs) {
                    if (e != null && e.getSkill().isRemovedOnDamage() && (awake || e.getSkill().getSkillType() != L2SkillType.SLEEP)) { e.exit(true); }
                }
            }
        }
    ...
    public void updateEffectIcons(boolean partyOnly) {
        if (_buffs == null && _debuffs == null) { return; }

        if (partyOnly) { _partyOnly = true; }
    ...
    public void queueEffect(L2Effect effect, boolean remove) {
        ...
        if (!_queuesInitialized) { init(); }

        if (remove) { _removeQueue.offer(effect); }
        else { _addQueue.offer(effect); }
    ...
    private void queueRunner() {
        if (!queueLock.compareAndSet(false, true)) { return; }
    ...
    protected void removeEffectFromQueue(L2Effect effect) {
        ...
        if ("none".equals(effect.getStackType())) {
            ...
            if (index >= 0) {
                ...
                if (stackQueue.isEmpty()) { _stackedEffects.remove(effect.getStackType()); }
                else
        ...
        if (effectList.remove(effect) && _owner instanceof L2PcInstance && effect.getShowIcon()) {
            SystemMessage sm;
            if (effect.getSkill().isToggle()) { sm = SystemMessage.getSystemMessage(SystemMessageId.S1_HAS_BEEN_ABORTED); }
            else { sm = SystemMessage.getSystemMessage(SystemMessageId.EFFECT_S1_DISAPPEARED); }
    ...
    protected void addEffectFromQueue(L2Effect newEffect) {
        ...
        if (newSkill.isDebuff()) {
            ...
            if (newSkill.isToggle()) { _buffs.add(newEffect); }
            else {
                int pos = 0;
                for (L2Effect e : _buffs) {
                    if (e == null || e.getSkill().isToggle() || e.getSkill().is7Signs()) { continue; }
        ...
        if (_stackedEffects == null) { _stackedEffects = new HashMap<>(); }
        ...
        if (stackQueue != null) {
            ...
            if (!stackQueue.isEmpty()) {
                ...
                while (queueIterator.hasNext()) {
                    if (newEffect.getStackOrder() < queueIterator.next().getStackOrder()) { pos++; }
                    else { break; }
                }
                ...
                if (Config.EFFECT_CANCELING && !newEffect.isHerbEffect() && stackQueue.size() > 1) {
                    if (newSkill.isDebuff()) { _debuffs.remove(stackQueue.remove(1)); }
                    else { _buffs.remove(stackQueue.remove(1)); }
                }
            }
            else { stackQueue.add(0, newEffect); }
        }
        else {
        ...
        // Get the first stacked effect of the Stack group selected
        if (!stackQueue.isEmpty()) { effectToAdd = listsContains(stackQueue.get(0)); }
    ...
    protected void updateEffectIcons() {
        ...
        if (_owner instanceof L2PcInstance) {
            if (_partyOnly) { _partyOnly = false; }
            else { mi = new AbnormalStatusUpdate(); }

            if (_owner.isInParty()) { ps = new PartySpelled(_owner); }

            if (((L2PcInstance) _owner).isInOlympiadMode() && ((L2PcInstance) _owner).isOlympiadStart()) { os = new ExOlympiadSpelledInfo((L2PcInstance) _owner); }
        }
        else if (_owner instanceof L2Summon) { ps = new PartySpelled(_owner); }
        ...
        if (_buffs != null && !_buffs.isEmpty()) {
            for (L2Effect e : _buffs) {
                ...
                if (e.getSkill().isRemovedOnAnyActionExceptMove()) { foundRemovedOnAction = true; }
                if (e.getSkill().isRemovedOnDamage()) { foundRemovedOnDamage = true; }
                ...
                if (e.getInUse()) {
                    if (mi != null) { e.addIcon(mi); }

                    if (ps != null) { e.addPartySpelledIcon(ps); }

                    if (os != null) { e.addOlympiadSpelledIcon(os); }
                }
            }
        }
        ...
        if (mi != null) { _owner.sendPacket(mi); }
        ...
        if (ps != null) {
            if (_owner instanceof L2Summon) {
                ...
                if (summonOwner != null) {
                    if (summonOwner.isInParty()) { summonOwner.getParty().broadcastToPartyMembers(ps); }
                    else { summonOwner.sendPacket(ps); }
                }
            }
            else if (_owner instanceof L2PcInstance && _owner.isInParty()) { _owner.getParty().broadcastToPartyMembers(ps); }
        }

        if (os != null) {
            final OlympiadGameTask game = OlympiadGameManager.getInstance().getOlympiadTask(((L2PcInstance) _owner).getOlympiadGameId());
            if (game != null && game.isBattleStarted()) { game.getZone().broadcastPacketToObservers(os); }
        }
    }
    ...
    private L2Effect listsContains(L2Effect effect) {
        if (_buffs != null && !_buffs.isEmpty() && _buffs.contains(effect)) { return effect; }
        if (_debuffs != null && !_debuffs.isEmpty() && _debuffs.contains(effect)) { return effect; }
