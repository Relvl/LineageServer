...
public class Q373_SupplierOfReagents extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        // Urn
            ...
            // Not enough items, cancel the operation.
            if (!st.hasQuestItems(catalyst)) { return "31149-04.htm"; }
            ...
            for (int[] formula : FORMULAS) {
                if (formula[1] != regent || formula[2] != catalyst) { continue; }

                // Not enough regents.
                if (st.getQuestItemsCount(regent) < formula[0]) { break; }

                // Not enough catalysts.
                if (!st.hasQuestItems(catalyst)) { break; }
                ...
                for (int temperature[] : TEMPERATURES) {
                    if (temperature[0] != tempIndex) { continue; }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                if (npc.getNpcId() == WESLEY) { htmltext = "30166-05.htm"; }
                else { htmltext = "31149-01.htm"; }
                break;
        }
        return htmltext;
    }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (drop[2] == 0) { st.dropItems(drop[0], 1, 0, drop[1]); }
        else {
            int random = Rnd.get(1000);
            if (random < drop[3]) { st.dropItemsAlways((random < drop[2]) ? drop[0] : drop[1], 1, 0); }
        }
