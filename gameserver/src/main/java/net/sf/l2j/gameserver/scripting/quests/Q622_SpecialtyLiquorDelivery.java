...
public class Q622_SpecialtyLiquorDelivery extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("31521-02.htm")) {
            ...
            if (Rnd.get(5) < 1) { st.giveItems(RECIPES[Rnd.get(RECIPES.length)], 1); }
            else {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case JEREMY:
                        if (cond < 6) { htmltext = "31521-04.htm"; }
                        else if (cond == 6) { htmltext = "31521-05.htm"; }
                        else if (cond == 7) { htmltext = "31521-06.htm"; }
                        break;

                    case BEOLIN:
                        if (cond == 1 && st.getQuestItemsCount(SPECIAL_DRINK) == 5) { htmltext = "31547-01.htm"; }
                        else if (cond > 1) { htmltext = "31547-03.htm"; }
                        break;

                    case KUBER:
                        if (cond == 2 && st.getQuestItemsCount(SPECIAL_DRINK) == 4) { htmltext = "31546-01.htm"; }
                        else if (cond > 2) { htmltext = "31546-03.htm"; }
                        break;

                    case CROCUS:
                        if (cond == 3 && st.getQuestItemsCount(SPECIAL_DRINK) == 3) { htmltext = "31545-01.htm"; }
                        else if (cond > 3) { htmltext = "31545-03.htm"; }
                        break;

                    case NAFF:
                        if (cond == 4 && st.getQuestItemsCount(SPECIAL_DRINK) == 2) { htmltext = "31544-01.htm"; }
                        else if (cond > 4) { htmltext = "31544-03.htm"; }
                        break;

                    case PULIN:
                        if (cond == 5 && st.getQuestItemsCount(SPECIAL_DRINK) == 1) { htmltext = "31543-01.htm"; }
                        else if (cond > 5) { htmltext = "31543-03.htm"; }
                        break;

                    case LIETTA:
                        if (cond == 7) { htmltext = "31267-01.htm"; }
                        break;
                }
        }
