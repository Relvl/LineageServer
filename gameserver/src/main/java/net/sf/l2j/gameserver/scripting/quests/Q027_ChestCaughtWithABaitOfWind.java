    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getLevel() < 27) { htmltext = "31570-02.htm"; }
                else {
                    QuestState st2 = player.getQuestState("Q050_LanoscosSpecialBait");
                    if (st2 != null && st2.isCompleted()) { htmltext = "31570-01.htm"; }
                    else { htmltext = "31570-03.htm"; }
                }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case LANOSCO:
                        if (cond == 1) { htmltext = (!st.hasQuestItems(LARGE_BLUE_TREASURE_CHEST)) ? "31570-06.htm" : "31570-05.htm"; }
                        else if (cond == 2) { htmltext = "31570-09.htm"; }
                        break;

                    case SHALING:
                        if (cond == 2) { htmltext = "31434-01.htm"; }
                        break;
                }
                break;
