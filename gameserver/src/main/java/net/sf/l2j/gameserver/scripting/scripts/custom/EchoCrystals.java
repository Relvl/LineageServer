    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (st != null && StringUtil.isDigit(event)) {
            ...
            if (SCORES.containsKey(score)) {
                ...
                if (st.getQuestItemsCount(score) == 0) { htmltext = npc.getNpcId() + "-" + noscore + ".htm"; }
                else if (st.getQuestItemsCount(ItemConst.ADENA_ID) < COST) { htmltext = npc.getNpcId() + "-" + noadena + ".htm"; }
                else {
