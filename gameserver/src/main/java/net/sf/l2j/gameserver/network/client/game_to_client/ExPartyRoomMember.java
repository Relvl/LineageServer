...
public class ExPartyRoomMember extends L2GameServerPacket {
    ...
    protected void writeImpl() {
        ...
        for (L2PcInstance _member : _room.getPartyMembers()) {
            ...
            if (_room.getOwner().equals(_member)) { writeD(1); }
            else {
                if ((_room.getOwner().isInParty() && _member.isInParty()) && (_room.getOwner().getParty().getPartyLeaderOID() == _member.getParty().getPartyLeaderOID())) { writeD(2); }
                else { writeD(0); }
            }
        }
    }
