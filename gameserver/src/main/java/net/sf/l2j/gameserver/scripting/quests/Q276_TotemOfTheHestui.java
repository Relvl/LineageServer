    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getRace() != PlayerRace.Orc) { htmltext = "30571-00.htm"; }
                else if (player.getLevel() < 15) { htmltext = "30571-01.htm"; }
                else { htmltext = "30571-02.htm"; }
                break;

            case QuestState.STATE_STARTED:
                if (st.getInt("cond") == 1) { htmltext = "30571-04.htm"; }
                else {
        ...
        if (!st.hasQuestItems(KASHA_CRYSTAL)) {
            switch (npc.getNpcId()) {
                case 20479:
                    ...
                    else { st.dropItemsAlways(KASHA_PARASITE, 1, 0); }
                    break;
