...
public abstract class AbstractNpcInfo extends L2GameServerPacket {
    ...
    public static class NpcInfo extends AbstractNpcInfo {
        ...
        public NpcInfo(L2Npc cha, L2Character attacker) {
            ...
            if (_npc.getTemplate().isCustomNpc()) { _name = _npc.getTemplate().getName(); }

            if (_npc.isChampion()) { _title = ("Champion"); }
            else if (_npc.getTemplate().isCustomNpc()) { _title = _npc.getTemplate().getTitle(); }
            else { _title = _npc.getTitle(); }

            if (Config.SHOW_NPC_LVL && _npc instanceof L2MonsterInstance) {
                String t = "Lv " + _npc.getLevel() + (_npc.getAggroRange() > 0 ? "*" : "");
                if (_title != null) { t += " " + _title; }
    ...
    public static class SummonInfo extends AbstractNpcInfo {
        ...
        @Override
        protected void writeImpl() {
            if (_owner != null && _owner.isInvisible()) { return; }
