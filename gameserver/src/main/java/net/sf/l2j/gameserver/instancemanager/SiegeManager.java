...
public class SiegeManager {
    ...
    protected SiegeManager() {
        ...
        for (Castle castle : CastleManager.getInstance().getCastles()) {
            ...
            for (int i = 1; i < 0xFF; i++) {
                ...
                try {
                    ...
                    LOGGER.error("Error while loading control tower(s) for " + castle.getName() + " castle.", e);
                }
            }
            ...
            for (int i = 1; i < 0xFF; i++) {
                ...
                try {
                    ...
                    while (st.hasMoreTokens()) { zoneList.add(Integer.parseInt(st.nextToken())); }

                    flameTowers.add(new TowerSpawn(npcId, new Location(x, y, z), zoneList));
                }
                catch (Exception e) {
                    LOGGER.error("Error while loading flame tower(s) for " + castle.getName() + " castle.", e);
                }
            }
        ...
        LOGGER.info("SiegeManager: Loaded " + _controlTowers.size() + " Control Towers & " + _flameTowers.size() + " Flame Towers.");
    }
    ...
    public static boolean checkIsRegistered(L2Clan clan) {
        if (clan == null || clan.hasCastle()) { return true; }
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.error("Exception: checkIsRegistered(): ", e);
        }
        return register;
    }

    public static void addSiegeSkills(L2PcInstance character) {
        for (L2Skill sk : SkillTable.getInstance().getSiegeSkills(character.isNoble())) { character.addSkill(sk, false); }
    }

    public static void removeSiegeSkills(L2PcInstance character) {
        for (L2Skill sk : SkillTable.getInstance().getSiegeSkills(character.isNoble())) { character.removeSkill(sk); }
    }
    ...
    public static Siege getSiege(int x, int y, int z) {
        for (Castle castle : CastleManager.getInstance().getCastles()) { if (castle.getSiege().checkIfInZone(x, y, z)) { return castle.getSiege(); } }
    ...
    public static List<Siege> getSieges() {
        ...
        for (Castle castle : CastleManager.getInstance().getCastles()) { sieges.add(castle.getSiege()); }
    ...
    private void loadTrapUpgrade(int castleId) {
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            while (rs.next()) { _flameTowers.get(castleId).get(rs.getInt("towerIndex")).setUpgradeLevel(rs.getInt("level")); }

            rs.close();
            statement.close();
        }
        catch (Exception e) {
            LOGGER.error("Exception: loadTrapUpgrade(): ", e);
        }
    }
