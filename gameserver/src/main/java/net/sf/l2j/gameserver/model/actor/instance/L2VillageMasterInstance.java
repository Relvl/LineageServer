...
public class L2VillageMasterInstance extends L2NpcInstance {
    ...
    public void onBypassFeedback(L2PcInstance player, String command) {
        ...
        if (actualCommand.equalsIgnoreCase("create_clan")) {
            ...
            if (player.getClan() == null) { player.sendPacket(SystemMessageId.ONLY_CLAN_LEADER_CREATE_ALLIANCE); }
            ...
            if (player.getClan().levelUpClan(player)) { player.broadcastPacket(new MagicSkillUse(player, player, 5103, 1, 0, 0)); }
        }
            ...
            if (OlympiadManager.getInstance().isRegisteredInComp(player)) { OlympiadManager.getInstance().unRegisterNoble(player); }
            ...
            try {
                ...
                if (command.length() > endIndex) { paramTwo = Integer.parseInt(command.substring(endIndex).trim()); }
            }
            ...
            switch (cmdChoice) {
                ...
                case 2: // Change Class - Initial
                    ...
                    if (player.getSubClasses().isEmpty()) { html.setFile("data/html/villagemaster/SubClass_ChangeNo.htm"); }
                    else {
                        sb = new StringBuilder(300);

                        if (checkVillageMaster(player.getBaseClass())) { StringUtil.append(sb, "<a action=\"bypass -h npc_%objectId%_Subclass 5 0\">", CharTemplateTable.getInstance().getClassNameById(player.getBaseClass()), "</a><br>"); }
                        ...
                        else { html.setFile("data/html/villagemaster/SubClass_ChangeNotFound.htm"); }
                    }
                    break;
                ...
                case 3: // Change/Cancel Subclass - Initial
                    ...
                    if (player.getTotalSubClasses() > 3) {
                        ...
                        for (Iterator<SubClass> subList = iterSubClasses(player); subList.hasNext(); ) {
                            SubClass subClass = subList.next();
                            StringUtil.append(sb, "Sub-class ", classIndex++, "<br>", "<a action=\"bypass -h npc_%objectId%_Subclass 6 ", subClass.getClassIndex(), "\">", CharTemplateTable.getInstance()
                        ...
                        if (player.getSubClasses().containsKey(1)) { html.replace("%sub1%", CharTemplateTable.getInstance().getClassNameById(player.getSubClasses().get(1).getClassId())); }
                        else { html.replace("<a action=\"bypass -h npc_%objectId%_Subclass 6 1\">%sub1%</a><br>", ""); }

                        if (player.getSubClasses().containsKey(2)) { html.replace("%sub2%", CharTemplateTable.getInstance().getClassNameById(player.getSubClasses().get(2).getClassId())); }
                        else { html.replace("<a action=\"bypass -h npc_%objectId%_Subclass 6 2\">%sub2%</a><br>", ""); }

                        if (player.getSubClasses().containsKey(3)) { html.replace("%sub3%", CharTemplateTable.getInstance().getClassNameById(player.getSubClasses().get(3).getClassId())); }
                        else { html.replace("<a action=\"bypass -h npc_%objectId%_Subclass 6 3\">%sub3%</a><br>", ""); }
                    }
                    break;

                case 4: // Add Subclass - Action (Subclass 4 x[x])
                    if (!FloodProtectors.performAction(player.getClient(), Action.SUBCLASS)) { return; }

                    boolean allowAddition = true;

                    if (player.getTotalSubClasses() >= 3) { allowAddition = false; }
					 ...
					 * the sub-class.
					 */
                    if (allowAddition && !Config.ALT_GAME_SUBCLASS_WITHOUT_QUESTS) { allowAddition = checkQuests(player); }
                    ...
                    else { html.setFile("data/html/villagemaster/SubClass_Fail.htm"); }
                    break;
                ...
                case 5: // Change Class - Action
                    ...
                    if (paramOne == 0) {
                        ...
                        try {
                            if (!checkVillageMaster(player.getSubClasses().get(paramOne).getClassDefinition())) { return; }
                        }
                ...
                case 6: // Change/Cancel Subclass - Choice
                    // validity check
                    if (paramOne < 1 || paramOne > 3) { return; }
    ...
    private static final String formatClassForDisplay(PlayerClass className) {
        ...
        for (int i = 1; i < charArray.length; i++) {
            if (Character.isUpperCase(charArray[i])) { classNameStr = classNameStr.substring(0, i) + " " + classNameStr.substring(i); }
        }
    ...
    private static final void dissolveClan(L2PcInstance player) {
        ...
        else { ClanTable.getInstance().destroyClan(clan.getClanId()); }
    ...
    private static final void createSubPledge(L2PcInstance player, String clanName, String leaderName, int pledgeType, int minClanLvl) {
        ...
        if (clan.getLevel() < minClanLvl) {
            if (pledgeType == L2Clan.SUBUNIT_ACADEMY) { player.sendPacket(SystemMessageId.YOU_DO_NOT_MEET_CRITERIA_IN_ORDER_TO_CREATE_A_CLAN_ACADEMY); }
        ...
        for (L2Clan tempClan : ClanTable.getInstance().getClans()) {
            if (tempClan.getSubPledge(clanName) != null) {
                if (pledgeType == L2Clan.SUBUNIT_ACADEMY) { player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_ALREADY_EXISTS).addString(clanName)); }
                else { player.sendPacket(SystemMessageId.ANOTHER_MILITARY_UNIT_IS_ALREADY_USING_THAT_NAME); }
        ...
        if (pledgeType != L2Clan.SUBUNIT_ACADEMY) {
            if (clan.getClanMember(leaderName) == null || clan.getClanMember(leaderName).getPledgeType() != 0) {
                if (pledgeType >= L2Clan.SUBUNIT_KNIGHT1) { player.sendPacket(SystemMessageId.CAPTAIN_OF_ORDER_OF_KNIGHTS_CANNOT_BE_APPOINTED); }
                else if (pledgeType >= L2Clan.SUBUNIT_ROYAL1) { player.sendPacket(SystemMessageId.CAPTAIN_OF_ROYAL_GUARD_CANNOT_BE_APPOINTED); }
        ...
        else { sm = SystemMessage.getSystemMessage(SystemMessageId.CLAN_CREATED); }
    ...
    private static final void assignSubPledgeLeader(L2PcInstance player, String clanName, String leaderName) {
        ...
        if (leaderSubPledge == null || leaderSubPledge.getPledgeType() != 0) {
            if (subPledge.getId() >= L2Clan.SUBUNIT_KNIGHT1) { player.sendPacket(SystemMessageId.CAPTAIN_OF_ORDER_OF_KNIGHTS_CANNOT_BE_APPOINTED); }
            else if (subPledge.getId() >= L2Clan.SUBUNIT_ROYAL1) { player.sendPacket(SystemMessageId.CAPTAIN_OF_ROYAL_GUARD_CANNOT_BE_APPOINTED); }
