...
public class Q348_AnArrogantSearch extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30864-05.htm")) {
            ...
            st.set("cond", "24");
            st.playSound(ESound.ItemSound_quest_middle);
            st.rewardItems(ItemConst.ADENA_ID, Rnd.get(1, 2) * 12000);
        }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (st.hasQuestItems(BLOODED_FABRIC)) { htmltext = "30864-00.htm"; }
                else if (player.getLevel() < 60) { htmltext = "30864-01.htm"; }
                else { htmltext = "30864-02.htm"; }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case HANELLIN:
                        if (cond == 1) { htmltext = "30864-02.htm"; }
                        else if (cond == 2) { htmltext = (!st.hasQuestItems(TITAN_POWERSTONE)) ? "30864-06.htm" : "30864-07.htm"; }
                        else if (cond == 4) { htmltext = "30864-09.htm"; }
                        else if (cond > 4 && cond < 21) { htmltext = (player.getInventory().hasAtLeastOneItem(BOOK_OF_SAINT, BLOOD_OF_SAINT, BOUGH_OF_SAINT)) ? "30864-28.htm" : "30864-24.htm"; }
                        else if (cond == 21) {
                            ...
                            if (st.hasQuestItems(WHITE_FABRIC_TRIBE)) { htmltext = "30864-31.htm"; }
                            else if (st.getQuestItemsCount(ANTIDOTE) < 5 || !st.hasQuestItems(HEALING_POTION)) { htmltext = "30864-30.htm"; }
                            else {
                        ...
                        else if (cond == 24) { htmltext = "30864-38.htm"; }
                        else if (cond == 25) {
                            if (st.hasQuestItems(WHITE_FABRIC_TRIBE)) { htmltext = "30864-39.htm"; }
                            else if (st.hasQuestItems(BLOODED_FABRIC)) { htmltext = "30864-49.htm"; }
                            ...
                            if (count + st.getQuestItemsCount(WHITE_FABRIC_ANGELS) < 10) {
                                ...
                                st.rewardItems(ItemConst.ADENA_ID, (1000 * count) + 4000);
                                st.exitQuest(true);
                            }
                            else if (count < 10) { htmltext = "30864-52.htm"; }
                            else if (count >= 10) { htmltext = "30864-53.htm"; }
                        }
                        else if (cond == 27) {
                            if (st.getInt("gustav") + st.getInt("hardin") + st.getInt("iason") == 3) {
                                htmltext = "30864-60.htm";
                                st.set("cond", "28");
                                st.rewardItems(ItemConst.ADENA_ID, 49000);
                                st.playSound(ESound.ItemSound_quest_middle);
                            }
                            else if (st.hasQuestItems(BLOODED_FABRIC) && st.getInt("usedonbaium") != 1) { htmltext = "30864-59.htm"; }
                            else {
                        ...
                        else if (cond == 28) { htmltext = "30864-55.htm"; }
                        else if (cond == 29) {
                            int count = st.getQuestItemsCount(BLOODED_FABRIC);
                            ...
                            if (count + st.getQuestItemsCount(WHITE_FABRIC_ANGELS) < 10) {
                                htmltext = "30864-54.htm";
                                st.takeItems(BLOODED_FABRIC, -1);
                                st.rewardItems(ItemConst.ADENA_ID, 5000 * count);
                                st.playSound(ESound.ItemSound_quest_finish);
                                st.exitQuest(true);
                            }
                            else if (count < 10) { htmltext = "30864-52.htm"; }
                    ...
                    case GUSTAV_ATHEBALDT:
                        if (cond == 27) {
                            ...
                            else if (st.getInt("gustav") == 1) { htmltext = "30760-02.htm"; }
                            else {
                    ...
                    case HARDIN:
                        if (cond == 27) {
                            ...
                            else if (st.getInt("hardin") == 1) { htmltext = "30832-02.htm"; }
                            else {
                    ...
                    case IASON_HEINE:
                        if (cond == 27) {
                            ...
                            else if (st.getInt("iason") == 1) { htmltext = "30969-02.htm"; }
                            else {
                    ...
                    case HARNE:
                        if (cond >= 5 && cond <= 22) {
                            if (!st.hasQuestItems(BLOOD_OF_SAINT)) {
                                ...
                                else { htmltext = "30144-04.htm"; }
                            }
                            else { htmltext = "30144-05.htm"; }
                        }
                        break;
                    ...
                    case CLAUDIA_ATHEBALDT:
                        if (cond >= 5 && cond <= 22) {
                            if (!st.hasQuestItems(BOOK_OF_SAINT)) {
                                ...
                                else { htmltext = "31001-04.htm"; }
                            }
                            else { htmltext = "31001-05.htm"; }
                        }
                        break;
                    ...
                    case MARTIEN:
                        if (cond >= 5 && cond <= 22) {
                            if (!st.hasQuestItems(BOUGH_OF_SAINT)) {
                                ...
                                else { htmltext = "30645-04.htm"; }
                            }
                            else { htmltext = "30645-05.htm"; }
                        }
                        break;
                        ...
                        if (!st.hasQuestItems(HANELLIN_FIRST_LETTER) && cond >= 5 && cond <= 22) {
                            if (!st.hasQuestItems(FIRST_KEY_OF_ARK) && !st.hasQuestItems(BLOOD_OF_SAINT)) {
                                if (st.getInt("angelkiller") == 0) {
                                    ...
                                    if (_angelKiller == null) { _angelKiller = addSpawn(ANGEL_KILLER, npc, false, 0, true); }
                        ...
                        if (!st.hasQuestItems(HANELLIN_FIRST_LETTER) && cond >= 5 && cond <= 22) {
                            if (!st.hasQuestItems(BLOOD_OF_SAINT)) {
                                if (st.hasQuestItems(FIRST_KEY_OF_ARK)) {
                                    ...
                                    if (st.hasQuestItems(BOOK_OF_SAINT, BOUGH_OF_SAINT)) { st.set("cond", "21"); }
                                }
                                else { htmltext = "30977-04.htm"; }
                            }
                            else { htmltext = "30977-03.htm"; }
                        }
                        break;
                        ...
                        if (!st.hasQuestItems(HANELLIN_SECOND_LETTER) && cond >= 5 && cond <= 22) {
                            if (!st.hasQuestItems(BOOK_OF_SAINT)) {
                                if (!st.hasQuestItems(SECOND_KEY_OF_ARK)) {
                                    ...
                                    if (_elberoth == null) { _elberoth = addSpawn(ARK_GUARDIAN_ELBEROTH, npc, false, 0, true); }
                                }
                                else {
                                    ...
                                    if (st.hasQuestItems(BLOOD_OF_SAINT, BOUGH_OF_SAINT)) { st.set("cond", "21"); }
                                }
                            }
                            else { htmltext = "30978-03.htm"; }
                        }
                        break;
                        ...
                        if (!st.hasQuestItems(HANELLIN_THIRD_LETTER) && cond >= 5 && cond <= 22) {
                            if (!st.hasQuestItems(BOUGH_OF_SAINT)) {
                                if (!st.hasQuestItems(THIRD_KEY_OF_ARK)) {
                                    ...
                                    if (_shadowFang == null) { _shadowFang = addSpawn(ARK_GUARDIAN_SHADOW_FANG, npc, false, 0, true); }
                                }
                                else {
                                    ...
                                    if (st.hasQuestItems(BLOOD_OF_SAINT, BOOK_OF_SAINT)) { st.set("cond", "21"); }
                                }
                            }
                            else { htmltext = "30979-03.htm"; }
                        }
                        break;
                }
                break;
        ...
        switch (npc.getNpcId()) {
            ...
            case PLANTINUM_TRIBE_SHAMAN:
                ...
                if ((cond == 24 || cond == 25) && Rnd.get(500) < 1 && st.hasQuestItems(WHITE_FABRIC_TRIBE)) {
                    ...
                    if (cond != 24) { st.playSound(ESound.ItemSound_quest_itemget); }
                    else {
                        st.playSound(ESound.ItemSound_quest_finish);
                        st.exitQuest(true);
                    }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            ...
            case LESSER_GIANT_MAGE:
                if (cond == 2) { st.dropItems(TITAN_POWERSTONE, 1, 1, 100000); }
                break;
