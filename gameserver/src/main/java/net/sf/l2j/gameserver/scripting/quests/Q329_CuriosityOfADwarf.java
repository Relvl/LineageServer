...
public class Q329_CuriosityOfADwarf extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                if (golem + broken == 0) { htmltext = "30437-04.htm"; }
                else {
                    htmltext = "30437-05.htm";
                    st.takeItems(GOLEM_HEARTSTONE, -1);
                    st.takeItems(BROKEN_HEARTSTONE, -1);
                    st.rewardItems(ItemConst.ADENA_ID, broken * 50 + golem * 1000 + ((golem + broken > 10) ? 1183 : 0));
                }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (chance < 2) { st.dropItemsAlways(GOLEM_HEARTSTONE, 1, 0); }
        else if (chance < ((npc.getNpcId() == 20083) ? 44 : 50)) { st.dropItemsAlways(BROKEN_HEARTSTONE, 1, 0); }
