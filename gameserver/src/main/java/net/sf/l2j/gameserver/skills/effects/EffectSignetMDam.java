...
public class EffectSignetMDam extends L2Effect {
    ...
    public boolean onStart() {
        ...
        if (getSkill() instanceof L2SkillSignetCasttime) { template = NpcTable.getInstance().getTemplate(((L2SkillSignetCasttime) getSkill()).effectNpcId); }
        else { return false; }
        ...
        if (getEffector() instanceof L2PcInstance && getSkill().getTargetType() == ESkillTargetType.TARGET_GROUND) {
    ...
    public boolean onActionTime() {
        ...
        if (!targets.isEmpty()) {
            ...
            for (L2Character target : targets) {
                ...
                if (target instanceof L2Summon) { target.broadcastStatusUpdate(); }
