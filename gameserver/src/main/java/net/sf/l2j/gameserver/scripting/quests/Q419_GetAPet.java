...
public class Q419_GetAPet extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("task")) {
            ...
            if (st.getInt("progress") == 7) { st.playSound(ESound.ItemSound_quest_middle); }
        }
                ...
                switch (npc.getNpcId()) {
                    case MARTIN:
                        if (st.hasAtLeastOneQuestItem(ANIMAL_SLAYER_LIST_1, ANIMAL_SLAYER_LIST_2, ANIMAL_SLAYER_LIST_3, ANIMAL_SLAYER_LIST_4, ANIMAL_SLAYER_LIST_5)) {
                            int proofs = st.getQuestItemsCount(BLOODY_FANG) + st.getQuestItemsCount(BLOODY_CLAW) + st.getQuestItemsCount(BLOODY_NAIL) + st.getQuestItemsCount(BLOODY_KASHA_FANG) + st.getQuestItemsCount(BLOODY_TARANTULA_NAIL);
                            if (proofs == 0) { htmltext = "30731-09.htm"; }
                            else if (proofs < 50) { htmltext = "30731-10.htm"; }
                            else { htmltext = "30731-11.htm"; }
                        }
                        else if (st.getInt("progress") == 7) { htmltext = "30731-13.htm"; }
                        else { htmltext = "30731-16.htm"; }
                        break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.hasQuestItems(drop[0] - 5)) { st.dropItems(drop[0], 1, 50, drop[1]); }
