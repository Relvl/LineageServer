...
public class AdminMonsterRace implements IAdminCommandHandler {
    ...
    public boolean useAdminCommand(String command, L2PcInstance activeChar) {
        if (command.equalsIgnoreCase("admin_mons")) {
            ...
            if (state == -1) {
                ...
                activeChar.broadcastPacket(new PlaySound(ESound.S_RACE));
                activeChar.broadcastPacket(new PlaySound(ESound.ItemSound2_race_start, 121209259, 12125, 182487, -3559));
                activeChar.broadcastPacket(new MonRaceInfo(codes[state][0], codes[state][1], race.getMonsters(), race.getSpeeds()));
    ...
    class RunEnd implements Runnable {
        ...
        @Override
        public void run() {
            for (int i = 0; i < 8; i++) { activeChar.broadcastPacket(new DeleteObject(MonsterRace.getInstance().getMonsters()[i])); }
