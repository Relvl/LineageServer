...
public class Sailren extends AbstractNpcAI {
    ...
    public Sailren() {
        ...
        switch (GrandBossManager.getInstance().getBossStatus(SAILREN)) {
            case DEAD: // Launch the timer to set DORMANT, or set DORMANT directly if timer expired while offline.
                ...
                if (temp > 0) { startQuestTimer("unlock", temp, null, null, false); }
                else { GrandBossManager.getInstance().setBossStatus(SAILREN, DORMANT); }
                break;
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        if (event.equalsIgnoreCase("beginning")) {
        ...
        else if (event.equalsIgnoreCase("skill")) { SAILREN_LAIR.broadcastPacket(new MagicSkillUse(npc, npc, 5090, 1, 2500, 0)); }
        else if (event.equalsIgnoreCase("camera_0")) { SAILREN_LAIR.broadcastPacket(new SpecialCamera(npc.getObjectId(), 100, 180, 30, 3000, 3000, 0, 50, 1, 0)); }
        else if (event.equalsIgnoreCase("camera_1")) { SAILREN_LAIR.broadcastPacket(new SpecialCamera(npc.getObjectId(), 150, 270, 25, 3000, 3000, 0, 30, 1, 0)); }
        else if (event.equalsIgnoreCase("camera_2")) { SAILREN_LAIR.broadcastPacket(new SpecialCamera(npc.getObjectId(), 160, 360, 20, 3000, 3000, 10, 15, 1, 0)); }
        else if (event.equalsIgnoreCase("camera_3")) { SAILREN_LAIR.broadcastPacket(new SpecialCamera(npc.getObjectId(), 160, 450, 10, 3000, 3000, 0, 10, 1, 0)); }
        ...
        else if (event.equalsIgnoreCase("camera_5")) { SAILREN_LAIR.broadcastPacket(new SpecialCamera(npc.getObjectId(), 70, 560, 0, 500, 7000, -15, 10, 1, 0)); }
        else if (event.equalsIgnoreCase("unlock")) { GrandBossManager.getInstance().setBossStatus(SAILREN, DORMANT); }
            ...
            if ((System.currentTimeMillis() - _lastAttackTime) >= INTERVAL_CHECK) {
                ...
                if (!_mobs.isEmpty()) {
                    for (L2Npc mob : _mobs) { mob.deleteMe(); }
    ...
    public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet) {
        if (!_mobs.contains(npc) || !SAILREN_LAIR.getAllowedPlayers().contains(killer.getObjectId())) { return null; }
        ...
        switch (npc.getNpcId()) {
            ...
            case TREX:
                ...
                if (_mobs.remove(npc)) { startQuestTimer("spawn", Config.WAIT_TIME_SAILREN, npc, killer, false); }
                break;
    ...
    public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isPet) {
        if (!_mobs.contains(npc) || !SAILREN_LAIR.getAllowedPlayers().contains(attacker.getObjectId())) { return null; }
