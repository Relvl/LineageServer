...
import net.sf.l2j.gameserver.network.client.game_to_client.SendMacroList;
...
public class MacroList {
    ...
    public void registerMacro(L2Macro macro) {
        if (macro.id == 0) {
            ...
            while (_macroses.get(macro.id) != null) { macro.id = _macroId++; }
            ...
            if (old != null) { deleteMacroFromDb(old); }
    ...
    public void deleteMacro(int id) {
        ...
        if (toRemove != null) { deleteMacroFromDb(toRemove); }

        _macroses.remove(id);

        final L2ShortCut[] allShortCuts = _owner.getAllShortCuts();
        for (L2ShortCut sc : allShortCuts) {
            if (sc.getId() == id && sc.getType() == L2ShortCut.TYPE_MACRO) { _owner.deleteShortCut(sc.getSlot(), sc.getPage()); }
        }
    ...
    public void sendUpdate() {
        ...
        if (all.length == 0) { _owner.sendPacket(new SendMacroList(_revision, all.length, null)); }
        else {
            for (L2Macro m : all) { _owner.sendPacket(new SendMacroList(_revision, all.length, m)); }
        }
    }
    ...
    private void registerMacroInDb(L2Macro macro) {
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            for (L2MacroCmd cmd : macro.commands) {
                ...
                if (cmd.cmd != null && cmd.cmd.length() > 0) { StringUtil.append(sb, ",", cmd.cmd); }

                sb.append(';');
            }

            if (sb.length() > 255) { sb.setLength(255); }
