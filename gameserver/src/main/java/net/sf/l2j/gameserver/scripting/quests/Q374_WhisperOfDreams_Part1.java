    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30515-03.htm")) {
            ...
            if (st.getQuestItemsCount(CAVE_BEAST_TOOTH) >= 65 && st.getQuestItemsCount(DEATH_WAVE_LIGHT) >= 65) {
                ...
                st.rewardItems(ItemConst.ADENA_ID, reward[2]);
                ...
                switch (npc.getNpcId()) {
                    case MANAKIA:
                        if (!st.hasQuestItems(SEALED_MYSTERIOUS_STONE)) {
                            if (st.getQuestItemsCount(CAVE_BEAST_TOOTH) >= 65 && st.getQuestItemsCount(DEATH_WAVE_LIGHT) >= 65) { htmltext = "30515-05.htm"; }
