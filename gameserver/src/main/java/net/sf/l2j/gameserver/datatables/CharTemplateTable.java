...
public class CharTemplateTable {
    ...
    protected CharTemplateTable() {
        ...
        if (!mainDir.isDirectory()) {
            LOGGER.error("CharTemplateTable: Main dir {} hasn't been found.", mainDir.getAbsolutePath());
            return;
        }

        for (final File file : mainDir.listFiles()) {
            if (file.isFile() && file.getName().endsWith(".xml")) { loadFileClass(file); }
        }

        LOGGER.info("CharTemplateTable: Loaded {} character templates.", _templates.size());
        LOGGER.info("CharTemplateTable: Loaded {} classes skills trees.", SkillTreeTable.getInstance().getSkillTreesSize());
    }
    ...
    private void loadFileClass(final File f) {
        try {
            ...
            for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling()) {
                if ("class".equalsIgnoreCase(d.getNodeName())) {
                    ...
                    if (items != null) {
                        ...
                        for (String element : itemsSplit) { pcT.addItem(Integer.parseInt(element)); }
                    }
            ...
            LOGGER.error("CharTemplateTable: Error loading from file: {}", f.getName(), e);
        }
    }
    ...
    public final String getClassNameById(int classId) {
        ...
        if (pcTemplate == null) { throw new IllegalArgumentException("No template for classId: " + classId); }
