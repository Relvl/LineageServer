...
public class OracleTeleport extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("Return")) {
            ...
            if (playerLevel >= 20 && playerLevel < 30) { st.takeItems(ItemConst.ADENA_ID, 2000); }
            else if (playerLevel >= 30 && playerLevel < 40) { st.takeItems(ItemConst.ADENA_ID, 4500); }
            else if (playerLevel >= 40 && playerLevel < 50) { st.takeItems(ItemConst.ADENA_ID, 8000); }
            else if (playerLevel >= 50 && playerLevel < 60) { st.takeItems(ItemConst.ADENA_ID, 12500); }
            else if (playerLevel >= 60 && playerLevel < 70) { st.takeItems(ItemConst.ADENA_ID, 18000); }
            else if (playerLevel >= 70) { st.takeItems(ItemConst.ADENA_ID, 24500); }

            int i = 0;
            for (int zigurrat : TELEPORTERS) {
                if (zigurrat == npcId) { break; }
                i++;
            }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        if (Util.contains(TOWN_DUSK, npcId)) {
            ...
            else if (playerLevel >= 20 && playerLevel < 30 && st.getQuestItemsCount(ItemConst.ADENA_ID) < 2000) {
                htmltext = "ziggurat_noadena.htm";
                st.exitQuest(true);
            }
            else if (playerLevel >= 30 && playerLevel < 40 && st.getQuestItemsCount(ItemConst.ADENA_ID) < 4500) {
                htmltext = "ziggurat_noadena.htm";
                st.exitQuest(true);
            }
            else if (playerLevel >= 40 && playerLevel < 50 && st.getQuestItemsCount(ItemConst.ADENA_ID) < 8000) {
                htmltext = "ziggurat_noadena.htm";
                st.exitQuest(true);
            }
            else if (playerLevel >= 50 && playerLevel < 60 && st.getQuestItemsCount(ItemConst.ADENA_ID) < 12500) {
                htmltext = "ziggurat_noadena.htm";
                st.exitQuest(true);
            }
            else if (playerLevel >= 60 && playerLevel < 70 && st.getQuestItemsCount(ItemConst.ADENA_ID) < 18000) {
                htmltext = "ziggurat_noadena.htm";
                st.exitQuest(true);
            }
            else if (playerLevel >= 70 && st.getQuestItemsCount(ItemConst.ADENA_ID) < 24500) {
                htmltext = "ziggurat_noadena.htm";
                st.exitQuest(true);
            }
            else { htmltext = "ziggurat.htm"; }
        }
