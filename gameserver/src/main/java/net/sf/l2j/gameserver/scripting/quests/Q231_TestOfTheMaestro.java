...
public class Q231_TestOfTheMaestro extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        // BALANKI
        else if (event.equalsIgnoreCase("30533-02.htm")) { st.set("bCond", "1"); }
        // CROTO
        else if (event.equalsIgnoreCase("30671-02.htm")) { st.giveItems(PAINT_OF_KAMURU, 1); }
        // TOMA
        ...
        // Spawns 3 King Bugbears
            ...
            L2Attackable bugbear1 = (L2Attackable) addSpawn(KING_BUGBEAR, 140333, -194153, -3138, 0, false, 200000, true);
            bugbear1.addDamageHate(player, 0, 999);
            bugbear1.getAI().setIntention(EIntention.ATTACK, player);

            L2Attackable bugbear2 = (L2Attackable) addSpawn(KING_BUGBEAR, 140395, -194147, -3146, 0, false, 200000, true);
            bugbear2.addDamageHate(player, 0, 999);
            bugbear2.getAI().setIntention(EIntention.ATTACK, player);

            L2Attackable bugbear3 = (L2Attackable) addSpawn(KING_BUGBEAR, 140304, -194082, -3157, 0, false, 200000, true);
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getClassId() != ClassId.artisan) { htmltext = "30531-01.htm"; }
                else if (player.getLevel() < 39) { htmltext = "30531-02.htm"; }
                else { htmltext = "30531-03.htm"; }
                break;
            ...
            case QuestState.STATE_STARTED:
                switch (npc.getNpcId()) {
                    case LOCKIRIN:
                        ...
                        if (cond == 1) { htmltext = "30531-05.htm"; }
                        else if (cond == 2) {
                    ...
                    case BALANKI:
                        ...
                        if (bCond == 0) { htmltext = "30533-01.htm"; }
                        else if (bCond == 1) { htmltext = "30533-03.htm"; }
                        else if (bCond == 2) {
                        ...
                        else if (bCond == 3) { htmltext = "30533-05.htm"; }
                        break;

                    case CROTO:
                        bCond = st.getInt("bCond");
                        if (bCond == 1) {
                            if (!st.hasQuestItems(PAINT_OF_KAMURU)) { htmltext = "30671-01.htm"; }
                            else if (!st.hasQuestItems(NECKLACE_OF_KAMURU)) { htmltext = "30671-03.htm"; }
                            else {
                        ...
                        else if (bCond > 1) { htmltext = "30671-05.htm"; }
                        break;
                    ...
                    case ARIN:
                        ...
                        else if (aCond == 1) { htmltext = "30536-02.htm"; }
                        else if (aCond == 2) {
                        ...
                        else if (aCond == 3) { htmltext = "30536-04.htm"; }
                        break;

                    case TOMA:
                        aCond = st.getInt("aCond");
                        if (aCond == 1) {
                            if (!st.hasQuestItems(BROKEN_TELEPORT_DEVICE)) { htmltext = "30556-01.htm"; }
                        ...
                        else if (aCond > 1) { htmltext = "30556-07.htm"; }
                        break;
                    ...
                    case FILAUR:
                        ...
                        else if (fCond == 1) { htmltext = "30535-02.htm"; }
                        else if (fCond == 2) {
                        ...
                        else if (fCond == 3) { htmltext = "30535-04.htm"; }
                        break;
                    ...
                    case LORAIN:
                        ...
                        if (fCond == 1) {
                            if (!st.hasQuestItems(REPORT_OF_KRUMA)) {
                                ...
                                else if (st.getQuestItemsCount(STINGER_WASP_NEEDLE) < 10 || st.getQuestItemsCount(MARSH_SPIDER_WEB) < 10 || st.getQuestItemsCount(BLOOD_OF_LEECH) < 10) { htmltext = "30673-02.htm"; }
                                else { htmltext = "30673-03.htm"; }
                            }
                        }
                        else if (fCond > 1) { htmltext = "30673-05.htm"; }
                        break;
                }
                break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            case GIANT_MIST_LEECH:
                if (st.hasQuestItems(INGREDIENTS_OF_ANTIDOTE)) { st.dropItemsAlways(BLOOD_OF_LEECH, 1, 10); }
                break;

            case STINGER_WASP:
                if (st.hasQuestItems(INGREDIENTS_OF_ANTIDOTE)) { st.dropItemsAlways(STINGER_WASP_NEEDLE, 1, 10); }
                break;

            case MARSH_SPIDER:
                if (st.hasQuestItems(INGREDIENTS_OF_ANTIDOTE)) { st.dropItemsAlways(MARSH_SPIDER_WEB, 1, 10); }
                break;

            case EVIL_EYE_LORD:
                if (st.hasQuestItems(PAINT_OF_KAMURU)) { st.dropItemsAlways(NECKLACE_OF_KAMURU, 1, 1); }
                break;
        }
