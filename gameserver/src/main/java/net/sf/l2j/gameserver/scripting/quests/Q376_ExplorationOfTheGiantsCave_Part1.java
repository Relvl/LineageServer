...
public class Q376_ExplorationOfTheGiantsCave_Part1 extends Quest {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    ...
                    case CLIFF:
                        if (cond == 2 && st.hasQuestItems(MYSTERIOUS_BOOK)) { htmltext = "30182-01.htm"; }
                        else if (cond == 3) { htmltext = "30182-03.htm"; }
                        break;
                }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropItems(MYSTERIOUS_BOOK, 1, 1, 1000)) { st.unset("condBook"); }
    ...
    private static String checkItems(QuestState st) {
        ...
        for (int type = 0; type < BOOKS.length; type++) {
            ...
            for (int book : BOOKS[type]) {
                if (!st.hasQuestItems(book)) { complete = false; }
            }

            if (complete) {
                for (int book : BOOKS[type]) { st.takeItems(book, 1); }
