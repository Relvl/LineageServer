...
public class L2Spawn {
    ...
    public L2Npc doSpawn(boolean isSummonSpawn) {
        ...
        try {
            ...
            if (_template.isType("L2Pet") || _template.isType("L2Minion")) { return mob; }
            ...
            if (isSummonSpawn && tmp instanceof L2Character) { ((L2Character) tmp).setShowSummonAnimation(isSummonSpawn); }

            // Check if the Instance is a L2Npc
            if (!(tmp instanceof L2Npc)) { return mob; }
    ...
    private L2Npc initializeNpcInstance(L2Npc mob) {
        ...
        if (Math.abs(newlocz - getLocz()) > 200) { newlocz = getLocz(); }
        ...
        if (getHeading() == -1) { mob.setHeading(Rnd.get(61794)); }
        else { mob.setHeading(getHeading()); }
    ...
    public static void notifyNpcSpawned(L2Npc npc) {
        synchronized (_spawnListeners) {
            for (SpawnListener listener : _spawnListeners) { listener.npcSpawned(npc); }
        }
    }
