...
public class L2BufferInstance extends L2NpcInstance {
    ...
    public void onBypassFeedback(L2PcInstance player, String command) {
        ...
        if (currentCommand.startsWith("menu")) {
            ...
            L2Summon summon = player.getPet();
            if (summon != null) { summon.setCurrentHpMp(summon.getMaxHp(), summon.getMaxMp()); }
            ...
            if (currentCommand.startsWith("skillselect") && !schemeName.equalsIgnoreCase("none")) {
                ...
                else { player.sendMessage("This scheme has reached the maximum amount of buffs."); }
            }
            else if (currentCommand.startsWith("skillunselect")) { skills.remove(Integer.valueOf(skillId)); }
    ...
    private void showEditSchemeWindow(L2PcInstance player, String groupType, String schemeName) {
        ...
        if (schemeName.equalsIgnoreCase("none")) { html.setFile(getHtmlPath(getNpcId(), 3)); }
        else {
            if (groupType.equalsIgnoreCase("none")) { html.setFile(getHtmlPath(getNpcId(), 4)); }
            else {
    ...
    private static String getGroupSkillList(L2PcInstance player, String groupType, String schemeName) {
        ...
        for (int skillId : BufferTable.getSkillsIdsByType(groupType)) {
            if (BufferTable.getInstance().getSchemeContainsSkill(player.getObjectId(), schemeName, skillId)) { continue; }

            skills.add(skillId);
        }

        if (skills.isEmpty()) { return "That group doesn't contain any skills."; }
    ...
    private static String getPlayerSchemeSkillList(L2PcInstance player, String groupType, String schemeName) {
        ...
        for (int sk : skills) {
            ...
            else { StringUtil.append(sb, "<td><button action=\"bypass -h npc_%objectId%_skillunselect ", groupType, " ", schemeName, " ", sk, "\" width=32 height=32 back=\"icon.skill", sk, "\" fore=\"icon.skill", sk, "\"></td>"); }
    ...
    private static String getTypesFrame(String groupType, String schemeName) {
        ...
        for (String s : BufferTable.getSkillTypes()) {
            ...
            if (groupType.equalsIgnoreCase(s)) { StringUtil.append(sb, "<td width=65>", s, "</td>"); }
            else { StringUtil.append(sb, "<td width=65><a action=\"bypass -h npc_%objectId%_editschemes ", s, " ", schemeName, "\">", s, "</a></td>"); }
    ...
    private static int getFee(ArrayList<Integer> list) {
        if (Config.BUFFER_STATIC_BUFF_COST >= 0) { return list.size() * Config.BUFFER_STATIC_BUFF_COST; }

        int fee = 0;
        for (int sk : list) { fee += Config.BUFFER_BUFFLIST.get(sk).getValue(); }
