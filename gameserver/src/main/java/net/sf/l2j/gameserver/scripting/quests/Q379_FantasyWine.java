...
public class Q379_FantasyWine extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        else if (event.equalsIgnoreCase("30074-2a.htm")) { st.exitQuest(true); }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                if (leaf == 80 && stone == 100) { htmltext = "30074-5.htm"; }
                else if (leaf == 80) { htmltext = "30074-4a.htm"; }
                else if (stone == 100) { htmltext = "30074-4b.htm"; }
                else { htmltext = "30074-4.htm"; }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (npc.getNpcId() == ENKU_CHAMPION) {
            if (st.dropItemsAlways(LEAF, 1, 80) && st.getQuestItemsCount(STONE) >= 100) { st.set("cond", "2"); }
        }
        else if (st.dropItemsAlways(STONE, 1, 100) && st.getQuestItemsCount(LEAF) >= 80) { st.set("cond", "2"); }
