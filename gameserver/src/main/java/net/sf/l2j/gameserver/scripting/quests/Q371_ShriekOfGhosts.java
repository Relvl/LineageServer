...
public class Q371_ShriekOfGhosts extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30867-03.htm")) {
            ...
            if (st.hasQuestItems(PORCELAIN)) {
                ...
                else { htmltext = "30929-07.htm"; }
            }
            else { htmltext = "30929-02.htm"; }
        }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                switch (npc.getNpcId()) {
                    case REVA:
                        if (st.hasQuestItems(URN)) { htmltext = (st.hasQuestItems(PORCELAIN)) ? "30867-05.htm" : "30867-04.htm"; }
                        else { htmltext = "30867-06.htm"; }
                        break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (random < chances[1]) { st.dropItemsAlways((random < chances[0]) ? URN : PORCELAIN, 1, 0); }
