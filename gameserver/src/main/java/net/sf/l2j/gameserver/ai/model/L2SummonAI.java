...
public class L2SummonAI extends L2PlayableAI implements Runnable {
    ...
    protected void onIntentionActive() {
        ...
        if (_startFollow) { setIntention(EIntention.FOLLOW, summon.getOwner()); }
    ...
    protected void onEvtThink() {
        if (_thinking || actor.isCastingNow() || actor.isAllSkillsDisabled()) { return; }
    ...
    protected void onEvtFinishCasting() {
        if (_lastAttack == null) { ((L2Summon) actor).setFollow(_startFollow); }
        else {
    ...
    private void avoidAttack(L2Character attacker) {
        ...
        if (((L2Summon) actor).getOwner() != null && ((L2Summon) actor).getOwner() != attacker && ((L2Summon) actor).getOwner().isInsideRadius(actor, 2 * AVOID_RADIUS, true, false)) { _startAvoid = true; }
    }
    ...
    public void run() {
        if (_startAvoid) {
            ...
            if (!actorMoving && !actor.isDead() && !actor.isMovementDisabled()) {
                ...
                if (PathFinding.getInstance().canMoveToTarget(actor.getX(), actor.getY(), actor.getZ(), targetX, targetY, actor.getZ())) { moveTo(targetX, targetY, actor.getZ()); }
            }
        }
    }
    ...
    protected void onIntentionCast(L2Skill skill, L2Object target) {
        if (getIntention() == EIntention.ATTACK) { _lastAttack = (L2Character) getTarget(); }
        else { _lastAttack = null; }

        super.onIntentionCast(skill, target);
    }

    private void startAvoidTask() {
        if (_avoidTask == null) { _avoidTask = ThreadPoolManager.getInstance().scheduleAiAtFixedRate(this, 100, 100); }
    }
