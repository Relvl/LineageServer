...
public class Q403_PathToARogue extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30379-05.htm")) {
            if (player.getClassId() != ClassId.fighter) { htmltext = (player.getClassId() == ClassId.rogue) ? "30379-02a.htm" : "30379-02.htm"; }
            else if (player.getLevel() < 19) { htmltext = "30379-02.htm"; }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case BEZIQUE:
                        if (cond == 1) { htmltext = "30379-07.htm"; }
                        else if (cond == 2 || cond == 3) { htmltext = "30379-10.htm"; }
                        else if (cond == 4) {
                        ...
                        else if (cond == 5) { htmltext = "30379-11.htm"; }
                        else if (cond == 6) {
                    ...
                    case NETI:
                        if (cond == 1) { htmltext = "30425-01.htm"; }
                        else if (cond == 2) { htmltext = "30425-06.htm"; }
                        else if (cond == 3) {
                        ...
                        else if (cond > 3) { htmltext = "30425-08.htm"; }
                        break;
                }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if ((equippedItemId != NETI_BOW) && (equippedItemId != NETI_DAGGER)) { return null; }

        switch (npc.getNpcId()) {
            case 20035:
            case 20045:
            case 20051:
                if (st.getInt("cond") == 2 && st.dropItems(SPARTOI_BONES, 1, 10, 200000)) { st.set("cond", "3"); }
                break;

            case 20042:
                if (st.getInt("cond") == 2 && st.dropItems(SPARTOI_BONES, 1, 10, 300000)) { st.set("cond", "3"); }
                break;

            case 20054:
            case 20060:
                if (st.getInt("cond") == 2 && st.dropItems(SPARTOI_BONES, 1, 10, 800000)) { st.set("cond", "3"); }
                break;

            case 27038:
                if (st.getInt("cond") == 5) {
                    int randomItem = Rnd.get(STOLEN_JEWELRY, STOLEN_NECKLACE);
