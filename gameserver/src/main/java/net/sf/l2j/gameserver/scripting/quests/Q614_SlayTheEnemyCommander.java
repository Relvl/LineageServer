        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getLevel() >= 75) {
                    if (player.getAllianceWithVarkaKetra() <= -4 && st.hasQuestItems(VARKA_ALLIANCE_4) && !st.hasQuestItems(FEATHER_OF_WISDOM)) { htmltext = "31377-01.htm"; }
                    else { htmltext = "31377-02.htm"; }
                }
                else { htmltext = "31377-03.htm"; }
                break;
