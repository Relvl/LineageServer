...
public class Q275_DarkWingedSpies extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getRace() != PlayerRace.Orc) { htmltext = "30567-00.htm"; }
                else if (player.getLevel() < 11) { htmltext = "30567-01.htm"; }
                else { htmltext = "30567-02.htm"; }
                break;

            case QuestState.STATE_STARTED:
                if (st.getInt("cond") == 1) { htmltext = "30567-04.htm"; }
                else {
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            case DARKWING_BAT:
                if (st.dropItemsAlways(DARKWING_BAT_FANG, 1, 70)) { st.set("cond", "2"); }
            ...
            case VARANGKA_TRACKER:
                if (st.hasQuestItems(VARANGKA_PARASITE)) {
                    ...
                    if (st.dropItemsAlways(DARKWING_BAT_FANG, 5, 70)) { st.set("cond", "2"); }
                }
                break;
        }
