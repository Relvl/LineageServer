...
public class L2SiegeZone extends L2ZoneType {
    ...
    public void setParameter(String name, String value) {
        if (name.equals("castleId")) {
            if (_siegableId != -1) { throw new IllegalArgumentException("Siege object already defined!"); }
    ...
    protected void onExit(L2Character character) {
        ...
        if (character instanceof L2PcInstance) {
            ...
            if (_isActiveSiege) {
                ...
                if (activeChar.getMountType() == 2) { activeChar.exitedNoLanding(); }

                PvpFlagTaskManager.getInstance().add(activeChar, Config.PVP_NORMAL_TIME);

                // Set pvp flag
                if (activeChar.getPvpFlag() == 0) { activeChar.updatePvPFlag(1); }
            }

            activeChar.setIsInSiege(false);
        }
        else if (character instanceof L2SiegeSummonInstance) { ((L2SiegeSummonInstance) character).unSummon(((L2SiegeSummonInstance) character).getOwner()); }
    }
    ...
    public void updateZoneStatusForCharactersInside() {
        if (_isActiveSiege) {
            for (L2Character character : _characterList) {
                if (character != null) { onEnter(character); }
            }
        }
        else {
            for (L2Character character : _characterList) {
                ...
                if (character instanceof L2PcInstance) {
                    ...
                    if (player.getMountType() == 2) { player.exitedNoLanding(); }
                }
                else if (character instanceof L2SiegeSummonInstance) { ((L2SiegeSummonInstance) character).unSummon(((L2SiegeSummonInstance) character).getOwner()); }
            }
        }
    ...
    public void announceToPlayers(String message) {
        for (L2PcInstance player : getKnownTypeInside(L2PcInstance.class)) { player.sendMessage(message); }
    }
    ...
    public void banishForeigners(L2Clan owningClan) {
        for (L2PcInstance player : getKnownTypeInside(L2PcInstance.class)) {
            if (player.getClan() == owningClan || player.isGM()) { continue; }
