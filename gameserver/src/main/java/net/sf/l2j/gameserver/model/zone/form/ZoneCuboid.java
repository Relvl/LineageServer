...
public class ZoneCuboid extends L2ZoneForm {
    ...
    @Override
    public boolean isIntersectsRectangle(int ax1, int ax2, int ay1, int ay2) {
        ...
        if (isLineSegmentsIntersect(_x1, _y1, _x2, _y1, ax1, ay1, ax1, ay2)) { return true; }
        if (isLineSegmentsIntersect(_x1, _y1, _x2, _y1, ax2, ay1, ax2, ay2)) { return true; }
        if (isLineSegmentsIntersect(_x1, _y2, _x2, _y2, ax1, ay1, ax1, ay2)) { return true; }
        if (isLineSegmentsIntersect(_x1, _y2, _x2, _y2, ax2, ay1, ax2, ay2)) { return true; }

        // Vertical lines may intersect horizontal lines
        if (isLineSegmentsIntersect(_x1, _y1, _x1, _y2, ax1, ay1, ax2, ay1)) { return true; }
        if (isLineSegmentsIntersect(_x1, _y1, _x1, _y2, ax1, ay2, ax2, ay2)) { return true; }
        if (isLineSegmentsIntersect(_x2, _y1, _x2, _y2, ax1, ay1, ax2, ay1)) { return true; }
        if (isLineSegmentsIntersect(_x2, _y1, _x2, _y2, ax1, ay2, ax2, ay2)) { return true; }
    ...
    public void visualizeZone(int z) {
        ...
        for (int x = _x1; x < _x2; x += STEP) {
            dropDebugItem(ItemConst.ADENA_ID, 1, x, _y1, z);
            dropDebugItem(ItemConst.ADENA_ID, 1, x, _y2, z);
        }
        // y1->y2
        for (int y = _y1; y < _y2; y += STEP) {
            dropDebugItem(ItemConst.ADENA_ID, 1, _x1, y, z);
            dropDebugItem(ItemConst.ADENA_ID, 1, _x2, y, z);
        }
    }
