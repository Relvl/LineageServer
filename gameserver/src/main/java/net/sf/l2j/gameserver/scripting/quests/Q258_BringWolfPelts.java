...
public class Q258_BringWolfPelts extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                if (st.getQuestItemsCount(WOLF_PELT) < 40) { htmltext = "30001-05.htm"; }
                else {
                    st.takeItems(WOLF_PELT, -1);
                    int randomNumber = Rnd.get(16);

                    // Reward is based on a random number (1D16).
                    if (randomNumber == 0) { st.giveItems(COTTON_SHIRT, 1); }
                    else if (randomNumber < 6) { st.giveItems(LEATHER_PANTS, 1); }
                    else if (randomNumber < 9) { st.giveItems(LEATHER_SHIRT, 1); }
                    else if (randomNumber < 13) { st.giveItems(SHORT_LEATHER_GLOVES, 1); }
                    else { st.giveItems(TUNIC, 1); }

                    htmltext = "30001-06.htm";

                    if (randomNumber == 0) { st.playSound(ESound.ItemSound_quest_jackpot); }
                    else { st.playSound(ESound.ItemSound_quest_finish); }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropItemsAlways(WOLF_PELT, 1, 40)) { st.set("cond", "2"); }
