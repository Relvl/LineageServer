...
public final class RequestActionUse extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        switch (_actionId) {
            ...
            case 1:
                if (activeChar.isRunning()) { activeChar.setWalking(); }
                else { activeChar.setRunning(); }
                break;
            ...
            case 22: // Attack (pet attack)
                ...
                if (Util.contains(PASSIVE_SUMMONS, pet.getNpcId())) { return; }
                ...
                if (activeChar.isInOlympiadMode() && !activeChar.isOlympiadStart()) { return; }
            ...
            case 19: // Returns pet to control item
                if (pet == null || !(pet instanceof L2PetInstance)) { return; }

                if (pet.isDead()) { activeChar.sendPacket(SystemMessageId.DEAD_PET_CANNOT_BE_RETURNED); }
                else if (pet.isOutOfControl()) { activeChar.sendPacket(SystemMessageId.PET_REFUSING_ORDER); }
            ...
            case 52: // Unsummon a servitor
                if (pet == null || !(pet instanceof L2SummonInstance)) { return; }
            ...
            case 54: // move to target hatch/strider
                if (target == null || pet == null || pet == target) { return; }
            ...
            case 1001: // Sin Eater - Ultimate Bombastic Buster
                if (useSkill(4139, pet) && pet.getNpcId() == SIN_EATER_ID && Rnd.get(100) < 10) {
                    pet.broadcastPacket(new NpcSay(pet.getObjectId(), EChatType.ALL, pet.getNpcId(), SIN_EATER_ACTIONS_STRINGS[Rnd.get(SIN_EATER_ACTIONS_STRINGS.length)]));
                }
                break;
            ...
            default:
                _log.warn("{}: unhandled action type {}", activeChar.getName(), _actionId);
        }
    }
    ...
    private boolean useSkill(int skillId, L2Object target) {
        ...
        if (activeChar == null || activeChar.isInStoreMode()) { return false; }

        final L2Summon activeSummon = activeChar.getPet();
        if (activeSummon == null) { return false; }
        ...
        if (skill.isOffensive() && activeChar == target) { return false; }
