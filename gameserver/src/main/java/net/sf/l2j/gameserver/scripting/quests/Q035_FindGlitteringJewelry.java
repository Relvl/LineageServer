...
public class Q035_FindGlitteringJewelry extends Quest {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getLevel() >= 60) {
                    ...
                    if (fwear != null && fwear.getInt("cond") == 6) { htmltext = "30091-0.htm"; }
                    else { htmltext = "30091-0a.htm"; }
                }
                else { htmltext = "30091-0b.htm"; }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case ELLIE:
                        if (cond == 1 || cond == 2) { htmltext = "30091-1a.htm"; }
                        else if (cond == 3) { htmltext = "30091-2.htm"; }
                        else if (cond == 4) { htmltext = (st.getQuestItemsCount(ORIHARUKON) >= 5 && st.getQuestItemsCount(SILVER_NUGGET) >= 500 && st.getQuestItemsCount(THONS) >= 150) ? "30091-4.htm" : "30091-4a.htm"; }
                        break;

                    case FELTON:
                        if (cond == 1) { htmltext = "30879-0.htm"; }
                        else if (cond > 1) { htmltext = "30879-1a.htm"; }
                        break;
                }
                break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropItems(ROUGH_JEWEL, 1, 10, 500000)) { st.set("cond", "3"); }
