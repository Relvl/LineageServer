...
public class AdminBuffs implements IAdminCommandHandler {
    ...
    public boolean useAdminCommand(String command, L2PcInstance activeChar) {
        if (command.startsWith("admin_getbuffs")) {
            ...
            if (st.hasMoreTokens()) {
                ...
                int page = 1;
                if (st.hasMoreTokens()) { page = Integer.parseInt(st.nextToken()); }
            ...
            try {
                ...
                for (L2PcInstance knownChar : activeChar.getKnownList().getKnownTypeInRadius(L2PcInstance.class, radius)) {
                    if (!knownChar.equals(activeChar)) { knownChar.stopAllEffects(); }
                }
            ...
            else if (activeChar.getTarget() instanceof L2PcInstance) { player = (L2PcInstance) activeChar.getTarget(); }
    ...
    public static void showBuffs(L2PcInstance activeChar, L2Character target, int page) {
        ...
        if (page > effects.length / PAGE_LIMIT + 1 || page < 1) { return; }

        int max = effects.length / PAGE_LIMIT;
        if (effects.length > PAGE_LIMIT * max) { max++; }

        final StringBuilder sb = new StringBuilder("<html><table width=\"100%\"><tr><td width=45><button value=\"Main\" action=\"bypass -h admin_admin\" width=45 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td><td width=180><center><font color=\"LEVEL\">Effects of " + target
                .getName() + "</font></td><td width=45><button value=\"Back\" action=\"bypass -h admin_current_player\" width=45 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td></tr></table><br><table width=\"100%\"><tr><td width=160>Skill</td><td width=60>Time Left</td><td width=60>Action</td></tr>");
        ...
        for (int i = start; i < end; i++) {
            ...
            if (e != null) {
                StringUtil.append(sb, "<tr><td>", e.getSkill().getName(), "</td><td>", (e.getSkill().isToggle())
                                                                                       ? "toggle"
                                                                                       : e.getPeriod() - e.getTime() + "s", "</td><td><a action=\"bypass -h admin_stopbuff ", target.getObjectId(), " ", e.getSkill()
        ...
        for (int x = 0; x < max; x++) {
            int pagenr = x + 1;
            if (page == pagenr) { StringUtil.append(sb, "<td>Page ", pagenr, "</td>"); }
            else { StringUtil.append(sb, "<td><a action=\"bypass -h admin_getbuffs ", target.getName(), " ", x + 1, "\"> Page ", pagenr, "</a></td>"); }
        }
