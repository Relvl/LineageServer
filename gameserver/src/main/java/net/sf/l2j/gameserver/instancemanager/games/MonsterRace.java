...
public class MonsterRace {
    ...
    protected static final PlaySound PLAY_SOUND_RACE = new PlaySound(ESound.S_RACE);
    protected static final PlaySound PLAY_SOUND_START = new PlaySound(ESound.ItemSound2_race_start, 121209259, 12125, 182487, -3559);
    ...
    protected static int _finalCountdown;
    ...
    protected MonsterRace() {
        ...
        for (int i = 31003; i < 31027; i++) { _npcTemplates.add(i); }
    ...
    private class Announcement implements Runnable {
        ...
        @Override
        public void run() {
            if (_finalCountdown > 1200) { _finalCountdown = 0; }
            ...
            switch (_finalCountdown) {
                ...
                case 300: // 5 min
                    Broadcast.toAllPlayersInZoneType(L2DerbyTrackZone.class, SystemMessage.getSystemMessage(SystemMessageId.MONSRACE_TICKETS_NOW_AVAILABLE_FOR_S1_RACE)
                                                                                          .addNumber(_raceNumber), SystemMessage.getSystemMessage(SystemMessageId.MONSRACE_TICKETS_STOP_IN_S1_MINUTES).addNumber(10));
                    break;

                case 600: // 10 min
                    Broadcast.toAllPlayersInZoneType(L2DerbyTrackZone.class, SystemMessage.getSystemMessage(SystemMessageId.MONSRACE_TICKETS_NOW_AVAILABLE_FOR_S1_RACE)
                                                                                          .addNumber(_raceNumber), SystemMessage.getSystemMessage(SystemMessageId.MONSRACE_TICKETS_STOP_IN_S1_MINUTES).addNumber(5));
                    break;

                case 840: // 14 min
                    Broadcast.toAllPlayersInZoneType(L2DerbyTrackZone.class, SystemMessage.getSystemMessage(SystemMessageId.MONSRACE_TICKETS_NOW_AVAILABLE_FOR_S1_RACE)
                                                                                          .addNumber(_raceNumber), SystemMessage.getSystemMessage(SystemMessageId.MONSRACE_TICKETS_STOP_IN_S1_MINUTES).addNumber(1));
                    break;
                ...
                case 900: // 15 min
                    ...
                    Broadcast.toAllPlayersInZoneType(L2DerbyTrackZone.class, SystemMessage.getSystemMessage(SystemMessageId.MONSRACE_TICKETS_NOW_AVAILABLE_FOR_S1_RACE)
                                                                                          .addNumber(_raceNumber), SystemMessage.getSystemMessage(SystemMessageId.MONSRACE_S1_TICKET_SALES_CLOSED));
                    break;
                ...
                case 1080: // 18 min
                    ...
                    Broadcast.toAllPlayersInZoneType(L2DerbyTrackZone.class, SystemMessage.getSystemMessage(SystemMessageId.MONSRACE_RACE_START), PLAY_SOUND_RACE, PLAY_SOUND_START, _packet);
                    break;
                ...
                case 1115: // 18 min 35 sec
                    ...
                    Broadcast.toAllPlayersInZoneType(L2DerbyTrackZone.class, SystemMessage.getSystemMessage(SystemMessageId.MONSRACE_FIRST_PLACE_S1_SECOND_S2)
                                                                                          .addNumber(getFirstPlace())
                                                                                          .addNumber(getSecondPlace()), SystemMessage.getSystemMessage(SystemMessageId.MONSRACE_S1_RACE_END).addNumber(_raceNumber));
                    _raceNumber++;
                    break;
    ...
    public void newSpeeds() {
        ...
        for (int i = 0; i < 8; i++) {
            total = 0;
            for (int j = 0; j < 20; j++) {
                if (j == 19) { _speeds[i][j] = 100; }
                else { _speeds[i][j] = Rnd.get(60) + 65; }
                total += _speeds[i][j];
            }
    ...
    protected static void loadHistory() {
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.error("MonsterRace: can't load history: {}", e.getMessage(), e);
        }
        LOGGER.info("MonsterRace : loaded {} records, currently on race #{}", _history.size(), _raceNumber);
    }
    ...
    protected static void saveHistory(HistoryInfo history) {
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.error("MonsterRace: can't save history: {}", e.getMessage(), e);
        }
    }
    ...
    protected static void loadBets() {
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            while (rset.next()) { setBetOnLane(rset.getInt("lane_id"), rset.getLong("bet"), false); }

            rset.close();
            statement.close();
        }
        catch (SQLException e) {
            LOGGER.error("MonsterRace: can't load bets: {}", e.getMessage(), e);
        }
    }
    ...
    protected static void saveBet(int lane, long sum) {
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.error("MonsterRace: can't save bet: {}", e.getMessage(), e);
        }
    }
    ...
    protected static void clearBets() {
        for (int key : _betsPerLane.keySet()) { _betsPerLane.put(key, 0L); }
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.error("MonsterRace: can't clear bets: {}", e.getMessage(), e);
        }
    }
    ...
    protected static void calculateOdds() {
        ...
        for (long amount : sortedLanes.values()) { sumOfAllLanes += amount; }

        // As we get the sum, we can now calculate the odd rate of each lane.
        for (long amount : sortedLanes.values()) { _odds.add((amount == 0) ? 0D : Math.max(1.25, sumOfAllLanes * 0.7 / amount)); }
    }
