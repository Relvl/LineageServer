    ...
    public String onSkillSee(L2Npc npc, L2PcInstance caster, L2Skill skill, L2Object[] targets, boolean isPet) {
        if (npc instanceof L2ChestInstance) {
            ...
            if (!chest.isInteracted()) {
                ...
                if (Rnd.get(100) < 40) {
                    switch (skill.getId()) {
                        ...
                        case SKILL_DELUXE_KEY:
                            ...
                            if (keyLevelNeeded < 0) { keyLevelNeeded *= -1; }

                            // Regular keys got 60% to succeed.
                            int chance = ((skill.getId() == SKILL_BOX_KEY) ? 60 : 100) - keyLevelNeeded * 40;
                ...
                else { attack(chest, isPet ? caster.getPet() : caster); }
            }
        }
    ...
    public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isPet) {
        if (npc instanceof L2ChestInstance) {
            ...
            if (!chest.isInteracted()) {
                ...
                if (Rnd.get(100) < 40) { chest.doCast(SkillTable.getInfo(4143, Math.min(10, Math.round(npc.getLevel() / 10)))); }
                // Mimic behavior : attack the caster.
                else { attack(chest, isPet ? attacker.getPet() : attacker, (damage * 100) / (chest.getLevel() + 7)); }
            }
        }
