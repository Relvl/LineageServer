    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getRace() != PlayerRace.Dwarf) { htmltext = "30532-00.htm"; }
                else if (player.getLevel() < 5) { htmltext = "30532-01.htm"; }
                else { htmltext = "30532-02.htm"; }
                break;

            case QuestState.STATE_STARTED:
                switch (npc.getNpcId()) {
                    case SPIRON:
                        int goblinNecklaces = st.getQuestItemsCount(GOBLIN_NECKLACE);
                        int goblinPendants = st.getQuestItemsCount(GOBLIN_PENDANT);
                        int goblinLordPendants = st.getQuestItemsCount(GOBLIN_LORD_PENDANT);
                        int suspiciousMemos = st.getQuestItemsCount(SUSPICIOUS_MEMO);

                        int countAll = goblinNecklaces + goblinPendants + goblinLordPendants;

                        boolean hasContract = st.hasQuestItems(SUSPICIOUS_CONTRACT);

                        if (countAll == 0) { htmltext = "30532-04.htm"; }
                        else {
                            if (hasContract) { htmltext = "30532-10.htm"; }
                            else if (suspiciousMemos > 0) {
                                if (suspiciousMemos > 1) { htmltext = "30532-09.htm"; }
                                else { htmltext = "30532-08.htm"; }
                            }
                            else { htmltext = "30532-05.htm"; }
                    ...
                    case BALANKI:
                        if (!st.hasQuestItems(SUSPICIOUS_CONTRACT)) { htmltext = "30533-01.htm"; }
                        else {
