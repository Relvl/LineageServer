    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getLevel() < 36) { htmltext = "31572-02.htm"; }
                else {
                    QuestState st2 = player.getQuestState("Q051_OFullesSpecialBait");
                    if (st2 != null && st2.isCompleted()) { htmltext = "31572-01.htm"; }
                    else { htmltext = "31572-03.htm"; }
                }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case OFULLE:
                        if (cond == 1) { htmltext = (!st.hasQuestItems(BIG_YELLOW_TREASURE_CHEST)) ? "31572-06.htm" : "31572-05.htm"; }
                        else if (cond == 2) { htmltext = "31572-09.htm"; }
                        break;

                    case KIKI:
                        if (cond == 2) { htmltext = "31442-01.htm"; }
                        break;
                }
                break;
