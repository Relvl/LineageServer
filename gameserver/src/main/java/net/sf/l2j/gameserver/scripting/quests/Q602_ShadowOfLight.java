...
public class Q602_ShadowOfLight extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("31683-02.htm")) {
            if (player.getLevel() < 68) { htmltext = "31683-02a.htm"; }
            else {
            ...
            for (int[] element : REWARDS) {
                if (random < element[4]) {
                    st.rewardItems(ItemConst.ADENA_ID, element[1]);

                    if (element[0] != 0) { st.giveItems(element[0], 3); }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                if (cond == 1) { htmltext = "31683-03.htm"; }
                else if (cond == 2) { htmltext = "31683-04.htm"; }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropItems(EYE_OF_DARKNESS, 1, 100, (npc.getNpcId() == 21299) ? 450000 : 500000)) { st.set("cond", "2"); }
