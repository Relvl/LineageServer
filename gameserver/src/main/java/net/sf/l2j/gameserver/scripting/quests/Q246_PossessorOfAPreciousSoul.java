    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (st.hasQuestItems(CARADINE_LETTER_1)) { htmltext = (!player.isSubClassActive() || player.getLevel() < 65) ? "31740-02.htm" : "31740-01.htm"; }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case CARADINE:
                        if (cond == 1) { htmltext = "31740-05.htm"; }
                        break;

                    case OSSIAN:
                        if (cond == 1) { htmltext = "31741-01.htm"; }
                        else if (cond == 2) { htmltext = "31741-03.htm"; }
                        else if (cond == 3) {
                            if (st.hasQuestItems(WATERBINDER, EVERGREEN)) { htmltext = "31741-04.htm"; }
                        }
                        else if (cond == 4) { htmltext = "31741-06.htm"; }
                        else if (cond == 5) {
                            if (st.hasQuestItems(RAIN_SONG)) { htmltext = "31741-07.htm"; }
                        }
                        else if (cond == 6) { htmltext = "31741-09.htm"; }
                        break;

                    case LADD:
                        if (cond == 6 && st.hasQuestItems(RELIC_BOX)) { htmltext = "30721-01.htm"; }
                        break;
                }
                break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (npcId == BARAKIEL) {
            for (L2PcInstance plr : getPartyMembers(player, npc, "cond", "4")) {
                if (!plr.isSubClassActive()) { continue; }
            ...
            if (!player.isSubClassActive()) { return null; }

            QuestState st = checkPlayerCondition(player, npc, "cond", "2");
            if (st == null) { return null; }

            if (Rnd.get(10) < 2) {
                int neklaceOrRing = (npcId == PILGRIM_OF_SPLENDOR) ? WATERBINDER : EVERGREEN;
