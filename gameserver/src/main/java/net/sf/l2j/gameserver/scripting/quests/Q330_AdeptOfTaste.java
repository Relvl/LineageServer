...
public class Q330_AdeptOfTaste extends Quest {
                ...
                switch (npc.getNpcId()) {
                    case JONAS:
                        if (st.hasQuestItems(INGREDIENT_LIST)) {
                            if (!hasAllIngredients(st)) { htmltext = "30469-04.htm"; }
                            else {
                                int dish;

                                int specialIngredientsNumber = st.getQuestItemsCount(WHITE_MANDRAGORA_SAP) + st.getQuestItemsCount(GOLDEN_HONEY) + st.getQuestItemsCount(BROWN_MOSS_BUNDLE);
                                ...
                                st.playSound((dish == JONAS_STEAK_DISH_5) ? ESound.ItemSound_quest_jackpot : ESound.ItemSound_quest_itemget);
                        ...
                        else if (st.hasAtLeastOneQuestItem(JONAS_STEAK_DISH_1, JONAS_STEAK_DISH_2, JONAS_STEAK_DISH_3, JONAS_STEAK_DISH_4, JONAS_STEAK_DISH_5)) { htmltext = "30469-06.htm"; }
                    ...
                    case MIRIEN:
                        if (st.hasQuestItems(INGREDIENT_LIST)) { htmltext = "30461-01.htm"; }
                        ...
                        else if (st.hasAtLeastOneQuestItem(MIRIEN_REVIEW_1, MIRIEN_REVIEW_2, MIRIEN_REVIEW_3, MIRIEN_REVIEW_4, MIRIEN_REVIEW_5)) { htmltext = "30461-04.htm"; }
                        break;
                    ...
                    case SONIA:
                        if (!st.hasQuestItems(RED_MANDRAGORA_SAP) && !st.hasQuestItems(WHITE_MANDRAGORA_SAP)) {
                            if (!st.hasQuestItems(SONIA_BOTANY_BOOK)) {
                                ...
                                if (st.getQuestItemsCount(RED_MANDRAGORA_ROOT) < 40 || st.getQuestItemsCount(WHITE_MANDRAGORA_ROOT) < 40) { htmltext = "30062-02.htm"; }
                                ...
                                else { htmltext = "30062-03.htm"; }
                            }
                        }
                        else { htmltext = "30062-07.htm"; }
                        break;
                    ...
                    case JACOB:
                        if (!st.hasQuestItems(HONEY) && !st.hasQuestItems(GOLDEN_HONEY)) {
                            if (!st.hasQuestItems(JACOB_INSECT_BOOK)) {
                                ...
                                if (st.getQuestItemsCount(NECTAR) < 20) { htmltext = "30073-02.htm"; }
                                else {
                                    if (st.getQuestItemsCount(ROYAL_JELLY) < 10) { htmltext = "30073-03.htm"; }
                                    else {
                    ...
                    case PANO:
                        if (!st.hasQuestItems(DIONIAN_POTATO)) {
                            if (!st.hasQuestItems(PANO_CONTRACT)) {
                                ...
                                if (st.getQuestItemsCount(HOBGOBLIN_AMULET) < 30) { htmltext = "30078-02.htm"; }
                                else {
                    ...
                    case GLYVKA:
                        if (!st.hasQuestItems(GREEN_MOSS_BUNDLE) && !st.hasQuestItems(BROWN_MOSS_BUNDLE)) {
                            if (!st.hasQuestItems(GLYVKA_BOTANY_BOOK)) {
                                ...
                                if (st.getQuestItemsCount(GREEN_MARSH_MOSS) < 20 || st.getQuestItemsCount(BROWN_MARSH_MOSS) < 20) { htmltext = "30067-02.htm"; }
                                ...
                                else { htmltext = "30067-03.htm"; }
                            }
                        }
                        else { htmltext = "30067-07.htm"; }
                        break;
                    ...
                    case ROLLANT:
                        if (!st.hasQuestItems(MONSTER_EYE_MEAT)) {
                            if (!st.hasQuestItems(ROLANT_CREATURE_BOOK)) {
                                ...
                                if (st.getQuestItemsCount(MONSTER_EYE_BODY) < 30) { htmltext = "30069-02.htm"; }
                                else {
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npcId) {
            case 20265:
                if (st.hasQuestItems(ROLANT_CREATURE_BOOK)) { st.dropItems(MONSTER_EYE_BODY, (Rnd.get(97) < 77) ? 2 : 3, 30, 970000); }
                break;

            case 20266:
                if (st.hasQuestItems(ROLANT_CREATURE_BOOK)) { st.dropItemsAlways(MONSTER_EYE_BODY, (Rnd.get(10) < 7) ? 1 : 2, 30); }
                break;

            case 20226:
                if (st.hasQuestItems(GLYVKA_BOTANY_BOOK)) { st.dropItems(Rnd.get(96) < 87 ? GREEN_MARSH_MOSS : BROWN_MARSH_MOSS, 1, 20, 960000); }
                break;

            case 20228:
                if (st.hasQuestItems(GLYVKA_BOTANY_BOOK)) { st.dropItemsAlways(Rnd.get(10) < 9 ? GREEN_MARSH_MOSS : BROWN_MARSH_MOSS, 1, 20); }
                break;

            case 20147:
                if (st.hasQuestItems(PANO_CONTRACT)) { st.dropItemsAlways(HOBGOBLIN_AMULET, 1, 30); }
                break;
            ...
            case 20229:
                if (st.hasQuestItems(JACOB_INSECT_BOOK)) {
                    ...
                    if (random < chances[0]) { st.dropItemsAlways(NECTAR, 1, 20); }
                    else if (random < chances[1]) { st.dropItemsAlways(ROYAL_JELLY, 1, 10); }
                }
                break;
            ...
            case 20156:
                if (st.hasQuestItems(SONIA_BOTANY_BOOK)) {
                    int random = Rnd.get(100);
                    int[] chances = CHANCES.get(npcId);
                    if (random < chances[1]) { st.dropItemsAlways((random < chances[0]) ? RED_MANDRAGORA_ROOT : WHITE_MANDRAGORA_ROOT, 1, 40); }
                }
                break;
        }
