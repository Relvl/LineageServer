...
public final class DuelManager {
    ...
    public void doSurrender(L2PcInstance player) {
        ...
        if (duel != null) { duel.doSurrender(player); }
    }
    ...
    public void broadcastToOppositeTeam(L2PcInstance player, L2GameServerPacket packet) {
        ...
        if (duel.getPlayerA() == player) { duel.broadcastToTeam2(packet); }
            ...
            if (duel.getPlayerA().getParty() != null && duel.getPlayerA().getParty().getPartyMembers().contains(player)) { duel.broadcastToTeam2(packet); }
            else if (duel.getPlayerB().getParty() != null && duel.getPlayerB().getParty().getPartyMembers().contains(player)) { duel.broadcastToTeam1(packet); }
        }
    }
