    ...
    public void useSkill(L2Character activeChar, L2Skill skill, L2Object[] targets) {
        ...
        for (L2Object obj : targets) {
            ...
            if (target != activeChar) {
                if (activeChar instanceof L2PcInstance && ((L2PcInstance) activeChar).isCursedWeaponEquipped()) { continue; }

                if (targetPlayer && ((L2PcInstance) target).isCursedWeaponEquipped()) { continue; }
            }
            ...
            if (targetPlayer) {
                ...
                if (cp) {
                    ...
                    if (activeChar != target) { sm = SystemMessage.getSystemMessage(SystemMessageId.S2_HP_RESTORED_BY_S1).addCharName(activeChar); }
                    else { sm = SystemMessage.getSystemMessage(SystemMessageId.S1_HP_RESTORED); }
                    ...
                    if (activeChar != target) { sm = SystemMessage.getSystemMessage(SystemMessageId.S2_MP_RESTORED_BY_S1).addCharName(activeChar); }
                    else { sm = SystemMessage.getSystemMessage(SystemMessageId.S1_MP_RESTORED); }
