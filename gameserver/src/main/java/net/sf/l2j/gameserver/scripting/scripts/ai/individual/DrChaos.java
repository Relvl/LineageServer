...
public class DrChaos extends AbstractNpcAI {
    ...
    public DrChaos() {
        ...
        else { addSpawn(DOCTOR_CHAOS, 96320, -110912, -3328, 8191, false, 0, false); }
    }
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        else if (event.equalsIgnoreCase("3")) { npc.broadcastPacket(new SocialAction(npc, 1)); }
        else if (event.equalsIgnoreCase("4")) {
            npc.broadcastPacket(new SpecialCamera(npc.getObjectId(), 1, -150, 10, 3500, 5000, 0, 0, 1, 0));
            npc.getAI().setIntention(EIntention.MOVE_TO, new HeadedLocation(95928, -110671, -3340, 0));
        }
        ...
        // Check every sec if someone is in range, if found, launch one task to decrease the timer.
            ...
            if (GrandBossManager.getInstance().getBossStatus(CHAOS_GOLEM) == NORMAL) {
                for (L2PcInstance obj : npc.getKnownList().getKnownTypeInRadius(L2PcInstance.class, 500)) {
                    ...
                    // Make him speak.
                    if (_pissedOffTimer == 15) { npc.broadcastNpcSay("How dare you trespass into my territory! Have you no fear?"); }

                    // That was "too much" for that time.
                    if (_pissedOffTimer <= 0) { crazyMidgetBecomesAngry(npc); }
                }
            }
        }
    ...
    private void crazyMidgetBecomesAngry(L2Npc npc) {
        if (GrandBossManager.getInstance().getBossStatus(CHAOS_GOLEM) == NORMAL) {
            ...
            npc.getAI().setIntention(EIntention.MOVE_TO, new HeadedLocation(96323, -110914, -3328, 0));
