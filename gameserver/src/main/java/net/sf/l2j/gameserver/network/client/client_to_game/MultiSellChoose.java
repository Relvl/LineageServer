...
public class MultiSellChoose extends L2GameClientPacket {
    ...
    private void doExchange(L2PcInstance player, L2Npc merchant, Entry templateEntry, boolean applyTaxes, boolean maintainEnchantment, int enchantment) {
        ...
        for (Ingredient e : entry.getProducts()) {
            ...
            if (!template.isStackable()) { slots += e.getItemCount() * _amount; }
            else if (player.getInventory().getItemByItemId(id) == null) { slots++; }
        ...
        for (Ingredient e : entry.getIngredients()) {
            ...
            if (newIng) { _ingredientsList.add(new Ingredient(e)); }
        }
        ...
        for (Ingredient e : entry.getIngredients()) {
            if (e.getItemId() != 65336) {
                ...
                if (itemToTake == null) {
                    ...
                    _log.warn("{} is trying to cheat using multisell, merchant id: {}", player.getName(), merchant.getNpcId());
                    return;
                }
                                ...
                                if (itemToTake.getEnchantLevel() > 0) {
                                    for (L2ItemInstance item : inventoryContents) {
                                        if (item.getEnchantLevel() < itemToTake.getEnchantLevel()) {
                                            ...
                                            if (itemToTake.getEnchantLevel() == 0) { break; }
                                        }
                                    }
                                }
