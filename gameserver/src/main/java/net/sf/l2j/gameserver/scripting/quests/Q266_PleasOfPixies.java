...
public class Q266_PleasOfPixies extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getRace() != PlayerRace.Elf) { htmltext = "31852-00.htm"; }
                else if (player.getLevel() < 3) { htmltext = "31852-01.htm"; }
                else { htmltext = "31852-02.htm"; }
                break;

            case QuestState.STATE_STARTED:
                if (st.getQuestItemsCount(PREDATOR_FANG) < 100) { htmltext = "31852-04.htm"; }
                else {
                    ...
                    else if (n < 30) { st.rewardItems(BLUE_ONYX, 1); }
                    else if (n < 60) { st.rewardItems(ONYX, 1); }
                    else { st.rewardItems(GLASS_SHARD, 1); }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            case 20525:
                if (st.dropItemsAlways(PREDATOR_FANG, Rnd.get(2, 3), 100)) { st.set("cond", "2"); }
                break;

            case 20530:
                if (st.dropItems(PREDATOR_FANG, 1, 100, 800000)) { st.set("cond", "2"); }
                break;

            case 20534:
                if (st.dropItems(PREDATOR_FANG, (Rnd.get(3) == 0) ? 1 : 2, 100, 600000)) { st.set("cond", "2"); }
                break;

            case 20537:
                if (st.dropItemsAlways(PREDATOR_FANG, 2, 100)) { st.set("cond", "2"); }
                break;
        }
