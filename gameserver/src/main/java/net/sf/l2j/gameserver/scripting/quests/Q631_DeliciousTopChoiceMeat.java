...
public class Q631_DeliciousTopChoiceMeat extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                if (cond == 1) { htmltext = "31537-03a.htm"; }
                else if (cond == 2) {
                    if (st.getQuestItemsCount(TOP_QUALITY_MEAT) >= 120) { htmltext = "31537-04.htm"; }
                    else {
                        st.set("cond", "1");
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropItems(TOP_QUALITY_MEAT, 1, 120, CHANCES.get(npc.getNpcId()))) { st.set("cond", "2"); }
