...
public final class L2BabyPetInstance extends L2PetInstance {
    ...
    public void onSpawn() {
        ...
        for (L2Skill skill : getTemplate().getHealSkills()) {
            if (skill.getTargetType() != ESkillTargetType.TARGET_OWNER_PET || skill.getSkillType() != L2SkillType.HEAL) { continue; }
            ...
            if (healPower == 0) {
                ...
                if (skill.getPower() > healPower) { _majorHeal = new IntIntHolder(skill.getId(), skillLevel); }
                else { _minorHeal = new IntIntHolder(skill.getId(), skillLevel); }
            }
        }
        startCastTask();
    }
    ...
    protected void castSkill(L2Skill skill) {
        ...
        if (!previousFollowStatus && !isInsideRadius(getOwner(), skill.getCastRange(), true, true)) { return; }
        ...
        if (previousFollowStatus != isFollow()) { setFollow(previousFollowStatus); }
    }
