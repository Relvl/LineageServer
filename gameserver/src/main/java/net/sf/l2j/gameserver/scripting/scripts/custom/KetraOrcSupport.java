    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        if (player.isInParty()) {
            for (L2PcInstance partyMember : player.getParty().getPartyMembers()) {
                if (partyMember != null) { testKetraDemote(partyMember); }
            }
        }
        else { testKetraDemote(player); }
    ...
    private static void testKetraDemote(L2PcInstance player) {
        if (player.isAlliedWithKetra()) {
            ...
            for (int i = 7215; i >= 7211; i--) {
                ...
                if (item != null) {
                    // Destroy the badge.
                    player.getInventory().destroyItemByItemId(EItemProcessPurpose.QUEST, i, item.getCount(), player, player, true);

                    // Badge lvl 1 ; no addition of badge of lower level.
                    if (i != 7211) { player.addItem(EItemProcessPurpose.QUEST, i - 1, 1, player, true); }
