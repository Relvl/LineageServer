...
public class DoorTable {
                        ...
                        if (d.getNodeName().equalsIgnoreCase("door")) {
                            ...
                            if (castleId > 0) {
                                ...
                                if (castle != null) {
                                    ...
                                    if (name.contains("wall")) { door.setIsWall(true); }
                            ...
                            if (_regions.containsKey(door.getMapRegion())) { _regions.get(door.getMapRegion()).add(door); }
                            else {
    ...
    public boolean checkIfDoorsBetween(int ox, int oy, int oz, int tx, int ty, int tz) {
        ...
        for (L2DoorInstance door : doors) {
            if (door.isOpened() || door.getCurrentHp() <= 0) { continue; }
    ...
    private void onStart() {
        try {
            ...
            for (L2DoorInstance doorInst : getDoors()) {
                ...
                if (doorInst.getName().startsWith("Eva")) { doorInst.setAutoActionDelay(420000); }
                // Tower of Insolence (every 5 minutes)
                else if (doorInst.getName().startsWith("hubris")) { doorInst.setAutoActionDelay(300000); }
            }
