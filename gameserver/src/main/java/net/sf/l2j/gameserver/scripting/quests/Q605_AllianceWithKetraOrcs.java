...
public class Q605_AllianceWithKetraOrcs extends Quest {
    ...
    public Q605_AllianceWithKetraOrcs() {
        ...
        for (int mobs : CHANCES.keySet()) { addKillId(mobs); }
    }
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("31371-03a.htm")) {
            if (player.isAlliedWithVarka()) { htmltext = "31371-02a.htm"; }
            else {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getLevel() >= 74) { htmltext = "31371-01.htm"; }
                else {
            ...
            case QuestState.STATE_STARTED:
                ...
                if (cond == 1) {
                    if (st.getQuestItemsCount(VARKA_BADGE_SOLDIER) < 100) { htmltext = "31371-03b.htm"; }
                    else { htmltext = "31371-09.htm"; }
                }
                else if (cond == 2) {
                    if (st.getQuestItemsCount(VARKA_BADGE_SOLDIER) < 200 || st.getQuestItemsCount(VARKA_BADGE_OFFICER) < 100) { htmltext = "31371-12.htm"; }
                    else { htmltext = "31371-13.htm"; }
                }
                else if (cond == 3) {
                    if (st.getQuestItemsCount(VARKA_BADGE_SOLDIER) < 300 || st.getQuestItemsCount(VARKA_BADGE_OFFICER) < 200 || st.getQuestItemsCount(VARKA_BADGE_CAPTAIN) < 100) { htmltext = "31371-15.htm"; }
                    else { htmltext = "31371-16.htm"; }
                }
                else if (cond == 4) {
                ...
                else if (cond == 6) { htmltext = "31371-08.htm"; }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npcId) {
            ...
            case 21354:
            case 21355:
                if (cond == 1) { st.dropItems(VARKA_BADGE_SOLDIER, 1, 100, CHANCES.get(npcId)); }
                else if (cond == 2) { st.dropItems(VARKA_BADGE_SOLDIER, 1, 200, CHANCES.get(npcId)); }
                else if (cond == 3 || cond == 4) { st.dropItems(VARKA_BADGE_SOLDIER, 1, 300, CHANCES.get(npcId)); }
                else if (cond == 5) { st.dropItems(VARKA_BADGE_SOLDIER, 1, 400, CHANCES.get(npcId)); }
                break;
            ...
            case 21369:
            case 21370:
                if (cond == 2) { st.dropItems(VARKA_BADGE_OFFICER, 1, 100, CHANCES.get(npcId)); }
                else if (cond == 3) { st.dropItems(VARKA_BADGE_OFFICER, 1, 200, CHANCES.get(npcId)); }
                else if (cond == 4) { st.dropItems(VARKA_BADGE_OFFICER, 1, 300, CHANCES.get(npcId)); }
                else if (cond == 5) { st.dropItems(VARKA_BADGE_OFFICER, 1, 400, CHANCES.get(npcId)); }
                break;
            ...
            case 21374:
            case 21375:
                if (cond == 3) { st.dropItems(VARKA_BADGE_CAPTAIN, 1, 100, CHANCES.get(npcId)); }
                else if (cond == 4 || cond == 5) { st.dropItems(VARKA_BADGE_CAPTAIN, 1, 200, CHANCES.get(npcId)); }
                break;
        }
