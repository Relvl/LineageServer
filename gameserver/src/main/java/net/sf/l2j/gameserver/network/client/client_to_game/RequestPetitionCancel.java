...
public final class RequestPetitionCancel extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (PetitionManager.getInstance().isPlayerInConsultation(activeChar)) {
            ...
            else { activeChar.sendPacket(SystemMessageId.PETITION_UNDER_PROCESS); }
        }
        else {
            if (PetitionManager.getInstance().isPlayerPetitionPending(activeChar)) {
                if (PetitionManager.getInstance().cancelActivePetition(activeChar)) {
                    ...
                    GmListTable.broadcastToGMs(new CreatureSay(activeChar.getObjectId(), EChatType.HERO_VOICE, "Petition System", msgContent));
                }
                else { activeChar.sendPacket(SystemMessageId.FAILED_CANCEL_PETITION_TRY_LATER); }
            }
            else { activeChar.sendPacket(SystemMessageId.PETITION_NOT_SUBMITTED); }
        }
    }
}