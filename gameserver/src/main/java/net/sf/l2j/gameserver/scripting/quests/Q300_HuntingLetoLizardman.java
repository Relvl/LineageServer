...
public class Q300_HuntingLetoLizardman extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30126-03.htm")) {
            ...
            if (st.getQuestItemsCount(BRACELET) >= 60) {
                ...
                int luck = Rnd.get(3);
                if (luck == 0) { st.rewardItems(ItemConst.ADENA_ID, 30000); }
                else if (luck == 1) { st.rewardItems(1867, 50); }
                else if (luck == 2) { st.rewardItems(1872, 50); }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropItems(BRACELET, 1, 60, CHANCES.get(npc.getNpcId()))) { st.set("cond", "2"); }
