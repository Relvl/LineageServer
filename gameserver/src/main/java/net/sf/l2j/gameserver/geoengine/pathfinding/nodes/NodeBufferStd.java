...
public class NodeBufferStd {
    ...
    public NodeBufferStd(int size) {
        ...
        for (int x = 0; x < size; x++) { for (int y = 0; y < size; y++) { _buffer[x][y] = new Node(); } }
    }
    ...
    public final Node findPath(int gox, int goy, short goz, int gtx, int gty, short gtz) {
        ...
        do {
            // reached target?
            if (_current.getLoc().getGeoX() == _gtx && _current.getLoc().getGeoY() == _gty && Math.abs(_current.getLoc().getZ() - _gtz) < 8) { return _current; }
    ...
    public final List<Node> debugPath() {
        ...
        for (Node[] nodes : _buffer) {
            for (Node node : nodes) {
                if (node.getLoc() == null || node.getCost() <= 0) { continue; }
    ...
    public final void free() {
        ...
        for (Node[] nodes : _buffer) { for (Node node : nodes) { if (node.getLoc() != null) { node.free(); } } }
    ...
    private final void expand() {
        ...
        if ((nswe & GeoStructure.CELL_FLAG_N) != 0) {
            ...
            if (node != null) { nsweN = node.getLoc().getNSWE(); }
        }

        // can move south, expand
        if ((nswe & GeoStructure.CELL_FLAG_S) != 0) {
            node = addNode(x, y + 1, z, Config.BASE_WEIGHT);
            if (node != null) { nsweS = node.getLoc().getNSWE(); }
        }

        // can move west, expand
        if ((nswe & GeoStructure.CELL_FLAG_W) != 0) {
            node = addNode(x - 1, y, z, Config.BASE_WEIGHT);
            if (node != null) { nsweW = node.getLoc().getNSWE(); }
        }

        // can move east, expand
        if ((nswe & GeoStructure.CELL_FLAG_E) != 0) {
            node = addNode(x + 1, y, z, Config.BASE_WEIGHT);
            if (node != null) { nsweE = node.getLoc().getNSWE(); }
        }

        // can move north-west, expand
        if ((nsweN & GeoStructure.CELL_FLAG_W) != 0 || (nsweW & GeoStructure.CELL_FLAG_N) != 0) { addNode(x - 1, y - 1, z, Config.DIAGONAL_WEIGHT); }

        // can move north-east, expand
        if ((nsweN & GeoStructure.CELL_FLAG_E) != 0 || (nsweE & GeoStructure.CELL_FLAG_N) != 0) { addNode(x + 1, y - 1, z, Config.DIAGONAL_WEIGHT); }

        // can move south-west, expand
        if ((nsweS & GeoStructure.CELL_FLAG_W) != 0 || (nsweW & GeoStructure.CELL_FLAG_S) != 0) { addNode(x - 1, y + 1, z, Config.DIAGONAL_WEIGHT); }

        // can move south-east, expand
        if ((nsweS & GeoStructure.CELL_FLAG_E) != 0 || (nsweE & GeoStructure.CELL_FLAG_S) != 0) { addNode(x + 1, y + 1, z, Config.DIAGONAL_WEIGHT); }
    }
    ...
    private final Node getNode(int x, int y, short z) {
        ...
        final int ix = x - _cx;
        if (ix < 0 || ix >= _size) { return null; }

        // check node Y out of coordinates
        final int iy = y - _cy;
        if (iy < 0 || iy >= _size) { return null; }

        // get node
        Node result = _buffer[ix][iy];

        // check and update
        if (result.getLoc() == null) { result.setLoc(x, y, z); }
    ...
    private final Node addNode(int x, int y, short z, int weight) {
        ...
        if (node.getCost() >= 0) { return node; }

        node.setParent(_current);
        if (node.getLoc().getNSWE() != (byte) 0x0F) { node.setCost(getCostH(x, y, node.getLoc().getZ()) + weight * Config.OBSTACLE_MULTIPLIER); }
        else { node.setCost(getCostH(x, y, node.getLoc().getZ()) + weight); }
        ...
        if (count >= Config.MAX_ITERATIONS * 4) { System.err.println("Pathfinding: too long loop detected, cost:" + node.getCost()); }
