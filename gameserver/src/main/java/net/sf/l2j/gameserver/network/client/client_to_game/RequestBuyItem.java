...
public final class RequestBuyItem extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (!Config.KARMA_PLAYER_CAN_SHOP && player.getKarma() > 0) { return; }

        L2Npc merchant = null;
        if (!player.isGM()) {
            merchant = (player.getTarget() instanceof L2MerchantInstance) ? (L2Npc) player.getTarget() : null;
            if (merchant == null || !merchant.canInteract(player)) { return; }
        }

        NpcBuyList buyList = BuyListTable.getInstance().getBuyList(_listId);
        if (buyList == null) {
        ...
        if (merchant != null) {
            if (!buyList.isNpcAllowed(merchant.getNpcId())) { return; }

            if (merchant instanceof L2MerchantInstance) { castleTaxRate = merchant.getCastle().getTaxRate(); }
        }
        ...
        for (IntIntHolder i : _items) {
            ...
            if (i.getId() >= 3960 && i.getId() <= 4026) { price *= Config.RATE_SIEGE_GUARDS_PRICE; }
            ...
            if (product.hasLimitedStock()) {
                ...
                if (i.getValue() > product.getCount()) { return; }
            }
            ...
            if (!product.getItem().isStackable()) { slots += i.getValue(); }
            else if (player.getInventory().getItemByItemId(i.getId()) == null) { slots++; }
        }
        ...
        if (subTotal < 0 || !player.getInventory().reduceAdena(EItemProcessPurpose.BUY, subTotal, player.getCurrentFolkNPC(), false)) {
        ...
        for (IntIntHolder i : _items) {
            ...
            if (product.hasLimitedStock()) {
                if (product.decreaseCount(i.getValue())) { player.getInventory().addItem(EItemProcessPurpose.BUY, i.getId(), i.getValue(), player, merchant); }
            }
            else { player.getInventory().addItem(EItemProcessPurpose.BUY, i.getId(), i.getValue(), player, merchant); }
        }

        // add to castle treasury
        if (merchant instanceof L2MerchantInstance) { merchant.getCastle().addToTreasury((int) (subTotal * castleTaxRate)); }
