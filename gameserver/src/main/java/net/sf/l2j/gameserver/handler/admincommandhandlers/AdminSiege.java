...
public class AdminSiege implements IAdminCommandHandler {
    ...
    public boolean useAdminCommand(String command, L2PcInstance activeChar) {
        ...
        if (command.startsWith("admin_clanhall")) { clanhall = ClanHallManager.getInstance().getClanHallById(Integer.parseInt(st.nextToken())); }
        ...
        if (castle != null) {
            if (command.equalsIgnoreCase("admin_add_attacker")) {
                ...
                if (player == null || player.getClan() == null) { activeChar.sendPacket(SystemMessageId.TARGET_IS_INCORRECT); }
                else if (player.getClan().hasCastle()) { activeChar.sendMessage(player.getName() + "'s clan already owns a castle."); }
            ...
            if (command.equalsIgnoreCase("admin_clanhallset")) {
                if (player == null || player.getClan() == null) { activeChar.sendPacket(SystemMessageId.TARGET_IS_INCORRECT); }
                else if (!ClanHallManager.getInstance().isFree(clanhall.getId())) { activeChar.sendMessage("This ClanHall isn't free!"); }
                else if (!player.getClan().hasHideout()) {
                    ClanHallManager.getInstance().setOwner(clanhall.getId(), player.getClan());
                    if (AuctionManager.getInstance().getAuction(clanhall.getId()) != null) { AuctionManager.getInstance().getAuction(clanhall.getId()).deleteAuctionFromDB(); }
                }
