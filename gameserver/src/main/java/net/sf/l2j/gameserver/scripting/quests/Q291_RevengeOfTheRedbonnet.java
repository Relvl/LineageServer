...
public class Q291_RevengeOfTheRedbonnet extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                if (cond == 1) { htmltext = "30553-04.htm"; }
                else if (cond == 2) {
                    htmltext = "30553-05.htm";
                    st.takeItems(BLACK_WOLF_PELT, -1);

                    int random = Rnd.get(100);
                    if (random < 3) { st.rewardItems(GRANDMA_PEARL, 1); }
                    else if (random < 21) { st.rewardItems(GRANDMA_MIRROR, 1); }
                    else if (random < 46) { st.rewardItems(GRANDMA_NECKLACE, 1); }
                    else {
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropItemsAlways(BLACK_WOLF_PELT, 1, 40)) { st.set("cond", "2"); }
