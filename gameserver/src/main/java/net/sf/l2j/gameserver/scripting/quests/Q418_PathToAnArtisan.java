...
public class Q418_PathToAnArtisan extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30527-05.htm")) {
            if (player.getClassId() != ClassId.dwarvenFighter) { htmltext = (player.getClassId() == ClassId.artisan) ? "30527-02a.htm" : "30527-02.htm"; }
            else if (player.getLevel() < 19) { htmltext = "30527-03.htm"; }
            else if (st.hasQuestItems(FINAL_PASS_CERTIFICATE)) { htmltext = "30527-04.htm"; }
        }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case SILVERA:
                        if (cond == 1) { htmltext = "30527-07.htm"; }
                        else if (cond == 2) { htmltext = "30527-08.htm"; }
                        else if (cond == 3) { htmltext = "30527-09.htm"; }
                        else if (cond == 8) { htmltext = "30527-09a.htm"; }
                        break;

                    case KLUTO:
                        if (cond == 3) { htmltext = "30317-01.htm"; }
                        else if (cond == 4) { htmltext = "30317-08.htm"; }
                        else if (cond == 7) { htmltext = "30317-09.htm"; }
                        break;

                    case PINTER:
                        if (cond == 4) { htmltext = "30298-01.htm"; }
                        else if (cond == 5) { htmltext = "30298-04.htm"; }
                        else if (cond == 6) { htmltext = "30298-05.htm"; }
                        else if (cond == 7) { htmltext = "30298-07.htm"; }
                        break;

                    case OBI:
                        if (cond == 8) { htmltext = "32052-01.htm"; }
                        else if (cond == 9) { htmltext = "32052-06a.htm"; }
                        else if (cond == 11) { htmltext = "32052-07.htm"; }
                        break;

                    case HITCHI:
                        if (cond == 9) { htmltext = "31963-01.htm"; }
                        else if (cond == 10) { htmltext = "31963-04.htm"; }
                        else if (cond == 11) { htmltext = "31963-06a.htm"; }
                        else if (cond == 12) { htmltext = "31963-08.htm"; }
                        break;

                    case LOCKIRIN:
                        if (cond == 10) { htmltext = "30531-01.htm"; }
                        break;

                    case RYDEL:
                        if (cond == 12) { htmltext = "31956-01.htm"; }
                        break;
                }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            case 20389:
                if (st.getInt("cond") == 1 && st.dropItems(BOOGLE_RATMAN_TOOTH, 1, 10, 700000) && st.getQuestItemsCount(BOOGLE_RATMAN_LEADER_TOOTH) == 2) { st.set("cond", "2"); }
                break;

            case 20390:
                if (st.getInt("cond") == 1 && st.dropItems(BOOGLE_RATMAN_LEADER_TOOTH, 1, 2, 500000) && st.getQuestItemsCount(BOOGLE_RATMAN_TOOTH) == 10) { st.set("cond", "2"); }
                break;

            case 20017:
                if (st.getInt("cond") == 5 && st.dropItems(STOLEN_SECRET_BOX, 1, 1, 200000)) { st.set("cond", "6"); }
                break;
        }
