...
public class AdminSkill implements IAdminCommandHandler {
    ...
    public boolean useAdminCommand(String command, L2PcInstance activeChar) {
        if (command.equals("admin_show_skills")) { showMainPage(activeChar); }
        ...
        else if (command.equals("admin_reset_skills")) { adminResetSkills(activeChar); }
        else if (command.equals("admin_give_all_skills")) { adminGiveAllSkills(activeChar); }
        else if (command.equals("admin_remove_all_skills")) {
            if (activeChar.getTarget() instanceof L2PcInstance) {
                L2PcInstance player = (L2PcInstance) activeChar.getTarget();

                for (L2Skill skill : player.getAllSkills()) { player.removeSkill(skill); }

                activeChar.sendMessage("You removed all skills from " + player.getName() + ".");
                if (player != activeChar) { player.sendMessage("Admin removed all skills from you."); }
    ...
    private static void adminTestSkill(L2PcInstance activeChar, int id) {
        ...
        if (target == null || !(target instanceof L2Character)) { player = activeChar; }
        else { player = (L2Character) target; }
    ...
    private static void removeSkillsPage(L2PcInstance activeChar, int page) {
        ...
        StringUtil.append(sb, "<html><body><table width=270><tr><td width=45><button value=\"Main\" action=\"bypass -h admin_admin\" width=40 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td><td width=180><center>Delete Skills Menu</center></td><td width=45><button value=\"Back\" action=\"bypass -h admin_show_skills\" width=40 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"></td></tr></table><br><br><center>Editing <font color=\"LEVEL\">", player
                .getName(), "</font>, ", player.getTemplate().getClassName(), " lvl ", player.getLevel(), ".<br><center><table width=270><tr>");

        for (int i = 0; i < max; i++) {
            final int pagenr = i + 1;
            if (page == pagenr) { StringUtil.append(sb, "<td>", pagenr, "</td>"); }
            else { StringUtil.append(sb, "<td><a action=\"bypass -h admin_remove_skills ", pagenr, "\">", pagenr, "</a></td>"); }
        }
    ...
    private static void adminGetSkills(L2PcInstance activeChar) {
        ...
        if (player == activeChar) { player.sendPacket(SystemMessageId.CANNOT_USE_ON_YOURSELF); }
        else {
            L2Skill[] skills = player.getAllSkills();
            ADMIN_SKILLS = activeChar.getAllSkills();

            for (L2Skill skill : ADMIN_SKILLS) { activeChar.removeSkill(skill); }

            for (L2Skill skill : skills) { activeChar.addSkill(skill, true); }
    ...
    private static void adminResetSkills(L2PcInstance activeChar) {
        if (ADMIN_SKILLS == null) { activeChar.sendMessage("Ninja first skills of someone to use that command."); }
        else {
            L2Skill[] skills = activeChar.getAllSkills();

            for (L2Skill skill : skills) { activeChar.removeSkill(skill); }

            for (L2Skill skill : ADMIN_SKILLS) { activeChar.addSkill(skill, true); }
    ...
    private static void adminAddSkill(L2PcInstance activeChar, String val) {
        ...
        if (st.countTokens() != 2) { showMainPage(activeChar); }
        else {
            L2Skill skill = null;
            try {
            ...
            if (skill != null) {
                ...
                if (player != activeChar) { activeChar.sendMessage("You gave the skill " + name + " to " + player.getName() + "."); }

                player.sendSkillList();
            }
            else { activeChar.sendMessage("Error: there is no such skill."); }
    ...
    private static void adminRemoveSkill(L2PcInstance activeChar, int idval) {
        ...
        if (skill != null) {
            ...
            if (player != activeChar) { player.sendMessage("Admin removed the skill " + skillname + " from your skills list."); }
