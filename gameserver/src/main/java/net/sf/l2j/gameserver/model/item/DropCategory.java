...
public class DropCategory {
    ...
    public int getCategoryChance() {
        if (getCategoryType() >= 0) { return _categoryChance; }

        return DropData.MAX_CHANCE;
    }

    public int getCategoryBalancedChance() {
        if (getCategoryType() >= 0) { return _categoryBalancedChance; }
    ...
    public synchronized DropData dropSeedAllowedDropsOnly() {
        ...
        for (DropData drop : getAllDrops()) {
            if ((drop.getItemId() == ItemConst.ADENA_ID) || (drop.getItemId() == 6360) || (drop.getItemId() == 6361) || (drop.getItemId() == 6362)) {
                drops.add(drop);
