...
public final class RequestConfirmCancelItem extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        switch (item.getItem().getCrystalType()) {
            case C:
                if (item.getCrystalCount() < 1720) { price = 95000; }
                else if (item.getCrystalCount() < 2452) { price = 150000; }
                else { price = 210000; }
                break;

            case B:
                if (item.getCrystalCount() < 1746) { price = 240000; }
                else { price = 270000; }
                break;

            case A:
                if (item.getCrystalCount() < 2160) { price = 330000; }
                else if (item.getCrystalCount() < 2824) { price = 390000; }
                else { price = 420000; }
                break;
