...
public class ItemPassiveSkillsListener implements OnEquipListener {
    ...
    public void onEquip(EPaperdollSlot slot, L2ItemInstance item, L2Playable actor) {
        ...
        if (it instanceof Weapon) {
            ...
            if (item.isAugmented()) { item.getAugmentation().applyBonus(player); }

            // Verify if the grade penalty is occuring. If yes, then forget +4 dual skills and SA attached to weapon.
            if (player.getExpertiseIndex() < it.getCrystalType().getId()) { return; }
        ...
        if (update) {
            ...
            if (updateTimeStamp) { player.sendPacket(new SkillCoolTime(player)); }
        }
    }
    ...
    public void onUnequip(EPaperdollSlot slot, L2ItemInstance item, L2Playable actor) {
        ...
        if (it instanceof Weapon) {
            ...
            if (item.isAugmented()) { item.getAugmentation().removeBonus(player); }
        ...
        if (update) { player.sendSkillList(); }
    }
}