...
public class Q127_KamaelAWindowToTheFuture extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        else if (event.equalsIgnoreCase("30187-06.htm")) { st.set("cond", "2"); }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                switch (npc.getNpcId()) {
                    case KLAUS:
                        if (cond == 1) { htmltext = "30187-01.htm"; }
                        else if (cond == 2) { htmltext = "30187-06.htm"; }
                        break;

                    case ALDER:
                        if (cond == 3) { htmltext = "32092-01.htm"; }
                        break;

                    case AKLAN:
                        if (cond == 4) { htmltext = "31288-01.htm"; }
                        break;

                    case OLTLIN:
                        if (cond == 5) { htmltext = "30862-01.htm"; }
                        break;

                    case JURIS:
                        if (cond == 6) { htmltext = "30113-01.htm"; }
                        break;

                    case RODEMAI:
                        if (cond == 7) { htmltext = "30756-01.htm"; }
                        else if (cond == 8) { htmltext = "30756-04.htm"; }
                        break;

                    case DOMINIC:
                        if (cond == 9) { htmltext = "31350-05.htm"; }
                        break;
                }
                break;
