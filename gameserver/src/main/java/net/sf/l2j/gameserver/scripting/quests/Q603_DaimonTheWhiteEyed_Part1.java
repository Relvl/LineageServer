...
public class Q603_DaimonTheWhiteEyed_Part1 extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case EYE_OF_ARGOS:
                        if (cond < 6) { htmltext = "31683-04.htm"; }
                        else if (cond == 6) { htmltext = "31683-05.htm"; }
                        else if (cond == 7) { htmltext = "31683-08.htm"; }
                        else if (cond == 8) { htmltext = "31683-09.htm"; }
                        break;

                    case MYSTERIOUS_TABLET_1:
                        if (cond == 1) { htmltext = "31548-01.htm"; }
                        else { htmltext = "31548-03.htm"; }
                        break;

                    case MYSTERIOUS_TABLET_2:
                        if (cond == 2) { htmltext = "31549-01.htm"; }
                        else if (cond > 2) { htmltext = "31549-03.htm"; }
                        break;

                    case MYSTERIOUS_TABLET_3:
                        if (cond == 3) { htmltext = "31550-01.htm"; }
                        else if (cond > 3) { htmltext = "31550-03.htm"; }
                        break;

                    case MYSTERIOUS_TABLET_4:
                        if (cond == 4) { htmltext = "31551-01.htm"; }
                        else if (cond > 4) { htmltext = "31551-03.htm"; }
                        break;

                    case MYSTERIOUS_TABLET_5:
                        if (cond == 5) { htmltext = "31552-01.htm"; }
                        else if (cond > 5) { htmltext = "31552-03.htm"; }
                        break;
                }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropItems(EVIL_SPIRIT_BEADS, 1, 200, CHANCES.get(npc.getNpcId()))) { st.set("cond", "8"); }
