...
public class Q417_PathToBecomeAScavenger extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30524-05.htm")) {
            if (player.getClassId() != ClassId.dwarvenFighter) { htmltext = (player.getClassId() == ClassId.scavenger) ? "30524-02a.htm" : "30524-08.htm"; }
            else if (player.getLevel() < 19) { htmltext = "30524-02.htm"; }
            else if (st.hasQuestItems(RING_OF_RAVEN)) { htmltext = "30524-04.htm"; }
            else {
        ...
        // MION
        ...
        else if (event.equalsIgnoreCase("30519-07.htm")) { st.set("id", String.valueOf(st.getInt("id") + 1)); }
            ...
            else if (id / 10 == 2) { st.set("id", String.valueOf(id + 1)); }
            else if (id / 10 >= 3) {
        ...
        else if (event.equalsIgnoreCase("30519-11.htm") && Rnd.nextBoolean()) { htmltext = "30519-06.htm"; }
        // TOMA
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case PIPPI:
                        if (cond == 1) { htmltext = "30524-06.htm"; }
                        else if (cond > 1) { htmltext = "30524-07.htm"; }
                        break;

                    case MION:
                        if (st.hasQuestItems(PIPPI_LETTER)) { htmltext = "30519-01.htm"; }
                        else if (st.hasAtLeastOneQuestItem(BRONK_INGOT, SHARI_AXE, ZIMENF_POTION)) {
                            int id = st.getInt("id");
                            if (id / 10 == 0) { htmltext = "30519-05.htm"; }
                            else { htmltext = "30519-08.htm"; }
                        }
                        else if (st.hasAtLeastOneQuestItem(BRONK_PAY, SHARI_PAY, ZIMENF_PAY)) {
                            int id = st.getInt("id");
                            if (id < 50) { htmltext = "30519-12.htm"; }
                            else {
                        ...
                        else if (cond == 4) { htmltext = "30519-13.htm"; }
                        else if (cond > 4) { htmltext = "30519-14.htm"; }
                        break;

                    case SHARI:
                        if (st.hasQuestItems(SHARI_AXE)) {
                            int id = st.getInt("id");
                            if (id < 20) { htmltext = "30517-01.htm"; }
                            else {
                        ...
                        else if (st.hasQuestItems(SHARI_PAY)) { htmltext = "30517-03.htm"; }
                        break;

                    case BRONK:
                        if (st.hasQuestItems(BRONK_INGOT)) {
                            int id = st.getInt("id");
                            if (id < 20) { htmltext = "30525-01.htm"; }
                            else {
                        ...
                        else if (st.hasQuestItems(BRONK_PAY)) { htmltext = "30525-03.htm"; }
                        break;

                    case ZIMENF:
                        if (st.hasQuestItems(ZIMENF_POTION)) {
                            int id = st.getInt("id");
                            if (id < 20) { htmltext = "30538-01.htm"; }
                            else {
                        ...
                        else if (st.hasQuestItems(ZIMENF_PAY)) { htmltext = "30538-03.htm"; }
                        break;
                    ...
                    case TOMA:
                        ...
                        else if (cond == 5) { htmltext = "30556-02.htm"; }
                        else if (cond == 6) {
                        ...
                        else if (cond == 7) { htmltext = "30556-04.htm"; }
                        else if (cond == 8) { htmltext = "30556-05a.htm"; }
                        else if (cond == 9) { htmltext = "30556-06a.htm"; }
                        else if (cond == 10 || cond == 11) { htmltext = "30556-07.htm"; }
                        else if (cond == 12) { htmltext = "30556-06c.htm"; }
                        break;

                    case RAUT:
                        if (cond == 9) { htmltext = "30316-01.htm"; }
                        else if (cond == 10) { htmltext = "30316-04.htm"; }
                        else if (cond == 11) {
                    ...
                    case TORAI:
                        if (cond == 10) { htmltext = "30557-01.htm"; }
                        break;

                    case YASHENI:
                        if (cond == 12) { htmltext = "31958-01.htm"; }
                        break;
                }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            case HUNTER_BEAR:
                if (st.getInt("cond") == 5) {
                    ...
                    if (step > 20) {
                        ...
                        else { st.set("step", String.valueOf(step + 1)); }
                    }
                    else { st.set("step", String.valueOf(step + 1)); }
                }
                break;
            ...
            case HONEY_BEAR:
                if (st.getInt("cond") == 5 && npc.getSpoilerId() == player.getObjectId() && st.dropItemsAlways(HONEY_JAR, 1, 5)) { st.set("cond", "6"); }
                break;

            case HUNTER_TARANTULA:
            case PLUNDER_TARANTULA:
                if (st.getInt("cond") == 7 && npc.getSpoilerId() == player.getObjectId() && st.dropItems(BEAD, 1, 20, (npc.getNpcId() == HUNTER_TARANTULA) ? 333333 : 600000)) { st.set("cond", "8"); }
                break;
        }
