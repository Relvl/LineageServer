...
public class RequestBuySeed extends L2GameClientPacket {
    ...
    protected void runImpl() {
        if (!FloodProtectors.performAction(getClient(), Action.MANOR)) { return; }
        ...
        if (!player.isInsideRadius(manager, INTERACTION_DISTANCE, true, false)) { return; }
        ...
        for (Seed i : _seeds) {
            if (!i.setProduction(castle)) { return; }
            ...
            else if (player.getInventory().getItemByItemId(i.getSeedId()) == null) { slots++; }
        }
        ...
        for (Seed i : _seeds) {
            ...
            player.addItem(EItemProcessPurpose.BUY, i.getSeedId(), i.getCount(), manager, true);
        }
    ...
    private static class Seed {
        ...
        public boolean setProduction(Castle c) {
            ...
            if (_seed.getPrice() <= 0) { return false; }
            // try to buy more than castle can produce
            if (_seed.getCanProduce() < _count) { return false; }
            // check for overflow
            if ((Integer.MAX_VALUE / _count) < _seed.getPrice()) { return false; }
        ...
        public boolean updateProduction(Castle c) {
            ...
            if (Config.ALT_MANOR_SAVE_ALL_ACTIONS) { c.updateSeed(_seedId, _seed.getCanProduce(), CastleManorManager.PERIOD_CURRENT); }
