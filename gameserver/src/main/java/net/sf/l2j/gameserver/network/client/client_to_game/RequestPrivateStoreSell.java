...
public final class RequestPrivateStoreSell extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (storePlayer == null) { return; }

        if (!player.isInsideRadius(storePlayer, 150, true, false)) { return; }

        if (storePlayer.getPrivateStoreType() != PrivateStoreType.BUY) { return; }
        ...
        if (!storeList.privateStoreSell(player, _items)) {
            _log.warn("PrivateStore sell has failed due to invalid list or request. Player: {}, Private store of: {}", player.getName(), storePlayer.getName());
            return;
        }
