...
public final class L2ClassMasterInstance extends L2NpcInstance {
    ...
    public void showChatWindow(L2PcInstance player) {
        ...
        if (Config.ALLOW_CLASS_MASTERS) { filename = "data/html/classmaster/" + getNpcId() + ".htm"; }
    ...
    public void onBypassFeedback(L2PcInstance player, String command) {
        if (!Config.ALLOW_CLASS_MASTERS) { return; }

        if (command.startsWith("1stClass")) { showHtmlMenu(player, getObjectId(), 1); }
        else if (command.startsWith("2ndClass")) { showHtmlMenu(player, getObjectId(), 2); }
        else if (command.startsWith("3rdClass")) { showHtmlMenu(player, getObjectId(), 3); }
    ...
    private static final void showHtmlMenu(L2PcInstance player, int objectId, int level) {
        ...
        if (!Config.CLASS_MASTER_SETTINGS.isAllowed(level)) {
            ...
            switch (player.getClassId().level()) {
                case 0:
                    if (Config.CLASS_MASTER_SETTINGS.isAllowed(1)) { sb.append("Come back here when you reached level 20 to change your class.<br>"); }
                    else if (Config.CLASS_MASTER_SETTINGS.isAllowed(2)) { sb.append("Come back after your first occupation change.<br>"); }
                    else if (Config.CLASS_MASTER_SETTINGS.isAllowed(3)) { sb.append("Come back after your second occupation change.<br>"); }
                    else { sb.append("I can't change your occupation.<br>"); }
                    break;

                case 1:
                    if (Config.CLASS_MASTER_SETTINGS.isAllowed(2)) { sb.append("Come back here when you reached level 40 to change your class.<br>"); }
                    else if (Config.CLASS_MASTER_SETTINGS.isAllowed(3)) { sb.append("Come back after your second occupation change.<br>"); }
                    else { sb.append("I can't change your occupation.<br>"); }
                    break;

                case 2:
                    if (Config.CLASS_MASTER_SETTINGS.isAllowed(3)) { sb.append("Come back here when you reached level 76 to change your class.<br>"); }
                    else { sb.append("I can't change your occupation.<br>"); }
                    break;
            ...
            if (currentClassId.level() >= level) { html.setFile("data/html/classmaster/nomore.htm"); }
            else {
    ...
    private static final boolean checkAndChangeClass(L2PcInstance player, int val) {
        final ClassId currentClassId = player.getClassId();
        if (getMinLevel(currentClassId.level()) > player.getLevel() && !Config.ALLOW_ENTIRE_TREE) { return false; }
        ...
        for (IntIntHolder item : Config.CLASS_MASTER_SETTINGS.getRewardItems(newJobLevel)) { player.addItem(EItemProcessPurpose.CLASS_MASTER, item.getId(), item.getValue(), player, true); }
    ...
    private static final boolean validateClassId(ClassId oldCID, ClassId newCID) {
        if (newCID == null || newCID.getRace() == null) { return false; }

        if (oldCID.equals(newCID.getParent())) { return true; }

        if (Config.ALLOW_ENTIRE_TREE && newCID.childOf(oldCID)) { return true; }
    ...
    private static String getRequiredItems(int level) {
        ...
        if (neededItems == null || neededItems.isEmpty()) { return "<tr><td>none</td></r>"; }

        final StringBuilder sb = new StringBuilder();
        for (IntIntHolder item : neededItems) { StringUtil.append(sb, "<tr><td><font color=\"LEVEL\">", item.getValue(), "</font></td><td>", ItemTable.getInstance().getTemplate(item.getId()).getName(), "</td></tr>"); }
