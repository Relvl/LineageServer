...
public class ClanHallManager {
    ...
    private void load() {
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            while (rs.next()) {
                ...
                if (!_allClanHalls.containsKey(Location)) { _allClanHalls.put(Location, new ArrayList<ClanHall>()); }
                ...
                if (auc == null && lease > 0) { AuctionManager.getInstance().initNPC(id); }
            }
            rs.close();
            statement.close();

            LOGGER.info("ClanHallManager: Loaded {} clan halls.", getClanHalls().size());
            LOGGER.info("ClanHallManager: Loaded {} free clan halls.", getFreeClanHalls().size());
            _loaded = true;
        }
        catch (Exception e) {
            LOGGER.error("Exception: ClanHallManager.load(): {}", e.getMessage(), e);
        }
    }
    ...
    public final List<ClanHall> getClanHallsByLocation(String location) {
        if (!_allClanHalls.containsKey(location)) { return null; }
    ...
    public final ClanHall getClanHallById(int clanHallId) {
        if (_clanHall.containsKey(clanHallId)) { return _clanHall.get(clanHallId); }

        if (_freeClanHall.containsKey(clanHallId)) { return _freeClanHall.get(clanHallId); }

        LOGGER.warn("ClanHall (id: {}) isn't found in clanhall table.", clanHallId);
        return null;
    }
    ...
    public final ClanHall getNearbyClanHall(int x, int y, int maxDist) {
        ...
        for (Entry<Integer, ClanHall> clanHallEntry : _clanHall.entrySet()) {
            zone = clanHallEntry.getValue().getZone();
            if (zone != null && zone.getDistanceToZone(x, y) < maxDist) { return clanHallEntry.getValue(); }
        }
        for (Entry<Integer, ClanHall> clanHallEntry : _freeClanHall.entrySet()) {
            zone = clanHallEntry.getValue().getZone();
            if (zone != null && zone.getDistanceToZone(x, y) < maxDist) { return clanHallEntry.getValue(); }
        }
        return null;
    ...
    public final ClanHall getClanHallByOwner(L2Clan clan) {
        for (Entry<Integer, ClanHall> clanHallEntry : _clanHall.entrySet()) {
            if (clan.getClanId() == clanHallEntry.getValue().getOwnerId()) { return clanHallEntry.getValue(); }
        }
        return null;
    }
