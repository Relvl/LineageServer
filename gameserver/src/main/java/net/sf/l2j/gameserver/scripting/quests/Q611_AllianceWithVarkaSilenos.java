...
public class Q611_AllianceWithVarkaSilenos extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("31378-03a.htm")) {
            if (player.isAlliedWithKetra()) { htmltext = "31378-02a.htm"; }
            else {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getLevel() >= 74) { htmltext = "31378-01.htm"; }
                else {
            ...
            case QuestState.STATE_STARTED:
                ...
                if (cond == 1) {
                    if (st.getQuestItemsCount(KETRA_BADGE_SOLDIER) < 100) { htmltext = "31378-03b.htm"; }
                    else { htmltext = "31378-09.htm"; }
                }
                else if (cond == 2) {
                    if (st.getQuestItemsCount(KETRA_BADGE_SOLDIER) < 200 || st.getQuestItemsCount(KETRA_BADGE_OFFICER) < 100) { htmltext = "31378-12.htm"; }
                    else { htmltext = "31378-13.htm"; }
                }
                else if (cond == 3) {
                    if (st.getQuestItemsCount(KETRA_BADGE_SOLDIER) < 300 || st.getQuestItemsCount(KETRA_BADGE_OFFICER) < 200 || st.getQuestItemsCount(KETRA_BADGE_CAPTAIN) < 100) { htmltext = "31378-15.htm"; }
                    else { htmltext = "31378-16.htm"; }
                }
                else if (cond == 4) {
                ...
                else if (cond == 6) { htmltext = "31378-08.htm"; }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npcId) {
            ...
            case 21328:
            case 21329:
                if (cond == 1) { st.dropItems(KETRA_BADGE_SOLDIER, 1, 100, CHANCES.get(npcId)); }
                else if (cond == 2) { st.dropItems(KETRA_BADGE_SOLDIER, 1, 200, CHANCES.get(npcId)); }
                else if (cond == 3 || cond == 4) { st.dropItems(KETRA_BADGE_SOLDIER, 1, 300, CHANCES.get(npcId)); }
                else if (cond == 5) { st.dropItems(KETRA_BADGE_SOLDIER, 1, 400, CHANCES.get(npcId)); }
                break;
            ...
            case 21343:
            case 21344:
                if (cond == 2) { st.dropItems(KETRA_BADGE_OFFICER, 1, 100, CHANCES.get(npcId)); }
                else if (cond == 3) { st.dropItems(KETRA_BADGE_OFFICER, 1, 200, CHANCES.get(npcId)); }
                else if (cond == 4) { st.dropItems(KETRA_BADGE_OFFICER, 1, 300, CHANCES.get(npcId)); }
                else if (cond == 5) { st.dropItems(KETRA_BADGE_OFFICER, 1, 400, CHANCES.get(npcId)); }
                break;
            ...
            case 21348:
            case 21349:
                if (cond == 3) { st.dropItems(KETRA_BADGE_CAPTAIN, 1, 100, CHANCES.get(npcId)); }
                else if (cond == 4 || cond == 5) { st.dropItems(KETRA_BADGE_CAPTAIN, 1, 200, CHANCES.get(npcId)); }
                break;
        }
