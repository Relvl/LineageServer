...
public class ExManagePartyRoomMember extends L2GameServerPacket {
    ...
    protected void writeImpl() {
        ...
        if (_room.getOwner().equals(_activeChar)) { writeD(1); }
        else {
            if ((_room.getOwner().isInParty() && _activeChar.isInParty()) && (_room.getOwner().getParty().getPartyLeaderOID() == _activeChar.getParty().getPartyLeaderOID())) { writeD(2); }
            else { writeD(0); }
        }
    }
}