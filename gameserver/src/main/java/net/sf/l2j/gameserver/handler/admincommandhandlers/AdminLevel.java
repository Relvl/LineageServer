...
public class AdminLevel implements IAdminCommandHandler {
    ...
    public boolean useAdminCommand(String command, L2PcInstance activeChar) {
        ...
        if (actualCommand.equalsIgnoreCase("admin_addlevel")) {
            try {
                if (targetChar instanceof L2Playable) { ((L2Playable) targetChar).getStat().addLevel(Byte.parseByte(val)); }
            }
            ...
            try {
                ...
                if (lvl >= 1 && lvl <= Experience.MAX_LEVEL) {
                    ...
                    if (pXp > tXp) { targetPlayer.removeExpAndSp(pXp - tXp, 0); }
                    else if (pXp < tXp) { targetPlayer.addExpAndSp(tXp - pXp, 0); }
                }
                else {
