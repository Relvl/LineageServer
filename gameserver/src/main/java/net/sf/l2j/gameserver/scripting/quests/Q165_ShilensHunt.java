...
public class Q165_ShilensHunt extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getRace() != PlayerRace.DarkElf) { htmltext = "30348-00.htm"; }
                else if (player.getLevel() < 3) { htmltext = "30348-01.htm"; }
                else { htmltext = "30348-02.htm"; }
                break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropItems(DARK_BEZOAR, 1, 13, CHANCES.get(npc.getNpcId()))) { st.set("cond", "2"); }
