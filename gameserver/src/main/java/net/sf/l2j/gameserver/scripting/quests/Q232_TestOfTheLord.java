...
public class Q232_TestOfTheLord extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30565-05.htm")) {
            ...
            if (_firstOrc == null) { _firstOrc = addSpawn(FIRST_ORC, 21036, -107690, -3038, 200000, false, 0, true); }
        }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getRace() != PlayerRace.Orc) { htmltext = "30565-01.htm"; }
                else if (player.getClassId() != ClassId.orcShaman) { htmltext = "30565-02.htm"; }
                else if (player.getLevel() < 39) { htmltext = "30565-03.htm"; }
                else { htmltext = "30565-04.htm"; }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case VARKEES:
                        if (st.hasQuestItems(HUGE_ORC_FANG)) { htmltext = "30566-05.htm"; }
                            ...
                            else { htmltext = "30566-03.htm"; }
                        }
                        else { htmltext = "30566-01.htm"; }
                        break;

                    case MANAKIA:
                        if (st.hasQuestItems(HUGE_ORC_FANG)) { htmltext = "30515-05.htm"; }
                        else if (st.hasQuestItems(MANAKIAS_AMULET)) { htmltext = "30515-04.htm"; }
                    ...
                    case TANTUS:
                        if (st.hasQuestItems(AXE_OF_CEREMONY)) { htmltext = "30567-05.htm"; }
                            ...
                            else { htmltext = "30567-03.htm"; }
                        }
                        else { htmltext = "30567-01.htm"; }
                        break;

                    case JAKAL:
                        if (st.hasQuestItems(AXE_OF_CEREMONY)) { htmltext = "30558-05.htm"; }
                        else if (st.hasQuestItems(NERUGA_AXE_BLADE)) { htmltext = "30558-04.htm"; }
                        else if (st.hasQuestItems(TANTUS_CHARM)) {
                            if (st.getQuestItemsCount(ItemConst.ADENA_ID) >= 1000) { htmltext = "30558-01.htm"; }
                            else { htmltext = "30558-03.htm"; }
                        }
                        break;

                    case HATOS:
                        if (st.hasQuestItems(SWORD_INTO_SKULL)) { htmltext = "30568-05.htm"; }
                            ...
                            else { htmltext = "30568-03.htm"; }
                        }
                        else { htmltext = "30568-01.htm"; }
                        break;

                    case SUMARI:
                        if (st.hasQuestItems(URUTU_BLADE)) { htmltext = "30564-03.htm"; }
                        else if (st.hasQuestItems(SUMARIS_LETTER)) { htmltext = "30564-02.htm"; }
                        else if (st.hasQuestItems(HATOS_CHARM)) {
                    ...
                    case SOMAK:
                        if (st.hasQuestItems(SWORD_INTO_SKULL)) { htmltext = "30510-03.htm"; }
                        else if (st.hasQuestItems(URUTU_BLADE)) { htmltext = "30510-02.htm"; }
                    ...
                    case TAKUNA:
                        if (st.hasQuestItems(HANDIWORK_SPIDER_BROOCH)) { htmltext = "30641-05.htm"; }
                            ...
                            else { htmltext = "30641-03.htm"; }
                        }
                        else { htmltext = "30641-01.htm"; }
                        break;

                    case CHIANTA:
                        if (st.hasQuestItems(MONSTEREYE_WOODCARVING)) { htmltext = "30642-05.htm"; }
                            ...
                            else { htmltext = "30642-03.htm"; }
                        }
                        else { htmltext = "30642-01.htm"; }
                        break;

                    case KAKAI:
                        if (cond == 1) { htmltext = "30565-06.htm"; }
                        else if (cond == 2) { htmltext = "30565-07.htm"; }
                        else if (cond == 3) { htmltext = "30565-09.htm"; }
                        else if (cond > 3 && cond < 7) { htmltext = "30565-10.htm"; }
                        else if (cond == 7) {
                    ...
                    case ANCESTOR_MARTANKUS:
                        if (cond == 3) { htmltext = "30649-01.htm"; }
                        else if (cond == 4) { htmltext = "30649-05.htm"; }
                        else if (cond == 5) {
                        ...
                        else if (cond == 6) { htmltext = "30649-07.htm"; }
                        else if (cond == 7) { htmltext = "30649-08.htm"; }
                        break;

                    case FIRST_ORC:
                        if (cond == 6) { htmltext = "30643-01.htm"; }
                        else if (cond == 7) { htmltext = "30643-03.htm"; }
                        break;
                }
                break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            case 20564:
                if (st.hasQuestItems(CHIANTA_CHARM)) { st.dropItemsAlways(MONSTEREYE_CORNEA, 1, 20); }
                break;

            case 20583:
            case 20584:
            case 20585:
                if (st.hasQuestItems(HATOS_CHARM)) { st.dropItems(TIMAK_ORC_SKULL, 1, 10, 710000); }
                break;

            case 20586:
                if (st.hasQuestItems(HATOS_CHARM)) { st.dropItems(TIMAK_ORC_SKULL, 1, 10, 810000); }
                break;

            case 20587:
            case 20588:
                if (st.hasQuestItems(HATOS_CHARM)) { st.dropItemsAlways(TIMAK_ORC_SKULL, 1, 10); }
                break;

            case 20233:
                if (st.hasQuestItems(TAKUNA_CHARM)) { st.dropItemsAlways((st.getQuestItemsCount(MARSH_SPIDER_FEELER) >= 10) ? MARSH_SPIDER_FEET : MARSH_SPIDER_FEELER, 1, 10); }
                break;

            case 20269:
                if (st.hasQuestItems(MANAKIAS_ORDERS)) { st.dropItems(BREKA_ORC_FANG, 1, 20, 410000); }
                break;

            case 20270:
                if (st.hasQuestItems(MANAKIAS_ORDERS)) { st.dropItems(BREKA_ORC_FANG, 1, 20, 510000); }
                break;
