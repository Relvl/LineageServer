...
public class Q264_KeenClaws extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                if (count < 50) { htmltext = "30136-04.htm"; }
                else {
                    ...
                    else if (n < 2) { st.giveItems(ItemConst.ADENA_ID, 1000); }
                    else if (n < 5) { st.giveItems(LEATHER_SANDALS, 1); }
                    else if (n < 8) {
                        st.giveItems(STOCKINGS, 1);
                        st.giveItems(ItemConst.ADENA_ID, 50);
                    }
                    else if (n < 11) { st.giveItems(HEALING_POTION, 1); }
                    else if (n < 14) { st.giveItems(SHORT_GLOVES, 1); }
                    else { st.giveItems(CLOTH_SHOES, 1); }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (npc.getNpcId() == 20003) {
            if (st.dropItems(WOLF_CLAW, Rnd.nextBoolean() ? 2 : 4, 50, 500000)) { st.set("cond", "2"); }
        }
        else if (st.dropItemsAlways(WOLF_CLAW, (Rnd.get(5) < 4) ? 1 : 2, 50)) { st.set("cond", "2"); }
