...
public class ChatHandlerTrade implements IChatHandler {
    @Override
    public void handleChat(EChatType type, L2PcInstance activeChar, String tellTarget, String text) {
        ...
        for (L2PcInstance player : L2World.getInstance().getPlayers()) {
            if (!player.getContactController().isBlocked(activeChar) && region == MapRegionTable.getMapRegion(player.getX(), player.getY())) {
