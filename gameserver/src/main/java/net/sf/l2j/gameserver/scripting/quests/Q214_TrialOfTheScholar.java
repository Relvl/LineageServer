...
public class Q214_TrialOfTheScholar extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30461-04a.htm")) {
            ...
            if (player.getLevel() < 36) { st.giveItems(MIRIEN_INSTRUCTION, 1); }
            else {
        ...
        // VALKON
        else if (event.equalsIgnoreCase("30103-04.htm")) { st.giveItems(VALKON_REQUEST, 1); }
        // CASIAN
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getClassId() != ClassId.wizard && player.getClassId() != ClassId.elvenWizard && player.getClassId() != ClassId.darkWizard) { htmltext = "30461-01.htm"; }
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    ...
                    case MARIA:
                        ...
                        else if (st.hasAtLeastOneQuestItem(SYMBOL_OF_SYLVAIN, MIRIEN_SIGIL_2)) { htmltext = "30608-16.htm"; }
                        else if (cond > 18) {
                    ...
                    case DIETER:
                        ...
                        else if (cond > 24 && cond < 31) { htmltext = (!st.hasQuestItems(SCRIPTURE_CHAPTER_1, SCRIPTURE_CHAPTER_2, SCRIPTURE_CHAPTER_3, SCRIPTURE_CHAPTER_4)) ? "30111-12.htm" : "30111-13.htm"; }
                    ...
                    case CASIAN:
                        if ((cond == 26 || cond == 27) && st.hasQuestItems(POITAN_NOTES)) {
                            if (st.hasQuestItems(SCRIPTURE_CHAPTER_1, SCRIPTURE_CHAPTER_2, SCRIPTURE_CHAPTER_3)) { htmltext = "30612-02.htm"; }
                            else {
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            case LETO_LIZARDMAN_WARRIOR:
                if (st.getInt("cond") == 11 && st.dropItems(BROWN_SCROLL_SCRAP, 1, 5, 500000)) { st.set("cond", "12"); }
                break;

            case SHACKLE_1:
            case SHACKLE_2:
                if (st.getInt("cond") == 16 && st.dropItems(SHACKLE_SCALP, 1, 2, 500000)) { if (st.getQuestItemsCount(MONSTER_EYE_DESTROYER_SKIN) == 5 && st.getQuestItemsCount(SHAMAN_NECKLACE) == 5) { st.set("cond", "17"); } }
                break;

            case MONSTER_EYE_DESTROYER:
                if (st.getInt("cond") == 16 && st.dropItems(MONSTER_EYE_DESTROYER_SKIN, 1, 5, 500000)) { if (st.getQuestItemsCount(SHACKLE_SCALP) == 2 && st.getQuestItemsCount(SHAMAN_NECKLACE) == 5) { st.set("cond", "17"); } }
                break;

            case BREKA_ORC_SHAMAN:
                if (st.getInt("cond") == 16 && st.dropItems(SHAMAN_NECKLACE, 1, 5, 500000)) { if (st.getQuestItemsCount(SHACKLE_SCALP) == 2 && st.getQuestItemsCount(MONSTER_EYE_DESTROYER_SKIN) == 5) { st.set("cond", "17"); } }
                break;
            ...
            case MEDUSA:
                if (st.getInt("cond") == 28 && st.dropItemsAlways(MEDUSA_BLOOD, 1, 12)) {
                    if (st.getQuestItemsCount(GHOUL_SKIN) == 10 && st.getQuestItemsCount(FETTERED_SOUL_ICHOR) == 5 && st.getQuestItemsCount(ENCHANTED_GARGOYLE_NAIL) == 5) { st.set("cond", "29"); }
                }
                break;

            case GHOUL:
                if (st.getInt("cond") == 28 && st.dropItemsAlways(GHOUL_SKIN, 1, 10)) {
                    if (st.getQuestItemsCount(MEDUSA_BLOOD) == 12 && st.getQuestItemsCount(FETTERED_SOUL_ICHOR) == 5 && st.getQuestItemsCount(ENCHANTED_GARGOYLE_NAIL) == 5) { st.set("cond", "29"); }
                }
                break;

            case FETTERED_SOUL:
                if (st.getInt("cond") == 28 && st.dropItemsAlways(FETTERED_SOUL_ICHOR, 1, 5)) {
                    if (st.getQuestItemsCount(MEDUSA_BLOOD) == 12 && st.getQuestItemsCount(GHOUL_SKIN) == 10 && st.getQuestItemsCount(ENCHANTED_GARGOYLE_NAIL) == 5) { st.set("cond", "29"); }
                }
                break;

            case ENCHANTED_GARGOYLE:
                if (st.getInt("cond") == 28 && st.dropItemsAlways(ENCHANTED_GARGOYLE_NAIL, 1, 5)) {
                    if (st.getQuestItemsCount(MEDUSA_BLOOD) == 12 && st.getQuestItemsCount(GHOUL_SKIN) == 10 && st.getQuestItemsCount(FETTERED_SOUL_ICHOR) == 5) { st.set("cond", "29"); }
                }
                break;
        }
