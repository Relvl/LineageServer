...
public class AdminBookmark implements IAdminCommandHandler {
    ...
    public boolean useAdminCommand(String command, L2PcInstance activeChar) {
        if (command.startsWith("admin_bkpage")) {
            ...
            else { activeChar.sendMessage("The command delbk must be followed by a valid name."); }
    ...
    private static void showBookmarks(L2PcInstance activeChar, int page) {
        ...
        for (int i = 0; i < max; i++) {
            ...
            else { StringUtil.append(sb, "<a action=\"bypass -h admin_bkpage ", pagenr, "\">", pagenr, "</a>&nbsp;"); }
        }
