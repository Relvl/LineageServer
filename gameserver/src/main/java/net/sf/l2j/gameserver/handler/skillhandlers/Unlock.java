...
public class Unlock implements ISkillHandler {
    ...
    public void useSkill(L2Character activeChar, L2Skill skill, L2Object[] targets) {
        ...
        if (object instanceof L2DoorInstance) {
            ...
            else { activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.FAILED_TO_UNLOCK_DOOR)); }
        }
        else if (object instanceof L2ChestInstance) {
            final L2ChestInstance chest = (L2ChestInstance) object;
            if (chest.isDead() || chest.isInteracted()) { return; }
        ...
        else { activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.INCORRECT_TARGET)); }
    }

    private static final boolean doorUnlock(L2Skill skill) {
        if (skill.getSkillType() == L2SkillType.UNLOCK_SPECIAL) { return Rnd.get(100) < skill.getPower(); }
    ...
    private static final boolean chestUnlock(L2Skill skill, L2Character chest) {
        ...
        if (chest.getLevel() > 60) {
            if (skill.getLevel() < 10) { return false; }

            chance = (skill.getLevel() - 10) * 5 + 30;
        }
        else if (chest.getLevel() > 40) {
            if (skill.getLevel() < 6) { return false; }

            chance = (skill.getLevel() - 6) * 5 + 10;
        }
        else if (chest.getLevel() > 30) {
            if (skill.getLevel() < 3) { return false; }

            if (skill.getLevel() > 12) { return true; }

            chance = (skill.getLevel() - 3) * 5 + 30;
        }
        else {
            if (skill.getLevel() > 10) { return true; }
