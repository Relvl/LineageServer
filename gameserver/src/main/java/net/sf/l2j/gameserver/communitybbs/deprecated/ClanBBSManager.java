...
public class ClanBBSManager extends BaseBBSManager {
    ...
    public void parseCmd(String command, L2PcInstance activeChar) {
        if (command.equalsIgnoreCase("_bbsclan")) {
            if (activeChar.getClan() == null) { sendClanList(activeChar, 1); }
            else { sendClanDetails(activeChar, activeChar.getClan().getClanId()); }
        }
            ...
            if (clanCommand.equalsIgnoreCase("clan")) { sendClanList(activeChar, Integer.parseInt(st.nextToken())); }
            else if (clanCommand.equalsIgnoreCase("home")) { sendClanDetails(activeChar, Integer.parseInt(st.nextToken())); }
            else if (clanCommand.equalsIgnoreCase("mail")) { sendClanMail(activeChar, Integer.parseInt(st.nextToken())); }
            else if (clanCommand.equalsIgnoreCase("management")) { sendClanManagement(activeChar, Integer.parseInt(st.nextToken())); }
            else if (clanCommand.equalsIgnoreCase("notice")) {
                if (st.hasMoreTokens()) {
                    final String noticeCommand = st.nextToken();
                    if (!noticeCommand.isEmpty() && activeChar.getClan() != null) { activeChar.getClan().setNoticeEnabledAndStore(Boolean.parseBoolean(noticeCommand)); }
                }
    ...
    public void parseWrite(String ar1, String ar2, String ar3, String ar4, String ar5, L2PcInstance activeChar) {
        if (ar1.equalsIgnoreCase("intro")) {
            if (Integer.valueOf(ar2) != activeChar.getClanId()) { return; }
            ...
            for (L2ClanMember player : clan.getMembers()) {
                if (player != null) {
                    if (membersList.length() > 0) { membersList.append(";"); }
    ...
    private static void sendClanNotice(L2PcInstance activeChar, int clanId) {
        final L2Clan clan = ClanTable.getInstance().getClan(clanId);
        if (clan == null || activeChar.getClanId() != clanId) { return; }
    ...
    private static void sendClanList(L2PcInstance activeChar, int index) {
        ...
        if (playerClan != null) { StringUtil.append(sb, "<table width=610 bgcolor=A7A19A><tr><td width=5></td><td width=605><a action=\"bypass _bbsclan;home;", playerClan.getClanId(), "\">[GO TO MY CLAN]</a></td></tr></table>"); }
        ...
        for (L2Clan cl : ClanTable.getInstance().getClans()) {
            if (i > (index + 1) * 7) { break; }

            if (i++ >= (index - 1) * 7) {
                StringUtil.append(sb, "<table width=610><tr><td width=5></td><td width=150 align=center><a action=\"bypass _bbsclan;home;", cl.getClanId(), "\">", cl.getName(), "</a></td><td width=150 align=center>", cl.getLeaderName(), "</td><td width=100 align=center>", cl
                        .getLevel(), "</td><td width=200 align=center>", cl.getMembersCount(), "</td><td width=5></td></tr></table><br1><img src=\"L2UI.Squaregray\" width=605 height=1><br1>");
            }
        }
        sb.append("<table><tr>");

        if (index == 1) { sb.append("<td><button action=\"\" back=\"l2ui_ch3.prev1_down\" fore=\"l2ui_ch3.prev1\" width=16 height=16></td>"); }
        else { StringUtil.append(sb, "<td><button action=\"_bbsclan;clan;", index - 1, "\" back=\"l2ui_ch3.prev1_down\" fore=\"l2ui_ch3.prev1\" width=16 height=16 ></td>"); }

        i = 0;
        int nbp = ClanTable.getInstance().getClans().length / 8;
        if (nbp * 8 != ClanTable.getInstance().getClans().length) { nbp++; }

        for (i = 1; i <= nbp; i++) {
            if (i == index) { StringUtil.append(sb, "<td> ", i, " </td>"); }
            else { StringUtil.append(sb, "<td><a action=\"bypass _bbsclan;clan;", i, "\"> ", i, " </a></td>"); }
        }

        if (index == nbp) { sb.append("<td><button action=\"\" back=\"l2ui_ch3.next1_down\" fore=\"l2ui_ch3.next1\" width=16 height=16></td>"); }
        else { StringUtil.append(sb, "<td><button action=\"bypass _bbsclan;clan;", index + 1, "\" back=\"l2ui_ch3.next1_down\" fore=\"l2ui_ch3.next1\" width=16 height=16 ></td>"); }
    ...
    private static void sendClanDetails(L2PcInstance activeChar, int clanId) {
        ...
        String content;
        if (activeChar.getClanId() != clanId) { content = HtmCache.getInstance().getHtm(CB_PATH + "clan/clanhome.htm"); }
        else if (activeChar.isClanLeader()) { content = HtmCache.getInstance().getHtm(CB_PATH + "clan/clanhome-leader.htm"); }
        else { content = HtmCache.getInstance().getHtm(CB_PATH + "clan/clanhome-member.htm"); }
