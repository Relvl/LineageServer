...
abstract class DocumentBase {
    protected static final Logger LOGGER = LoggerFactory.getLogger(DocumentBase.class);
    ...
    protected void attachEffect(Node n, Object template, Condition attachCond) {
        ...
        if (attrs.getNamedItem("count") != null) { count = Integer.decode(getValue(attrs.getNamedItem("count").getNodeValue(), template)); }

        if (attrs.getNamedItem("time") != null) { time = Integer.decode(getValue(attrs.getNamedItem("time").getNodeValue(), template)); }
        else if (((L2Skill) template).getBuffDuration() > 0) { time = ((L2Skill) template).getBuffDuration() / 1000 / count; }

        boolean self = false;
        if (attrs.getNamedItem("self") != null) {
            if (Integer.decode(getValue(attrs.getNamedItem("self").getNodeValue(), template)) == 1) { self = true; }
        }
        ...
        if (attrs.getNamedItem("stackType") != null) { stackType = attrs.getNamedItem("stackType").getNodeValue(); }

        float stackOrder = 0;
        if (attrs.getNamedItem("stackOrder") != null) { stackOrder = Float.parseFloat(getValue(attrs.getNamedItem("stackOrder").getNodeValue(), template)); }

        double effectPower = -1;
        if (attrs.getNamedItem("effectPower") != null) { effectPower = Double.parseDouble(getValue(attrs.getNamedItem("effectPower").getNodeValue(), template)); }
        ...
        int trigLvl = 1;
        if (attrs.getNamedItem("triggeredLevel") != null) { trigLvl = Integer.parseInt(getValue(attrs.getNamedItem("triggeredLevel").getNodeValue(), template)); }

        String chanceCond = null;
        if (attrs.getNamedItem("chanceType") != null) { chanceCond = getValue(attrs.getNamedItem("chanceType").getNodeValue(), template); }
        else if (isChanceSkillTrigger) { throw new NoSuchElementException(name + " requires chanceType"); }

        int activationChance = -1;
        if (attrs.getNamedItem("activationChance") != null) { activationChance = Integer.parseInt(getValue(attrs.getNamedItem("activationChance").getNodeValue(), template)); }

        ChanceCondition chance = ChanceCondition.parse(chanceCond, activationChance);

        if (chance == null && isChanceSkillTrigger) { throw new NoSuchElementException("Invalid chance condition: " + chanceCond + " " + activationChance); }
    ...
    protected Condition parsePlayerCondition(Node n, Object template) {
        ...
        if (forces[0] + forces[1] > 0) { cond = joinAnd(cond, new ConditionForceBuff(forces)); }
    ...
    protected void parseTable(Node n) {
        ...
        while (data.hasMoreTokens()) { array.add(data.nextToken()); }
