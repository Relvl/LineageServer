...
import net.sf.l2j.gameserver.network.client.game_to_client.AcquireSkillInfo;
    ...
    protected void runImpl() {
        ...
        switch (_skillType) {
            ...
            case 0:
                ...
                if (!trainer.getTemplate().canTeach(activeChar.getSkillLearningClassId())) { return; }
                ...
                for (L2SkillLearn sl : SkillTreeTable.getInstance().getAvailableSkills(activeChar, activeChar.getSkillLearningClassId())) {
                    if (sl.getId() == _skillId && sl.getLevel() == _skillLevel) {
                        ...
                        if (spellbookItemId != 0) { asi.addRequirement(99, spellbookItemId, 1, 50); }
                        sendPacket(asi);
                        break;
                    }
                }
            ...
            case 2:
                ...
                for (L2PledgeSkillLearn psl : SkillTreeTable.getInstance().getAvailablePledgeSkills(activeChar)) {
                    if (psl.getId() == _skillId && psl.getLevel() == _skillLevel) {
                        ...
                        if (Config.LIFE_CRYSTAL_NEEDED && psl.getItemId() != 0) { asi.addRequirement(1, psl.getItemId(), 1, 0); }
                        sendPacket(asi);
                        break;
                    }
                }
