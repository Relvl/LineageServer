...
public class GmListTable {
    ...
    public List<L2PcInstance> getAllGms(boolean includeHidden) {
        ...
        for (Entry<L2PcInstance, Boolean> entry : _gmList.entrySet()) {
            if (includeHidden || !entry.getValue()) { tmpGmList.add(entry.getKey()); }
        }
    ...
    public List<String> getAllGmNames(boolean includeHidden) {
        ...
        for (Entry<L2PcInstance, Boolean> entry : _gmList.entrySet()) {
            ...
            if (!entry.getValue()) { tmpGmList.add(name); }
            else if (includeHidden) { tmpGmList.add(name + " (invis)"); }
        }
    ...
    public void showOrHideGm(L2PcInstance player, boolean showOrHide) {
        if (_gmList.containsKey(player)) { _gmList.put(player, showOrHide); }
    }
    ...
    public boolean isGmOnline(boolean includeHidden) {
        for (Entry<L2PcInstance, Boolean> entry : _gmList.entrySet()) {
            if (includeHidden || !entry.getValue()) { return true; }
        }
    ...
    public void sendListToPlayer(L2PcInstance player) {
        if (isGmOnline(player.isGM())) {
            ...
            for (String name : getAllGmNames(player.isGM())) { player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.GM_S1).addString(name)); }
        }
        else {
            player.sendPacket(SystemMessageId.NO_GM_PROVIDING_SERVICE_NOW);
            player.sendPacket(new PlaySound(ESound.systemmsg_e_702));
        }
    }

    public static void broadcastToGMs(L2GameServerPacket packet) {
        for (L2PcInstance gm : getInstance().getAllGms(true)) { gm.sendPacket(packet); }
    }

    public static void broadcastMessageToGMs(String message) {
        for (L2PcInstance gm : getInstance().getAllGms(true)) { gm.sendMessage(message); }
    }
