...
public final class L2AttackableAIScript extends Quest {
    public L2AttackableAIScript() {
        ...
        for (NpcTemplate template : NpcTable.getInstance().getAllNpcs()) {
            try {
                ...
                LOGGER.info("Class not found: {}Instance", template.getType());
            }
        }
    }
    ...
    public String onSkillSee(L2Npc npc, L2PcInstance caster, L2Skill skill, L2Object[] targets, boolean isPet) {
        ...
        if (caster.getPet() != null) {
            if (targets.length == 1 && Util.contains(targets, caster.getPet())) { skillAggroPoints = 0; }
        }
    ...
    public String onFactionCall(L2Npc npc, L2Npc caller, L2PcInstance attacker, boolean isPet) {
        ...
        if (attacker.isInParty() && attacker.getParty().isInDimensionalRift()) {
            ...
            if (caller instanceof L2RiftInvaderInstance && !DimensionalRiftManager.getInstance().getRoom(riftType, riftRoom).checkIfInZone(npc.getX(), npc.getY(), npc.getZ())) { return null; }
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet) {
        if (npc instanceof L2MonsterInstance) {
            ...
            if (mob.getLeader() != null) { mob.getLeader().getMinionList().onMinionDie(mob, -1); }

            if (mob.hasMinions()) { mob.getMinionList().onMasterDie(false); }
        }
        return null;
    }
}