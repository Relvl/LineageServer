...
public class Q216_TrialOfTheGuildsman extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        else if (event.equalsIgnoreCase("30210-10.htm")) { st.giveItems(NORMAN_LIST, 1); }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getClassId() != ClassId.scavenger && player.getClassId() != ClassId.artisan) { htmltext = "30103-01.htm"; }
                else if (player.getLevel() < 35) { htmltext = "30103-02.htm"; }
                else { htmltext = "30103-03.htm"; }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case VALKON:
                        if (cond == 1) { htmltext = "30103-06c.htm"; }
                        else if (cond < 5) { htmltext = "30103-07.htm"; }
                        else if (cond == 5) { htmltext = "30103-08.htm"; }
                        else if (cond == 6) { htmltext = (st.getQuestItemsCount(JOURNEYMAN_RING) == 7) ? "30103-09.htm" : "30103-08.htm"; }
                        break;
                    ...
                    case ALTRAN:
                        ...
                        else if (cond == 4) { htmltext = "30283-02.htm"; }
                        else if (cond > 4) { htmltext = "30283-04.htm"; }
                        break;

                    case NORMAN:
                        if (cond == 5) {
                            if (st.hasQuestItems(ALTRAN_RECOMMENDATION_1)) { htmltext = "30210-01.htm"; }
                            else if (st.hasQuestItems(NORMAN_RECEIPT)) { htmltext = "30210-05.htm"; }
                            else if (st.hasQuestItems(DUNING_INSTRUCTIONS)) { htmltext = "30210-06.htm"; }
                    ...
                    case DUNING:
                        if (cond == 5) {
                            if (st.hasQuestItems(NORMAN_RECEIPT)) { htmltext = "30688-01.htm"; }
                            else if (st.hasQuestItems(DUNING_INSTRUCTIONS)) {
                                if (st.getQuestItemsCount(DUNING_KEY) < 30) { htmltext = "30688-03.htm"; }
                                else {
                                    htmltext = "30688-04.htm";
                                    st.takeItems(DUNING_INSTRUCTIONS, 1);
                                }
                            }
                            else { htmltext = "30688-05.htm"; }
                        }
                        break;

                    case PINTER:
                        if (cond == 5) {
                            if (st.hasQuestItems(ALTRAN_RECOMMENDATION_2)) { htmltext = (player.getLevel() < 36) ? "30298-01.htm" : "30298-02.htm"; }
                            else if (st.hasQuestItems(PINTER_INSTRUCTIONS)) {
                                if (st.getQuestItemsCount(AMBER_BEAD) < 70) { htmltext = "30298-06.htm"; }
                                else {
                        ...
                        else if (st.hasQuestItems(JOURNEYMAN_DECO_BEADS)) { htmltext = "30298-08.htm"; }
                        break;
                }
                break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            ...
            case MANDRAGORA_BLOSSOM:
                if (st.getInt("cond") == 3 && st.dropItemsAlways(MANDRAGORA_BERRY, 1, 1)) { st.set("cond", "4"); }
                break;

            case BREKA_ORC_WARRIOR:
            case BREKA_ORC_OVERLORD:
            case BREKA_ORC_SHAMAN:
                if (st.hasQuestItems(DUNING_INSTRUCTIONS)) { st.dropItemsAlways(DUNING_KEY, 1, 30); }
                break;

            case GHOUL:
            case STRAIN:
                if (st.hasQuestItems(NORMAN_LIST)) { st.dropItemsAlways(GRAY_BONE_POWDER, 5, 70); }
                break;

            case GRANITE_GOLEM:
                if (st.hasQuestItems(NORMAN_LIST)) { st.dropItemsAlways(GRANITE_WHETSTONE, 7, 70); }
                break;

            case DEAD_SEEKER:
                if (st.hasQuestItems(NORMAN_LIST)) { st.dropItemsAlways(RED_PIGMENT, 7, 70); }
                break;

            case SILENOS:
                if (st.hasQuestItems(NORMAN_LIST)) { st.dropItemsAlways(BRAIDED_YARN, 10, 70); }
                break;
            ...
            case ANT_CAPTAIN:
                if (st.hasQuestItems(PINTER_INSTRUCTIONS)) {
                    ...
                    if (st.dropItemsAlways(AMBER_BEAD, (player.getClassId() == ClassId.scavenger && npc.getSpoilerId() == player.getObjectId()) ? 10 : 5, 70)) {
                        if (player.getClassId() == ClassId.artisan && Rnd.nextBoolean()) { st.giveItems(AMBER_LUMP, 1); }
                    }
                }
                break;
        }
