...
public final class Formulas {
    ...
    static {
        for (int i = 0; i < STRbonus.length; i++) { STRbonus[i] = Math.floor(Math.pow(STRCompute[0], i - STRCompute[1]) * 100 + .5d) / 100; }
        for (int i = 0; i < INTbonus.length; i++) { INTbonus[i] = Math.floor(Math.pow(INTCompute[0], i - INTCompute[1]) * 100 + .5d) / 100; }
        for (int i = 0; i < DEXbonus.length; i++) { DEXbonus[i] = Math.floor(Math.pow(DEXCompute[0], i - DEXCompute[1]) * 100 + .5d) / 100; }
        for (int i = 0; i < WITbonus.length; i++) { WITbonus[i] = Math.floor(Math.pow(WITCompute[0], i - WITCompute[1]) * 100 + .5d) / 100; }
        for (int i = 0; i < CONbonus.length; i++) { CONbonus[i] = Math.floor(Math.pow(CONCompute[0], i - CONCompute[1]) * 100 + .5d) / 100; }
        for (int i = 0; i < MENbonus.length; i++) { MENbonus[i] = Math.floor(Math.pow(MENCompute[0], i - MENCompute[1]) * 100 + .5d) / 100; }

        // Precompute square root values
        for (int i = 0; i < sqrtCONbonus.length; i++) { sqrtCONbonus[i] = Math.sqrt(CONbonus[i]); }
        for (int i = 0; i < sqrtMENbonus.length; i++) { sqrtMENbonus[i] = Math.sqrt(MENbonus[i]); }
    }
    ...
    public static final double calcHpRegen(L2Character cha) {
        ...
        if (cha.isChampion()) { hpRegenMultiplier *= Config.CHAMPION_HP_REGEN; }
        ...
        if (cha instanceof L2PcInstance) {
            ...
            if (SevenSignsFestival.getInstance().isFestivalInProgress() && player.isFestivalParticipant()) { hpRegenMultiplier *= calcFestivalRegenModifier(player); }
            else if (calcSiegeRegenModifer(player)) { hpRegenMultiplier *= 1.5; }

            if (player.isInsideZone(ZoneId.CLAN_HALL) && player.getClan() != null) {
                int clanHallIndex = player.getClan().getHideoutId();
                if (clanHallIndex > 0) {
                    ClanHall clansHall = ClanHallManager.getInstance().getClanHallById(clanHallIndex);
                    if (clansHall != null) { if (clansHall.getFunction(ClanHall.FUNC_RESTORE_HP) != null) { hpRegenMultiplier *= 1 + clansHall.getFunction(ClanHall.FUNC_RESTORE_HP).getLvl() / 100; } }
                }
            }
    ...
    public static final double calcMpRegen(L2Character cha) {
        ...
        if (cha instanceof L2PcInstance) {
            ...
            // SevenSigns Festival modifier
            if (SevenSignsFestival.getInstance().isFestivalInProgress() && player.isFestivalParticipant()) { mpRegenMultiplier *= calcFestivalRegenModifier(player); }
            ...
            if (player.isInsideZone(ZoneId.CLAN_HALL) && player.getClan() != null) {
                ...
                if (clanHallIndex > 0) {
                    ClanHall clansHall = ClanHallManager.getInstance().getClanHallById(clanHallIndex);
                    if (clansHall != null) { if (clansHall.getFunction(ClanHall.FUNC_RESTORE_MP) != null) { mpRegenMultiplier *= 1 + clansHall.getFunction(ClanHall.FUNC_RESTORE_MP).getLvl() / 100; } }
                }
            }
    ...
    public static double calcBlowDamage(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean ss) {
        ...
        if (ss) {
            ...
            if (skill.getSSBoost() > 0) { power *= skill.getSSBoost(); }
        }
        ...
        if (isPvP) { damage *= attacker.calcStat(Stats.PVP_PHYS_SKILL_DMG, 1, null, null); }
    ...
    public static final double calcPhysDam(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean crit, boolean ss) {
        if (attacker instanceof L2PcInstance) {
            ...
            if (pcInst.isGM() && !pcInst.getAccessLevel().canGiveDamage()) { return 0; }
        }

        double defence = target.getPDef(attacker);
        switch (shld) {
            case SHIELD_DEFENSE_SUCCEED:
                if (!Config.ALT_GAME_SHIELD_BLOCKS) { defence += target.getShldDef(); }
                break;
        ...
        if (skill != null) {
            ...
            if (ssBoost > 0 && ss) { skillpower *= ssBoost; }
        ...
        else { damage = 70 * damage / defence; }

        if (stat != null) { damage = target.calcStat(stat, damage, target, null); }

        // Weapon random damage ; invalid for CHARGEDAM skills.
        if (skill == null || skill.getEffectType() != L2SkillType.CHARGEDAM) { damage *= attacker.getRandomDamageMultiplier(); }
        ...
        if (isPvP) {
            if (skill == null) { damage *= attacker.calcStat(Stats.PVP_PHYSICAL_DMG, 1, null, null); }
            else { damage *= attacker.calcStat(Stats.PVP_PHYS_SKILL_DMG, 1, null, null); }
        }
    ...
    public static final double calcMagicDam(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean ss, boolean bss, boolean mcrit) {
        ...
        if (Config.ALT_GAME_MAGICFAILURES && !calcMagicSuccess(attacker, target, skill)) {
            if (attacker instanceof L2PcInstance) {
                if (calcMagicSuccess(attacker, target, skill) && (target.getLevel() - attacker.getLevel()) <= 9) {
                    if (skill.getSkillType() == L2SkillType.DRAIN) { attacker.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.DRAIN_HALF_SUCCESFUL)); }
                    else { attacker.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.ATTACK_FAILED)); }
            ...
            if (target instanceof L2PcInstance) {
                if (skill.getSkillType() == L2SkillType.DRAIN) { target.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.RESISTED_S1_DRAIN).addCharName(attacker)); }
                else { target.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.RESISTED_S1_MAGIC).addCharName(attacker)); }
            }
        }
        ...
        if (attacker instanceof L2Playable && target instanceof L2Playable) {
            if (skill.isMagic()) { damage *= attacker.calcStat(Stats.PVP_MAGICAL_DMG, 1, null, null); }
            else { damage *= attacker.calcStat(Stats.PVP_PHYS_SKILL_DMG, 1, null, null); }
        }
    ...
    public static final double calcMagicDam(L2CubicInstance attacker, L2Character target, L2Skill skill, boolean mcrit, byte shld) {
        ...
        if (Config.ALT_GAME_MAGICFAILURES && !calcMagicSuccess(owner, target, skill)) {
            if (calcMagicSuccess(owner, target, skill) && (target.getLevel() - skill.getMagicLevel()) <= 9) {
                if (skill.getSkillType() == L2SkillType.DRAIN) { owner.sendPacket(SystemMessageId.DRAIN_HALF_SUCCESFUL); }
                else { owner.sendPacket(SystemMessageId.ATTACK_FAILED); }
            ...
            if (target instanceof L2PcInstance) {
                if (skill.getSkillType() == L2SkillType.DRAIN) { target.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.RESISTED_S1_DRAIN).addCharName(owner)); }
                else { target.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.RESISTED_S1_MAGIC).addCharName(owner)); }
            }
        }
        else if (mcrit) { damage *= 4; }
    ...
    public static final double calcLethal(L2Character activeChar, L2Character target, int baseLethal, int magiclvl) {
        ...
        if (magiclvl > 0) {
            ...
            if (delta >= -3) { chance = (baseLethal * ((double) activeChar.getLevel() / target.getLevel())); }
            else if (delta < -3 && delta >= -9) { chance = (-3) * (baseLethal / (delta)); }
            else { chance = baseLethal / 15; }
        }
        else { chance = (baseLethal * ((double) activeChar.getLevel() / target.getLevel())); }
    ...
    public static final void calcLethalHit(L2Character activeChar, L2Character target, L2Skill skill) {
        if (target.isRaid() || target instanceof L2DoorInstance) { return; }
        ...
        if (skill.getLethalChance2() > 0 && Rnd.get(1000) < calcLethal(activeChar, target, skill.getLethalChance2(), skill.getMagicLevel())) {
            if (target instanceof L2Npc) { target.reduceCurrentHp(target.getCurrentHp() - 1, activeChar, skill); }
        ...
        // First lethal effect (hp/2 for npc, cp to 1 for player).
            ...
            if (target instanceof L2Npc) { target.reduceCurrentHp(target.getCurrentHp() / 2, activeChar, skill); }
    ...
    public static final void calcCastBreak(L2Character target, double dmg) {
        ...
        if (target.isRaid() || target.isInvul()) { return; }
        ...
        if (!target.isCastingNow() && (target.getLastSkillCast() != null && !target.getLastSkillCast().isMagic())) { return; }
        ...
        if (Rnd.get(100) < rate) { target.breakCast(); }
    }
    ...
    public static boolean calcHitMiss(L2Character attacker, L2Character target) {
        ...
        if (attacker.getZ() - target.getZ() > 50) { modifier += 3; }
        else if (attacker.getZ() - target.getZ() < -50) { modifier -= 3; }

        // Get weather bonus. TODO: rain support (-3%).
        if (GameTimeTaskManager.getInstance().isNight()) { modifier -= 10; }
    ...
    public static byte calcShldUse(L2Character attacker, L2Character target, L2Skill skill) {
        ...
        if (degreeside < 360 && (!target.isFacing(attacker, degreeside))) { return 0; }
    ...
    public static double calcSkillVulnerability(L2Character attacker, L2Character target, L2Skill skill, L2SkillType type) {
        ...
        if (skill.getElement() > 0) { multiplier *= Math.sqrt(calcElemental(attacker, target, skill)); }
    ...
    private static double getLevelModifier(L2Character attacker, L2Character target, L2Skill skill) {
        if (skill.getLevelDepend() == 0) { return 1; }
    ...
    public static boolean calcEffectSuccess(L2Character attacker, L2Character target, EffectTemplate effect, L2Skill skill, byte shld, boolean bss) {
        ...
        if (type == null) { return Rnd.get(100) < baseChance; }
    ...
    public static boolean calcCubicSkillSuccess(L2CubicInstance attacker, L2Character target, L2Skill skill, byte shld, boolean bss) {
        ...
        if (calcSkillReflect(target, skill) != SKILL_REFLECT_FAILED) { return false; }
    ...
    public static boolean calcMagicSuccess(L2Character attacker, L2Character target, L2Skill skill) {
        ...
        if (lvlDifference > 0) { rate = (Math.pow(1.3, lvlDifference)) * 100; }

        if (attacker instanceof L2PcInstance && ((L2PcInstance) attacker).getExpertiseWeaponPenalty()) { rate += 6000; }
    ...
    public static boolean calcSkillMastery(L2Character actor, L2Skill sk) {
        ...
        if (!(actor instanceof L2PcInstance)) { return false; }

        if (sk.getSkillType() == L2SkillType.FISHING) { return false; }

        double val = actor.getStat().calcStat(Stats.SKILL_MASTERY, 0, null, null);

        if (((L2PcInstance) actor).isMageClass()) { val *= INTbonus[actor.getINT()]; }
        else { val *= STRbonus[actor.getSTR()]; }
    ...
    public static double calcElemental(L2Character attacker, L2Character target, L2Skill skill) {
        if (skill != null) {
            ...
            if (element > 0) { return 1 + ((attacker.getAttackElementValue(element) / 10.0 - target.getDefenseElementValue(element)) / 100.0); }
    ...
    public static byte calcSkillReflect(L2Character target, L2Skill skill) {
        ...
        switch (skill.getSkillType()) {
            ...
            case CHARGEDAM:
                ...
                if (venganceChance > Rnd.get(100)) { reflect |= SKILL_REFLECT_VENGEANCE; }
                break;
        }
        ...
        if (Rnd.get(100) < reflectChance) { reflect |= SKILL_REFLECT_SUCCEED; }
    ...
    public static int calculateKarmaGain(int pkCount, boolean isSummon) {
        int result = 14400;
        if (pkCount < 100) { result = (int) (((((pkCount - 1) * 0.5) + 1) * 60) * 4); }
        else if (pkCount < 180) { result = (int) (((((pkCount + 1) * 0.125) + 37.5) * 60) * 4); }

        if (isSummon) { result = ((pkCount & 3) + result) >> 2; }
