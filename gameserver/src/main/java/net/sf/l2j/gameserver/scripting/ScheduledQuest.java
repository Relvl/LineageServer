...
public abstract class ScheduledQuest extends Quest {
    private static final Logger LOGGER = LoggerFactory.getLogger(ScheduledQuest.class);
    ...
    public final boolean setSchedule(String type, String start, String end) {
        try {
            ...
            if (_end.getTimeInMillis() > _start.getTimeInMillis()) {
                LOGGER.warn(getName() + ": Start is scheduled after the end, script is not scheduled.");
                return false;
            }
            ...
            LOGGER.error("{}: Error while loading schedule data.", getName(), e);
            _type = null;
            _start = null;
            _end = null;
            return false;
    ...
    public final void notifyAndSchedule() {
        ...
        if (_started) { notifyEndScheduleStart(); }
        else { notifyStartScheduleEnd(); }
    }
    ...
    private static final int getDayOfWeek(String day) throws Exception {
        ...
        else if (day.equals("SUN")) { return Calendar.SUNDAY; }
