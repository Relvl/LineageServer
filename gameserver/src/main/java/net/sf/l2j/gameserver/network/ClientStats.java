...
public class ClientStats {
    ...
    protected final boolean countPacket(int queueSize) {
        ...
        if (maxQueueSize < queueSize) { maxQueueSize = queueSize; }
        if (_queueOverflowDetected && queueSize < 2) { _queueOverflowDetected = false; }
    ...
    protected final boolean countBurst(int count) {
        if (count > maxBurstSize) { maxBurstSize = count; }

        if (count < Config.CLIENT_PACKET_QUEUE_MAX_BURST_SIZE) { return false; }
    ...
    private final synchronized boolean countPacket() {
        ...
        if (tick - _packetCountStartTick > 1000) {
            ...
            if (_floodDetected && !longFloodDetected() && _packetsInSecond[_head] < Config.CLIENT_PACKET_QUEUE_MAX_PACKETS_PER_SECOND / 2) { _floodDetected = false; }

            // wrap head of the buffer around the tail
            if (_head <= 0) { _head = BUFFER_SIZE; }
            _head--;
        ...
        if (!_floodDetected) {
            if (count > Config.CLIENT_PACKET_QUEUE_MAX_PACKETS_PER_SECOND) { shortFloods++; }
            else if (longFloodDetected()) { longFloods++; }
            else { return false; }
