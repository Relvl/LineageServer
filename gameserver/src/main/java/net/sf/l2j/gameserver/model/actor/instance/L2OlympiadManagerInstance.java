...
import net.sf.l2j.gameserver.network.client.game_to_client.ExHeroList;
    ...
    public void showChatWindow(L2PcInstance player, int val) {
        ...
        switch (npcId) {
            case 31688: // Olympiad managers
                if (player.isNoble() && val == 0) { filename = "noble_main.htm"; }
                break;
            ...
            case 31771:
            case 31772:
                if (player.isHero() || Hero.getInstance().isInactiveHero(player.getObjectId())) { filename = "hero_main.htm"; }
                else { filename = "hero_main2.htm"; }
                break;
        }
        ...
        if (filename == "hero_main.htm") {
            String hiddenText = "";
            if (Hero.getInstance().isInactiveHero(player.getObjectId())) { hiddenText = "<a action=\"bypass -h npc_%objectId%_Olympiad 5\">\"I want to be a Hero.\"</a><br>"; }
        ...
        if (command.startsWith("OlympiadNoble")) {
            ...
            switch (val) {
                ...
                case 10: // Give tokens to player
                    player.addItem(EItemProcessPurpose.OLYMPIAD, GATE_PASS, Olympiad.getInstance().getNoblessePasses(player, true), player, true);
                    break;
