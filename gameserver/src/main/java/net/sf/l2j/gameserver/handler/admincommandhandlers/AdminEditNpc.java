...
public class AdminEditNpc implements IAdminCommandHandler {
    ...
    private static void showShopList(L2PcInstance activeChar, int listId) {
        ...
        StringUtil.append(sb, "<html><body><center><font color=\"LEVEL\">", NpcTable.getInstance()
                                                                                    .getTemplate(buyList.getNpcId())
                                                                                    .getName(), " (", buyList.getNpcId(), ") buylist id: ", buyList.getListId(), "</font></center><br><table width=\"100%\"><tr><td width=200>Item</td><td width=80>Price</td></tr>");

        for (Product product : buyList.getProducts()) { StringUtil.append(sb, "<tr><td>", product.getItem().getName(), "</td><td>", product.getPrice(), "</td></tr>"); }
    ...
    private static void showShop(L2PcInstance activeChar, int npcId) {
        ...
        for (NpcBuyList buyList : buyLists) { StringUtil.append(sb, "<tr><td><a action=\"bypass -h admin_show_shoplist ", buyList.getListId(), " 1\">Buylist id: ", buyList.getListId(), "</a></td></tr>"); }
    ...
    private static void showNpcDropList(L2PcInstance activeChar, int npcId, int page) {
        ...
        if (!npcData.getDropData().isEmpty()) {
            ...
            for (DropCategory cat : npcData.getDropData()) {
                ...
                for (DropData drop : cat.getAllDrops()) {
                    ...
                    StringUtil.append(sb, "<tr><td><font color=\"", ((cat.isSweep()) ? "00FF00" : "3BB9FF"), "\">", cat.getCategoryType(), "</td><td>", ItemTable.getInstance()
                                                                                                                                                                 .getTemplate(drop.getItemId())
                                                                                                                                                                 .getName(), " (", drop.getItemId(), ")</td></tr>");
                    shown++;
                }
            }
            ...
            if (page > 1) {
                ...
                if (!hasMore) { StringUtil.append(sb, "<td width=100>Page ", page, "</td><td width=70></td></tr>"); }
            }

            if (hasMore) {
                if (page <= 1) { sb.append("<td width=120></td>"); }
        ...
        else { sb.append("This NPC has no drops."); }
    ...
    private static void showScriptsList(L2PcInstance activeChar, int npcId) {
        ...
        if (!npcData.getEventQuests().isEmpty()) {
            ...
            for (Map.Entry<EventType, List<Quest>> entry : npcData.getEventQuests().entrySet()) {
                ...
                for (Quest quest : entry.getValue()) { StringUtil.append(sb, quest.getName(), "<br1>"); }
            }
        }
        else { sb.append("This NPC isn't affected by scripts."); }
