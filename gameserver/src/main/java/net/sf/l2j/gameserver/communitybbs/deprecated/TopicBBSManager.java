...
public class TopicBBSManager extends BaseBBSManager {
    ...
    private static void showNewTopic(Forum forum, L2PcInstance activeChar, int idf) {
        ...
        if (forum.getType() == Forum.MEMO) { showMemoNewTopics(forum, activeChar); }
        else {
    ...
    public void parseWrite(String ar1, String ar2, String ar3, String ar4, String ar5, L2PcInstance activeChar) {
        if (ar1.equals("crea")) {
            ...
            f.vload();
            Topic t = new Topic(Topic.ConstructorType.CREATE, TopicBBSManager.getInstance().getMaxID(f) + 1, Integer.parseInt(ar2), ar5, Calendar.getInstance()
                                                                                                                                                 .getTimeInMillis(), activeChar.getName(), activeChar.getObjectId(), Topic.MEMO, 0);
            f.addTopic(t);
    ...
    private void showTopics(Forum forum, L2PcInstance activeChar, int index, int idf) {
        ...
        if (forum.getType() == Forum.MEMO) { showMemoTopics(forum, activeChar, index); }
        else {
            separateAndSend("<html><body><br><br><center>The forum named '" + forum.getName() + "' doesn't exist.</center></body></html>", activeChar);
        }
    }
    ...
    private void showMemoTopics(Forum forum, L2PcInstance activeChar, int index) {
        ...
        for (int i = 0, j = getMaxID(forum) + 1; i < 12 * index; j--) {
            ...
            if (t != null) {
                if (i++ >= 12 * (index - 1)) {
                    StringUtil.append(sb, "<table border=0 cellspacing=0 cellpadding=5 WIDTH=610><tr><td FIXWIDTH=5></td><td FIXWIDTH=415><a action=\"bypass _bbsposts;read;", forum.getID(), ";", t.getID(), "\">", t.getName(), "</a></td><td FIXWIDTH=120 align=center></td><td FIXWIDTH=70 align=center>", dateFormat
                            .format(new Date(t.getDate())), "</td></tr></table><img src=\"L2UI.Squaregray\" width=\"610\" height=\"1\">");
                }
            }
        }
