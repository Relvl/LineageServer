...
public class Q163_LegacyOfThePoet extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getRace() == PlayerRace.DarkElf) { htmltext = "30220-00.htm"; }
                else if (player.getLevel() < 11) { htmltext = "30220-02.htm"; }
                else { htmltext = "30220-03.htm"; }
                break;
            ...
            case QuestState.STATE_STARTED:
                if (st.getInt("cond") == 2) {
                    ...
                    for (int poem : RUMIELS_POEMS) { st.takeItems(poem, -1); }

                    st.rewardItems(ItemConst.ADENA_ID, 13890);
                    st.playSound(ESound.ItemSound_quest_finish);
                    st.exitQuest(false);
                }
                else { htmltext = "30220-08.htm"; }
                break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropMultipleItems(DROPLIST)) { st.set("cond", "2"); }
