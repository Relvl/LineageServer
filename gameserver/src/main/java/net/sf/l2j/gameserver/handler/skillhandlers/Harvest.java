...
public class Harvest implements ISkillHandler {
    ...
    public void useSkill(L2Character activeChar, L2Skill skill, L2Object[] targets) {
        ...
        if (target.isSeeded()) {
                ...
                if (!items.isEmpty()) {
                    ...
                    for (IntIntHolder ritem : items) {
                        ...
                        if (player.isInParty()) { player.getParty().distributeItem(player, ritem, true, target); }
                            ...
                            L2ItemInstance item = player.getInventory().addItem(EItemProcessPurpose.MANOR, ritem.getId(), ritem.getValue(), player, target);
                            iu.addItem(item);
                    ...
                    if (send) {
                        ...
                        if (player.isInParty()) { player.getParty().broadcastToPartyMembers(player, SystemMessage.getSystemMessage(SystemMessageId.S1_HARVESTED_S3_S2S).addPcName(player).addItemName(cropId).addNumber(total)); }
            ...
            else { player.sendPacket(SystemMessageId.THE_HARVEST_HAS_FAILED); }
        }
        else { player.sendPacket(SystemMessageId.THE_HARVEST_FAILED_BECAUSE_THE_SEED_WAS_NOT_SOWN); }
    }
    ...
    private static boolean calcSuccess(L2Character activeChar, L2Character target) {
        ...
        if (diff > 5) { basicSuccess -= (diff - 5) * 5; }
