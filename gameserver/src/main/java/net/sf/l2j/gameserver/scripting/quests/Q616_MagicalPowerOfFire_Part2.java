...
public class Q616_MagicalPowerOfFire_Part2 extends Quest {
    ...
    public Q616_MagicalPowerOfFire_Part2() {
        ...
        switch (RaidBossSpawnManager.getInstance().getRaidBossStatusId(SOUL_OF_FIRE_NASTRON)) {
            case UNDEFINED:
                LOGGER.warn(qn + ": can not find spawned L2RaidBoss id=" + SOUL_OF_FIRE_NASTRON);
                break;
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equals("check")) {
            ...
            if (raid != null && raid.getRaidStatus() == StatusEnum.ALIVE) {
                if (_status >= 0 && _status-- == 0) { despawnRaid(raid); }
        ...
        // Ketra's Holy Altar
            ...
            if (st.hasQuestItems(RED_TOTEM)) {
                ...
                else { htmltext = "31558-04.htm"; }
            }
            else { htmltext = "31558-03.htm"; }
        }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case UDAN_MARDUI:
                        if (cond == 1) { htmltext = "31379-05.htm"; }
                        else if (cond == 2) { htmltext = "31379-06.htm"; }
                        else { htmltext = "31379-07.htm"; }
                        break;

                    case KETRAS_HOLY_ALTAR:
                        if (cond == 1) { htmltext = "31558-01.htm"; }
                        else if (cond == 2) { htmltext = "31558-05.htm"; }
                        break;
                }
                break;
        }
    ...
    private void spawnNpc() {
        ...
        if (_npc == null) { _npc = addSpawn(KETRAS_HOLY_ALTAR, 142368, -82512, -6487, 58000, false, 0, false); }
    }
    ...
    private static void despawnRaid(L2Npc raid) {
        ...
        if (!raid.isDead()) { raid.teleToLocation(-105300, -252700, -15542, 0); }
