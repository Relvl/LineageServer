...
public class MercTicketManager {
    private static final Logger LOGGER = LoggerFactory.getLogger(MercTicketManager.class);
    ...
    private static final void load() {
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
                ...
                for (int i = startindex; i < NPC_IDS.length; i++) {
                    if (NPC_IDS[i] == npcId) // Find the index of the item used
                    {
                        ...
                        if (castle != null && !(castle.getSiege().isInProgress())) {
                            ...
                            dropticket.setLocation(EItemLocation.INVENTORY);
            ...
            LOGGER.error("Exception: loadMercenaryData(): {}", e.getMessage(), e);
        }
        LOGGER.info("MercTicketManager: Loaded {} tickets.", _droppedTickets.size());
    }
    ...
    public boolean isAtTypeLimit(int itemId) {
        ...
        for (L2ItemInstance ticket : _droppedTickets) {
            if (ticket != null && ticket.getItemId() == itemId) { count++; }
        }
    ...
    public boolean isAtCasleLimit(int itemId) {
        ...
        int count = 0;
        for (L2ItemInstance ticket : _droppedTickets) {
            if ((ticket != null) && (getTicketCastleId(ticket.getItemId()) == castleId)) { count++; }
        }
