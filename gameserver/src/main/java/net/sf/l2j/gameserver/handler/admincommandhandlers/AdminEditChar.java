...
public class AdminEditChar implements IAdminCommandHandler {
    ...
    public boolean useAdminCommand(String command, L2PcInstance activeChar) {
        if (command.equals("admin_current_player")) { showCharacterInfo(activeChar, null); }
            ...
            try {
                ...
                else { activeChar.sendPacket(SystemMessageId.CHARACTER_DOES_NOT_EXIST); }
            }
            catch (Exception e) {
            ...
            try {
                ...
                if (valid && (player.getClassId().getId() != classidval)) {
                    ...
                    // Messages
                    if (player != activeChar) { player.sendMessage("A GM changed your class to " + newclass + "."); }
            ...
            L2Object target = activeChar.getTarget();
            if (target instanceof L2Summon) { gatherSummonInfo((L2Summon) target, activeChar); }
            ...
            L2Object target = activeChar.getTarget();
            if (target instanceof L2Summon) { ((L2Summon) target).unSummon(((L2Summon) target).getOwner()); }
            else { activeChar.sendPacket(SystemMessageId.INCORRECT_TARGET); }
        }
            ...
            if (target instanceof L2PetInstance) {
                ...
                try {
                    ...
                    if (oldExp > newExp) { pet.getStat().removeExp(oldExp - newExp); }
                    else if (oldExp < newExp) { pet.getStat().addExp(newExp - oldExp); }
                }
                catch (Exception e) {
                }
            }
            ...
            if (target instanceof L2PcInstance) {
                if (((L2PcInstance) target).isInParty()) { gatherPartyInfo((L2PcInstance) target, activeChar); }
                else { activeChar.sendMessage(((L2PcInstance) target).getName() + " isn't in a party."); }
            }
            else { activeChar.sendPacket(SystemMessageId.INCORRECT_TARGET); }
        }
            ...
            try {
                ...
                if (player != null) {
                    ...
                    if (changeCreateExpiryTime) {
                        player.variables().remove(EPlayerVariableKey.CLAN_CREATE_EXPIRY_TIME);
                    }
                    else {
                        player.variables().remove(EPlayerVariableKey.CLAN_JOIN_EXPIRY_TIME);
                    }
                }
    ...
    private static void listCharacters(L2PcInstance activeChar, int page) {
        ...
        for (int x = 0; x < max; x++) {
            ...
            if (page == pagenr) { StringUtil.append(sb, pagenr, "&nbsp;"); }
            else { StringUtil.append(sb, "<a action=\"bypass -h admin_show_characters ", pagenr, "\">", pagenr, "</a>&nbsp;"); }
        }
    ...
    private static void gatherCharacterInfo(L2PcInstance activeChar, L2PcInstance player, String filename) {
        ...
        html.replace("%xp%", player.getStat().getExp());
        html.replace("%sp%", player.getStat().getSp());
    ...
    private static void setTargetKarma(L2PcInstance activeChar, int newKarma) {
        ...
        else { activeChar.sendMessage("The karma value must be greater or equal to 0."); }
    }
    ...
    private static void findCharactersPerIp(L2PcInstance activeChar, String IpAdress) throws IllegalArgumentException {
        ...
        if (IpAdress.equals("disconnected")) { findDisconnected = true; }
        else {
        ...
        if (charactersFound == 0) { replyMSG2 = "."; }
        else if (charactersFound > 20) {
            html.replace("%number%", " more than 20.");
            replyMSG2 = "s.";
        }
        else if (charactersFound == 1) { replyMSG2 = "."; }
        else { replyMSG2 = "s."; }
    ...
    private static void findDualbox(L2PcInstance activeChar, int multibox) {
        ...
        for (L2PcInstance player : L2World.getInstance().getPlayers()) {
            L2GameClient client = player.getClient();
            if (client == null || client.isDetached()) { continue; }
        ...
        final StringBuilder sb = new StringBuilder();
        for (String dualboxIP : keys) { StringUtil.append(sb, "<a action=\"bypass -h admin_find_ip ", dualboxIP, "\">", dualboxIP, " (", dualboxIPs.get(dualboxIP), ")</a><br1>"); }
