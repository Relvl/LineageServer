...
public class L2Fishing implements Runnable {
    ...
    public L2Fishing(L2PcInstance Fisher, FishData fish, boolean isNoob, boolean isUpperGrade, int lureId) {
        ...
        if (_fishAiTask == null) { _fishAiTask = ThreadPoolManager.getInstance().scheduleEffectAtFixedRate(this, 1000, 1000); }
    }
    ...
    public synchronized void doDie(boolean win) {
        ...
        if (win) {
            if (Rnd.get(100) < 5) {
                ...
                _fisher.addItem(EItemProcessPurpose.FISHING, _fishId, 1, null, true);
    ...
    protected void aiTask() {
        ...
        try {
            if (_mode == 1) {
                if (_deceptiveMode == 0) { _fishCurHp += (int) _regenHp; }
            }
            else {
                if (_deceptiveMode == 1) { _fishCurHp += (int) _regenHp; }
            }

            if (_stop == 0) {
                _stop = 1;
                int check = Rnd.get(100);
                if (check >= 70) { _mode = _mode == 0 ? 1 : 0; }

                if (_isUpperGrade) {
                    check = Rnd.get(100);
                    if (check >= 90) { _deceptiveMode = _deceptiveMode == 0 ? 1 : 0; }
                }
            }
            else { _stop--; }
        }
            ...
            _thinking = false;
            if (_anim != 0) { _fisher.broadcastPacket(new ExFishingHpRegen(_fisher, _time, _fishCurHp, _mode, 0, _anim, 0, _deceptiveMode)); }
            else { _fisher.sendPacket(new ExFishingHpRegen(_fisher, _time, _fishCurHp, _mode, 0, _anim, 0, _deceptiveMode)); }
        }
    }
    ...
    public void useRealing(int dmg, int pen) {
        ...
        if (_mode == 1) {
            if (_deceptiveMode == 0) {
                ...
                if (pen == 50) { _fisher.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.REELING_SUCCESSFUL_PENALTY_S1).addNumber(pen)); }
    ...
    public void usePomping(int dmg, int pen) {
        ...
        if (_mode == 0) {
            if (_deceptiveMode == 0) {
                ...
                if (pen == 50) { _fisher.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.PUMPING_SUCCESSFUL_PENALTY_S1).addNumber(pen)); }
