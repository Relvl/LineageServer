...
public class Q402_PathToAHumanKnight extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30417-05.htm")) {
            if (player.getClassId() != ClassId.fighter) { htmltext = (player.getClassId() == ClassId.knight) ? "30417-02a.htm" : "30417-03.htm"; }
            else if (player.getLevel() < 19) { htmltext = "30417-02.htm"; }
            else if (st.hasQuestItems(SWORD_OF_RITUAL)) { htmltext = "30417-04.htm"; }
        }
            ...
            int coinCount = st.getQuestItemsCount(COIN_OF_LORDS_1) + st.getQuestItemsCount(COIN_OF_LORDS_2) + st.getQuestItemsCount(COIN_OF_LORDS_3) + st.getQuestItemsCount(COIN_OF_LORDS_4) + st.getQuestItemsCount(COIN_OF_LORDS_5) + st
                    .getQuestItemsCount(COIN_OF_LORDS_6);
            ...
            case QuestState.STATE_STARTED:
                switch (npc.getNpcId()) {
                    case SIR_KLAUS_VASPER:
                        int coins = st.getQuestItemsCount(COIN_OF_LORDS_1) + st.getQuestItemsCount(COIN_OF_LORDS_2) + st.getQuestItemsCount(COIN_OF_LORDS_3) + st.getQuestItemsCount(COIN_OF_LORDS_4) + st.getQuestItemsCount(COIN_OF_LORDS_5) + st
                                .getQuestItemsCount(COIN_OF_LORDS_6);
                        if (coins < 3) { htmltext = "30417-09.htm"; }
                        else if (coins == 3) { htmltext = "30417-10.htm"; }
                        else if (coins > 3 && coins < 6) { htmltext = "30417-11.htm"; }
                        else if (coins == 6) {
                    ...
                    case BATHIS:
                        if (st.hasQuestItems(COIN_OF_LORDS_1)) { htmltext = "30332-05.htm"; }
                        else if (st.hasQuestItems(GLUDIO_GUARD_MARK_1)) {
                            if (st.getQuestItemsCount(BUGBEAR_NECKLACE) < 10) { htmltext = "30332-03.htm"; }
                            else {
                        ...
                        else { htmltext = "30332-01.htm"; }
                        break;

                    case RAYMOND:
                        if (st.hasQuestItems(COIN_OF_LORDS_2)) { htmltext = "30289-06.htm"; }
                        else if (st.hasQuestItems(EINHASAD_CHURCH_MARK_1)) {
                            if (st.getQuestItemsCount(EINHASAD_CRUCIFIX) < 12) { htmltext = "30289-04.htm"; }
                            else {
                        ...
                        else { htmltext = "30289-01.htm"; }
                        break;

                    case BEZIQUE:
                        if (st.hasQuestItems(COIN_OF_LORDS_3)) { htmltext = "30379-05.htm"; }
                        else if (st.hasQuestItems(GLUDIO_GUARD_MARK_2)) {
                            if (st.getQuestItemsCount(SPIDER_LEG) < 20) { htmltext = "30379-03.htm"; }
                            else {
                        ...
                        else { htmltext = "30379-01.htm"; }
                        break;

                    case LEVIAN:
                        if (st.hasQuestItems(COIN_OF_LORDS_4)) { htmltext = "30037-05.htm"; }
                        else if (st.hasQuestItems(EINHASAD_CHURCH_MARK_2)) {
                            if (st.getQuestItemsCount(LIZARDMAN_TOTEM) < 20) { htmltext = "30037-03.htm"; }
                            else {
                        ...
                        else { htmltext = "30037-01.htm"; }
                        break;

                    case GILBERT:
                        if (st.hasQuestItems(COIN_OF_LORDS_5)) { htmltext = "30039-05.htm"; }
                        else if (st.hasQuestItems(GLUDIO_GUARD_MARK_3)) {
                            if (st.getQuestItemsCount(GIANT_SPIDER_HUSK) < 20) { htmltext = "30039-03.htm"; }
                            else {
                        ...
                        else { htmltext = "30039-01.htm"; }
                        break;

                    case BIOTIN:
                        if (st.hasQuestItems(COIN_OF_LORDS_6)) { htmltext = "30031-05.htm"; }
                        else if (st.hasQuestItems(EINHASAD_CHURCH_MARK_3)) {
                            if (st.getQuestItemsCount(HORRIBLE_SKULL) < 10) { htmltext = "30031-03.htm"; }
                            else {
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            case 20775: // Bugbear Raider
                if (st.hasQuestItems(GLUDIO_GUARD_MARK_1)) { st.dropItemsAlways(BUGBEAR_NECKLACE, 1, 10); }
                break;

            case 27024: // Undead Priest
                if (st.hasQuestItems(EINHASAD_CHURCH_MARK_1)) { st.dropItems(EINHASAD_CRUCIFIX, 1, 12, 500000); }
                break;

            case 20038: // Poison Spider
            case 20043: // Arachnid Tracker
            case 20050: // Arachnid Predator
                if (st.hasQuestItems(GLUDIO_GUARD_MARK_2)) { st.dropItemsAlways(SPIDER_LEG, 1, 20); }
                break;

            case 20030: // Langk Lizardman
            case 20027: // Langk Lizardman Scout
            case 20024: // Langk Lizardman Warrior
                if (st.hasQuestItems(EINHASAD_CHURCH_MARK_2)) { st.dropItems(LIZARDMAN_TOTEM, 1, 20, 500000); }
                break;

            case 20103: // Giant Spider
            case 20106: // Talon Spider
            case 20108: // Blade Spider
                if (st.hasQuestItems(GLUDIO_GUARD_MARK_3)) { st.dropItems(GIANT_SPIDER_HUSK, 1, 20, 400000); }
                break;

            case 20404: // Silent Horror
                if (st.hasQuestItems(EINHASAD_CHURCH_MARK_3)) { st.dropItems(HORRIBLE_SKULL, 1, 10, 400000); }
                break;
        }
