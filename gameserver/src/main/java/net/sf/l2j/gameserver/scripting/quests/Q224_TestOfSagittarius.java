        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    ...
                    case HAMIL:
                        if (cond == 1) { htmltext = "30626-01.htm"; }
                        else if (cond == 2 || cond == 3) { htmltext = "30626-04.htm"; }
                        else if (cond == 4) { htmltext = "30626-05.htm"; }
                        else if (cond > 4 && cond < 8) { htmltext = "30626-08.htm"; }
                        else if (cond == 8) {
                        ...
                        else if (cond > 8 && cond < 12) { htmltext = "30626-10.htm"; }
                        else if (cond == 12) {
                    ...
                    case SIR_ARON_TANFORD:
                        if (cond == 2) { htmltext = "30653-01.htm"; }
                        else if (cond > 2) { htmltext = "30653-03.htm"; }
                        break;

                    case VOKIAN:
                        if (cond == 5) { htmltext = "30514-01.htm"; }
                        else if (cond == 6) { htmltext = "30514-03.htm"; }
                        else if (cond == 7) {
                    ...
                    case GAUEN:
                        ...
                        else if (cond > 11) { htmltext = "30717-04.htm"; }
                        break;
                }
                break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            ...
            case NOBLE_ANT_LEADER:
                if (st.getInt("cond") == 3 && st.dropItems(HUNTER_RUNE_1, 1, 10, 500000)) { st.set("cond", "4"); }
                break;
            ...
            case MARSH_STAKATO_DRONE:
                if (st.getInt("cond") == 10 && st.dropItems(STAKATO_CHITIN, 1, 1, 100000) && st.hasQuestItems(MANASHEN_HORN, MITHRIL_CLIP, REINFORCED_BOWSTRING)) { st.set("cond", "11"); }
                break;

            case MARSH_SPIDER:
                if (st.getInt("cond") == 10 && st.dropItems(REINFORCED_BOWSTRING, 1, 1, 100000) && st.hasQuestItems(MANASHEN_HORN, MITHRIL_CLIP, STAKATO_CHITIN)) { st.set("cond", "11"); }
                break;

            case ROAD_SCAVENGER:
                if (st.getInt("cond") == 10 && st.dropItems(MITHRIL_CLIP, 1, 1, 100000) && st.hasQuestItems(MANASHEN_HORN, REINFORCED_BOWSTRING, STAKATO_CHITIN)) { st.set("cond", "11"); }
                break;

            case MANASHEN_GARGOYLE:
                if (st.getInt("cond") == 10 && st.dropItems(MANASHEN_HORN, 1, 1, 100000) && st.hasQuestItems(REINFORCED_BOWSTRING, MITHRIL_CLIP, STAKATO_CHITIN)) { st.set("cond", "11"); }
                break;
            ...
            case LETO_LIZARDMAN_OVERLORD:
                if (st.getInt("cond") == 13) {
                    ...
                    else { st.dropItemsAlways(BLOOD_OF_LIZARDMAN, 1, 0); }
                }
                break;
