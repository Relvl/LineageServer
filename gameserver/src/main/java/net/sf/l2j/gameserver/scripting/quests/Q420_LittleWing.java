...
public class Q420_LittleWing extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        // BYRON
            ...
            st.set("cond", "4");
            st.playSound(ESound.ItemSound_quest_middle);
            if (st.hasQuestItems(DELUXE_FAIRY_STONE)) { htmltext = "30711-04.htm"; }
        }
        // MIMYU
            ...
            if (st.hasQuestItems(FAIRY_DUST)) {
                ...
                if (Rnd.get(20) == 1) { st.giveItems(HATCHLING_SOFT_LEATHER, 1); }
                else {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    ...
                    case CRONOS:
                        if (cond == 1) { htmltext = "30610-01.htm"; }
                        else if (st.getInt("deluxestone") == 2) { htmltext = "30610-10.htm"; }
                        else if (cond == 2) {
                            if (st.hasAtLeastOneQuestItem(FAIRY_STONE, DELUXE_FAIRY_STONE)) {
                                if (st.getInt("deluxestone") == 1) { htmltext = "30610-14.htm"; }
                                else {
                            ...
                            else { htmltext = "30610-07.htm"; }
                        }
                        else if (cond == 3) { htmltext = "30610-09.htm"; }
                        else if (cond == 4 && st.hasAtLeastOneQuestItem(FAIRY_STONE, DELUXE_FAIRY_STONE)) { htmltext = "30610-11.htm"; }
                        break;

                    case MARIA:
                        if (st.hasAtLeastOneQuestItem(FAIRY_STONE, DELUXE_FAIRY_STONE)) { htmltext = "30608-06.htm"; }
                        else if (cond == 2) {
                            if (st.hasQuestItems(FAIRY_STONE_LIST)) { htmltext = (checkItems(st, false)) ? "30608-02.htm" : "30608-01.htm"; }
                            else if (st.hasQuestItems(DELUXE_FAIRY_STONE_LIST)) { htmltext = (checkItems(st, true)) ? "30608-04.htm" : "30608-01.htm"; }
                        }
                        break;

                    case BYRON:
                        int deluxestone = st.getInt("deluxestone");
                        if (deluxestone == 1) {
                            ...
                            else { htmltext = "30711-10.htm"; }
                        }
                        else if (deluxestone == 2) { htmltext = "30711-09.htm"; }
                        else if (cond == 3) { htmltext = "30711-01.htm"; }
                        else if (cond == 4) {
                            if (st.hasQuestItems(FAIRY_STONE)) { htmltext = "30711-07.htm"; }
                            else if (st.hasQuestItems(DELUXE_FAIRY_STONE)) { htmltext = "30711-08.htm"; }
                        }
                        break;

                    case MIMYU:
                        if (cond == 4) {
                            if (st.getInt("mimyu") == 1) { htmltext = "30747-06.htm"; }
                            else if (st.hasQuestItems(FAIRY_STONE)) { htmltext = "30747-01.htm"; }
                            else if (st.hasQuestItems(DELUXE_FAIRY_STONE)) { htmltext = "30747-03.htm"; }
                        }
                        else if (cond == 5) { htmltext = "30747-08.htm"; }
                        else if (cond == 6) {
                            int eggs = st.getQuestItemsCount(EGG_OF_DRAKE_EXARION) + st.getQuestItemsCount(EGG_OF_DRAKE_ZWOV) + st.getQuestItemsCount(EGG_OF_DRAKE_KALIBRAN) + st.getQuestItemsCount(EGG_OF_WYVERN_SUZET) + st.getQuestItemsCount(EGG_OF_WYVERN_SHAMHAI);
                            if (eggs < 20) { htmltext = "30747-09.htm"; }
                            else { htmltext = "30747-10.htm"; }
                        }
                        else if (cond == 7) { htmltext = "30747-11.htm"; }
                        break;

                    case EXARION:
                        if (cond == 5) { htmltext = "30748-01.htm"; }
                        else if (cond == 6) {
                            if (st.getQuestItemsCount(EGG_OF_DRAKE_EXARION) < 20) { htmltext = "30748-03.htm"; }
                            else {
                        ...
                        else if (cond == 7) { htmltext = "30748-05.htm"; }
                        break;

                    case ZWOV:
                        if (cond == 5) { htmltext = "30749-01.htm"; }
                        else if (cond == 6) {
                            if (st.getQuestItemsCount(EGG_OF_DRAKE_ZWOV) < 20) { htmltext = "30749-03.htm"; }
                            else {
                        ...
                        else if (cond == 7) { htmltext = "30749-05.htm"; }
                        break;

                    case KALIBRAN:
                        if (cond == 5) { htmltext = "30750-01.htm"; }
                        else if (cond == 6) { htmltext = (st.getQuestItemsCount(EGG_OF_DRAKE_KALIBRAN) < 20) ? "30750-03.htm" : "30750-04.htm"; }
                        else if (cond == 7) { htmltext = "30750-06.htm"; }
                        break;

                    case SUZET:
                        if (cond == 5) { htmltext = "30751-01.htm"; }
                        else if (cond == 6) {
                            if (st.getQuestItemsCount(EGG_OF_WYVERN_SUZET) < 20) { htmltext = "30751-04.htm"; }
                            else {
                        ...
                        else if (cond == 7) { htmltext = "30751-06.htm"; }
                        break;

                    case SHAMHAI:
                        if (cond == 5) { htmltext = "30752-01.htm"; }
                        else if (cond == 6) {
                            if (st.getQuestItemsCount(EGG_OF_WYVERN_SHAMHAI) < 20) { htmltext = "30752-03.htm"; }
                            else {
                        ...
                        else if (cond == 7) { htmltext = "30752-05.htm"; }
                        break;
                }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            case 20231:
                if (st.hasQuestItems(FAIRY_STONE_LIST)) { st.dropItems(TOAD_LORD_BACK_SKIN, 1, 10, 300000); }
                else if (st.hasQuestItems(DELUXE_FAIRY_STONE_LIST)) { st.dropItems(TOAD_LORD_BACK_SKIN, 1, 20, 300000); }
                break;

            case 20580:
                if (st.hasQuestItems(SCALE_OF_DRAKE_EXARION) && !st.dropItems(EGG_OF_DRAKE_EXARION, 1, 20, 500000)) { npc.broadcastNpcSay("If the eggs get taken, we're dead!"); }
                break;

            case 20233:
                if (st.hasQuestItems(SCALE_OF_DRAKE_ZWOV)) { st.dropItems(EGG_OF_DRAKE_ZWOV, 1, 20, 500000); }
                break;

            case 20551:
                if (st.hasQuestItems(SCALE_OF_DRAKE_KALIBRAN) && !st.dropItems(EGG_OF_DRAKE_KALIBRAN, 1, 20, 500000)) { npc.broadcastNpcSay("Hey! Everybody watch the eggs!"); }
                break;

            case 20270:
                if (st.hasQuestItems(SCALE_OF_WYVERN_SUZET) && !st.dropItems(EGG_OF_WYVERN_SUZET, 1, 20, 500000)) { npc.broadcastNpcSay("I thought I'd caught one share... Whew!"); }
                break;

            case 20202:
                if (st.hasQuestItems(SCALE_OF_WYVERN_SHAMHAI)) { st.dropItems(EGG_OF_WYVERN_SHAMHAI, 1, 20, 500000); }
                break;
    ...
    private static boolean checkItems(QuestState st, boolean isDeluxe) {
        ...
        if (st.getQuestItemsCount(COAL) < 10 || st.getQuestItemsCount(CHARCOAL) < 10) { return false; }

        if (isDeluxe) {
            if (st.getQuestItemsCount(GEMSTONE_C) >= 1 && st.getQuestItemsCount(SILVER_NUGGET) >= 5 && st.getQuestItemsCount(STONE_OF_PURITY) >= 1 && st.getQuestItemsCount(TOAD_LORD_BACK_SKIN) >= 20) { return true; }
        }
        else {
            if (st.getQuestItemsCount(GEMSTONE_D) >= 1 && st.getQuestItemsCount(SILVER_NUGGET) >= 3 && st.getQuestItemsCount(TOAD_LORD_BACK_SKIN) >= 10) { return true; }
        }
        return false;
    }
    ...
    private static void giveRandomPet(QuestState st, boolean hasFairyDust) {
        ...
        if (st.hasQuestItems(EGG_OF_DRAKE_EXARION)) {
            ...
            if (hasFairyDust) {
                if (chance < 45) { pet = DRAGONFLUTE_OF_WIND; }
                else if (chance < 75) { pet = DRAGONFLUTE_OF_STAR; }
            }
            else if (chance < 50) { pet = DRAGONFLUTE_OF_WIND; }
            else if (chance < 85) { pet = DRAGONFLUTE_OF_STAR; }
        }
        else if (st.hasQuestItems(EGG_OF_WYVERN_SUZET)) {
            st.takeItems(EGG_OF_WYVERN_SUZET, 1);
            if (hasFairyDust) {
                if (chance < 55) { pet = DRAGONFLUTE_OF_WIND; }
                else if (chance < 85) { pet = DRAGONFLUTE_OF_STAR; }
            }
            else if (chance < 65) { pet = DRAGONFLUTE_OF_WIND; }
            else if (chance < 95) { pet = DRAGONFLUTE_OF_STAR; }
        }
        else if (st.hasQuestItems(EGG_OF_DRAKE_KALIBRAN)) {
            st.takeItems(EGG_OF_DRAKE_KALIBRAN, 1);
            if (hasFairyDust) {
                if (chance < 60) { pet = DRAGONFLUTE_OF_WIND; }
                else if (chance < 90) { pet = DRAGONFLUTE_OF_STAR; }
            }
            else if (chance < 70) { pet = DRAGONFLUTE_OF_WIND; }
            else { pet = DRAGONFLUTE_OF_STAR; }
        }
        else if (st.hasQuestItems(EGG_OF_WYVERN_SHAMHAI)) {
            st.takeItems(EGG_OF_WYVERN_SHAMHAI, 1);
            if (hasFairyDust) {
                if (chance < 70) { pet = DRAGONFLUTE_OF_WIND; }
                else { pet = DRAGONFLUTE_OF_STAR; }
            }
            else if (chance < 85) { pet = DRAGONFLUTE_OF_WIND; }
            else { pet = DRAGONFLUTE_OF_STAR; }
        }
        else if (st.hasQuestItems(EGG_OF_DRAKE_ZWOV)) {
            st.takeItems(EGG_OF_DRAKE_ZWOV, 1);
            if (hasFairyDust) {
                if (chance < 90) { pet = DRAGONFLUTE_OF_WIND; }
                else { pet = DRAGONFLUTE_OF_STAR; }
            }
            else { pet = DRAGONFLUTE_OF_WIND; }
        }
