        ...
        if (command.startsWith("admin_geo_bug")) {
            ...
            if (GeoData.getInstance().hasGeoPos(geoX, geoY)) {
                try {
                    ...
                    if (GeoData.getInstance().addGeoBug(activeChar.getPosition(), activeChar.getName() + ": " + comment)) { activeChar.sendMessage("GeoData bug saved."); }
                }
                catch (Exception e) {
                    activeChar.sendMessage("Usage: //admin_geo_bug comments");
                }
            }
            else { activeChar.sendMessage("There is no geodata at this position."); }
        }
            ...
            if (target != null) {
                if (PathFinding.getInstance().canSeeTarget(activeChar, target)) { activeChar.sendMessage("Can see target."); }
                else { activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.CANT_SEE_TARGET)); }
            }
            ...
            L2Object target = activeChar.getTarget();
            if (target != null) {
                if (PathFinding.getInstance().canMoveToTarget(activeChar.getX(), activeChar.getY(), activeChar.getZ(), target.getX(), target.getY(), target.getZ())) { activeChar.sendMessage("Can move beeline."); }
                else { activeChar.sendMessage("Can not move beeline!"); }
            }
            else { activeChar.sendPacket(SystemMessageId.INCORRECT_TARGET); }
        }
        else { return false; }
