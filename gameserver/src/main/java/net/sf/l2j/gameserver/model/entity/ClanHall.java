...
public class ClanHall {
    ...
    public void openCloseDoor(L2PcInstance activeChar, int doorId, boolean open) {
        if (activeChar != null && activeChar.getClanId() == getOwnerId()) { openCloseDoor(doorId, open); }
    }
    ...
    public boolean updateFunctions(L2PcInstance player, int type, int lvl, int lease, long rate, boolean addNew) {
        ...
        if (addNew) { _functions.put(type, new ClanHallFunction(type, lvl, lease, 0, rate, 0, false)); }
            ...
            if (lvl == 0 && lease == 0) { removeFunction(type); }
                ...
                if (diffLease > 0) { _functions.put(type, new ClanHallFunction(type, lvl, lease, diffLease, rate, -1, false)); }
                else {
                    chf.setLease(lease);
                    chf.setLvl(lvl);
                    chf.dbSave();
        ...
        public void run() {
            try {
                ...
                if (clan.getWarehouse().getAdena() >= getLease()) {
                    if (_paidUntil != 0) {
                        while (_paidUntil <= time) { _paidUntil += CH_RATE; }
                    }
