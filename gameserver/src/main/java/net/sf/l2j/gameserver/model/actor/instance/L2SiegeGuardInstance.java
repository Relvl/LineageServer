...
public final class L2SiegeGuardInstance extends L2Attackable {
    ...
    public L2CharacterAI getAI() {
        ...
        if (ai == null) {
            synchronized (this) {
                if (this.ai == null) { this.ai = new L2SiegeGuardAI(this); }
    ...
    public void returnHome() {
        if (getWalkSpeed() <= 0) { return; }

        if (!isInsideRadius(getSpawn().getLocx(), getSpawn().getLocy(), 40, false)) {
            setIsReturningToSpawnPoint(true);
            clearAggroList();

            if (hasAI()) { getAI().setIntention(EIntention.MOVE_TO, new HeadedLocation(getSpawn().getLocx(), getSpawn().getLocy(), getSpawn().getLocz(), 0)); }
        }
    }
    ...
    public void addDamageHate(L2Character attacker, int damage, int aggro) {
        ...
        if (!(attacker instanceof L2SiegeGuardInstance)) { super.addDamageHate(attacker, damage, aggro); }
    }
