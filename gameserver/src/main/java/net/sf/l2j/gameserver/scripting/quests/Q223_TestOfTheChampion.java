    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                ...
                if (classId != ClassId.warrior && classId != ClassId.orcRaider) { htmltext = "30624-01.htm"; }
                else if (player.getLevel() < 39) { htmltext = "30624-02.htm"; }
                else { htmltext = (classId == ClassId.warrior) ? "30624-03.htm" : "30624-04.htm"; }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case ASCALON:
                        if (cond == 1) { htmltext = "30624-07.htm"; }
                        else if (cond < 4) { htmltext = "30624-08.htm"; }
                        else if (cond == 4) { htmltext = "30624-09.htm"; }
                        else if (cond == 5) { htmltext = "30624-11.htm"; }
                        else if (cond > 5 && cond < 8) { htmltext = "30624-12.htm"; }
                        else if (cond == 8) { htmltext = "30624-13.htm"; }
                        else if (cond == 9) { htmltext = "30624-15.htm"; }
                        else if (cond > 9 && cond < 14) { htmltext = "30624-16.htm"; }
                        else if (cond == 14) {
                    ...
                    case MASON:
                        if (cond == 1) { htmltext = "30625-01.htm"; }
                        else if (cond == 2) { htmltext = "30625-04.htm"; }
                        else if (cond == 3) {
                        ...
                        else if (cond == 4) { htmltext = "30625-06.htm"; }
                        else if (cond > 4) { htmltext = "30625-07.htm"; }
                        break;

                    case GROOT:
                        if (cond == 5) { htmltext = "30093-01.htm"; }
                        else if (cond == 6) { htmltext = "30093-03.htm"; }
                        else if (cond == 7) {
                        ...
                        else if (cond == 8) { htmltext = "30093-05.htm"; }
                        else if (cond > 8) { htmltext = "30093-06.htm"; }
                        break;

                    case MOUEN:
                        if (cond == 9) { htmltext = "30196-01.htm"; }
                        else if (cond == 10) { htmltext = "30196-04.htm"; }
                        else if (cond == 11) { htmltext = "30196-05.htm"; }
                        else if (cond == 12) { htmltext = "30196-07.htm"; }
                        else if (cond == 13) {
                        ...
                        else if (cond > 13) { htmltext = "30196-09.htm"; }
                        break;
                }
                break;
        ...
        switch (npc.getNpcId()) {
            case HARPY: // Possibility to spawn an HARPY _MATRIARCH.
                if (st.getInt("cond") == 6 && Rnd.nextBoolean() && !npc.isScriptValue(1)) {
                    L2Character originalKiller = isPet ? attacker.getPet() : attacker;

                    // Spawn one or two matriarchs.
                    for (int i = 1; i < ((Rnd.get(10) < 7) ? 2 : 3); i++) {
                        L2Attackable collector = (L2Attackable) addSpawn(HARPY_MATRIARCH, npc, true, 0, false);
            ...
            case ROAD_SCAVENGER: // Possibility to spawn a Road Collector.
                if (st.getInt("cond") == 10 && Rnd.nextBoolean() && !npc.isScriptValue(1)) {
                    ...
                    for (int i = 1; i < ((Rnd.get(10) < 7) ? 2 : 3); i++) {
                        L2Attackable collector = (L2Attackable) addSpawn(ROAD_COLLECTOR, npc, true, 0, false);
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npcId) {
            case BLOODY_AXE_ELITE:
                if (st.getInt("cond") == 2 && st.dropItemsAlways(BLOODY_AXE_HEAD, 1, 100)) { st.set("cond", "3"); }
                break;

            case HARPY:
            case HARPY_MATRIARCH:
                if (st.getInt("cond") == 6 && st.dropItems(HARPY_EGG, 1, 30, 500000)) { if (st.getQuestItemsCount(MEDUSA_VENOM) == 30 && st.getQuestItemsCount(WINDSUS_BILE) == 30) { st.set("cond", "7"); } }
                break;

            case MEDUSA:
                if (st.getInt("cond") == 6 && st.dropItems(MEDUSA_VENOM, 1, 30, 500000)) { if (st.getQuestItemsCount(HARPY_EGG) == 30 && st.getQuestItemsCount(WINDSUS_BILE) == 30) { st.set("cond", "7"); } }
                break;

            case WINDSUS:
                if (st.getInt("cond") == 6 && st.dropItems(WINDSUS_BILE, 1, 30, 500000)) { if (st.getQuestItemsCount(HARPY_EGG) == 30 && st.getQuestItemsCount(MEDUSA_VENOM) == 30) { st.set("cond", "7"); } }
                break;

            case ROAD_COLLECTOR:
            case ROAD_SCAVENGER:
                if (st.getInt("cond") == 10 && st.dropItemsAlways(ROAD_RATMAN_HEAD, 1, 100)) { st.set("cond", "11"); }
                break;
            ...
            case LETO_LIZARDMAN_OVERLORD:
                if (st.getInt("cond") == 12 && st.dropItems(LETO_LIZARDMAN_FANG, 1, 100, 500000 + (100000 * (npcId - 20577)))) { st.set("cond", "13"); }
                break;
        }
