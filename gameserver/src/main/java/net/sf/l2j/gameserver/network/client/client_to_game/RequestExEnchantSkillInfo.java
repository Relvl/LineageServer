...
import net.sf.l2j.gameserver.network.client.game_to_client.ExEnchantSkillInfo;
...
public final class RequestExEnchantSkillInfo extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (!trainer.getTemplate().canTeach(activeChar.getClassId())) { return; }
        ...
        for (L2EnchantSkillLearn esl : SkillTreeTable.getInstance().getAvailableEnchantSkills(activeChar)) {
            ...
            if (esl.getId() == _skillId && esl.getLevel() == _skillLevel) {
                ...
                if (Config.ES_SP_BOOK_NEEDED) { if (data.getItemId() != 0 && data.getItemCount() != 0) { esi.addRequirement(4, data.getItemId(), data.getItemCount(), 0); } }
                sendPacket(esi);
