...
public class L2OlympiadStadiumZone extends L2SpawnZone {
    ...
    public final void broadcastStatusUpdate(L2PcInstance player) {
        ...
        for (L2PcInstance plyr : getKnownTypeInside(L2PcInstance.class)) {
            if (plyr.isInObserverMode() || plyr.getOlympiadSide() != player.getOlympiadSide()) { plyr.sendPacket(packet); }
        }
    }
    ...
    public final void broadcastPacketToObservers(L2GameServerPacket packet) {
        for (L2PcInstance player : getKnownTypeInside(L2PcInstance.class)) {
            if (player.isInObserverMode()) { player.sendPacket(packet); }
        }
    }
    ...
    protected final void onEnter(L2Character character) {
        ...
        if (character instanceof L2Playable) {
            ...
            if (player != null) {
                ...
                if (!player.isGM() && !player.isInOlympiadMode() && !player.isInObserverMode()) { ThreadPoolManager.getInstance().executeTask(new KickPlayer(player)); }
            }
        }
    }
    ...
    public final void updateZoneStatusForCharactersInside() {
        ...
        if (battleStarted) { sm = SystemMessage.getSystemMessage(SystemMessageId.ENTERED_COMBAT_ZONE); }
        else { sm = SystemMessage.getSystemMessage(SystemMessageId.LEFT_COMBAT_ZONE); }
        ...
        for (L2Character character : _characterList) {
            ...
            if (battleStarted) {
                character.setInsideZone(ZoneId.PVP, true);
                if (character instanceof L2PcInstance) { character.sendPacket(sm); }
            }
            else {
                character.setInsideZone(ZoneId.PVP, false);
                if (character instanceof L2PcInstance) {
    ...
    private static final class KickPlayer implements Runnable {
        ...
        public void run() {
            if (_player != null) {
                ...
                if (summon != null) { summon.unSummon(_player); }
