...
public class Q234_FatesWhisper extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("31002-03.htm")) {
            ...
            if (st.getInt("bypass") == 1) { return null; }
            ...
            if (st.getInt("bypass") == 1) {
                ...
                else { htmltext = getHtmlText("31002-15.htm").replace("%weaponname%", WEAPONS.get(itemId)); }
            }
            else { htmltext = "31002-16.htm"; }
        }
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case 31002:
                        if (cond == 1) {
                            if (!st.hasQuestItems(REIRIA_SOUL_ORB)) { htmltext = "31002-04b.htm"; }
                            else {
                            ...
                            if (!st.hasQuestItems(KERMON_INFERNIUM_SCEPTER) || !st.hasQuestItems(GOLKONDA_INFERNIUM_SCEPTER) || !st.hasQuestItems(HALLATE_INFERNIUM_SCEPTER)) { htmltext = "31002-05c.htm"; }
                            else {
                            ...
                            if (!st.hasQuestItems(INFERNIUM_VARNISH)) { htmltext = "31002-06b.htm"; }
                            else {
                            ...
                            if (!st.hasQuestItems(REORIN_HAMMER)) { htmltext = "31002-07b.htm"; }
                            else {
                        ...
                        else if (cond > 4 && cond < 8) { htmltext = "31002-08b.htm"; }
                        else if (cond == 8) {
                            ...
                            if (st.getQuestItemsCount(CRYSTAL_B) < 984) { htmltext = "31002-09b.htm"; }
                            else {
                            ...
                            else { htmltext = "31002-BGradeList.htm"; }
                        }
                        break;

                    case 30182:
                        if (cond == 3) { htmltext = (!st.hasQuestItems(INFERNIUM_VARNISH)) ? "30182-01.htm" : "30182-02.htm"; }
                        break;
                    ...
                    case 30847:
                        ...
                        else if (cond >= 4 && st.hasQuestItems(REORIN_HAMMER)) { htmltext = "30847-02.htm"; }
                        break;

                    case 30178:
                        if (cond == 5) { htmltext = "30178-01.htm"; }
                        else if (cond > 5) { htmltext = "30178-02.htm"; }
                        break;

                    case 30833:
                        if (cond == 6) { htmltext = "30833-01.htm"; }
                        else if (cond == 7) {
                            if (st.hasQuestItems(PIPETTE_KNIFE) && !st.hasQuestItems(RED_PIPETTE_KNIFE)) { htmltext = "30833-02.htm"; }
                            else {
                        ...
                        else if (cond > 7) { htmltext = "30833-04.htm"; }
                        break;
                    ...
                    case 31027:
                        ...
                        else { htmltext = "31027-02.htm"; }
                        break;

                    case 31028:
                    case 31029:
                    case 31030:
                        int itemId = npc.getNpcId() - 26361;
                        if (cond == 2 && !st.hasQuestItems(itemId)) {
                            htmltext = npc.getNpcId() + "-01.htm";
                            st.playSound(ESound.ItemSound_quest_itemget);
                            st.giveItems(itemId, 1);
                        }
                        else { htmltext = npc.getNpcId() + "-02.htm"; }
                        break;
                }
                break;
