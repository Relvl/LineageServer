...
public class FriendsBBSManager extends BaseBBSManager {
    ...
    public void parseCmd(String command, L2PcInstance activeChar) {
        if (command.startsWith("_friendlist")) { showFriendsList(activeChar, false); }
        else if (command.startsWith("_blocklist")) { showBlockList(activeChar, false); }
            ...
            if (action.equals("select")) {
                ...
                try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
                    ...
                    LOGGER.error("could not delete friends objectid: ", e);
                }
                ...
                try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
                    ...
                    LOGGER.error("could not delete friend objectid: ", e);
                }
                ...
                if (!activeChar.getSelectedFriendList().isEmpty()) { showMailWrite(activeChar); }
            }
        }
            ...
            if (action.equals("select")) {
                ...
                for (Integer blockId : list) { BlockList.removeFromBlockList(activeChar, blockId); }

                activeChar.getSelectedBlocksList().clear();
                showBlockList(activeChar, false);
            }
            else if (action.equals("delconfirm")) { showBlockList(activeChar, true); }
            else if (action.equals("del")) {
                for (Integer blockId : activeChar.getSelectedBlocksList()) { BlockList.removeFromBlockList(activeChar, blockId); }
