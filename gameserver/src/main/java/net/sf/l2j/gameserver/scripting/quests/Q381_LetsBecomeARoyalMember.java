...
public class Q381_LetsBecomeARoyalMember extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                switch (npc.getNpcId()) {
                    case SORINT:
                        if (!st.hasQuestItems(KAIL_COIN)) { htmltext = "30232-04.htm"; }
                        else if (!st.hasQuestItems(COIN_ALBUM)) { htmltext = "30232-05.htm"; }
                        else {
                    ...
                    case SANDRA:
                        if (!st.hasQuestItems(COIN_ALBUM)) {
                            if (st.getInt("aCond") == 0) { htmltext = "30090-01.htm"; }
                            else {
                                if (!st.hasQuestItems(GOLDEN_CLOVER_COIN)) { htmltext = "30090-03.htm"; }
                                else {
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (npc.getNpcId() == 21018) { st.dropItems(KAIL_COIN, 1, 1, 50000); }
        else if (st.getInt("aCond") == 1) { st.dropItemsAlways(GOLDEN_CLOVER_COIN, 1, 1); }
