...
public class L2Attackable extends L2Npc {
    ...
    public L2CharacterAI getAI() {
        ...
        if (ai == null) {
            synchronized (this) {
                if (this.ai == null) { this.ai = new L2AttackableAI(this); }
    ...
    public void reduceCurrentHp(double damage, L2Character attacker, boolean awake, boolean isDOT, L2Skill skill) {
        ...
        if (this instanceof L2MonsterInstance) {
            ...
            if (master.hasMinions()) { master.getMinionList().onAssist(this, attacker); }

            master = master.getLeader();
            if (master != null && master.hasMinions()) { master.getMinionList().onAssist(this, attacker); }
        }
    ...
    public boolean doDie(L2Character killer) {
        ...
        try {
            ...
            if (player != null) {
                ...
                if (quests != null) { for (Quest quest : quests) { ThreadPoolManager.getInstance().scheduleEffect(new OnKillNotifyTask(this, quest, player, killer instanceof L2Summon), 3000); } }
            }
        }
        catch (Exception e) {
            LOGGER.error("", e);
    ...
    protected void calculateRewards(L2Character lastAttacker) {
        ...
        for (AggroInfo info : _aggroList.values()) {
            ...
            L2PcInstance attacker = info.getAttacker().getActingPlayer();
            ...
            if (!Util.checkIfInRange(Config.ALT_PARTY_RANGE, this, attacker, true)) { continue; }
        ...
        for (RewardInfo reward : rewards.values()) {
            ...
            L2PcInstance attacker = reward.getAttacker();
            ...
            float penalty = attacker.hasServitor() ? ((L2SummonInstance) attacker.getPet()).getExpPenalty() : 0;
            ...
            if (attackerParty == null) {
                ...
                if (attacker.getKnownList().isObjectKnown(this) && !attacker.isDead()) {
                    ...
                    int[] expSp = calculateExpAndSp(levelDiff, damage, totalDamage);
                    long exp = expSp[0];
                    int sp = expSp[1];
                    ...
                    if (_overhit && _overhitAttacker != null && _overhitAttacker.getActingPlayer() != null && attacker == _overhitAttacker.getActingPlayer()) {
            ...
            // Share with party members.
                ...
                List<L2PcInstance> rewardedMembers = new ArrayList<>();
                ...
                for (L2PcInstance partyPlayer : groupMembers) {
                    ...
                    if (reward2 != null) {
                        if (Util.checkIfInRange(Config.ALT_PARTY_RANGE, this, partyPlayer, true)) {
                            ...
                            if (partyPlayer.getLevel() > partyLvl) { partyLvl = (attackerParty.isInCommandChannel()) ? attackerParty.getCommandChannel().getLevel() : partyPlayer.getLevel(); }
                        }
                ...
                if (partyDmg < totalDamage) { partyMul = (float) partyDmg / totalDamage; }
    ...
    public boolean hasRandomAnimation() {
        return Config.MAX_MONSTER_ANIMATION > 0 && !_isRaid;
    }
    ...
    public void addDamage(L2Character attacker, int damage) {
        ...
        if (player != null) {
            ...
            if (quests != null) { for (Quest quest : quests) { quest.notifyAttack(this, player, damage, attacker instanceof L2Summon); } }
        }
    ...
    public void addDamageHate(L2Character attacker, int damage, int aggro) {
        ...
        if (aggro == 0) {
            ...
            if (targetPlayer != null) {
                ...
                if (quests != null) { for (Quest quest : quests) { quest.notifyAggro(this, targetPlayer, attacker instanceof L2Summon); } }
            }
            else {
                aggro = 1;
                ai.addHate(1);
            ...
            if (aggro > 0 && getAI().getIntention() == EIntention.IDLE) { getAI().setIntention(EIntention.ACTIVE); }
        }
    }
    ...
    public void reduceHate(L2Character target, int amount) {
        ...
        if (target == null) // whole aggrolist
        {
            ...
            for (AggroInfo ai : _aggroList.values()) { ai.addHate(-amount); }
    ...
    public L2Character getMostHated() {
        if (_aggroList.isEmpty() || isAlikeDead()) { return null; }
    ...
    public List<L2Character> getHateList() {
        if (_aggroList.isEmpty() || isAlikeDead()) { return Collections.emptyList(); }
    ...
    public int getHating(L2Character target) {
        if (_aggroList.isEmpty() || target == null) { return 0; }
    ...
    private IntIntHolder calculateRewardItem(L2PcInstance lastAttacker, DropData drop, int levelModifier, boolean isSweep) {
        ...
        if (isChampion()) { dropChance *= Config.CHAMPION_REWARDS; }
    ...
    private IntIntHolder calculateCategorizedRewardItem(L2PcInstance lastAttacker, DropCategory categoryDrops, int levelModifier) {
        if (categoryDrops == null) { return null; }
        ...
        if (isChampion()) { categoryDropChance *= Config.CHAMPION_REWARDS; }
        ...
        if (Rnd.get(DropData.MAX_CHANCE) < categoryDropChance) {
            DropData drop = categoryDrops.dropOne(_isRaid && !_isRaidMinion);
            ...
            if (dropChance < DropData.MAX_CHANCE) { dropChance = DropData.MAX_CHANCE; }
    ...
    private int calculateLevelModifierForDrop(L2PcInstance lastAttacker) {
        if (Config.DEEPBLUE_DROP_RULES) {
            ...
            for (L2Character atkChar : _attackByList) { if (atkChar.getLevel() > highestLevel) { highestLevel = atkChar.getLevel(); } }
    ...
    public void doItemDrop(NpcTemplate npcTemplate, L2Character mainDamageDealer) {
        if (mainDamageDealer == null) { return; }
        ...
        for (DropCategory cat : npcTemplate.getDropData()) {
            ...
            if (cat.isSweep()) {
                ...
                else { item = calculateCategorizedRewardItem(player, cat, levelModifier); }

                if (item != null) {
                    // Check if the autoLoot mode is active
                    if (!_isRaid && Config.AUTO_LOOT) {
                    ...
                    if (_isRaid && !_isRaidMinion) { broadcastPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_DIED_DROPPED_S3_S2).addCharName(this).addItemName(item.getId()).addNumber(item.getValue())); }
                }
            }
        }
        ...
        if (isChampion() && Config.CHAMPION_REWARD > 0) {
            ...
            if (Rnd.get(100) < dropChance) {
                ...
                if (Config.AUTO_LOOT) { player.addItem(EItemProcessPurpose.CHAMPION_LOOT, item.getId(), item.getValue(), this, true); }
        ...
        if (getTemplate().getDropHerbGroup() > 0) {
            for (DropCategory cat : HerbDropTable.getInstance().getHerbDroplist(getTemplate().getDropHerbGroup())) {
                ...
                if (item != null) {
                    if (Config.AUTO_LOOT_HERBS) { player.addItem(EItemProcessPurpose.CHAMPION_LOOT, item.getId(), 1, this, true); }
                    else {
                        // If multiple similar herbs drop, split them and make a unique drop per item.
                        int count = item.getValue();
                        if (count > 1) {
                            item.setValue(1);
                            for (int i = 0; i < count; i++) { dropItem(player, item); }
                        }
                        else { dropItem(player, item); }
                    }
                }
    ...
    public L2ItemInstance dropItem(L2PcInstance mainDamageDealer, IntIntHolder item) {
        ...
        for (int i = 0; i < item.getValue(); i++) {
            ...
            if (ItemTable.getInstance().getTemplate(item.getId()) != null) {
                ...
                ditem = ItemTable.getInstance().createItem(EItemProcessPurpose.LOOT, item.getId(), item.getValue(), mainDamageDealer, this);
    ...
    public void useMagic(L2Skill skill) {
        if (skill == null || isAlikeDead()) { return; }
        ...
        if (getCurrentMp() < getStat().getMpConsume(skill) + getStat().getMpInitialConsume(skill)) { return; }

        if (getCurrentHp() <= skill.getHpConsume()) { return; }

        if (skill.isMagic()) {
            if (isMuted()) { return; }
        }
        else {
            if (isPhysicalMuted()) { return; }
        }
    ...
    public void returnHome() {
        ...
        if (hasAI() && getSpawn() != null) { getAI().setIntention(EIntention.MOVE_TO, new HeadedLocation(getSpawn().getLocx(), getSpawn().getLocy(), getSpawn().getLocz(), 0)); }
    }
    ...
    public void addAbsorber(L2PcInstance user, L2ItemInstance crystal) {
        ...
        if (ai == null) {
            ...
            if (!ai.isRegistered()) { ai.setItemId(crystal.getObjectId()); }
        }
    }
    ...
    public void registerAbsorber(L2PcInstance user) {
        ...
        if (user.getInventory().getItemByObjectId(ai.getItemId()) == null) { return; }
    ...
    private int[] calculateExpAndSp(int diff, int damage, long totalDamage) {
        ...
        L2Skill hpSkill = getKnownSkill(4408);
        if (hpSkill != null) {
            xp *= hpSkill.getPower();
            sp *= hpSkill.getPower();
        }
    ...
    public long calculateOverhitExp(long normalExp) {
        ...
        if (overhitPercentage > 25) { overhitPercentage = 25; }
