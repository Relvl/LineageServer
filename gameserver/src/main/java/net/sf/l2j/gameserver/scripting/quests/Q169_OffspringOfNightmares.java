...
public class Q169_OffspringOfNightmares extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getRace() != PlayerRace.DarkElf) { htmltext = "30145-00.htm"; }
                else if (player.getLevel() < 15) { htmltext = "30145-02.htm"; }
                else { htmltext = "30145-03.htm"; }
                break;

            case QuestState.STATE_STARTED:
                int cond = st.getInt("cond");
                if (cond == 1) {
                    if (st.hasQuestItems(CRACKED_SKULL)) { htmltext = "30145-06.htm"; }
                    else { htmltext = "30145-05.htm"; }
                }
                else if (cond == 2) { htmltext = "30145-07.htm"; }
                break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.getInt("cond") == 1 && st.dropItems(PERFECT_SKULL, 1, 1, 200000)) { st.set("cond", "2"); }
        else { st.dropItems(CRACKED_SKULL, 1, 0, 500000); }
