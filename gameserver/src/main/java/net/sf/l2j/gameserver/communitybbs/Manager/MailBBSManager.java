...
public class MailBBSManager extends BaseBBSManager {
    private enum MailType {
        ...
        MailType(String description, String bypass) {
    ...
    public void parseCmd(String command, L2PcInstance activeChar) {
        if (command.equals("_bbsmail") || command.equals("_maillist_0_1_0_")) { showMailList(activeChar, 1, MailType.INBOX); }
            ...
            if (action.equals("inbox") || action.equals("sentbox") || action.equals("archive") || action.equals("temparchive")) {
            ...
            else if (action.equals("crea")) { showWriteView(activeChar); }
            else if (action.equals("view")) {
                int letterId = (st.hasMoreTokens()) ? Integer.parseInt(st.nextToken()) : -1;

                Mail letter = getLetter(activeChar, letterId);
                if (letter == null) { showLastForum(activeChar); }
                else {
                    showLetterView(activeChar, letter);
                    if (letter.unread) { setLetterToRead(activeChar, letter.letterId); }
                }
            }
                ...
                Mail letter = getLetter(activeChar, letterId);
                if (letter == null) { showLastForum(activeChar); }
                else { showWriteView(activeChar, getCharName(letter.senderId), letter); }
            }
            else if (action.equals("del")) {
                int letterId = (st.hasMoreTokens()) ? Integer.parseInt(st.nextToken()) : -1;

                Mail letter = getLetter(activeChar, letterId);
                if (letter != null) { deleteLetter(activeChar, letter.letterId); }
                ...
                Mail letter = getLetter(activeChar, letterId);
                if (letter != null) { setLetterLocation(activeChar, letter.letterId, MailType.ARCHIVE); }

                showMailList(activeChar, 1, MailType.ARCHIVE);
            }
        }
        else { super.parseCmd(command, activeChar); }
    }
    ...
    public void parseWrite(String ar1, String ar2, String ar3, String ar4, String ar5, L2PcInstance activeChar) {
        ...
        else { super.parseWrite(ar1, ar2, ar3, ar4, ar5, activeChar); }
    }
    ...
    private void initId() {
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            while (result.next()) {
                if (result.getInt(1) > _lastid) { _lastid = result.getInt(1); }
            }
            result.close();
            statement.close();
        }
        catch (Exception e) {
            LOGGER.error(getClass().getSimpleName() + ": data error on MailBBS (initId): " + e);
            e.printStackTrace();
        }
    }
    ...
    private List<Mail> getPlayerMails(int objId) {
        ...
        if (_letters == null) {
            ...
            try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
                ...
                LOGGER.error("couldnt load mail for ID:{} {}", objId, e.getMessage());
            }
    ...
    private void showMailList(L2PcInstance activeChar, int page, MailType type, String sType, String search) {
        ...
        if (!sType.equals("") && !search.equals("")) {
            ...
            for (Mail letter : getPlayerMails(activeChar.getObjectId())) {
                if (byTitle && letter.subject.toLowerCase().contains(search.toLowerCase())) { letters.add(letter); }
                else if (!byTitle) {
                    String writer = getCharName(letter.senderId);
                    if (writer.toLowerCase().contains(search.toLowerCase())) { letters.add(letter); }
                }
            }
        }
        else { letters = getPlayerMails(activeChar.getObjectId()); }

        int countMails = getCountLetters(activeChar.getObjectId(), type, sType, search);
        ...
        for (Mail letter : letters) {
            if (letter.location.equals(type)) {
                ...
                if (letter.unread) { sb.append("<font color=\"LEVEL\">"); }

                sb.append(abbreviate(letter.subject, 51));

                if (letter.unread) { sb.append("</font>"); }
        ...
        if (maxpage > 21) {
            if (page <= 11) {
                for (i = 1; i <= (10 + page); i++) {
                    if (i == page) { StringUtil.append(sb, "<td> ", i, " </td>"); }
                    else { StringUtil.append(sb, "<td><a action=\"bypass _bbsmail;", type, ";", i, fullSearch, "\"> ", i, " </a></td>"); }
                }
            }
    ...
    public void sendLetter(String recipients, String subject, String message, L2PcInstance activeChar) {
        ...
        for (Mail letter : getPlayerMails(activeChar.getObjectId())) { if (letter.sentDate.after(ts) && letter.location == MailType.SENTBOX) { countTodaysLetters++; } }
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            for (String r : recipAr) { recipts.add(r.trim()); }
            ...
            for (String recipient : recipts) {
                ...
                if (recipId <= 0 || recipId == activeChar.getObjectId()) { activeChar.sendPacket(SystemMessageId.INCORRECT_TARGET); }
                else if (!activeChar.isGM()) {
                    if (isGM(recipId)) { activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.CANNOT_MAIL_GM_S1).addString(recipient)); }
                    else if (isBlocked(activeChar, recipId)) { activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_BLOCKED_YOU_CANNOT_MAIL).addString(recipient)); }
                        ...
                        if (PCrecipient != null) { PCrecipient.sendPacket(SystemMessageId.MAILBOX_FULL); }
                    }
                }
            ...
            if (countRecips > 5 && !activeChar.isGM()) { activeChar.sendPacket(SystemMessageId.ONLY_FIVE_RECIPIENTS); }

            if (sent) { activeChar.sendPacket(SystemMessageId.SENT_MAIL); }
        }
        catch (Exception e) {
            LOGGER.error("couldnt send letter for {} {}", activeChar.getName(), e.getMessage());
        }
    }
    ...
    private int getCountLetters(int objId, MailType location, String sType, String search) {
        ...
        if (!sType.equals("") && !search.equals("")) {
            ...
            for (Mail letter : getPlayerMails(objId)) {
                if (!letter.location.equals(location)) { continue; }

                if (byTitle && letter.subject.toLowerCase().contains(search.toLowerCase())) { count++; }
                else if (!byTitle) {
                    String writer = getCharName(letter.senderId);
                    if (writer.toLowerCase().contains(search.toLowerCase())) { count++; }
                }
            }
        }
        else {
            for (Mail letter : getPlayerMails(objId)) {
                if (letter.location.equals(location)) { count++; }
            }
        }
        return count;
    }
    ...
    private void deleteLetter(L2PcInstance activeChar, int letterId) {
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.error("couldnt delete letter {}", letterId, e);
        }
    }
    ...
    private void setLetterToRead(L2PcInstance activeChar, int letterId) {
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.error("couldnt set unread to false for {}", letterId, e);
        }
    }
    ...
    private void setLetterLocation(L2PcInstance activeChar, int letterId, MailType location) {
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.error("couldnt set location to false for {}", letterId, e);
        }
    }
    ...
    private static int getMaxPageId(int letterCount) {
        ...
        if (letterCount % 10 == 0) { return letterCount / 10; }
