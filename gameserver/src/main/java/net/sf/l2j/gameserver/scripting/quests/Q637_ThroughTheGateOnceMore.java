...
public class Q637_ThroughTheGateOnceMore extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        else if (event.equalsIgnoreCase("32010-10.htm")) { st.exitQuest(true); }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getLevel() < 73 || !st.hasQuestItems(FADED_VISITOR_MARK)) { htmltext = "32010-01a.htm"; }
                else if (st.hasQuestItems(PAGAN_MARK)) { htmltext = "32010-00.htm"; }
                else { htmltext = "32010-01.htm"; }
                break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropItems(NECROMANCER_HEART, 1, 10, 400000)) { st.set("cond", "2"); }
