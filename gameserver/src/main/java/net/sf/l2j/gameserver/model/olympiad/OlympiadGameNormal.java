...
abstract public class OlympiadGameNormal extends AbstractOlympiadGame {
    ...
    protected final void broadcastPacket(L2GameServerPacket packet) {
        ...
        if (_playerOne.player != null) { _playerOne.player.sendPacket(packet); }

        _playerTwo.updatePlayer();
        if (_playerTwo.player != null) { _playerTwo.player.sendPacket(packet); }
    }
    ...
    protected final boolean makeCompetitionStart() {
        if (!super.makeCompetitionStart()) { return false; }

        if (_playerOne.player == null || _playerTwo.player == null) { return false; }
    ...
    protected final void cleanEffects() {
        if (_playerOne.player != null && !_playerOne.defaulted && !_playerOne.disconnected && _playerOne.player.getOlympiadGameId() == _stadiumID) { cleanEffects(_playerOne.player); }

        if (_playerTwo.player != null && !_playerTwo.defaulted && !_playerTwo.disconnected && _playerTwo.player.getOlympiadGameId() == _stadiumID) { cleanEffects(_playerTwo.player); }
    }

    @Override
    protected final void portPlayersBack() {
        if (_playerOne.player != null && !_playerOne.defaulted && !_playerOne.disconnected) { portPlayerBack(_playerOne.player); }
        if (_playerTwo.player != null && !_playerTwo.defaulted && !_playerTwo.disconnected) { portPlayerBack(_playerTwo.player); }
    }

    @Override
    protected final void playersStatusBack() {
        if (_playerOne.player != null && !_playerOne.defaulted && !_playerOne.disconnected && _playerOne.player.getOlympiadGameId() == _stadiumID) { playerStatusBack(_playerOne.player); }

        if (_playerTwo.player != null && !_playerTwo.defaulted && !_playerTwo.disconnected && _playerTwo.player.getOlympiadGameId() == _stadiumID) { playerStatusBack(_playerTwo.player); }
    }
    ...
    protected final void handleDisconnect(L2PcInstance player) {
        if (player.getObjectId() == _playerOne.objectId) { _playerOne.disconnected = true; }
        else if (player.getObjectId() == _playerTwo.objectId) { _playerTwo.disconnected = true; }
    }
    ...
    protected final boolean checkBattleStatus() {
        ...
        if (_playerOne.player == null || _playerOne.disconnected) { return false; }

        if (_playerTwo.player == null || _playerTwo.disconnected) { return false; }
    ...
    protected final boolean haveWinner() {
        if (!checkBattleStatus()) { return true; }

        boolean playerOneLost = true;
        try {
            if (_playerOne.player.getOlympiadGameId() == _stadiumID) { playerOneLost = _playerOne.player.isDead(); }
        }
        catch (Exception e) {
            playerOneLost = true;
        }
        ...
        try {
            if (_playerTwo.player.getOlympiadGameId() == _stadiumID) { playerTwoLost = _playerTwo.player.isDead(); }
        }
        catch (Exception e) {
            playerTwoLost = true;
        }
    ...
    protected void validateWinner(L2OlympiadStadiumZone stadium) {
        ...
        if (pointDiff <= 0) { pointDiff = 1; }
        else if (pointDiff > Config.ALT_OLY_MAX_POINTS) { pointDiff = Config.ALT_OLY_MAX_POINTS; }
            ...
            try {
                if (_playerOne.defaulted) {
                    try {
                        ...
                        LOGGER.error("Exception on validateWinner(): {}", e.getMessage(), e);
                    }
                }
        ...
        try {
            ...
            if (_playerOne.player != null && !_playerOne.player.isDead()) {
                ...
                if (playerOneHp < 0.5) { playerOneHp = 0; }
            }
            ...
            if (_playerTwo.player != null && !_playerTwo.player.isDead()) {
                ...
                if (playerTwoHp < 0.5) { playerTwoHp = 0; }
            }
    ...
    protected final void addDamage(L2PcInstance player, int damage) {
        if (_playerOne.player == null || _playerTwo.player == null) { return; }
        if (player == _playerOne.player) { _damageP1 += damage; }
        else if (player == _playerTwo.player) { _damageP2 += damage; }
    }
    ...
    public boolean checkDefaulted() {
        ...
        if (reason != null) {
            ...
            if (_playerTwo.player != null) { _playerTwo.player.sendPacket(reason); }
        }

        reason = checkDefaulted(_playerTwo.player);
        if (reason != null) {
            _playerTwo.defaulted = true;
            if (_playerOne.player != null) { _playerOne.player.sendPacket(reason); }
        }
