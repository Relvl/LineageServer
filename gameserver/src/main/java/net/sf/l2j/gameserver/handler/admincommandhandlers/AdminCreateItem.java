...
public class AdminCreateItem implements IAdminCommandHandler {
    ...
    public boolean useAdminCommand(String command, L2PcInstance activeChar) {
        ...
        if (command.equals("admin_itemcreate")) {
            ...
            try {
                ...
                for (L2PcInstance player : players) { createItem(activeChar, player, id, count, 0, false); }
            ...
            if (activeChar.getTarget() != null && activeChar.getTarget() instanceof L2PcInstance) { target = (L2PcInstance) activeChar.getTarget(); }
            ...
            if (command.equals("admin_create_item")) {
                try {
                    ...
                    if (st.hasMoreTokens()) {
                        ...
                        if (st.hasMoreTokens()) { radius = Integer.parseInt(st.nextToken()); }
                    }
                ...
                try {
                    ...
                    for (int itemId : set.getSetItemsId()) {
                        if (ItemTable.getInstance().getTemplate(itemId) != null) { target.getInventory().addItem(EItemProcessPurpose.ADMIN, itemId, 1, target, activeChar); }
                    }

                    if (set.getShield() > 0) { target.getInventory().addItem(EItemProcessPurpose.ADMIN, set.getShield(), 1, target, activeChar); }
        ...
        if (radius > 0) {
            ...
            for (L2PcInstance obj : activeChar.getKnownList().getKnownTypeInRadius(L2PcInstance.class, radius)) {
                if (!(obj.equals(activeChar))) {
                    obj.getInventory().addItem(EItemProcessPurpose.ADMIN, id, num, obj, activeChar);
            ...
            if (sendGmMessage) { activeChar.sendMessage(counter + " players rewarded with " + num + " " + template.getName() + " in a " + radius + " radius."); }
        }
        else {
            target.getInventory().addItem(EItemProcessPurpose.ADMIN, id, num, target, activeChar);
            if (activeChar != target) { target.sendMessage("A GM spawned " + num + " " + template.getName() + " in your inventory."); }

            if (sendGmMessage) { activeChar.sendMessage("You have spawned " + num + " " + template.getName() + " (" + id + ") in " + target.getName() + "'s inventory."); }
    ...
    private static int getCoinId(String name) {
        ...
        if (name.equalsIgnoreCase("festivaladena")) { return 6673; }
