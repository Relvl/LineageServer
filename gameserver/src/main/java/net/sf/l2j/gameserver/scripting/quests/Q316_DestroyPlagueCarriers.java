    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getRace() != PlayerRace.Elf) { htmltext = "30155-00.htm"; }
                else if (player.getLevel() < 18) { htmltext = "30155-02.htm"; }
                else { htmltext = "30155-03.htm"; }
                break;

            case QuestState.STATE_STARTED:
                int ratFangs = st.getQuestItemsCount(WERERAT_FANG);
                int varoolFangs = st.getQuestItemsCount(VAROOL_FOULCLAW_FANG);

                if (ratFangs + varoolFangs == 0) { htmltext = "30155-05.htm"; }
                else {
