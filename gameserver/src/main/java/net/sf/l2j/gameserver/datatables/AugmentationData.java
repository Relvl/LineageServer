                ...
                if ("list".equalsIgnoreCase(n.getNodeName())) {
                    for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling()) {
                        if ("augmentation".equalsIgnoreCase(d.getNodeName())) {
                            ...
                            if (type.equalsIgnoreCase("blue")) { ((ArrayList<Integer>) _blueSkills[k]).add(augmentationId); }
                            else if (type.equalsIgnoreCase("purple")) { ((ArrayList<Integer>) _purpleSkills[k]).add(augmentationId); }
                            else { ((ArrayList<Integer>) _redSkills[k]).add(augmentationId); }
                            ...
                            if ("stat".equalsIgnoreCase(d.getNodeName())) {
                                ...
                                for (Node cd = d.getFirstChild(); cd != null; cd = cd.getNextSibling()) {
                                    if ("table".equalsIgnoreCase(cd.getNodeName())) {
                                        ...
                                        while (data.hasMoreTokens()) { array.add(Float.parseFloat(data.nextToken())); }

                                        if (tableName.equalsIgnoreCase("#soloValues")) {
                                            soloValues = new float[array.size()];
                                            int x = 0;
                                            for (float value : array) { soloValues[x++] = value; }
                                        }
                                        else {
                                            combinedValues = new float[array.size()];
                                            int x = 0;
                                            for (float value : array) { combinedValues[x++] = value; }
                                        }
                                    }
                                }
                                // store this stat
    ...
    public L2Augmentation generateRandomAugmentation(int lifeStoneLevel, int lifeStoneGrade) {
        ...
        switch (lifeStoneGrade) {
            case AbstractRefinePacket.GRADE_NONE:
                if (Rnd.get(1, 100) <= Config.AUGMENTATION_NG_SKILL_CHANCE) { generateSkill = true; }
                if (Rnd.get(1, 100) <= Config.AUGMENTATION_NG_GLOW_CHANCE) { generateGlow = true; }
                break;
            case AbstractRefinePacket.GRADE_MID:
                if (Rnd.get(1, 100) <= Config.AUGMENTATION_MID_SKILL_CHANCE) { generateSkill = true; }
                if (Rnd.get(1, 100) <= Config.AUGMENTATION_MID_GLOW_CHANCE) { generateGlow = true; }
                break;
            case AbstractRefinePacket.GRADE_HIGH:
                if (Rnd.get(1, 100) <= Config.AUGMENTATION_HIGH_SKILL_CHANCE) { generateSkill = true; }
                if (Rnd.get(1, 100) <= Config.AUGMENTATION_HIGH_GLOW_CHANCE) { generateGlow = true; }
                break;
            case AbstractRefinePacket.GRADE_TOP:
                if (Rnd.get(1, 100) <= Config.AUGMENTATION_TOP_SKILL_CHANCE) { generateSkill = true; }
                if (Rnd.get(1, 100) <= Config.AUGMENTATION_TOP_GLOW_CHANCE) { generateGlow = true; }
                break;
        }

        if (!generateSkill && Rnd.get(1, 100) <= Config.AUGMENTATION_BASESTAT_CHANCE) { stat34 = Rnd.get(BASESTAT_STR, BASESTAT_MEN); }
        ...
        if (stat34 == 0 && !generateSkill) {
            if (resultColor <= (15 * lifeStoneGrade) + 40) { resultColor = 1; }
            else { resultColor = 0; }
        }
        else {
            if (resultColor <= (10 * lifeStoneGrade) + 5 || stat34 != 0) { resultColor = 3; }
            else if (resultColor <= (10 * lifeStoneGrade) + 10) { resultColor = 1; }
            else { resultColor = 2; }
        }
        ...
        if (stat34 == 0) {
            ...
            if (generateGlow && lifeStoneGrade >= 2) { offset = (lifeStoneLevel * STAT_SUBBLOCKSIZE) + (temp - 2) * STAT_BLOCKSIZE + lifeStoneGrade * (10 * STAT_SUBBLOCKSIZE) + 1; }
            else { offset = (lifeStoneLevel * STAT_SUBBLOCKSIZE) + (temp - 2) * STAT_BLOCKSIZE + Rnd.get(0, 1) * (10 * STAT_SUBBLOCKSIZE) + 1; }
        }
        else {
            if (!generateGlow) { offset = (lifeStoneLevel * STAT_SUBBLOCKSIZE) + Rnd.get(0, 1) * STAT_BLOCKSIZE + 1; }
            else { offset = (lifeStoneLevel * STAT_SUBBLOCKSIZE) + Rnd.get(0, 1) * STAT_BLOCKSIZE + (lifeStoneGrade + resultColor) / 2 * (10 * STAT_SUBBLOCKSIZE) + 1; }
        }
