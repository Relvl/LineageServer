    ...
    protected void runImpl() {
        ...
        if ((activeChar.getClanPrivileges() & L2Clan.CP_CL_REGISTER_CREST) == L2Clan.CP_CL_REGISTER_CREST) {
            if (_length == 0 || _data == null) {
                if (clan.getCrestLargeId() == 0) { return; }
                ...
                if (!CrestCache.getInstance().saveCrest(CrestType.PLEDGE_LARGE, crestLargeId, _data)) {
                    _log.warn("Error saving large crest for clan {} [{}]", clan.getName(), clan.getClanId());
                    return;
                }
        ...
        if (updated && crestLargeId != -1) { clan.changeLargeCrest(crestLargeId); }
    }
}