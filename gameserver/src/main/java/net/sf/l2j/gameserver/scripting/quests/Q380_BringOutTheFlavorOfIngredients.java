...
public class Q380_BringOutTheFlavorOfIngredients extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                if (cond == 1) { htmltext = "30069-06.htm"; }
                else if (cond == 2) {
                    ...
                    if (Rnd.get(100) < 55) { htmltext = "30069-11.htm"; }
                    else {
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            case DIRE_WOLF:
                if (st.dropItems(RITRON_FRUIT, 1, 4, 100000)) { if (st.getQuestItemsCount(MOON_FACE_FLOWER) == 20 && st.getQuestItemsCount(LEECH_FLUIDS) == 10) { st.set("cond", "2"); } }
                break;

            case KADIF_WEREWOLF:
                if (st.dropItems(MOON_FACE_FLOWER, 1, 20, 500000)) { if (st.getQuestItemsCount(RITRON_FRUIT) == 4 && st.getQuestItemsCount(LEECH_FLUIDS) == 10) { st.set("cond", "2"); } }
                break;

            case GIANT_MIST_LEECH:
                if (st.dropItems(LEECH_FLUIDS, 1, 10, 500000)) { if (st.getQuestItemsCount(RITRON_FRUIT) == 4 && st.getQuestItemsCount(MOON_FACE_FLOWER) == 20) { st.set("cond", "2"); } }
                break;
        }
