...
public class Q233_TestOfTheWarSpirit extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        // ORIM
        else if (event.equalsIgnoreCase("30630-04.htm")) { st.giveItems(ORIM_CONTRACT, 1); }
        // RACOY
        else if (event.equalsIgnoreCase("30507-02.htm")) { st.giveItems(RACOY_TOTEM, 1); }
        // VIVYAN
        else if (event.equalsIgnoreCase("30030-04.htm")) { st.giveItems(VIVYAN_LETTER, 1); }
        // PEKIRON
        else if (event.equalsIgnoreCase("30682-02.htm")) { st.giveItems(PEKIRON_TOTEM, 1); }
        // MANAKIA
        else if (event.equalsIgnoreCase("30515-02.htm")) { st.giveItems(MANAKIA_TOTEM, 1); }
        // ANCESTOR MARTANKUS
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getClassId() == ClassId.orcShaman) { htmltext = (player.getLevel() < 39) ? "30510-03.htm" : "30510-04.htm"; }
                else { htmltext = (player.getRace() == PlayerRace.Orc) ? "30510-02.htm" : "30510-01.htm"; }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case SOMAK:
                        if (cond == 1) { htmltext = "30510-06.htm"; }
                        else if (cond == 2) {
                        ...
                        else if (cond == 3) { htmltext = "30510-08.htm"; }
                        else if (cond == 4) {
                        ...
                        else if (cond == 5) { htmltext = "30510-10.htm"; }
                        break;

                    case ORIM:
                        if (cond == 1 && !st.hasQuestItems(BRAKI_REMAINS_1)) {
                            if (!st.hasQuestItems(ORIM_CONTRACT)) { htmltext = "30630-01.htm"; }
                            ...
                            else { htmltext = "30630-05.htm"; }
                        }
                        else { htmltext = "30630-07.htm"; }
                        break;

                    case RACOY:
                        if (cond == 1 && !st.hasQuestItems(KIRUNA_REMAINS_1)) {
                            if (!st.hasQuestItems(RACOY_TOTEM)) { htmltext = "30507-01.htm"; }
                            else if (st.hasQuestItems(VIVYAN_LETTER)) { htmltext = "30507-04.htm"; }
                                ...
                                else { htmltext = "30507-05.htm"; }
                            }
                            else { htmltext = "30507-03.htm"; }
                        }
                        else { htmltext = "30507-07.htm"; }
                        break;

                    case VIVYAN:
                        if (cond == 1 && st.hasQuestItems(RACOY_TOTEM)) {
                            if (st.hasQuestItems(VIVYAN_LETTER)) { htmltext = "30030-05.htm"; }
                            else if (st.hasQuestItems(INSECT_DIAGRAM_BOOK)) { htmltext = "30030-06.htm"; }
                            else { htmltext = "30030-01.htm"; }
                        }
                        else { htmltext = "30030-07.htm"; }
                        break;
                    ...
                    case SARIEN:
                        if (cond == 1 && st.hasQuestItems(RACOY_TOTEM)) {
                            ...
                            else if (st.hasQuestItems(INSECT_DIAGRAM_BOOK)) { htmltext = "30436-02.htm"; }
                        }
                        else { htmltext = "30436-03.htm"; }
                        break;

                    case PEKIRON:
                        if (cond == 1 && !st.hasQuestItems(TONAR_REMAINS_1)) {
                            if (!st.hasQuestItems(PEKIRON_TOTEM)) { htmltext = "30682-01.htm"; }
                            ...
                            else { htmltext = "30682-03.htm"; }
                        }
                        else { htmltext = "30682-05.htm"; }
                        break;

                    case MANAKIA:
                        if (cond == 1 && !st.hasQuestItems(HERMODT_REMAINS_1)) {
                            if (!st.hasQuestItems(MANAKIA_TOTEM)) { htmltext = "30515-01.htm"; }
                            ...
                            else { htmltext = "30515-03.htm"; }
                        }
                        else { htmltext = "30515-05.htm"; }
                        break;

                    case ANCESTOR_MARTANKUS:
                        if (cond == 5) { htmltext = "30649-01.htm"; }
                        break;
                }
                break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            case PORTA:
                if (st.hasQuestItems(ORIM_CONTRACT)) { st.dropItemsAlways(PORTA_EYE, 1, 10); }
                break;

            case EXCURO:
                if (st.hasQuestItems(ORIM_CONTRACT)) { st.dropItemsAlways(EXCURO_SCALE, 1, 10); }
                break;

            case MORDEO:
                if (st.hasQuestItems(ORIM_CONTRACT)) { st.dropItemsAlways(MORDEO_TALON, 1, 10); }
                break;
            ...
            case NOBLE_ANT_LEADER:
                if (st.hasQuestItems(INSECT_DIAGRAM_BOOK)) {
                    ...
                    if (rndAnt > 70) {
                        if (st.hasQuestItems(KIRUNA_THIGHBONE)) { st.dropItemsAlways(KIRUNA_ARMBONE, 1, 1); }
                        else { st.dropItemsAlways(KIRUNA_THIGHBONE, 1, 1); }
                    }
                    else if (rndAnt > 40) {
                        if (st.hasQuestItems(KIRUNA_SPINE)) { st.dropItemsAlways(KIRUNA_RIBBONE, 1, 1); }
                        else { st.dropItemsAlways(KIRUNA_SPINE, 1, 1); }
                    }
                    else if (rndAnt > 10) { st.dropItemsAlways(KIRUNA_SKULL, 1, 1); }
                }
                break;
            ...
            case LETO_LIZARDMAN_OVERLORD:
                if (st.hasQuestItems(PEKIRON_TOTEM) && Rnd.nextBoolean()) {
                    if (!st.hasQuestItems(TONAR_SKULL)) { st.dropItemsAlways(TONAR_SKULL, 1, 1); }
                    else if (!st.hasQuestItems(TONAR_RIBBONE)) { st.dropItemsAlways(TONAR_RIBBONE, 1, 1); }
                    else if (!st.hasQuestItems(TONAR_SPINE)) { st.dropItemsAlways(TONAR_SPINE, 1, 1); }
                    else if (!st.hasQuestItems(TONAR_ARMBONE)) { st.dropItemsAlways(TONAR_ARMBONE, 1, 1); }
                    else { st.dropItemsAlways(TONAR_THIGHBONE, 1, 1); }
                }
                break;

            case MEDUSA:
                if (st.hasQuestItems(MANAKIA_TOTEM) && Rnd.nextBoolean()) {
                    if (!st.hasQuestItems(HERMODT_RIBBONE)) { st.dropItemsAlways(HERMODT_RIBBONE, 1, 1); }
                    else if (!st.hasQuestItems(HERMODT_SPINE)) { st.dropItemsAlways(HERMODT_SPINE, 1, 1); }
                    else if (!st.hasQuestItems(HERMODT_ARMBONE)) { st.dropItemsAlways(HERMODT_ARMBONE, 1, 1); }
                    else { st.dropItemsAlways(HERMODT_THIGHBONE, 1, 1); }
                }
                break;

            case STENOA_GORGON_QUEEN:
                if (st.hasQuestItems(MANAKIA_TOTEM)) { st.dropItemsAlways(HERMODT_SKULL, 1, 1); }
                break;

            case TAMLIN_ORC:
            case TAMLIN_ORC_ARCHER:
                if (st.hasQuestItems(VENDETTA_TOTEM) && st.dropItems(TAMLIN_ORC_HEAD, 1, 13, 500000)) { st.set("cond", "4"); }
                break;
        }
