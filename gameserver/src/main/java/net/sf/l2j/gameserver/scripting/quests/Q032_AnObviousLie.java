...
public class Q032_AnObviousLie extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30120-1.htm")) {
            ...
            if (st.getQuestItemsCount(SPIRIT_ORE) < 500) { htmltext = "30094-5.htm"; }
            else {
                st.set("cond", "6");
                st.playSound(ESound.ItemSound_quest_middle);
        ...
        else if (event.equalsIgnoreCase("30094-13.htm")) { st.playSound(ESound.ItemSound_quest_middle); }
        else if (event.equalsIgnoreCase("cat")) {
            if (st.getQuestItemsCount(THREAD) < 1000 || st.getQuestItemsCount(SUEDE) < 500) { htmltext = "30094-11.htm"; }
            else {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    ...
                    case GENTLER:
                        if (cond == 1) { htmltext = "30094-0.htm"; }
                        else if (cond == 2 || cond == 3) { htmltext = "30094-2.htm"; }
                        else if (cond == 4) { htmltext = "30094-3.htm"; }
                        else if (cond == 5) { htmltext = (st.getQuestItemsCount(SPIRIT_ORE) < 500) ? "30094-5.htm" : "30094-6.htm"; }
                        else if (cond == 6) { htmltext = "30094-8.htm"; }
                        else if (cond == 7) { htmltext = "30094-9.htm"; }
                        else if (cond == 8) { htmltext = (st.getQuestItemsCount(THREAD) < 1000 || st.getQuestItemsCount(SUEDE) < 500) ? "30094-11.htm" : "30094-12.htm"; }
                        break;

                    case MIKI_THE_CAT:
                        if (cond == 2) { htmltext = "31706-0.htm"; }
                        else if (cond > 2 && cond < 6) { htmltext = "31706-2.htm"; }
                        else if (cond == 6) { htmltext = "31706-3.htm"; }
                        else if (cond > 6) { htmltext = "31706-5.htm"; }
                        break;
                }
                break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropItemsAlways(MEDICINAL_HERB, 1, 20)) { st.set("cond", "4"); }
