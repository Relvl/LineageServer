...
public class AdminChangeAccessLevel implements IAdminCommandHandler {
    ...
    public boolean useAdminCommand(String command, L2PcInstance activeChar) {
        ...
        if (parts.length == 2) {
            try {
                ...
                if (activeChar.getTarget() instanceof L2PcInstance) { onLineChange(activeChar, (L2PcInstance) activeChar.getTarget(), lvl); }
            ...
            if (player != null) { onLineChange(activeChar, player, lvl); }
            else {
                try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
                    ...
                    statement.close();
                    if (count == 0) { activeChar.sendMessage("Character not found or access level unaltered."); }
                    else { activeChar.sendMessage("Character's access level is now set to " + lvl); }
                }
                catch (Exception e) {
                }
            }
    ...
    private static void onLineChange(L2PcInstance activeChar, L2PcInstance player, int lvl) {
        ...
        if (lvl > 0) { player.sendMessage("Your access level has been changed to " + lvl); }
        else {
