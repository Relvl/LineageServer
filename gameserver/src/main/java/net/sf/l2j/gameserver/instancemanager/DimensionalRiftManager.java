...
public class DimensionalRiftManager {
    ...
    public void reload() {
        for (byte b : _rooms.keySet()) {
            for (byte i : _rooms.get(b).keySet()) { _rooms.get(b).get(i).getSpawns().clear(); }
    ...
    public boolean checkIfInRiftZone(int x, int y, int z, boolean ignorePeaceZone) {
        if (ignorePeaceZone) { return _rooms.get((byte) 0).get((byte) 1).checkIfInZone(x, y, z); }
    ...
    public synchronized void start(L2PcInstance player, byte type, L2Npc npc) {
        ...
        if (party.isInDimensionalRift()) { return; }
        ...
        do {
            ...
            while (room == 9) { room = emptyRooms.get(Rnd.get(1, emptyRooms.size()) - 1); }
        }
        // Find empty room
    ...
    public List<Byte> getFreeRooms(byte type) {
        ...
        for (DimensionalRiftRoom room : _rooms.get(type).values()) {
            if (!room.isPartyInside()) { list.add(room._room); }
        }
        return list;
    }
