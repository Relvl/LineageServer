        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getLevel() >= 60) {
                    ...
                    if (fwear != null && fwear.getInt("cond") == 7) { htmltext = "30838-0.htm"; }
                    else { htmltext = "30838-0a.htm"; }
                }
                else { htmltext = "30838-0b.htm"; }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case WOODLEY:
                        if (cond == 1) { htmltext = "30838-1.htm"; }
                        else if (cond == 2) { htmltext = "30838-2.htm"; }
                        else if (cond == 3) {
                            if (st.getQuestItemsCount(LEATHER) >= 200 && st.getQuestItemsCount(THREAD) >= 600 && st.getQuestItemsCount(ItemConst.ADENA_ID) >= 200000) { htmltext = "30838-4.htm"; }
                            else { htmltext = "30838-4a.htm"; }
                        }
                        else if (cond == 4) { htmltext = "30838-5a.htm"; }
                        else if (cond == 5) { htmltext = "30838-6.htm"; }
                        break;

                    case LEIKAR:
                        if (cond == 1) { htmltext = "31520-0.htm"; }
                        else if (cond > 1) { htmltext = "31520-1a.htm"; }
                        break;

                    case IAN:
                        if (cond == 4) { htmltext = "30164-0.htm"; }
                        else if (cond == 5) { htmltext = "30164-2.htm"; }
                        break;
                }
                break;
