...
public class MovieMakerManager {
    ...
    public void mainHtm(L2PcInstance player) {
        ...
        else { html.setFile("data/html/admin/movie/main_empty.htm"); }
    ...
    public void updateSequence(L2PcInstance player, int seqId, int objid, int dist, int yaw, int pitch, int time, int duration, int turn, int rise, int screen) {
        ...
        else { player.sendMessage("This sequence doesn't exist."); }

        mainHtm(player);
    }

    public void deleteSequence(int id, L2PcInstance player) {
        if (_sequence.containsKey(id)) { _sequence.remove(id); }
        else { player.sendMessage("This sequence id doesn't exist."); }

        mainHtm(player);
    }

    public void playMovie(int broadcast, L2PcInstance player) {
        if (!_sequence.isEmpty()) { ThreadPoolManager.getInstance().schedule(new Play(1, broadcast, player), 500); }
        else {
    ...
    private class Play implements Runnable {
        ...
        public void run() {
            if (_sequence.containsKey(_id)) {
                ...
                if (_broad == 1) { _player.broadcastPacket(new SpecialCamera(sec._objid, sec._dist, sec._yaw, sec._pitch, sec._time, sec._duration, sec._turn, sec._rise, sec._widescreen, 0)); }
                else { _player.sendPacket(new SpecialCamera(sec._objid, sec._dist, sec._yaw, sec._pitch, sec._time, sec._duration, sec._turn, sec._rise, sec._widescreen, 0)); }
