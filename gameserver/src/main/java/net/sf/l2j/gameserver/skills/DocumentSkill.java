...
public final class DocumentSkill extends DocumentBase {
    ...
    protected String getTableValue(String name) {
        try {
            ...
            LOGGER.error("Error in table: {} of Skill Id {}", name, _currentSkill.id, e);
            return "";
        }
    }
    ...
    protected void parseSkill(Node n) {
        ...
        if (attrs.getNamedItem("enchantLevels1") != null) { enchantLevels1 = Integer.parseInt(attrs.getNamedItem("enchantLevels1").getNodeValue()); }
        if (attrs.getNamedItem("enchantLevels2") != null) { enchantLevels2 = Integer.parseInt(attrs.getNamedItem("enchantLevels2").getNodeValue()); }
        ...
        if (_currentSkill.sets.length != lastLvl) { throw new RuntimeException("Skill id=" + skillId + " number of levels missmatch, " + lastLvl + " levels expected"); }
        ...
        for (int i = 1; i <= lastLvl; i++) {
            for (n = first; n != null; n = n.getNextSibling()) {
                if ("set".equalsIgnoreCase(n.getNodeName())) { parseBeanSet(n, _currentSkill.sets[i - 1], i); }
            }
        }
        for (int i = 0; i < enchantLevels1; i++) {
            ...
            for (n = first; n != null; n = n.getNextSibling()) {
                if ("set".equalsIgnoreCase(n.getNodeName())) { parseBeanSet(n, _currentSkill.enchsets1[i], _currentSkill.sets.length); }
            }

            for (n = first; n != null; n = n.getNextSibling()) {
                if ("enchant1".equalsIgnoreCase(n.getNodeName())) { parseBeanSet(n, _currentSkill.enchsets1[i], i + 1); }
            }
        }

        if (_currentSkill.enchsets1.length != enchantLevels1) { throw new RuntimeException("Skill id=" + skillId + " number of levels missmatch, " + enchantLevels1 + " levels expected"); }
        ...
        for (int i = 0; i < enchantLevels2; i++) {
            ...
            for (n = first; n != null; n = n.getNextSibling()) {
                if ("set".equalsIgnoreCase(n.getNodeName())) { parseBeanSet(n, _currentSkill.enchsets2[i], _currentSkill.sets.length); }
            }

            for (n = first; n != null; n = n.getNextSibling()) {
                if ("enchant2".equalsIgnoreCase(n.getNodeName())) { parseBeanSet(n, _currentSkill.enchsets2[i], i + 1); }
            }
        }

        if (_currentSkill.enchsets2.length != enchantLevels2) { throw new RuntimeException("Skill id=" + skillId + " number of levels missmatch, " + enchantLevels2 + " levels expected"); }
            ...
            for (n = first; n != null; n = n.getNextSibling()) {
                if ("cond".equalsIgnoreCase(n.getNodeName())) {
                    ...
                    if (condition != null && msg != null) { condition.setMessage(msg.getNodeValue()); }
                        ...
                        if (addName != null && Integer.decode(getValue(msgId.getNodeValue(), null)) > 0) { condition.addName(); }
                    }
    ...
    private void makeSkills() {
        ...
        for (int i = 0; i < _currentSkill.sets.length; i++) {
            try {
                ...
                LOGGER.error("Skill id={}level{}", _currentSkill.sets[i].getEnum("skillType", L2SkillType.class).makeSkill(_currentSkill.sets[i]).getId(), _currentSkill.sets[i].getEnum("skillType", L2SkillType.class).makeSkill(_currentSkill.sets[i]).getLevel(), e);
            }
        }
        int _count = count;
        for (int i = 0; i < _currentSkill.enchsets1.length; i++) {
            try {
                ...
                LOGGER.error("Skill id={} level={}", _currentSkill.enchsets1[i].getEnum("skillType", L2SkillType.class).makeSkill(_currentSkill.enchsets1[i]).getId(), _currentSkill.enchsets1[i].getEnum("skillType", L2SkillType.class).makeSkill(_currentSkill.enchsets1[i]).getLevel(), e);
            }
        }
        _count = count;
        for (int i = 0; i < _currentSkill.enchsets2.length; i++) {
            try {
                ...
                LOGGER.error("Skill id={} level={}", _currentSkill.enchsets2[i].getEnum("skillType", L2SkillType.class).makeSkill(_currentSkill.enchsets2[i]).getId(), _currentSkill.enchsets2[i].getEnum("skillType", L2SkillType.class).makeSkill(_currentSkill.enchsets2[i]).getLevel(), e);
            }
        }
    }
}
