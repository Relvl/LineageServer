...
public class Q401_PathToAWarrior extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30010-05.htm")) {
            if (player.getClassId() != ClassId.fighter) { htmltext = (player.getClassId() == ClassId.warrior) ? "30010-03.htm" : "30010-02b.htm"; }
            else if (player.getLevel() < 19) { htmltext = "30010-02.htm"; }
            else if (st.hasQuestItems(MEDALLION_OF_WARRIOR)) { htmltext = "30010-04.htm"; }
        }
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case AURON:
                        ...
                        else if (cond == 2 || cond == 3) { htmltext = "30010-08.htm"; }
                        else if (cond == 4) { htmltext = "30010-09.htm"; }
                        else if (cond == 5) { htmltext = "30010-12.htm"; }
                        else if (cond == 6) {
                    ...
                    case SIMPLON:
                        if (cond == 1) { htmltext = "30253-01.htm"; }
                        else if (cond == 2) {
                            if (!st.hasQuestItems(RUSTED_BRONZE_SWORD_1)) { htmltext = "30253-03.htm"; }
                            else if (st.getQuestItemsCount(RUSTED_BRONZE_SWORD_1) <= 9) { htmltext = "30253-03b.htm"; }
                        }
                        else if (cond == 3) {
                        ...
                        else if (cond == 4) { htmltext = "30253-05.htm"; }
                        break;
                }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            case 20035:
            case 20042:
                if (st.getInt("cond") == 2 && st.dropItems(RUSTED_BRONZE_SWORD_1, 1, 10, 400000)) { st.set("cond", "3"); }
                break;

            case 20038:
            case 20043:
                if (st.getInt("cond") == 5 && (st.getItemEquipped(EPaperdollSlot.PAPERDOLL_RHAND) == RUSTED_BRONZE_SWORD_3)) {
