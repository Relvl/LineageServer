    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case ALEXIS:
                        if (cond == 1) { htmltext = "30842-2.htm"; }
                        break;

                    case LEIKAR:
                        if (cond == 1) { htmltext = "31520-0.htm"; }
                        else if (cond == 2) { htmltext = "31520-1a.htm"; }
                        else if (cond == 5 || cond == 6) {
                            if (st.hasQuestItems(MYSTERIOUS_CLOTH, JEWEL_BOX, SEWING_KIT)) { htmltext = "31520-4.htm"; }
                            else if (st.hasQuestItems(BOX_OF_COOKIES)) { htmltext = "31520-2.htm"; }
                            else { htmltext = "31520-3a.htm"; }
                        }
                        else if (cond == 7) { htmltext = (st.hasQuestItems(DRESS_SHOES_BOX)) ? "31520-6.htm" : "31520-5a.htm"; }
                        break;

                    case JEREMY:
                        if (st.hasQuestItems(SIGNET_RING)) { htmltext = "31521-0.htm"; }
                        else if (cond == 3) { htmltext = "31521-1a.htm"; }
                        else if (cond == 4) { htmltext = "31521-2.htm"; }
                        else if (cond > 4) { htmltext = "31521-3a.htm"; }
                        break;

                    case MIST:
                        if (cond == 3) { htmltext = "31627-0.htm"; }
                        else if (cond > 3) { htmltext = "31627-2.htm"; }
                        break;
                }
                break;
