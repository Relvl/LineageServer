...
public class Auction {
    ...
    private void loadBid() {
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.error("Exception: Auction.loadBid(): {}", e.getMessage(), e);
        }
    }
    ...
    public synchronized void setBid(L2PcInstance bidder, int bid) {
        ...
        if (getHighestBidderName().equals(bidder.getClan().getLeaderName())) { requiredAdena = bid - getHighestBidderMaxBid(); }
        ...
        if ((bid < getStartingBid()) || (bid <= getHighestBidderMaxBid())) { bidder.sendPacket(SystemMessageId.BID_PRICE_MUST_BE_HIGHER); }
    }
    ...
    private void updateInDB(L2PcInstance bidder, int bid) {
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            if (_bidders.get(_highestBidderId) == null) { _bidders.put(_highestBidderId, new Bidder(_highestBidderName, bidder.getClan().getName(), bid, Calendar.getInstance().getTimeInMillis())); }
            else {
            ...
            LOGGER.error("Exception: Auction.updateInDB(L2PcInstance bidder, int bid): {}", e.getMessage(), e);
        }
    }
