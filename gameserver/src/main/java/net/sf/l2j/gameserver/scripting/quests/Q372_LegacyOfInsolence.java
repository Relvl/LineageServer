...
public class Q372_LegacyOfInsolence extends Quest {
    ...
    private static String checkAndRewardItems(QuestState st, int itemType, int rewardType, int npcId) {
        ...
        // Check set of items.
        for (int item = itemsToCheck[0]; item <= itemsToCheck[1]; item++) { if (!st.hasQuestItems(item)) { return npcId + ((npcId == WALDERAL) ? "-07a.htm" : "-01.htm"); } }

        // Remove set of items.
        for (int item = itemsToCheck[0]; item <= itemsToCheck[1]; item++) { st.takeItems(item, 1); }
