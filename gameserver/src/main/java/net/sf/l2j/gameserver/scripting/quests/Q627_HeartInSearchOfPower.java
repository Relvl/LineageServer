...
public class Q627_HeartInSearchOfPower extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("31518-01.htm")) {
            ...
            if (st.getQuestItemsCount(GEM_OF_SAINTS) == 1) {
                ...
                if (REWARDS.get(event)[0] > 0) { st.giveItems(REWARDS.get(event)[0], REWARDS.get(event)[1]); }
                st.rewardItems(ItemConst.ADENA_ID, REWARDS.get(event)[2]);

                st.playSound(ESound.ItemSound_quest_finish);
                st.exitQuest(true);
            }
            else { htmltext = "31518-7.htm"; }
        }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case NECROMANCER:
                        if (cond == 1) { htmltext = "31518-01a.htm"; }
                        else if (cond == 2) { htmltext = "31518-02.htm"; }
                        else if (cond == 3) { htmltext = "31518-04.htm"; }
                        else if (cond == 4) { htmltext = "31518-05.htm"; }
                        break;

                    case ENFEUX:
                        if (cond == 3) { htmltext = "31519-00.htm"; }
                        else if (cond == 4) { htmltext = "31519-02.htm"; }
                        break;
                }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropItems(BEAD_OF_OBEDIENCE, 1, 300, CHANCES.get(npc.getNpcId()))) { st.set("cond", "2"); }
