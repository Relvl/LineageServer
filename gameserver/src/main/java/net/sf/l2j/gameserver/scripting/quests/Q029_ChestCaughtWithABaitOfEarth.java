    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getLevel() < 48) { htmltext = "31574-02.htm"; }
                else {
                    QuestState st2 = player.getQuestState("Q052_WilliesSpecialBait");
                    if (st2 != null && st2.isCompleted()) { htmltext = "31574-01.htm"; }
                    else { htmltext = "31574-03.htm"; }
                }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case WILLIE:
                        if (cond == 1) { htmltext = (!st.hasQuestItems(SMALL_PURPLE_TREASURE_CHEST)) ? "31574-06.htm" : "31574-05.htm"; }
                        else if (cond == 2) { htmltext = "31574-09.htm"; }
                        break;

                    case ANABEL:
                        if (cond == 2) { htmltext = "30909-01.htm"; }
                        break;
                }
                break;
