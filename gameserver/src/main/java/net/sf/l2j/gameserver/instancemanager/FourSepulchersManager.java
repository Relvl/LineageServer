...
public class FourSepulchersManager {
    ...
    protected void timeCalculator() {
        ...
        if (tmp.get(Calendar.MINUTE) < _newCycleMin) { tmp.set(Calendar.HOUR, Calendar.getInstance().get(Calendar.HOUR) - 1); }
    ...
    public void clean() {
        ...
        if (!_archonSpawned.isEmpty()) {
            for (int npcId : _archonSpawned.keySet()) { _archonSpawned.put(npcId, false); }
        }
    }
    ...
    protected void spawnManagers() {
        ...
        for (L2Spawn spawn; i <= 31924; i++) {
            ...
            try {
                ...
                LOGGER.info("FourSepulchersManager: spawned {}", spawn.getTemplate().getName());
            }
            catch (Exception e) {
                LOGGER.error("Error while spawning managers: {}", e.getMessage(), e);
            }
        }
    }
    ...
    private void loadMysteriousBox() {
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            rset.close();
            statement.close();
            LOGGER.info("FourSepulchersManager: loaded {} Mysterious-Box spawns.", _mysteriousBoxSpawns.size());
        }
        catch (Exception e) {
            // problem with initializing spawn, go to next one
            LOGGER.error("FourSepulchersManager.LoadMysteriousBox: Spawn could not be initialized: {}", e.getMessage(), e);
        }
    }
        ...
        for (Entry<Integer, Integer> keyNpc : _keyBoxNpc.entrySet()) {
            try {
                ...
                if (template != null) {
                    ...
                    LOGGER.warn("FourSepulchersManager.InitKeyBoxSpawns: Data missing in NPC table for ID: {}.", keyNpc.getValue());
                }
            }
            catch (Exception e) {
                LOGGER.error("FourSepulchersManager.InitKeyBoxSpawns: Spawn could not be initialized: {}", e.getMessage(), e);
            }
        }
    }
    ...
    private void loadPhysicalMonsters() {
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.info("FourSepulchersManager: loaded {} Physical type monsters spawns.", loaded);
        }
        catch (Exception e) {
            // problem with initializing spawn, go to next one
            LOGGER.error("FourSepulchersManager.LoadPhysicalMonsters: Spawn could not be initialized: {}", e.getMessage(), e);
        }
    }
    ...
    private void loadMagicalMonsters() {
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.info("FourSepulchersManager: loaded {} Magical type monsters spawns.", loaded);
        }
        catch (Exception e) {
            // problem with initializing spawn, go to next one
            LOGGER.error("FourSepulchersManager.LoadMagicalMonsters: Spawn could not be initialized: {}", e.getMessage(), e);
        }
    }
    ...
    private void loadDukeMonsters() {
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.info("FourSepulchersManager: loaded {} Church of duke monsters spawns.", loaded);
        }
        catch (Exception e) {
            // problem with initializing spawn, go to next one
            LOGGER.error("FourSepulchersManager.LoadDukeMonsters: Spawn could not be initialized: {}", e.getMessage(), e);
        }
    }
    ...
    private void loadEmperorsGraveMonsters() {
        ...
        try (Connection con = L2DatabaseFactoryOld.getInstance().getConnection()) {
            ...
            LOGGER.info("FourSepulchersManager: loaded {} Emperor's grave NPC spawns.", loaded);
        }
        catch (Exception e) {
            // problem with initializing spawn, go to next one
            LOGGER.error("FourSepulchersManager.LoadEmperorsGraveMonsters: Spawn could not be initialized: {}", e.getMessage(), e);
        }
    }
        ...
        for (int i = 0; i <= 3; i++) {
            ...
            if (template != null) {
                try {
                    ...
                    LOGGER.error("Error on InitLocationShadowSpawns", e);
                }
            }
            else {
        ...
        for (Entry<Integer, Integer> victimNpc : _victim.entrySet()) {
            try {
                ...
                if (template != null) {
                    ...
                    LOGGER.warn("FourSepulchersManager.InitExecutionerSpawns: Data missing in NPC table for ID: {}.", victimNpc.getValue());
                }
            }
            catch (Exception e) {
                LOGGER.error("FourSepulchersManager.InitExecutionerSpawns: Spawn could not be initialized: {}", e.getMessage(), e);
            }
        }
    }
    ...
    public synchronized void tryEntry(L2Npc npc, L2PcInstance player) {
        ...
        switch (npcId) {
            ...
            default:
                if (!player.isGM()) {
                    LOGGER.warn("{} tried to cheat in four sepulchers.", player.getName());
    ...
    private void entry(int npcId, L2PcInstance player) {
        ...
        if (Config.FS_PARTY_MEMBER_COUNT > 1) {
            ...
            for (L2PcInstance mem : player.getParty().getPartyMembers()) {
                if (!mem.isDead() && Util.checkIfInRange(700, player, mem, true)) { members.add(mem); }
            }
            ...
            for (L2PcInstance mem : members) {
                ...
                mem.getInventory().destroyItemByItemId(EItemProcessPurpose.QUEST, ENTRANCE_PASS, 1, mem, null, true);
                if (mem.getInventory().getItemByItemId(ANTIQUE_BROOCH) == null) { mem.addItem(EItemProcessPurpose.QUEST, USED_PASS, 1, mem, true); }

                L2ItemInstance hallsKey = mem.getInventory().getItemByItemId(CHAPEL_KEY);
                if (hallsKey != null) { mem.getInventory().destroyItemByItemId(EItemProcessPurpose.QUEST, CHAPEL_KEY, hallsKey.getCount(), mem, null, true); }
            }
        ...
        if (Config.FS_PARTY_MEMBER_COUNT <= 1 && player.isInParty()) {
            ...
            for (L2PcInstance mem : members) {
                ...
                L2ItemInstance hallsKey = mem.getInventory().getItemByItemId(CHAPEL_KEY);
                if (hallsKey != null) {
                    mem.getInventory().destroyItemByItemId(EItemProcessPurpose.QUEST, CHAPEL_KEY, hallsKey.getCount(), mem, null, true);
                }
            }
            ...
            player.getInventory().destroyItemByItemId(EItemProcessPurpose.QUEST, ENTRANCE_PASS, 1, player, null, true);
            if (player.getInventory().getItemByItemId(ANTIQUE_BROOCH) == null) { player.addItem(EItemProcessPurpose.QUEST, USED_PASS, 1, player, true); }

            L2ItemInstance hallsKey = player.getInventory().getItemByItemId(CHAPEL_KEY);
            if (hallsKey != null) {
                player.getInventory().destroyItemByItemId(EItemProcessPurpose.QUEST, CHAPEL_KEY, hallsKey.getCount(), player, null, true);
            }
    ...
    public void spawnMonster(int npcId) {
        ...
        if (Rnd.nextBoolean()) { monsterList = _physicalMonsters.get(npcId); }
        else { monsterList = _magicalMonsters.get(npcId); }
        ...
        if (monsterList != null) {
            ...
            for (L2Spawn spawn : monsterList) {
                if (spawnedKeyBoxMob) { spawnKeyBoxMob = false; }
                else {
                    switch (npcId) {
                        case 31469:
                        case 31474:
                        case 31479:
                        case 31484:
                            if (Rnd.get(48) == 0) { spawnKeyBoxMob = true; }
                            break;
                ...
                if (spawnKeyBoxMob) {
                    try {
                        ...
                        LOGGER.error("FourSepulchersManager.SpawnMonster: Spawn could not be initialized: {}", e.getMessage(), e);
                    }
    ...
    public void deleteAllMobs() {
        for (L2Npc mob : _allMobs) {
            ...
            try {
                if (mob.getSpawn() != null) { mob.getSpawn().stopRespawn(); }

                mob.deleteMe();
            }
            catch (Exception e) {
                LOGGER.error("FourSepulchersManager: Failed deleting mob.", e);
            }
        }
        _allMobs.clear();
    }
        ...
        for (int doorId : _hallGateKeepers.values()) {
            try {
                ...
                if (door != null) { door.closeMe(); }
                    ...
                    LOGGER.warn("FourSepulchersManager: Attempted to close undefined door. doorId: {}", doorId);
                }
            }
            catch (Exception e) {
                LOGGER.error("FourSepulchersManager: Failed closing door", e);
            }
        }
    }
    ...
    public void managerSay(byte min) {
        ...
        if (_inAttackTime) {
            ...
            if (min == 90) { msg = "Game over. The teleport will appear momentarily"; }
            ...
            for (L2Spawn temp : _managers) {
                ...
                if (!_hallInUse.get(temp.getNpcId()).booleanValue()) { continue; }
    ...
    protected class ManagerSay implements Runnable {
        ...
        public void run() {
            ...
            else if (_inEntryTime) { managerSay((byte) 0); }
        }
    }
    ...
    protected class ChangeEntryTime implements Runnable {
        ...
        public void run() {
            ...
            if (_firstTimeRun) { interval = _entryTimeEnd - Calendar.getInstance().getTimeInMillis(); }
            else {
    ...
    protected class ChangeAttackTime implements Runnable {
        ...
        public void run() {
            ...
            if (!_firstTimeRun) { _warmUpTimeEnd = Calendar.getInstance().getTimeInMillis(); }
    ...
    protected class ChangeCoolDownTime implements Runnable {
        ...
        public void run() {
            ...
            if (Calendar.getInstance().get(Calendar.MINUTE) > _newCycleMin && !_firstTimeRun) { time.set(Calendar.HOUR, Calendar.getInstance().get(Calendar.HOUR) + 1); }

            time.set(Calendar.MINUTE, _newCycleMin);
            LOGGER.info("FourSepulchersManager: end of the round.");
    ...
    public void showHtmlFile(L2PcInstance player, String file, L2Npc npc, L2PcInstance member) {
        ...
        if (member != null) { html.replace("%member%", member.getName()); }
