...
public final class GeoDriverArray extends GeoData {
    ...
    public GeoDriverArray() {
        ...
        LOGGER.info("GeoDriverArray: Loaded " + loaded + " " + Config.GEODATA_FORMAT.toString() + " region files.");
    ...
    private boolean loadGeoBlocks(int regionX, int regionY) {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(Config.GEODATA_PATH + filename, "r"); FileChannel fc = raf.getChannel()) {
            ...
            if (buffer.remaining() > 0) { LOGGER.warn("GeoDriverArray: Region file " + filename + " can be corrupted, remaining " + buffer.remaining() + " bytes to read."); }
            ...
            LOGGER.error("GeoDriverArray: Error while loading " + filename + " region file.", e);
     ...
     */
    private static byte[] loadFlatBlock(ByteBuffer bb) {
        // get height
     ...
     */
    private static byte[] loadComplexBlock(ByteBuffer bb) {
        // initialize buffer
     ...
     */
    private static byte[] loadMultilayerBlock(ByteBuffer bb) {
        ...
        for (int cell = 0; cell < GeoStructure.BLOCK_CELLS; cell++) {
            ...
            if (layers <= 0 || layers > Byte.MAX_VALUE) { throw new RuntimeException("Invalid layer count for MultilayerBlock"); }
    ...
    public int getGeoX(int worldX) {
        if (worldX < L2World.WORLD_X_MIN || worldX > L2World.WORLD_X_MAX) { throw new IllegalArgumentException(); }
    ...
    public int getGeoY(int worldY) {
        if (worldY < L2World.WORLD_Y_MIN || worldY > L2World.WORLD_Y_MAX) { throw new IllegalArgumentException(); }
    ...
    public int getWorldX(int geoX) {
        if (geoX < 0 || geoX >= GeoStructure.GEO_CELLS_X) { throw new IllegalArgumentException(); }
    ...
    public int getWorldY(int geoY) {
        if (geoY < 0 || geoY >= GeoStructure.GEO_CELLS_Y) { throw new IllegalArgumentException(); }
     ...
     */
    private static int getCellIndexNearest(int geoX, int geoY, int worldZ, byte[] buffer) {
     ...
     */
    private static int getCellIndexAbove(int geoX, int geoY, int worldZ, byte[] buffer) {
        // move buffer index to cell
        int index = 1;
        for (int i = 0; i < (geoX % GeoStructure.BLOCK_CELLS_X) * GeoStructure.BLOCK_CELLS_Y + (geoY % GeoStructure.BLOCK_CELLS_Y); i++) {
            // move index by amount of layers for this cell
        ...
        // none layer found
        if (layers < 0) { throw new IndexOutOfBoundsException(); }
     ...
     * @throws IndexOutOfBoundsException : When cell is above given Z coordinate.
     */
    private static int getCellIndexBelow(int geoX, int geoY, int worldZ, byte[] buffer) {
        // move buffer index to cell
        int index = 1;
        for (int i = 0; i < (geoX % GeoStructure.BLOCK_CELLS_X) * GeoStructure.BLOCK_CELLS_Y + (geoY % GeoStructure.BLOCK_CELLS_Y); i++) {
            // move index by amount of layers for this cell
