...
public class Q327_RecoverTheFarmland extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        // Iris
            ...
            int n = st.getQuestItemsCount(CLAY_URN_FRAGMENT);
            if (n == 0) { htmltext = "30034-02.htm"; }
            else {
            ...
            int n = st.getQuestItemsCount(BRASS_TRINKET_PIECE);
            if (n == 0) { htmltext = "30034-02.htm"; }
            else {
                st.playSound(ESound.ItemSound_quest_itemget);
            ...
            int n = st.getQuestItemsCount(BRONZE_MIRROR_PIECE);
            if (n == 0) { htmltext = "30034-02.htm"; }
            else {
                st.playSound(ESound.ItemSound_quest_itemget);
            ...
            int n = st.getQuestItemsCount(JADE_NECKLACE_BEAD);
            if (n == 0) { htmltext = "30034-02.htm"; }
            else {
                st.playSound(ESound.ItemSound_quest_itemget);
            ...
            if (!isRewarded) { htmltext = "30034-02.htm"; }
            else { st.playSound(ESound.ItemSound_quest_itemget); }
        }
        // Nestle
        else if (event.equalsIgnoreCase("30314-03.htm")) {
            if (!st.hasQuestItems(ANCIENT_CLAY_URN)) { htmltext = "30314-07.htm"; }
            else {
            ...
            if (!st.hasQuestItems(ANCIENT_BRASS_TIARA)) { htmltext = "30314-07.htm"; }
            else {
                st.takeItems(ANCIENT_BRASS_TIARA, 1);
                int rnd = Rnd.get(100);
                if (rnd < 40) { st.rewardItems(HEALING_POTION, 1); }
                else if (rnd < 84) { st.rewardItems(HASTE_POTION, 1); }
                else { st.rewardItems(POTION_OF_ALACRITY, 1); }
            }
        }
        else if (event.equalsIgnoreCase("30314-05.htm")) {
            if (!st.hasQuestItems(ANCIENT_BRONZE_MIRROR)) { htmltext = "30314-07.htm"; }
            else {
            ...
            if (!st.hasQuestItems(ANCIENT_JADE_NECKLACE)) { htmltext = "30314-07.htm"; }
            else {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case PIOTUR:
                        if (!st.hasQuestItems(LEIKAN_LETTER)) {
                            if (st.hasAtLeastOneQuestItem(TUREK_DOGTAG, TUREK_MEDALLION)) {
                                ...
                                int dogtag = st.getQuestItemsCount(TUREK_DOGTAG);
                                int medallion = st.getQuestItemsCount(TUREK_MEDALLION);
                    ...
                    case LEIKAN:
                        if (cond == 2) { htmltext = "30382-04.htm"; }
                        else if (cond == 3 || cond == 4) {
                            htmltext = "30382-05.htm";
                            st.set("cond", "5");
                            st.playSound(ESound.ItemSound_quest_middle);
                        }
                        else if (cond == 5) { htmltext = "30382-05.htm"; }
                        break;
