...
public class Q272_WrathOfAncestors extends Quest {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getRace() != PlayerRace.Orc) { htmltext = "30572-00.htm"; }
                else if (player.getLevel() < 5) { htmltext = "30572-01.htm"; }
                else { htmltext = "30572-02.htm"; }
                break;

            case QuestState.STATE_STARTED:
                if (st.getInt("cond") == 1) { htmltext = "30572-04.htm"; }
                else {
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropItemsAlways(GRAVE_ROBBERS_HEAD, 1, 50)) { st.set("cond", "2"); }
