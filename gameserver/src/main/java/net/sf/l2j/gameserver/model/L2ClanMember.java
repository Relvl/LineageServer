...
public class L2ClanMember {
    ...
    public L2ClanMember(L2Clan clan, ResultSet clanMember) throws SQLException {
        if (clan == null) { throw new IllegalArgumentException("Cannot create a clan member with a null clan."); }
    ...
    public void setPlayerInstance(L2PcInstance player) {
        ...
        if (player != null) {
            if (_clan.getLevel() > 3 && player.isClanLeader()) { SiegeManager.addSiegeSkills(player); }

            if (_clan.getReputationScore() >= 0) {
                for (L2Skill sk : _clan.getClanSkills()) {
                    if (sk.getMinPledgeClass() <= player.getPledgeClass()) { player.addSkill(sk, false); }
                }
            }
        }
        _player = player;
    ...
    public boolean isOnline() {
        ...
        if (_player.getClient() == null) { return false; }
        if (_player.getClient().isDetached()) { return false; }
    ...
    public void setPledgeType(int pledgeType) {
        ...
        if (_player != null) { _player.setPledgeType(pledgeType); }
        else { updatePledgeType(); }
    }
    ...
    public void setPowerGrade(int powerGrade) {
        ...
        if (_player != null) { _player.setPowerGrade(powerGrade); }
        else { updatePowerGrade(); }
    }
    ...
    public static int calculatePledgeClass(L2PcInstance player) {
        ...
        if (clan != null) {
            switch (clan.getLevel()) {
                case 4:
                    if (player.isClanLeader()) { pledgeClass = 3; }
                    break;

                case 5:
                    if (player.isClanLeader()) { pledgeClass = 4; }
                    else { pledgeClass = 2; }
                    break;
                ...
                case 6:
                    switch (player.getPledgeType()) {
                        ...
                        case 0:
                            if (player.isClanLeader()) { pledgeClass = 5; }
                            else {
                ...
                case 7:
                    switch (player.getPledgeType()) {
                        ...
                        case 0:
                            if (player.isClanLeader()) { pledgeClass = 7; }
                            else {
                                switch (clan.getLeaderSubPledge(player.getObjectId())) {
                                    case 100:
                                    case 200:
                ...
                case 8:
                    switch (player.getPledgeType()) {
                        ...
                        case 0:
                            if (player.isClanLeader()) { pledgeClass = 8; }
                            else {
                                switch (clan.getLeaderSubPledge(player.getObjectId())) {
                                    case 100:
                                    case 200:
        ...
        if (player.isHero() && pledgeClass < 8) { pledgeClass = 8; }
        else if (player.isNoble() && pledgeClass < 5) { pledgeClass = 5; }
