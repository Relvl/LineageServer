...
public abstract class AbstractRefinePacket extends L2GameClientPacket {
    ...
    protected static final boolean isValid(L2PcInstance player, L2ItemInstance item, L2ItemInstance refinerItem, L2ItemInstance gemStones) {
        if (!isValid(player, item, refinerItem)) { return false; }

        // GemStones must belong to owner
        if (gemStones.getOwnerId() != player.getObjectId()) { return false; }
        // .. and located in inventory
        if (gemStones.getLocation() != EItemLocation.INVENTORY) { return false; }

        final CrystalType grade = item.getItem().getCrystalType();

        // Check for item id
        if (getGemStoneId(grade) != gemStones.getItemId()) { return false; }
        // Count must be greater or equal of required number
        if (getGemStoneCount(grade) > gemStones.getCount()) { return false; }
    ...
    protected static final boolean isValid(L2PcInstance player, L2ItemInstance item, L2ItemInstance refinerItem) {
        if (!isValid(player, item)) { return false; }

        // Item must belong to owner
        if (refinerItem.getOwnerId() != player.getObjectId()) { return false; }

        // Lifestone must be located in inventory
        if (refinerItem.getLocation() != EItemLocation.INVENTORY) { return false; }

        final LifeStone ls = _lifeStones.get(refinerItem.getItemId());
        if (ls == null) { return false; }

        // check for level of the lifestone
        if (player.getLevel() < ls.getPlayerLevel()) { return false; }
    ...
    protected static final boolean isValid(L2PcInstance player, L2ItemInstance item) {
        ...
        // Item must belong to owner
        if (item.getOwnerId() != player.getObjectId()) { return false; }
        if (item.isAugmented()) { return false; }
        if (item.isHeroItem()) { return false; }
        if (item.isShadowItem()) { return false; }
        if (item.getItem().getCrystalType().isLesser(CrystalType.C)) { return false; }
    ...
    protected static final boolean isValid(L2PcInstance player) {
        ...
        if (player.isCursedWeaponEquipped()) { return false; }
        if (player.isProcessingTransaction()) { return false; }
