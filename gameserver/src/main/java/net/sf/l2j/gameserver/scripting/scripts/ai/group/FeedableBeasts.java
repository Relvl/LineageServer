...
public class FeedableBeasts extends AbstractNpcAI {
    ...
    public void spawnNext(L2Npc npc, int growthLevel, L2PcInstance player, int food) {
        ...
        if (growthLevel == 2) {
            ...
            if (Rnd.get(2) == 0) {
                if (player.getClassId().isMage()) { nextNpcId = _GrowthCapableMobs.get(npcId).getMob(food, 1, 1); }
                else { nextNpcId = _GrowthCapableMobs.get(npcId).getMob(food, 1, 0); }
            }
            else {
                /*
        ...
        else { nextNpcId = _GrowthCapableMobs.get(npcId).getRandomMob(food); }

        // Remove the feedinfo of the mob that got despawned, if any
        if (_FeedInfo.containsKey(npc.getObjectId())) {
            if (_FeedInfo.get(npc.getObjectId()) == player.getObjectId()) { _FeedInfo.remove(npc.getObjectId()); }
        }
        ...
        if (Util.contains(TAMED_BEASTS, nextNpcId)) {
            ...
            if (rand < 5) {
                ...
                if (!message.isEmpty()) {
                    npc.broadcastPacket(new NpcSay(nextNpc.getObjectId(), EChatType.ALL, nextNpc.getNpcId(), message));
                }
            }
        }
        else {
    ...
    public String onSkillSee(L2Npc npc, L2PcInstance caster, L2Skill skill, L2Object[] targets, boolean isPet) {
        if (!Util.contains(targets, npc)) { return super.onSkillSee(npc, caster, skill, targets, isPet); }
        ...
        if (_GrowthCapableMobs.containsKey(npcId)) { growthLevel = _GrowthCapableMobs.get(npcId).getGrowthLevel(); }
        ...
        int food = 0;
        if (skillId == SKILL_GOLDEN_SPICE) { food = GOLDEN_SPICE; }
        else if (skillId == SKILL_CRYSTAL_SPICE) { food = CRYSTAL_SPICE; }
        ...
        if (_GrowthCapableMobs.containsKey(npcId)) {
            ...
            // Rare random talk...
            if (Rnd.get(20) == 0) {
                npc.broadcastPacket(new NpcSay(objectId, EChatType.ALL, npc.getNpcId(), TEXT[growthLevel][Rnd.get(TEXT[growthLevel].length)]));
            }
            ...
            if (Rnd.get(100) < _GrowthCapableMobs.get(npcId).getChance()) { spawnNext(npc, growthLevel, caster, food); }
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet) {
        ...
        if (_FeedInfo.containsKey(npc.getObjectId())) { _FeedInfo.remove(npc.getObjectId()); }
    ...
    private static class GrowthCapableMob {
        ...
        public Integer getMob(int spice, int mobType, int classType) {
            if (_spiceToMob.containsKey(spice)) { return _spiceToMob.get(spice)[mobType][classType]; }
