...
public class Q298_LizardmensConspiracy extends Quest {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                switch (npc.getNpcId()) {
                    ...
                    case ROHMER:
                        if (st.getInt("cond") == 1) { htmltext = (st.hasQuestItems(PATROL_REPORT)) ? "30344-0.htm" : "30344-0a.htm"; }
                        else { htmltext = "30344-2.htm"; }
                        break;
                }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            case 20922:
                if (st.dropItems(WHITE_GEM, 1, 50, 400000) && st.getQuestItemsCount(RED_GEM) >= 50) { st.set("cond", "3"); }
                break;

            case 20923:
                if (st.dropItems(WHITE_GEM, 1, 50, 450000) && st.getQuestItemsCount(RED_GEM) >= 50) { st.set("cond", "3"); }
                break;

            case 20924:
                if (st.dropItems(WHITE_GEM, 1, 50, 350000) && st.getQuestItemsCount(RED_GEM) >= 50) { st.set("cond", "3"); }
                break;

            case 20926:
            case 20927:
                if (st.dropItems(RED_GEM, 1, 50, 400000) && st.getQuestItemsCount(WHITE_GEM) >= 50) { st.set("cond", "3"); }
                break;
        }
