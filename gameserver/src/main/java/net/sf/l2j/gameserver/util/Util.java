...
public final class Util {
    public static void handleIllegalPlayerAction(L2PcInstance actor, String message, int punishment) {
        ThreadPoolManager.getInstance().schedule(new IllegalPlayerAction(actor, message, punishment), 5000);
    }
    ...
    public static float roundTo(float val, int numPlaces) {
        if (numPlaces <= 1) { return Math.round(val); }
    ...
    public static <T> boolean contains(T[] array1, T[] array2) {
        ...
        for (T element1 : array1) {
            for (T element2 : array2) { if (element2.equals(element1)) { return true; } }
        }
        return false;
    }
    ...
    public static boolean contains(int[] array, int obj) {
        ...
        for (int element : array) { if (element == obj) { return true; } }
