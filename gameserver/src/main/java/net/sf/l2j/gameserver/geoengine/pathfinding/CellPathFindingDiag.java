...
public class CellPathFindingDiag extends PathCheckerDiag {
    ...
    public CellPathFindingDiag() {
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            try {
                ...
                LOGGER.error("CellPathFindingDiag: Can not load buffer setting: " + buf);
            }
        }

        LOGGER.info("CellPathFindingDiag: Loaded " + count + " node buffers.");
    }
    ...
    public List<Location> findPath(int ox, int oy, int oz, int tx, int ty, int tz, boolean playable) {
        ...
        if (!GeoData.getInstance().hasGeoPos(gox, goy)) { return null; }
        ...
        if (!GeoData.getInstance().hasGeoPos(gtx, gty)) { return null; }
        ...
        _postFilterUses++;
        if (playable) { _postFilterPlayableUses++; }
    ...
    private final NodeBufferDiag getBuffer(int size, boolean playable) {
        ...
        for (BufferHolder holder : _buffers) {
            ...
            for (NodeBufferDiag buffer : holder._buffer) {
                ...
                holder._uses++;
                if (playable) { holder._playableUses++; }
            ...
            holder._overflows++;
            if (playable) { holder._playableOverflows++; }
        }
    ...
    private static final class BufferHolder {
        ...
        public BufferHolder(int size, int count) {
            ...
            for (int i = 0; i < count; i++) { _buffer.add(new NodeBufferDiag(size)); }
        }
        ...
        public String toString() {
            ...
            if (_uses > 0) { StringUtil.append(sb, " total/avg(ms)=", _elapsed, "/", String.format("%1.2f", (double) _elapsed / _uses)); }
    ...
    public List<String> getStat() {
        ...
        for (BufferHolder buffer : _buffers) { list.add(buffer.toString()); }
        ...
        if (_postFilterUses > 0) { list.add("Time (ms): total=" + String.valueOf(_postFilterElapsed) + " avg=" + String.format("%1.2f", (double) _postFilterElapsed / _postFilterUses)); }
