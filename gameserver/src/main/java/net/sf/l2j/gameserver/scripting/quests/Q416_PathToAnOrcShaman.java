...
public class Q416_PathToAnOrcShaman extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30585-05.htm")) {
            if (player.getClassId() != ClassId.orcMage) { htmltext = (player.getClassId() == ClassId.orcShaman) ? "30585-02a.htm" : "30585-02.htm"; }
            else if (player.getLevel() < 19) { htmltext = "30585-03.htm"; }
            else if (st.hasQuestItems(MASK_OF_MEDIUM)) { htmltext = "30585-04.htm"; }
        }
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case TATARU_ZU_HESTUI:
                        if (cond == 1) { htmltext = "30585-07.htm"; }
                        else if (cond == 2) {
                        ...
                        else if (cond == 3) { htmltext = "30585-09.htm"; }
                        else if (cond == 4) { htmltext = "30585-10.htm"; }
                        else if (cond == 5) { htmltext = "30585-12.htm"; }
                        else if (cond > 5 && cond < 12) { htmltext = "30585-13.htm"; }
                        else if (cond == 12) { htmltext = "30585-11c.htm"; }
                        break;

                    case HESTUI_TOTEM_SPIRIT:
                        if (cond == 3) { htmltext = "30592-01.htm"; }
                        else if (cond == 4) { htmltext = "30592-04.htm"; }
                        else if (cond > 4 && cond < 12) { htmltext = "30592-05.htm"; }
                        break;
                    ...
                    case UMOS:
                        ...
                        else if (cond == 6) { htmltext = "30502-02.htm"; }
                        else if (cond == 7) {
                        ...
                        else if (cond == 8) { htmltext = "30502-04.htm"; }
                        else if (cond == 9 || cond == 10) { htmltext = "30502-05.htm"; }
                        else if (cond == 11) { htmltext = "30502-06.htm"; }
                        break;
                    ...
                    case MOIRA:
                        ...
                        else if (cond > 12 && cond < 21) { htmltext = "31979-02.htm"; }
                        else if (cond == 21) {
                    ...
                    case TOTEM_SPIRIT_OF_GANDI:
                        if (cond == 13) { htmltext = "32057-01.htm"; }
                        else if (cond > 13 && cond < 20) { htmltext = "32057-03.htm"; }
                        else if (cond == 20) { htmltext = "32057-04.htm"; }
                        break;

                    case DUDA_MARA_TOTEM_SPIRIT:
                        if (cond == 8) { htmltext = "30593-01.htm"; }
                        else if (cond == 9) { htmltext = "30593-04.htm"; }
                        else if (cond == 10) {
                        ...
                        else if (cond == 11) { htmltext = "30593-06.htm"; }
                        break;

                    case DEAD_LEOPARD_CARCASS:
                        if (cond == 14) { htmltext = "32090-01a.htm"; }
                        else if (cond == 15) {
                            htmltext = "32090-01.htm";
                            st.set("cond", "16");
                            st.playSound(ESound.ItemSound_quest_middle);
                        }
                        else if (cond == 16) { htmltext = "32090-01b.htm"; }
                        else if (cond == 17) { htmltext = "32090-02.htm"; }
                        else if (cond == 18) { htmltext = "32090-05.htm"; }
                        else if (cond == 19) {
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            ...
            case GRIZZLY_BEAR:
                if (cond == 6 && st.dropItemsAlways(GRIZZLY_BLOOD, 1, 3)) { st.set("cond", "7"); }
                break;

            case VENOMOUS_SPIDER:
            case ARACHNID_TRACKER:
                if (cond == 9) {
                    int count = st.getQuestItemsCount(DURKA_PARASITE);
                    int rnd = Rnd.get(10);
                    ...
                    else { st.dropItemsAlways(DURKA_PARASITE, 1, 0); }
                }
                break;
            ...
            case BLACK_LEOPARD:
                if (cond == 14) {
                    if (st.getInt("leopard") > 0) {
                        ...
                        if (Rnd.get(3) < 2) { npc.broadcastNpcSay("My dear friend of " + player.getName() + ", who has gone on ahead of me!"); }
                    }
                    else { st.set("leopard", "1"); }
                }
                else if (cond == 16) {
                    st.set("cond", "17");
                    st.playSound(ESound.ItemSound_quest_middle);

                    if (Rnd.get(3) < 2) { npc.broadcastNpcSay("Listen to Tejakar Gandi, young Oroka! The spirit of the slain leopard is calling you, " + player.getName() + "!"); }
                }
                else if (cond == 18) {
                    st.set("cond", "19");
                    st.playSound(ESound.ItemSound_quest_middle);
