...
public class Q350_EnhanceYourWeapon extends Quest {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                for (L2ItemInstance item : player.getInventory().getItems()) {
                    ...
                    if (SoulCrystalsTable.getSoulCrystalInfos().get(item.getItemId()) != null) { return npc.getNpcId() + "-03.htm"; }
                }
    ...
    public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet) {
        ...
        LevelingInfo npcInfo = SoulCrystalsTable.getNpcInfos().get(npc.getNpcId());
        ...
        switch (npcInfo.getAbsorbCrystalType()) {
            case FULL_PARTY:
                ...
                for (L2PcInstance player : getPartyMembersState(killer, npc, QuestState.STATE_STARTED)) { tryToStageCrystal(player, mob, npcInfo, chance); }
                break;

            case PARTY_ONE_RANDOM:
                L2PcInstance player = getRandomPartyMemberState(killer, npc, QuestState.STATE_STARTED);
                if (player != null) { tryToStageCrystal(player, (L2Attackable) npc, npcInfo, Rnd.get(100)); }
                break;

            case LAST_HIT:
                if (checkPlayerState(killer, npc, QuestState.STATE_STARTED) != null) { tryToStageCrystal(killer, (L2Attackable) npc, npcInfo, Rnd.get(100)); }
                break;
        }
    ...
    private static void tryToStageCrystal(L2PcInstance player, L2Attackable mob, LevelingInfo npcInfo, int chance) {
        ...
        for (L2ItemInstance item : player.getInventory().getItems()) {
            ...
            if (crystalData != null) {
                ...
                if (npcInfo.isSkillRequired()) {
                    ...
                    SoulCrystalAbsorbInfo ai = mob.getAbsorbInfo(player.getObjectId());
                    if (ai != null && ai.isRegistered()) { player.sendPacket(SystemMessageId.SOUL_CRYSTAL_ABSORBING_FAILED_RESONATION); }
                }
        ...
        else if (chance < (npcInfo.getChanceStage() + npcInfo.getChanceBreak())) { exchangeCrystal(player, crystalData, false); }
