...
public class Q642_APowerfulPrimevalCreature extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("32105-04.htm")) {
            ...
            if (st.getQuestItemsCount(DINOSAUR_TISSUE) >= 150 && st.hasQuestItems(DINOSAUR_EGG)) { htmltext = "32105-06.htm"; }
        }
            ...
            if (tissues > 0) {
                ...
                st.rewardItems(ItemConst.ADENA_ID, tissues * 5000);
            }
            else { htmltext = "32105-08.htm"; }
        }
            ...
            if (st.getQuestItemsCount(DINOSAUR_TISSUE) >= 150 && st.hasQuestItems(DINOSAUR_EGG)) {
                ...
                st.rewardItems(ItemConst.ADENA_ID, 44000);
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (npc.getNpcId() == ANCIENT_EGG) {
            if (Rnd.get(100) < 1) {
                ...
                if (st.getQuestItemsCount(DINOSAUR_TISSUE) >= 150) { st.playSound(ESound.ItemSound_quest_middle); }
            ...
            if (st.getQuestItemsCount(DINOSAUR_TISSUE) >= 150 && st.hasQuestItems(DINOSAUR_EGG)) { st.playSound(ESound.ItemSound_quest_middle); }
            else { st.playSound(ESound.ItemSound_quest_itemget); }
        }
