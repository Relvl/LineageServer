...
public class L2Manor {
    private static final Logger LOGGER = LoggerFactory.getLogger(L2Manor.class);
    ...
    public List<Integer> getAllCrops() {
        ...
        for (SeedData seed : _seeds.values()) {
            if (!crops.contains(seed.getCrop()) && seed.getCrop() != 0 && !crops.contains(seed.getCrop())) { crops.add(seed.getCrop()); }
        }
        return crops;
    }
    ...
    public int getSeedBasicPrice(int seedId) {
        ...
        if (seedItem != null) { return seedItem.getReferencePrice(); }
    ...
    public int getSeedBasicPriceByCrop(int cropId) {
        for (SeedData seed : _seeds.values()) {
            if (seed.getCrop() == cropId) { return getSeedBasicPrice(seed.getId()); }
        }
        return 0;
    }
    ...
    public int getCropBasicPrice(int cropId) {
        ...
        if (cropItem != null) { return cropItem.getReferencePrice(); }
    ...
    public int getMatureCrop(int cropId) {
        for (SeedData seed : _seeds.values()) {
            if (seed.getCrop() == cropId) { return seed.getMature(); }
        }
        return 0;
    }
    ...
    public int getSeedMinLevel(int seedId) {
        ...
        if (seed != null) { return seed.getLevel() - 5; }
    ...
    public int getSeedLevelByCrop(int cropId) {
        for (SeedData seed : _seeds.values()) {
            if (seed.getCrop() == cropId) { return seed.getLevel(); }
        }
        return 0;
    }
    ...
    public int getSeedLevel(int seedId) {
        SeedData seed = _seeds.get(seedId);
        if (seed != null) { return seed.getLevel(); }
    ...
    public boolean isAlternative(int seedId) {
        for (SeedData seed : _seeds.values()) {
            if (seed.getId() == seedId) { return seed.isAlternative(); }
        }
        return false;
    }
    ...
    public int getCropType(int seedId) {
        SeedData seed = _seeds.get(seedId);
        if (seed != null) { return seed.getCrop(); }
    ...
    public synchronized int getRewardItem(int cropId, int type) {
        for (SeedData seed : _seeds.values()) {
            ...
            if (seed.getCrop() == cropId) { return seed.getReward(type); }
        }
        return -1;
    }
    ...
    public synchronized int getRewardItemBySeed(int seedId, int type) {
        SeedData seed = _seeds.get(seedId);
        if (seed != null) { return seed.getReward(type); }
    ...
    public List<Integer> getCropsForCastle(int castleId) {
        List<Integer> crops = new ArrayList<>();
        for (SeedData seed : _seeds.values()) {
            if (seed.getManorId() == castleId && !crops.contains(seed.getCrop())) { crops.add(seed.getCrop()); }
        }
        return crops;
    }
    ...
    public List<Integer> getSeedsForCastle(int castleId) {
        ...
        for (SeedData seed : _seeds.values()) {
            if (seed.getManorId() == castleId && !seedsID.contains(seed.getId())) { seedsID.add(seed.getId()); }
        }
        return seedsID;
    }
    ...
    public int getCastleIdForSeed(int seedId) {
        SeedData seed = _seeds.get(seedId);
        if (seed != null) { return seed.getManorId(); }
    ...
    public int getSeedSaleLimit(int seedId) {
        SeedData seed = _seeds.get(seedId);
        if (seed != null) { return seed.getSeedLimit(); }
    ...
    public int getCropPuchaseLimit(int cropId) {
        for (SeedData seed : _seeds.values()) {
            if (seed.getCrop() == cropId) { return seed.getCropLimit(); }
        }
        return 0;
    }
    ...
    private void parseData() {
        try {
            ...
            LOGGER.error("ManorManager: Error while creating table: ", e);
        }
        LOGGER.info("ManorManager: Loaded {} seeds.", _seeds.size());
    }
