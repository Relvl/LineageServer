...
public final class ConditionLogicOr extends ACondition {
    ...
    void setListener(IConditionListener listener) {
        if (listener != null) {
            for (ACondition condition : conditions) { condition.setListener(this); }
        }
        else {
            for (ACondition condition : conditions) { condition.setListener(null); }
        }
    ...
    public boolean testImpl(Env env) {
        for (ACondition condition : conditions) {
            if (condition.test(env)) { return true; }
        }
        return false;
    }
}
