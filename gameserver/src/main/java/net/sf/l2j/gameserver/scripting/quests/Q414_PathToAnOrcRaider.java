...
public class Q414_PathToAnOrcRaider extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30570-05.htm")) {
            if (player.getClassId() != ClassId.orcFighter) { htmltext = (player.getClassId() == ClassId.orcRaider) ? "30570-02a.htm" : "30570-03.htm"; }
            else if (player.getLevel() < 19) { htmltext = "30570-02.htm"; }
            else if (st.hasQuestItems(MARK_OF_RAIDER)) { htmltext = "30570-04.htm"; }
            else {
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case KARUKIA:
                        if (cond == 1) { htmltext = "30570-06.htm"; }
                        else if (cond == 2) { htmltext = "30570-07.htm"; }
                        else if (cond == 3 || cond == 4) { htmltext = "30570-08.htm"; }
                        else if (cond == 5) { htmltext = "30570-07b.htm"; }
                        break;

                    case KASMAN:
                        if (cond == 3) { htmltext = "30501-01.htm"; }
                        else if (cond == 4) {
                            if (st.getQuestItemsCount(HEAD_OF_BETRAYER) == 1) { htmltext = "30501-02.htm"; }
                            else {
                    ...
                    case TAZEER:
                        if (cond == 5) { htmltext = "31978-01.htm"; }
                        else if (cond == 6) { htmltext = "31978-04.htm"; }
                        else if (cond == 7) {
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        switch (npc.getNpcId()) {
            ...
            case KURUKA_RATMAN_LEADER:
                if (cond == 1 && st.dropItemsAlways(KURUKA_RATMAN_TOOTH, 1, 10)) { st.set("cond", "2"); }
                break;

            case UMBAR_ORC:
                if ((cond == 3 || cond == 4) && st.getQuestItemsCount(HEAD_OF_BETRAYER) < 2 && Rnd.get(10) < 2) {
                    if (cond == 3) { st.set("cond", "4"); }
            ...
            case TIMORA_ORC:
                if (cond == 6 && st.dropItems(TIMORA_ORC_HEAD, 1, 1, 600000)) { st.set("cond", "7"); }
                break;
        }
