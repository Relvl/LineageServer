...
public class Q031_SecretBuriedInTheSwamp extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("31555-01.htm")) {
            ...
            st.rewardItems(ItemConst.ADENA_ID, 40000);
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case ABERCROMBIE:
                        if (cond == 1) { htmltext = "31555-02.htm"; }
                        else if (cond == 2) { htmltext = "31555-03.htm"; }
                        else if (cond > 2 && cond < 7) { htmltext = "31555-05.htm"; }
                        else if (cond == 7) { htmltext = "31555-06.htm"; }
                        break;

                    case CORPSE_OF_DWARF:
                        if (cond == 1) { htmltext = "31665-00.htm"; }
                        else if (cond > 1) { htmltext = "31665-02.htm"; }
                        break;

                    case FORGOTTEN_MONUMENT_1:
                        if (cond == 3) { htmltext = "31661-00.htm"; }
                        else if (cond > 3) { htmltext = "31661-02.htm"; }
                        break;

                    case FORGOTTEN_MONUMENT_2:
                        if (cond == 4) { htmltext = "31662-00.htm"; }
                        else if (cond > 4) { htmltext = "31662-02.htm"; }
                        break;

                    case FORGOTTEN_MONUMENT_3:
                        if (cond == 5) { htmltext = "31663-00.htm"; }
                        else if (cond > 5) { htmltext = "31663-02.htm"; }
                        break;

                    case FORGOTTEN_MONUMENT_4:
                        if (cond == 6) { htmltext = "31664-00.htm"; }
                        else if (cond > 6) { htmltext = "31664-02.htm"; }
                        break;
                }
                break;
