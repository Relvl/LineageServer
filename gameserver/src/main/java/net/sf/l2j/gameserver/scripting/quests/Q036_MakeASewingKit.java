...
public class Q036_MakeASewingKit extends Quest {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getLevel() >= 60) {
                    ...
                    if (fwear != null && fwear.getInt("cond") == 6) { htmltext = "30847-0.htm"; }
                    else { htmltext = "30847-0a.htm"; }
                }
                else { htmltext = "30847-0b.htm"; }
                break;

            case QuestState.STATE_STARTED:
                int cond = st.getInt("cond");
                if (cond == 1) { htmltext = "30847-1a.htm"; }
                else if (cond == 2) { htmltext = "30847-2.htm"; }
                else if (cond == 3) { htmltext = (st.getQuestItemsCount(ORIHARUKON) < 10 || st.getQuestItemsCount(ARTISANS_FRAME) < 10) ? "30847-4a.htm" : "30847-4.htm"; }
                break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropItems(REINFORCED_STEEL, 1, 5, 500000)) { st.set("cond", "2"); }
