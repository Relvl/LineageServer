...
public class Q413_PathToAShillienOracle extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30330-05.htm")) {
            if (player.getClassId() != ClassId.darkMage) { htmltext = (player.getClassId() == ClassId.shillienOracle) ? "30330-02a.htm" : "30330-03.htm"; }
            else if (player.getLevel() < 19) { htmltext = "30330-02.htm"; }
            else if (st.hasQuestItems(ORB_OF_ABYSS)) { htmltext = "30330-04.htm"; }
        }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case SIDRA:
                        if (cond == 1) { htmltext = "30330-07.htm"; }
                        else if (cond > 1 && cond < 4) { htmltext = "30330-08.htm"; }
                        else if (cond > 3 && cond < 7) { htmltext = "30330-09.htm"; }
                        else if (cond == 7) {
                    ...
                    case TALBOT:
                        if (cond == 1) { htmltext = "30377-01.htm"; }
                        else if (cond == 2) { htmltext = (st.hasQuestItems(BLOODY_RUNE)) ? "30377-04.htm" : "30377-03.htm"; }
                        else if (cond == 3) {
                        ...
                        else if (cond > 3 && cond < 7) { htmltext = "30377-06.htm"; }
                        else if (cond == 7) { htmltext = "30377-07.htm"; }
                        break;

                    case ADONIUS:
                        if (cond == 4) { htmltext = "30375-01.htm"; }
                        else if (cond == 5) { htmltext = (st.hasQuestItems(ASHEN_BONES)) ? "30375-05.htm" : "30375-06.htm"; }
                        else if (cond == 6) {
                        ...
                        else if (cond == 7) { htmltext = "30375-08.htm"; }
                        break;
                }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (npc.getNpcId() == 20776) {
            if (st.getInt("cond") == 2) {
                ...
                if (st.dropItemsAlways(BLOODY_RUNE, 1, 5)) { st.set("cond", "3"); }
            }
        }
        else if (st.getInt("cond") == 5 && st.dropItemsAlways(ASHEN_BONES, 1, 10)) { st.set("cond", "6"); }
