...
public class Q325_GrimCollector extends Quest {
    ...
    public Q325_GrimCollector() {
        ...
        for (int npcId : DROPLIST.keySet()) { addKillId(npcId); }
    }

    private static int getNumberOfPieces(QuestState st) {
        return st.getQuestItemsCount(ZOMBIE_HEAD) + st.getQuestItemsCount(SPINE) + st.getQuestItemsCount(ARM_BONE) + st.getQuestItemsCount(ZOMBIE_HEART) + st.getQuestItemsCount(ZOMBIE_LIVER) + st.getQuestItemsCount(SKULL) + st.getQuestItemsCount(RIB_BONE) + st
                .getQuestItemsCount(THIGH_BONE) + st.getQuestItemsCount(COMPLETE_SKELETON);
    }

    private static void payback(QuestState st) {
        int count = getNumberOfPieces(st);
        if (count > 0) {
            int reward = 30 * st.getQuestItemsCount(ZOMBIE_HEAD) + 20 * st.getQuestItemsCount(ZOMBIE_HEART) + 20 * st.getQuestItemsCount(ZOMBIE_LIVER) + 100 * st.getQuestItemsCount(SKULL) + 40 * st.getQuestItemsCount(RIB_BONE) + 14 * st.getQuestItemsCount(SPINE) + 14 * st
                    .getQuestItemsCount(ARM_BONE) + 14 * st.getQuestItemsCount(THIGH_BONE) + 341 * st.getQuestItemsCount(COMPLETE_SKELETON);
            if (count > 10) { reward += 1629; }

            if (st.hasQuestItems(COMPLETE_SKELETON)) { reward += 543; }
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (event.equalsIgnoreCase("30336-03.htm")) {
            ...
            if (!st.hasQuestItems(SPINE, ARM_BONE, SKULL, RIB_BONE, THIGH_BONE)) { htmltext = "30342-02.htm"; }
            else {
                st.takeItems(SPINE, 1);
                st.takeItems(SKULL, 1);
                st.takeItems(ARM_BONE, 1);
                ...
                if (Rnd.get(10) < 9) { st.giveItems(COMPLETE_SKELETON, 1); }
                else { htmltext = "30342-04.htm"; }
            }
        }
                ...
                switch (npc.getNpcId()) {
                    ...
                    case SAMED:
                        if (!st.hasQuestItems(ANATOMY_DIAGRAM)) { htmltext = "30434-01.htm"; }
                            ...
                            else { htmltext = (!st.hasQuestItems(COMPLETE_SKELETON)) ? "30434-05.htm" : "30434-08.htm"; }
                        }
                        break;
