...
public class Q034_InSearchOfCloth extends Quest {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getLevel() >= 60) {
                    ...
                    if (fwear != null && fwear.getInt("cond") == 6) { htmltext = "30088-0.htm"; }
                    else { htmltext = "30088-0a.htm"; }
                }
                else { htmltext = "30088-0b.htm"; }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case RADIA:
                        if (cond == 1) { htmltext = "30088-1a.htm"; }
                        else if (cond == 2) { htmltext = "30088-2.htm"; }
                        else if (cond == 3) { htmltext = "30088-3a.htm"; }
                        else if (cond == 6) {
                            if (st.getQuestItemsCount(SUEDE) < 3000 || st.getQuestItemsCount(THREAD) < 5000 || !st.hasQuestItems(SPIDERSILK)) { htmltext = "30088-4a.htm"; }
                            else { htmltext = "30088-4.htm"; }
                        }
                        break;

                    case VARAN:
                        if (cond == 1) { htmltext = "30294-0.htm"; }
                        else if (cond > 1) { htmltext = "30294-1a.htm"; }
                        break;

                    case RALFORD:
                        if (cond == 3) { htmltext = "30165-0.htm"; }
                        else if (cond == 4 && st.getQuestItemsCount(SPINNERET) < 10) { htmltext = "30165-1a.htm"; }
                        else if (cond == 5) { htmltext = "30165-2.htm"; }
                        else if (cond > 5) { htmltext = "30165-3a.htm"; }
                        break;
                }
                break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (st.dropItems(SPINNERET, 1, 10, 500000)) { st.set("cond", "5"); }
