...
public class DrainSoul implements ISkillHandler {
    ...
    public void useSkill(L2Character activeChar, L2Skill skill, L2Object[] targets) {
        // Check player.
        if (activeChar == null || activeChar.isDead() || !(activeChar instanceof L2PcInstance)) { return; }

        // Check quest condition.
        final L2PcInstance player = (L2PcInstance) activeChar;
        QuestState st = player.getQuestState(qn);
        if (st == null || !st.isStarted()) { return; }

        // Get target.
        L2Object target = targets[0];
        if (target == null || !(target instanceof L2Attackable)) { return; }
        ...
        if (!player.isInsideRadius(mob, skill.getEffectRange(), true, true)) { return; }
