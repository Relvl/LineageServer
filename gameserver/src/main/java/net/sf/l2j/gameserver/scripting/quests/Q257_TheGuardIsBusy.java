    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            ...
            case QuestState.STATE_STARTED:
                int amulets = st.getQuestItemsCount(ORC_AMULET);
                int necklaces = st.getQuestItemsCount(ORC_NECKLACE);
                int fangs = st.getQuestItemsCount(WEREWOLF_FANG);

                if (amulets + necklaces + fangs == 0) { htmltext = "30039-04.htm"; }
                else {
                    ...
                    if (amulets + necklaces + fangs >= 10) { reward += 1000; }
