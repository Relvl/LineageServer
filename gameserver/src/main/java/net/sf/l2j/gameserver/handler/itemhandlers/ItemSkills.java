        ...
        for (IntIntHolder skillInfo : skills) {
            ...
            if (itemSkill.isPotion() || itemSkill.isSimultaneousCast()) {
                if (!item.isHerb()) {
                    ...
                    if (!playable.destroyItem(EItemProcessPurpose.CONSUME, item.getObjectId(), (itemSkill.getItemConsumeId() == 0 && itemSkill.getItemConsume() > 0) ? itemSkill.getItemConsume() : 1, null, false)) {
                ...
                if (!isPet && item.getItemType() == EtcItemType.HERB && activeChar.hasServitor()) { activeChar.getPet().doSimultaneousCast(itemSkill); }
            }
            else {
                // Normal item consumption is 1, if more, it must be given in DP with getItemConsume().
                if (!playable.destroyItem(EItemProcessPurpose.CONSUME, item.getObjectId(), (itemSkill.getItemConsumeId() == 0 && itemSkill.getItemConsume() > 0) ? itemSkill.getItemConsume() : 1, null, false)) {
            ...
            if (isPet) { activeChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.PET_USES_S1).addSkillName(itemSkill)); }
            else {
                ...
                switch (skillId) {
                    ...
                    case 2037:
                        ...
                        if (skillId == 2037) { activeChar.shortBuffStatusUpdate(skillId, skillInfo.getValue(), itemSkill.getBuffDuration() / 1000); }
                        // Healing potions.
                        else if (skillId == 2032 && buffId != 2037) { activeChar.shortBuffStatusUpdate(skillId, skillInfo.getValue(), itemSkill.getBuffDuration() / 1000); }
                        // Lesser healing potions.
                        else {
                            if (buffId != 2037 && buffId != 2032) { activeChar.shortBuffStatusUpdate(skillId, skillInfo.getValue(), itemSkill.getBuffDuration() / 1000); }
                        }
                        break;
                }
            }
            ...
            if (item.isEtcItem()) {
                if (item.getEtcItem().getReuseDelay() > reuseDelay) { reuseDelay = item.getEtcItem().getReuseDelay(); }

                playable.addTimeStamp(itemSkill, reuseDelay);
                if (reuseDelay != 0) { playable.disableSkill(itemSkill, reuseDelay); }

                if (!isPet) {
                    final int group = item.getEtcItem().getSharedReuseGroup();
                    if (group >= 0) { activeChar.sendPacket(new ExUseSharedGroupItem(item.getItemId(), group, reuseDelay, reuseDelay)); }
                }
            }
