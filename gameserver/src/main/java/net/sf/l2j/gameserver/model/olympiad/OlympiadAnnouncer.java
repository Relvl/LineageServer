...
public final class OlympiadAnnouncer implements Runnable {
    ...
    public OlympiadAnnouncer() {
        for (L2Spawn spawn : SpawnTable.getInstance().getSpawnTable()) {
            if (spawn != null && spawn.getNpcId() == OLY_MANAGER) { _managers.add(spawn); }
        }
    }
    ...
    public void run() {
        ...
        for (int i = OlympiadGameManager.getInstance().getNumberOfStadiums(); --i >= 0; _currentStadium++) {
            if (_currentStadium >= OlympiadGameManager.getInstance().getNumberOfStadiums()) { _currentStadium = 0; }
            ...
            if (task != null && task.getGame() != null && task.needAnnounce()) {
                ...
                for (L2Spawn spawn : _managers) {
                    ...
                    if (manager != null) {
                        manager.broadcastPacket(new NpcSay(manager.getObjectId(), EChatType.SHOUT, manager.getNpcId(), npcString));
                    }
                }
                break;
            }
