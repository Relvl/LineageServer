...
public class VarkaSilenosSupport extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        if (StringUtil.isDigit(event)) {
            ...
            if (player.getWarehouse().getSize() == 0) { htmltext = "31381-0.htm"; }
            else {
    ...
    public String onFirstTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (npc.getNpcId()) {
            case ASHAS:
                if (allianceLevel < 0) { htmltext = "31377-friend.htm"; }
                else { htmltext = "31377-no.htm"; }
                break;

            case NARAN:
                if (allianceLevel < 0) { htmltext = "31378-friend.htm"; }
                else { htmltext = "31378-no.htm"; }
                break;
            ...
            case UDAN:
                ...
                if (allianceLevel > -1) { htmltext = "31379-3.htm"; }
                    ...
                    if (st.hasQuestItems(SEED)) { htmltext = "31379-4.htm"; }
                    else { htmltext = "31379-2.htm"; }
                }
                break;

            case DIYABU:
                if (player.getKarma() >= 1) { htmltext = "31380-pk.htm"; }
                else if (allianceLevel >= 0) { htmltext = "31380-no.htm"; }
                else if (allianceLevel == -1 || allianceLevel == -2) { htmltext = "31380-1.htm"; }
                else { htmltext = "31380-2.htm"; }
                break;
            ...
            case HAGOS:
                switch (allianceLevel) {
                    ...
                    case -3:
                        ...
                        break;
                    default:
                        if (allianceLevel >= 0) { htmltext = "31381-no.htm"; }
                        else if (player.getWarehouse().getSize() == 0) { htmltext = "31381-3.htm"; }
                        else { htmltext = "31381-4.htm"; }
                        break;
                }
                break;
            ...
            case TERANU:
                if (allianceLevel >= 0) { htmltext = "31383-no.htm"; }
                else if (allianceLevel < 0 && allianceLevel > -4) { htmltext = "31383-1.htm"; }
                else if (allianceLevel == -4) { htmltext = "31383-2.htm"; }
                else { htmltext = "31383-3.htm"; }
                break;
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        if (player.isInParty()) {
            for (L2PcInstance partyMember : player.getParty().getPartyMembers()) {
                if (partyMember != null) { testVarkaDemote(partyMember); }
            }
        }
        else { testVarkaDemote(player); }
