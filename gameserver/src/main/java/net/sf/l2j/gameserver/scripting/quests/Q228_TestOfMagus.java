...
public class Q228_TestOfMagus extends Quest {
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
        ...
        // WIND SYLPH
        else if (event.equalsIgnoreCase("30412-02.htm")) { st.giveItems(SYLPH_CHARM, 1); }
        // EARTH SNAKE
        else if (event.equalsIgnoreCase("30409-03.htm")) { st.giveItems(SERPENT_CHARM, 1); }
    ...
    public String onTalk(L2Npc npc, L2PcInstance player) {
        ...
        switch (st.getState()) {
            case QuestState.STATE_CREATED:
                if (player.getClassId() != ClassId.wizard && player.getClassId() != ClassId.elvenWizard && player.getClassId() != ClassId.darkWizard) { htmltext = "30629-01.htm"; }
                else if (player.getLevel() < 39) { htmltext = "30629-02.htm"; }
                else { htmltext = "30629-03.htm"; }
                break;
            ...
            case QuestState.STATE_STARTED:
                ...
                switch (npc.getNpcId()) {
                    case RUKAL:
                        if (cond == 1) { htmltext = "30629-05.htm"; }
                        else if (cond == 2) { htmltext = "30629-06.htm"; }
                        else if (cond == 3) { htmltext = "30629-07.htm"; }
                        else if (cond == 4) { htmltext = "30629-08.htm"; }
                        else if (cond == 5) { htmltext = "30629-11.htm"; }
                        else if (cond == 6) {
                    ...
                    case PARINA:
                        if (cond == 1) { htmltext = "30391-01.htm"; }
                        else if (cond == 2) { htmltext = "30391-03.htm"; }
                        else if (cond == 3 || cond == 4) { htmltext = "30391-04.htm"; }
                        else if (cond > 4) { htmltext = "30391-05.htm"; }
                        break;

                    case CASIAN:
                        if (cond == 2) { htmltext = "30612-01.htm"; }
                        else if (cond == 3) { htmltext = "30612-03.htm"; }
                        else if (cond == 4) { htmltext = "30612-04.htm"; }
                        else if (cond > 4) { htmltext = "30612-05.htm"; }
                        break;

                    case WATER_UNDINE:
                        if (cond == 5) {
                            if (st.hasQuestItems(UNDINE_CHARM)) {
                                if (st.getQuestItemsCount(DAZZLING_DROP) < 20) { htmltext = "30413-02.htm"; }
                                else {
                            ...
                            else { htmltext = "30413-04.htm"; }
                        }
                        else if (cond == 6) { htmltext = "30413-04.htm"; }
                        break;

                    case FLAME_SALAMANDER:
                        if (cond == 5) {
                            if (st.hasQuestItems(SALAMANDER_CHARM)) {
                                if (st.getQuestItemsCount(FLAME_CRYSTAL) < 5) { htmltext = "30411-02.htm"; }
                                else {
                            ...
                            else { htmltext = "30411-04.htm"; }
                        }
                        else if (cond == 6) { htmltext = "30411-04.htm"; }
                        break;

                    case WIND_SYLPH:
                        if (cond == 5) {
                            if (st.hasQuestItems(SYLPH_CHARM)) {
                                if (st.getQuestItemsCount(HARPY_FEATHER) + st.getQuestItemsCount(WYRM_WINGBONE) + st.getQuestItemsCount(WINDSUS_MANE) < 40) { htmltext = "30412-03.htm"; }
                                else {
                            ...
                            else if (!st.hasQuestItems(TONE_OF_WIND)) { htmltext = "30412-01.htm"; }
                            else { htmltext = "30412-05.htm"; }
                        }
                        else if (cond == 6) { htmltext = "30412-05.htm"; }
                        break;

                    case EARTH_SNAKE:
                        if (cond == 5) {
                            if (st.hasQuestItems(SERPENT_CHARM)) {
                                if (st.getQuestItemsCount(EN_MONSTEREYE_SHELL) + st.getQuestItemsCount(EN_STONEGOLEM_POWDER) + st.getQuestItemsCount(EN_IRONGOLEM_SCRAP) < 30) { htmltext = "30409-04.htm"; }
                                else {
                            ...
                            else if (!st.hasQuestItems(TONE_OF_EARTH)) { htmltext = "30409-01.htm"; }
                            else { htmltext = "30409-06.htm"; }
                        }
                        else if (cond == 6) { htmltext = "30409-06.htm"; }
                        break;
                }
                break;
    ...
    public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
        ...
        if (cond == 3) {
            switch (npc.getNpcId()) {
                case SINGING_FLOWER_PHANTASM:
                    if (!st.hasQuestItems(GOLDEN_SEED_1)) {
                        ...
                        if (st.hasQuestItems(GOLDEN_SEED_2, GOLDEN_SEED_3)) { st.set("cond", "4"); }
                    }
                    break;
                ...
                case SINGING_FLOWER_NIGHTMARE:
                    if (!st.hasQuestItems(GOLDEN_SEED_2)) {
                        ...
                        if (st.hasQuestItems(GOLDEN_SEED_1, GOLDEN_SEED_3)) { st.set("cond", "4"); }
                    }
                    break;
                ...
                case SINGING_FLOWER_DARKLING:
                    if (!st.hasQuestItems(GOLDEN_SEED_3)) {
                        ...
                        if (st.hasQuestItems(GOLDEN_SEED_1, GOLDEN_SEED_2)) { st.set("cond", "4"); }
                    }
                    break;
            }
        }
            ...
            switch (npc.getNpcId()) {
                case GHOST_FIRE:
                    if (st.hasQuestItems(SALAMANDER_CHARM)) { st.dropItems(FLAME_CRYSTAL, 1, 5, 500000); }
                    break;

                case TOAD_LORD:
                case MARSH_STAKATO:
                case MARSH_STAKATO_WORKER:
                    if (st.hasQuestItems(UNDINE_CHARM)) { st.dropItems(DAZZLING_DROP, 1, 20, 300000); }
                    break;

                case MARSH_STAKATO_SOLDIER:
                    if (st.hasQuestItems(UNDINE_CHARM)) { st.dropItems(DAZZLING_DROP, 1, 20, 400000); }
                    break;

                case MARSH_STAKATO_DRONE:
                    if (st.hasQuestItems(UNDINE_CHARM)) { st.dropItems(DAZZLING_DROP, 1, 20, 500000); }
                    break;

                case HARPY:
                    if (st.hasQuestItems(SYLPH_CHARM)) { st.dropItemsAlways(HARPY_FEATHER, 1, 20); }
                    break;

                case WYRM:
                    if (st.hasQuestItems(SYLPH_CHARM)) { st.dropItems(WYRM_WINGBONE, 1, 10, 500000); }
                    break;

                case WINDSUS:
                    if (st.hasQuestItems(SYLPH_CHARM)) { st.dropItems(WINDSUS_MANE, 1, 10, 500000); }
                    break;

                case ENCHANTED_MONSTEREYE:
                    if (st.hasQuestItems(SERPENT_CHARM)) { st.dropItemsAlways(EN_MONSTEREYE_SHELL, 1, 10); }
                    break;

                case ENCHANTED_STONE_GOLEM:
                    if (st.hasQuestItems(SERPENT_CHARM)) { st.dropItemsAlways(EN_STONEGOLEM_POWDER, 1, 10); }
                    break;

                case ENCHANTED_IRON_GOLEM:
                    if (st.hasQuestItems(SERPENT_CHARM)) { st.dropItemsAlways(EN_IRONGOLEM_SCRAP, 1, 10); }
                    break;
            }
        }
